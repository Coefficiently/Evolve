...
const fortressModules = {
  prtl_fortress: {
    ...
    turret: {
      id: "portal-turret",
      title() {
        let type = global.tech["turret"]
          ? global.tech["turret"] >= 2
            ? "portal_turret_title3"
            : "portal_turret_title2"
          : "portal_turret_title1";
        return loc(type);
      },
      desc() {
        let type = global.tech["turret"]
          ? global.tech["turret"] >= 2
            ...
            : "portal_turret_title2"
          : "portal_turret_title1";
        return `<div>${loc(type)}</div><div class="has-text-special">${loc(
          "requires_power"
        )}</div>`;
      },
      reqs: { portal: 2 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("turret", offset, 350000, 1.28, "portal");
        },
        Copper(offset) {
          return spaceCostMultiplier("turret", offset, 50000, 1.28, "portal");
        },
        Adamantite(offset) {
          return spaceCostMultiplier("turret", offset, 8000, 1.28, "portal");
        },
        Elerium(offset) {
          return spaceCostMultiplier("turret", offset, 15, 1.28, "portal");
        },
        Nano_Tube(offset) {
          return spaceCostMultiplier("turret", offset, 28000, 1.28, "portal");
        },
      },
      powered() {
        return powerCostMod(
          global.tech["turret"] ? 4 + global.tech["turret"] : 4
        );
      },
      postPower(o) {
      ...
      effect() {
        let rating = global.tech["turret"]
          ? global.tech["turret"] >= 2
            ? 70
            : 50
          : 35;
        let power = $(this)[0].powered();
        return `<div>${loc("portal_turret_effect", [
          rating,
    ...
    carport: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("carport", offset, 250000, 1.3, "portal");
        },
        Cement(offset) {
          return spaceCostMultiplier("carport", offset, 18000, 1.3, "portal");
        },
        Oil(offset) {
    ...
    repair_droid: {
      ...
      effect() {
        return `<div>${loc("portal_repair_droid_effect", [5])}</div><div>${loc(
          "portal_repair_droid_effect2",
          [8]
        )}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
  ...
  prtl_badlands: {
    ...
    sensor_drone: {
      ...
      effect() {
        ...
        let sci_bonus = global.race["cataclysm"]
          ? `<div>${loc("space_moon_observatory_cata_effect", [2])}</div>`
          : `<div>${loc("space_moon_observatory_effect", [2])}</div><div>${loc(
              "portal_sensor_drone_effect2",
              [2]
            )}</div>`;
        let sci =
          global.tech["science"] >= 14
            ? `<div>${loc("city_max_knowledge", [know])}</div>${sci_bonus}`
            : "";
        return `<div>${loc("portal_sensor_drone_effect", [
          bonus,
        ])}</div>${sci}<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
      action() {
    ...
    attractor: {
      ...
      effect() {
        return `<div>${loc("portal_attractor_effect1")}</div><div>${loc(
          "portal_attractor_effect2",
  ...
  prtl_pit: {
    ...
    pit_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_pit_mission_result"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    assault_forge: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_assault_forge_result"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    soul_forge: {
        ...
        Money(offset) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("soul_forge")
              ? global.portal.soul_forge.count
              : 0) <
            1
            ? 25000000
            : 0;
      ...
      effect(wiki) {
        let desc = `<div>${loc("portal_soul_forge_effect", [
          global.resource.Soul_Gem.name,
        ])}</div>`;
        let count =
          (wiki?.count ?? 0) +
        ...
        if (count >= 1) {
          ...
          desc =
            desc +
            `<div>${loc("portal_soul_forge_effect2", [
              global.portal["soul_forge"]
                ? global.portal.soul_forge.kills.toLocaleString()
                : 0,
              Math.round(cap).toLocaleString(),
            ])}</div>`;
        }
        let soldiers = soulForgeSoldiers(wiki);
        return `${desc}<div><span class="has-text-caution">${loc(
          "portal_soul_forge_soldiers",
          [soldiers]
    ...
    gun_emplacement: {
      ...
      effect() {
        ...
        return `<div>${loc("portal_gun_emplacement_effect", [
          soldiers,
        ])}</div><div>${loc("portal_gun_emplacement_effect2", [
          min,
          max,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
    ...
    soul_attractor: {
      ...
      effect() {
        let link =
          global.tech.hell_pit >= 7
            ? `<div>${loc("portal_soul_attractor_effect2", [3])}</div>`
            : ``;
        let attract = global.blood["attract"] ? global.blood.attract * 5 : 0;
        if (global.tech["hell_pit"] && global.tech.hell_pit >= 8) {
          attract *= 2;
        }
        return `<div>${loc("portal_soul_attractor_effect", [
          40 + attract,
          120 + attract,
        ])}</div>${link}<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
      action() {
    ...
    soul_capacitor: {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("soul_capacitor")
        ...
        Soul_Gem(offset) {
          return (offset || 0) +
            ...
            40
            ? 0
            : spaceCostMultiplier("soul_capacitor", offset, 3, 1.01, "portal");
        },
        Mythril(offset) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("soul_capacitor")
      ...
      effect() {
        let cap = 2500000;
        let eCap = global.portal.hasOwnProperty("soul_capacitor")
          ? global.portal.soul_capacitor.ecap
          : 0;
        let energy = global.portal.hasOwnProperty("soul_capacitor")
          ? global.portal.soul_capacitor.energy
          : 0;
        let desc = `<div>${loc("portal_soul_capacitor_effect", [
          energy.toLocaleString(),
        ])}</div>`;
        desc += `<div>${loc("portal_soul_capacitor_effect2", [
          eCap.toLocaleString(),
        ])}</div>`;
        desc += `<div>${loc("portal_soul_capacitor_effect3", [
          cap.toLocaleString(),
        ])}</div>`;
    ...
    absorption_chamber: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("absorption_chamber") ||
          global.portal.absorption_chamber.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "portal_absorption_chamber_title"
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("absorption_chamber")
              ? global.portal.absorption_chamber.count
              : 0) <
            100
            ? 75000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          ...
          return `<div>${loc(
            "portal_absorption_chamber_incomplete"
          ...
          if (global.tech.forbidden === 5) {
            return `<div>${loc("portal_absorption_chamber_effect_eld", [
              (100000000).toLocaleString(),
            ])}</div><div class="has-text-special">${loc(
              "tech_demonic_infusion_effect2",
              [calcPrestige("descend").artifact]
            )}</div>`;
          } else {
            let reward = astrialProjection();
            return `<div>${loc(`portal_absorption_chamber_effect`, [
              (100000000).toLocaleString(),
            ])}</div><div>${reward}</div>`;
          }
        }
  ...
  prtl_ruins: {
    info: {
      ...
      support: "guard_post",
      prop() {
        let desc = ` - <span class="has-text-advanced">${loc(
          "portal_ruins_security"
        )}:</span> <span class="has-text-caution">{{ on | filter('army') }}</span>`;
        desc =
          desc +
          ` - <span class="has-text-advanced">${loc(
            "portal_ruins_supressed"
          )}:</span> <span class="has-text-caution">{{ on | filter('sup') }}</span>`;
        return desc;
      },
      filter(v, type) {
    ...
    ruins_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_ruins_mission_result"), "info", false, [
            "progress",
            "hell",
          ]);
    ...
    guard_post: {
      ...
      effect() {
        ...
        return `<div>${loc("portal_guard_post_effect1", [
          rating,
        ])}</div><div class="has-text-caution">${loc(
          "portal_guard_post_effect2",
          [jobScale(1), $(this)[0].powered()]
        )}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    vault: {
      ...
      cost: {
        Soul_Gem(offset) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("vault")
              ? global.portal.vault.count
              : 0) ===
            0
            ? 100
            : 0;
      ...
      effect(wiki) {
        let count =
          ...
          (global.portal.hasOwnProperty("vault")
            ...
            : 0);
        return count < 1
          ? loc("portal_vault_effect", [100])
          : loc("portal_vault_effect2");
      },
      action() {
        if (global.portal.vault.count < 2 && payCosts($(this)[0])) {
          ...
          if (global.portal.vault.count === 2) {
            ...
            messageQueue(loc("portal_vault_result"), "info", false, [
              "progress",
              "hell",
            ]);
          }
    ...
    archaeology: {
      ...
      effect() {
        return `<div>${loc("portal_archaeology_effect", [
          jobScale(2),
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    arcology: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "arcology",
            offset,
            traitCostMod("untrustworthy", 180000000),
            1.22,
            "portal"
          );
        },
        Graphene(offset) {
          return spaceCostMultiplier(
            "arcology",
            offset,
            traitCostMod("untrustworthy", 7500000),
            1.22,
            "portal"
          );
        },
        Bolognium(offset) {
          return spaceCostMultiplier(
            "arcology",
            offset,
            traitCostMod("untrustworthy", 2800000),
            1.22,
            "portal"
          );
        },
        Orichalcum(offset) {
          return spaceCostMultiplier(
            "arcology",
            offset,
            traitCostMod("untrustworthy", 5500000),
            1.22,
            "portal"
          );
        },
        Nanoweave(offset) {
          return spaceCostMultiplier(
            "arcology",
            offset,
            traitCostMod("untrustworthy", 650000),
            1.22,
            "portal"
          );
        },
      ...
      effect(wiki) {
        ...
        let container_string = `<div>${loc("plus_max_resource", [
          containers,
        ...
        ])}</div><div>${loc("portal_guard_post_effect1", [
          75,
        ])}</div>${container_string}<div class="has-text-caution">${loc(
          "minus_power",
          [$(this)[0].powered()]
        )}</div>`;
      },
    ...
    hell_forge: {
      ...
      effect(wiki) {
        ...
        let reactor = global.tech["inferno_power"]
          ? `<div>${loc("portal_hell_forge_effect2", [
              10,
              loc(`portal_inferno_power_title`),
            ])}</div>`
          : ``;
        return `<div>${loc("portal_hell_forge_effect", [
          jobScale(1),
        ])}</div>${reactor}<div>${loc("interstellar_stellar_forge_effect3", [
          $(this)[0].smelting(),
    ...
    inferno_power: {
      ...
      powered(wiki) {
        let power = 20;
        let infernal_forges_on = wiki
          ? global.portal?.hell_forge?.on ?? 0
          : p_on["hell_forge"];
    ...
    ancient_pillars: {
      ...
      reqs: { hell_ruins: 2 },
      queue_complete() {
        return global.tech["pillars"] &&
          global.tech.pillars === 1 &&
          global.race.universe !== "micro"
          ? 1
          : 0;
      },
      cost: {
        Harmony(offset, wiki) {
          if (offset !== undefined) {
            return offset + Object.keys(global.pillars).length <
              Object.keys(races).length - 1
              ? 1
              : 0;
          }
          return global.race.universe !== "micro" &&
            global.tech["pillars"] &&
            global.tech.pillars === 1
            ? 1
            : 0;
        ...
        Scarletite(offset, wiki) {
          if (offset !== undefined) {
            ...
            return pillars < Object.keys(races).length - 1
              ? pillars * 125000 + 1000000
              : 0;
          }
          return global.race.universe !== "micro" &&
            global.tech["pillars"] &&
            global.tech.pillars === 1
            ? Object.keys(global.pillars).length * 125000 + 1000000
            : 0;
        },
      },
      count() {
      ...
      effect(wiki) {
        ...
        if (pillars >= 1) {
          return `<div>${loc("portal_ancient_pillars_effect2", [
            Object.keys(races).length - 1,
            pillars,
          ])}</div>`;
        } else {
          return `<div>${loc("portal_ancient_pillars_effect", [
            Object.keys(races).length - 1,
          ])}</div>`;
        }
      },
  ...
  prtl_gate: {
    info: {
      ...
      desc() {
        return `${loc("portal_gate_desc")} ${loc(
          global.tech["wtower"] && global.tech["etower"]
            ? "portal_gate_open"
            : "portal_gate_closed"
        )}`;
      },
    ...
    gate_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_gate_mission_result"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    west_tower: {
      ...
      desc(wiki) {
        let size = towerSize();
        if (
          !global.portal.hasOwnProperty("west_tower") ||
          global.portal.west_tower.count < size ||
          wiki
        ) {
          return `<div>${loc(
            "portal_west_tower"
      ...
      cost: {
        Money(offset, wiki) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("west_tower")
              ? global.portal.west_tower.count
              : 0) <
            towerSize()
            ? towerPrice(10000000, wiki)
            : 0;
        },
        Stone(offset, wiki) {
          return (offset || 0) +
      ...
      action() {
        if (
          global.portal.west_tower.count < towerSize() &&
          payCosts($(this)[0])
        ) {
      ...
      post() {
        if (global.portal.west_tower.count >= towerSize()) {
          ...
          if (
            ...
            messageQueue(loc("portal_gate_open"), "info", false, [
              "progress",
              "hell",
            ]);
            renderFortress();
    ...
    east_tower: {
      ...
      desc(wiki) {
        let size = towerSize();
        if (
          !global.portal.hasOwnProperty("east_tower") ||
          global.portal.east_tower.count < size ||
          wiki
        ) {
          return `<div>${loc(
            "portal_east_tower"
      ...
      cost: {
        Money(offset, wiki) {
          return (offset || 0) +
            (global.portal.hasOwnProperty("east_tower")
              ? global.portal.east_tower.count
              : 0) <
            towerSize()
            ? towerPrice(10000000, wiki)
            : 0;
      ...
      action() {
        if (
          global.portal.east_tower.count < towerSize() &&
          payCosts($(this)[0])
        ) {
    ...
    gate_turret: {
      ...
      effect() {
        ...
        return `<div>${loc("portal_gate_turret_effect", [
          Math.round(security),
        ])}</div><div>${loc("portal_gate_turret_effect2", [
          min,
          max,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
    ...
    infernite_mine: {
      ...
      effect(wiki) {
        ...
        return `<div>${loc("portal_infernite_mine_effect", [
          +mining.toFixed(3),
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
  ...
  prtl_lake: {
    ...
    lake_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_lake_mission_result"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    harbor: {
      ...
      reqs: { hell_lake: 3 },
      powered(wiki) {
        let num_cooling_tower = wiki
          ? global.portal?.cooling_tower?.on ?? 0
          : p_on["cooling_tower"];
      ...
      effect(wiki) {
        ...
        for (const res of $(this)[0].res()) {
          if (global.resource[res].display) {
            let val = sizeApproximation(
              +spatialReasoning($(this)[0].val(res)).toFixed(0),
              1
            );
            storage =
              storage +
        ...
        storage = storage + "</div>";
        return `<div>${loc("portal_harbor_effect", [
          1,
        ])}</div>${storage}<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(wiki),
        ])}</div>`;
      },
      action() {
    ...
    cooling_tower: {
      ...
      effect() {
        return `<div>${loc("portal_cooling_tower_effect", [
          8,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    bireme: {
      ...
      cost: {
        ...
        Helium_3(offset) {
          return spaceCostMultiplier("bireme", offset, 225000, 1.24, "portal");
        },
        Adamantite(offset) {
          return spaceCostMultiplier(
            "bireme",
        ...
        Scarletite(offset) {
          return spaceCostMultiplier("bireme", offset, 125000, 1.24, "portal");
        },
      },
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("lake"),
        ])}</div><div>${loc("portal_bireme_effect", [
          rating,
        ])}</div><div class="has-text-caution">${loc(
    ...
    transport: {
      ...
      effect(wiki) {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("lake"),
        ])}</div><div>${loc("portal_transport_effect", [
          5,
        ])}</div><div class="has-text-danger">${loc(
          "portal_transport_effect2",
          [bireme]
        )}</div><div class="has-text-caution">${loc(
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (!global.settings.portal.spire) {
            ...
            messageQueue(loc("portal_transport_unlocked"), "info", false, [
              "progress",
              "hell",
            ]);
            renderFortress();
    ...
    oven: {
      ...
      desc(wiki) {
        if (
          !global.portal.hasOwnProperty("oven") ||
          global.portal.oven.count < 100 ||
          wiki
        ) {
          return (
            `<div>${loc(
      ...
      effect(wiki) {
        let count =
          (wiki?.count ?? 0) +
          (global.portal.hasOwnProperty("oven") ? global.portal.oven.count : 0);
        if (count < 100) {
          let remain = 100 - count;
          return `<div>${loc(
            "portal_oven_effect1"
    ...
    oven_complete: {
      ...
      effect(wiki) {
        ...
        return `<div>${loc(`portal_oven_desc`)}</div>${
          global.tech["dish"] === 4
            ? `<div class="has-text-special">${loc("portal_oven_desc2")}</div>`
            : ``
        }<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}, ${loc("spend", [fuel.a, fuel.r])}</div>`;
      },
      powered() {
    ...
    devilish_dish: {
      ...
      cost: {},
      effect() {
        const progress = global.portal["devilish_dish"]
          ? global.portal["devilish_dish"].done
          : 0;
        return `<div>${loc(`portal_devilish_dish_desc`, [
          progress.toFixed(1),
        ])}</div><div>${loc(
          `portal_devilish_dish_flavor${
            progress >= 100 ? 6 : Math.ceil(progress / 20)
          }`
        )}</div>`;
      },
      action() {
    ...
    dish_soul_steeper: {
      ...
      effect() {
        return `<div>${loc(
          "portal_dish_soul_steeper_effect1"
        )}</div><div class="has-text-danger">${loc(
          "portal_dish_soul_steeper_effect2",
          [
            3 +
              (global.race["malnutrition"] ? 1 : 0) +
              (global.race["angry"] ? -1 : 0),
          ]
        )}</div>`;
      },
      action() {
    ...
    dish_life_infuser: {
      ...
      effect() {
        return `<div>${loc("portal_dish_life_infuser_effect1", [
          15,
        ])}</div><div class="has-text-danger">${loc(
          "portal_dish_life_infuser_effect2",
          [5]
        )}</div>`;
      },
      action() {
  ...
  prtl_spire: {
    info: {
      ...
      prop() {
        let desc = ` - <span class="has-text-advanced">${loc(
          "portal_spire_supply"
        )}:</span> <span class="has-text-caution">{{ supply | filter }} / {{ sup_max | filter }}</span>`;
        return (
          desc +
          ` (<span class="has-text-success">+{{ diff | filter(2) }}/s</span>)`
        );
      },
      filter(v, fix) {
    ...
    spire_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("portal_spire_mission_result"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    purifier: {
      ...
      cost: {
        ...
        Supply(offset) {
          return global.portal["purifier"] && global.portal.purifier.count === 0
            ? 100
            : spaceCostMultiplier(
                "purifier",
                offset,
      ...
      support() {
        let base =
          global.tech["b_stone"] && global.tech.b_stone >= 3 ? 1.25 : 1;
        if (
          global.tech["hell_spire"] &&
          global.tech.hell_spire >= 11 &&
          global.eden["asphodel_harvester"] &&
      ...
      effect() {
        return `<div>${loc("portal_purifier_effect", [
          $(this)[0].support(),
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    port: {
      ...
      cost: {
        ...
        Supply(offset) {
          return global.portal.hasOwnProperty("port") &&
      ...
      effect(wiki) {
        ...
        let num_base_camps_on = wiki
          ? global.portal?.base_camp?.on ?? 0
          : spire_on["base_camp"];
        if (num_base_camps_on > 0) {
          port_value *= 1 + num_base_camps_on * 0.4;
        }
        return `<div class="has-text-caution">${loc("portal_port_effect1", [
          $(this)[0].support(),
        ])}</div><div>${loc("portal_port_effect2", [
          Math.round(port_value),
        ])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    base_camp: {
      ...
      effect() {
        return `<div class="has-text-caution">${loc("portal_port_effect1", [
          $(this)[0].support(),
        ])}</div><div>${loc("portal_base_camp_effect", [40])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (global.tech.hell_spire === 4) {
            ...
            messageQueue(loc("portal_spire_bridge_collapse"), "info", false, [
              "progress",
              "hell",
            ]);
            renderFortress();
    ...
    bridge: {
      ...
      desc(wiki) {
        if (
          !global.portal.hasOwnProperty("bridge") ||
          global.portal.bridge.count < 10 ||
          wiki
        ) {
          return `<div>${loc(
            "portal_bridge_title"
      ...
      effect(wiki) {
        ...
        if (count < size) {
          let remain = size - count;
          return `<div>${loc(
            "portal_bridge_effect"
          )}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          )}</div><div class="has-text-caution">${loc(
            "portal_bridge_effect2"
          )}</div>`;
        } else {
    ...
    sphinx: {
      id: "portal-sphinx",
      title() {
        return global.tech.hell_spire === 7
          ? loc("portal_sphinx_solve")
          : loc("portal_sphinx_title");
      },
      ...
      cost: {
        Knowledge(offset) {
          let count =
            (offset || 0) +
            (!global.tech["hell_spire"] || global.tech.hell_spire < 7
              ? 0
      ...
      action() {
        if (payCosts($(this)[0])) {
          if (global.tech.hell_spire === 6) {
            global.tech.hell_spire = 7;
            messageQueue(loc("portal_sphinx_msg"), "info", false, [
              "progress",
              "hell",
            ]);
            renderFortress();
            ...
            renderFortress();
            messageQueue(loc("portal_sphinx_answer_msg"), "info", false, [
              "progress",
              "hell",
            ]);
            return true;
    ...
    bribe_sphinx: {
      ...
      reqs: { hell_spire: 7 },
      condition() {
        return global.tech["hell_spire"] &&
          global.tech.hell_spire === 7 &&
          !global.tech["sphinx_bribe"]
          ? true
          : false;
      },
      cost: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          if (global.tech.hell_spire === 7 && !global.tech["sphinx_bribe"]) {
            ...
            global.resource.Codex.display = true;
            global.resource.Codex.amount = 1;
            messageQueue(loc("portal_sphinx_bribe_msg"), "info", false, [
              "progress",
              "hell",
            ]);
            return true;
    ...
    spire_survey: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          genSpireFloor();
          messageQueue(loc("portal_spire_survey_msg"), "info", false, [
            "progress",
            "hell",
          ]);
          return true;
    ...
    mechbay: {
      ...
      cost: {
        ...
        Supply(offset) {
          return spaceCostMultiplier("mechbay", offset, 250000, 1.2, "portal");
        },
      },
      powered() {
        return 0;
      ...
      effect() {
        let bay = global.portal.hasOwnProperty("mechbay")
          ? global.portal.mechbay.bay
          : 0;
        let max = global.portal.hasOwnProperty("mechbay")
          ? global.portal.mechbay.max
          : 0;
        return `<div class="has-text-caution">${loc("portal_port_effect1", [
          $(this)[0].support(),
        ])}</div><div>${loc("portal_mechbay_effect")}</div><div>${loc(
          "portal_mechbay_effect2",
          [bay, max]
        )}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (global.portal.mechbay.count === 1) {
            messageQueue(loc("portal_mechbay_unlocked"), "info", false, [
              "progress",
              "hell",
            ]);
            drawMechLab();
    ...
    spire: {
      ...
      cost: {},
      effect() {
        let floor = global.portal.hasOwnProperty("spire")
          ? global.portal.spire.count
          : 0;
        let terrain = global.portal.hasOwnProperty("spire")
          ? `<span class="has-text-warning">${loc(
              `portal_spire_type_${global.portal.spire.type}`
            )}</span>`
          : "?";
        let status = ``;
        if (
          global.portal.hasOwnProperty("spire") &&
          Object.keys(global.portal.spire.status).length > 0
        ) {
          status = `<div>${loc("portal_spire_hazard", [
            Object.keys(global.portal.spire.status)
              .map(
                (v) =>
                  `<span class="has-text-warning">${loc(
                    `portal_spire_status_${v}`
                  )}</span>`
              )
              .join(", "),
          ])}</div>`;
        }
        let progress = global.portal.hasOwnProperty("spire")
          ? `<span class="has-text-warning">${+global.portal.spire.progress.toFixed(
              3
            )}%</span>`
          : "0%";
        let leftSide = `<div>${loc("portal_spire_effect", [
          floor,
        ])}</div><div>${loc("portal_spire_type", [
          terrain,
        ])}</div>${status}<div>${loc("portal_spire_progress", [
          progress,
        ])}</div>`;

        let boss = global.portal.hasOwnProperty("spire")
          ? global.portal.spire.boss
          : "crazed";
        let threat = `<div>${loc("portal_spire_mob", [
          `<span class="has-text-danger">${loc(
            `portal_mech_boss_${boss}`
          )}</span>`,
        ])}</div>`;
        ...
        let rightSide = `<div>${threat}<div>${loc("portal_spire_mob_weak", [
          `<span class="has-text-warning">${weak}</span>`,
        ])}</div><div>${loc("portal_spire_mob_resist", [
          `<span class="has-text-warning">${resist}</span>`,
        ])}</div></div>`;
      ...
      mscan() {
        ...
        Object.keys(global.stats.spire).forEach(function (uni) {
          let boss = global.portal.hasOwnProperty("spire")
            ...
            : "crazed";
          if (
            global.stats.spire.hasOwnProperty(uni) &&
            global.stats.spire[uni].hasOwnProperty(boss) &&
            global.stats.spire[uni][boss] > level
          ) {
    ...
    waygate: {
      ...
      desc(wiki) {
        if (
          ...
          (global.tech["waygate"] && global.tech.waygate < 2) ||
          wiki
        ) {
          return `<div>${loc(
            "portal_waygate_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            10,
          ])}</div>`;
        } else {
      ...
      cost: {
        Species(offset) {
          if (offset) {
            return offset +
              (global.portal.hasOwnProperty("waygate")
                ? global.portal.waygate.count
                : 0) <
              10
              ? popCost(25)
              : 0;
          }
          return !global.portal.hasOwnProperty("waygate") ||
            (global.tech["waygate"] && global.tech.waygate < 2)
            ? popCost(25)
            : 0;
        },
      ...
      powered() {
        return global.portal.hasOwnProperty("waygate") &&
          global.portal.waygate.count >= 10
          ? 1
          : 0;
      },
      ...
      effect(wiki) {
        let count =
          ...
          (global.tech["waygate"] && global.tech.waygate >= 2
            ...
            : 0);
        if (count >= 10) {
          let progress = global.portal.hasOwnProperty("waygate")
            ? `<span class="has-text-warning">${+global.portal.waygate.progress.toFixed(
                3
              )}%</span>`
            : "0%";
          return `<div>${loc("portal_waygate_open")}</div><div>${loc(
            "portal_waygate_progress",
            [progress]
          )}</div>`;
        } else {
          let size = 10;
    ...
    edenic_gate: {
      ...
      title(wiki) {
        return loc(
          global.tech["edenic"] && global.tech.edenic >= 3
            ? "portal_edenic_gate_title"
            : "portal_waygate_title"
        );
      },
      desc(wiki) {
      ...
      cost: {
        Money(o) {
          return global.tech["edenic"] && global.tech.edenic < 3
            ? 10000000000
            : 0;
        },
        Supply(o) {
...
function spireCreep(base) {
  let creep =
    global.portal.hasOwnProperty("spire") && global.blood["spire"]
      ? base - (global.portal.spire.count - 1) / 2500
      : base;
...
export function soulForgeSoldiers(wiki) {
  ...
  let num_gun_emplacement = wiki
    ? global.portal?.gun_emplacement?.on ?? 0
    : p_on["gun_emplacement"];
  if (num_gun_emplacement) {
    soldiers -=
      num_gun_emplacement *
      (global.tech.hell_gun >= 2 ? jobScale(2) : jobScale(1));
    if (soldiers < 0) {
      soldiers = 0;
    }
  }
...
export function renderFortress() {
  if (
    ...
    (global.settings.civTabs !== 1 || global.settings.spaceTabs !== 4)
  ) {
    return;
  }
  let parent = $("#portal");
  ...
  Object.keys(fortressModules).forEach(function (region) {
    ...
    if (global.settings.portal[`${show}`]) {
      let name =
        typeof fortressModules[region].info.name === "string"
          ? fortressModules[region].info.name
          : fortressModules[region].info.name();
      ...
      popover(
        region,
        function () {
          return typeof fortressModules[region].info.desc === "string"
            ? fortressModules[region].info.desc
            : fortressModules[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
      ...
      Object.keys(fortressModules[region]).forEach(function (tech) {
        if (
          tech !== "info" &&
          checkRequirements(fortressModules, region, tech)
        ) {
  ...
  switch (dt) {
    case "hostiles": {
      if (global.portal.fortress.threat >= 2000) {
        return `${loc("fortress_threat", [
          global.portal.fortress.threat,
        ])} ${loc("fortress_threat_high")}`;
      } else if (global.portal.fortress.threat < 1000) {
        return `${loc("fortress_threat", [
          global.portal.fortress.threat,
        ])} ${loc("fortress_threat_low")}`;
      } else {
        return `${loc("fortress_threat", [
          global.portal.fortress.threat,
        ])} ${loc("fortress_threat_medium")}`;
      }
    }
    case "threatLevel": {
      let t = global.portal.fortress.threat;
      if (t < 1000) {
        return `${loc("fortress_threat_level")} ${loc(
          "fortress_threat_level1"
        )}`;
      } else if (t < 1500) {
        return `${loc("fortress_threat_level")} ${loc(
          "fortress_threat_level2"
        )}`;
      } else if (t >= 5000) {
        return `${loc("fortress_threat_level")} ${loc(
          "fortress_threat_level6"
        )}`;
      } else if (t >= 3000) {
        return `${loc("fortress_threat_level")} ${loc(
        ...
        return `${loc("fortress_threat_level")} ${loc(
          "fortress_threat_level4"
        )}`;
      } else {
        return `${loc("fortress_threat_level")} ${loc(
          "fortress_threat_level3"
        )}`;
      }
    }
    case "hireLabel": {
...
export function buildFortress(parent, full) {
  ...
  if (full) {
    ...
    fort.append(
      `<div><h3 class="has-text-warning">${loc(
        "portal_fortress_name"
      )}</h3><button class="button observe right" @click="observation">${loc(
        "hell_observation_button"
      )}</button></div>`
    );
  }
  ...
  let defense = $(
    `<span class="defense has-text-success" :aria-label="defense()">${loc(
      "fortress_defense"
    )} {{ f.garrison | defensive }}</span>`
  );
  status.append(defense);
  let activity = $(
    `<span class="has-text-danger pad hostiles" :aria-label="hostiles()">${loc(
      "fortress_spotted"
    )} {{ f.threat }}</span>`
  );
  status.append(activity);
  let threatLevel = $(
    `<span class="pad threatLevel" :class="threaten()" :aria-label="threatLevel()">{{ f.threat | threat }}</span>`
  );
  ...
  wallStatus.append(
    $(
      `<span class="has-text-warning" :aria-label="defense()">${loc(
        "fortress_wall"
      )} <span :class="wall()">{{ f.walls }}%</span></span>`
    )
  );
  ...
  station.append(
    $(
      '<span role="button" aria-label="remove soldiers from the fortress" class="sub has-text-danger" @click="aLast"><span>&laquo;</span></span>'
    )
  );
  station.append(
    $('<span class="current armyLabel">{{ f.garrison | patrolling }}</span>')
  );
  station.append(
    $(
      '<span role="button" aria-label="add soldiers to the fortress" class="add has-text-success" @click="aNext"><span>&raquo;</span></span>'
    )
  );
  ...
  station.append(
    $(
      '<span role="button" aria-label="reduce number of patrols" class="sub has-text-danger" @click="patDec"><span>&laquo;</span></span>'
    )
  );
  station.append($('<span class="current patLabel">{{ f.patrols }}</span>'));
  station.append(
    $(
      '<span role="button" aria-label="increase number of patrols" class="add has-text-success" @click="patInc"><span>&raquo;</span></span>'
    )
  );
  ...
  station.append(
    $(
      '<span role="button" aria-label="reduce size of each patrol" class="sub has-text-danger" @click="patSizeDec"><span>&laquo;</span></span>'
    )
  );
  station.append(
    $('<span class="current patSizeLabel">{{ f.patrol_size }}</span>')
  );
  station.append(
    $(
      '<span role="button" aria-label="increase size of each patrol" class="add has-text-success" @click="patSizeInc"><span>&raquo;</span></span>'
    )
  );

  station.append(
    $(
      `<span class="hireLabel"><button v-show="g.mercs" class="button merc" @click="hire" :aria-label="hireLabel()">${loc(
        "civics_garrison_hire_mercenary"
      )}</button></span>`
    )
  );
  ...
  bunks.append(
    $(`<span class="has-text-warning">${loc("civics_garrison")}: </span>`)
  );
  let soldier_title =
  ...
  bunks.append(
    $(
      `<span><span class="soldier">${soldier_title}</span> <span v-html="$options.filters.stationed(g.workers)"></span> / <span>{{ g.max | s_max }} | <span></span>`
    )
  );
  bunks.append(
    $(
      `<span v-show="g.crew > 0"><span class="crew">${loc(
        "civics_garrison_crew"
      )}</span> <span>{{ g.crew }} | </span></span>`
    )
  );
  bunks.append(
    $(
      `<span><span class="wounded">${loc(
        "civics_garrison_wounded"
      )}</span> <span>{{ g.wounded }}</span></span>`
    )
  );

  let color =
    global.settings.theme === "light" ? ` type="is-light"` : ` type="is-dark"`;
  let reports = $(`<div></div>`);
  station.append(reports);
  reports.append(
    $(
      `<b-checkbox class="patrol" v-model="f.notify" true-value="Yes" false-value="No"${color}>${loc(
        "fortress_patrol_reports"
      )}</b-checkbox>`
    )
  );
  reports.append(
    $(
      `<b-checkbox class="patrol" v-model="f.s_ntfy" true-value="Yes" false-value="No"${color}>${loc(
        "fortress_surv_reports"
      )}</b-checkbox>`
    )
  );
  reports.append(
    $(
      `<b-checkbox class="patrol" v-model="f.nocrew"${color} v-show="s.showGalactic">${loc(
        "fortress_nocrew"
      )}</b-checkbox>`
    )
  );
  ...
  if (full) {
    fort.append(
      $(
        `<div class="training"><span>${loc("civics_garrison_training")} - ${loc(
          "arpa_to_complete"
        )} {{ g.rate, g.progress | trainTime }}</span><button class="button observe right" @click="observation">${loc(
          "hell_observation_button"
        )}</button> <progress class="progress" :value="g.progress" max="100">{{ g.progress }}%</progress></div>`
      )
    );
  }
  ...
  vBind({
    ...
    methods: {
      ...
      aLast() {
        ...
        if (global.portal.fortress.garrison > min) {
          ...
          if (
            ...
            global.portal.fortress.patrols = Math.floor(
              global.portal.fortress.garrison /
      ...
      patSizeInc() {
        let inc = keyMultiplier();
        if (
          global.portal.fortress.patrol_size < global.portal.fortress.garrison
        ) {
    ...
    popover(
      ...
      function () {
        switch (k) {
          ...
          case "patLabel":
            return loc("fortress_patrol_desc", [
              global.portal.fortress.patrols,
            ]);
          case "patSizeLabel":
            return loc("fortress_patrol_size_desc", [
              global.portal.fortress.patrol_size,
            ]);
          case "hireLabel":
            return fortressData("hireLabel");
...
function fortressDefenseRating(v) {
  let army =
    v - global.portal.fortress.patrols * global.portal.fortress.patrol_size;
  if (p_on["soul_forge"]) {
    let forge = soulForgeSoldiers();
    if (forge <= army) {
      army -= forge;
  ...
  if (p_on["war_droid"]) {
    let droids =
      p_on["war_droid"] - global.portal.fortress.patrols > 0
        ? p_on["war_droid"] - global.portal.fortress.patrols
        : 0;
    army += global.tech["hdroid"] ? jobScale(droids * 2) : jobScale(droids);
  }
  let turret = global.tech["turret"]
    ? global.tech["turret"] >= 2
      ? 70
      : 50
    : 35;
  return (
    Math.round(armyRating(army, "hellArmy", wounded)) +
    (p_on["turret"] ? p_on["turret"] * turret : 0)
  );
}
...
function casualties(demons, pat_armor, ambush, report) {
  let casualties =
    Math.round(
      Math.log2(demons / global.portal.fortress.patrol_size / (pat_armor || 1))
    ) - Math.rand(0, pat_armor);
  let dead = 0;
  if (casualties > 0) {
...
export function bloodwar() {
  ...
  if (global.tech["portal"] >= 3 && p_on["war_drone"]) {
    ...
    for (let i = 0; i < p_on["war_drone"]; i++) {
      ...
      if (Math.rand(0, global.portal.fortress.threat) >= Math.rand(0, 999)) {
        let demons = Math.rand(
          Math.floor(global.portal.fortress.threat / 50),
          Math.floor(global.portal.fortress.threat / 10)
        );
        let killed =
          global.tech.portal >= 7 ? Math.rand(50, 125) : Math.rand(25, 75);
        if (demons < killed) {
          killed = demons;
        }
  ...
  let game_base =
    ...
    global.stats.achieve.technophobe.l >= 5
      ? 9000
      : 10000;
  ...
  if (global.race["ghostly"]) {
    gem_chance = Math.round(
      gem_chance * ((100 - traits.ghostly.vars()[2]) / 100)
    );
  }
  ...
  if (wendFathom > 0) {
    gem_chance = Math.round(
      gem_chance * ((100 - traits.ghostly.vars(1)[2] * wendFathom) / 100)
    );
  }
  ...
  for (let i = 0; i < global.portal.fortress.patrols; i++) {
    ...
    let hurt =
      brkpnt > (1 / global.portal.fortress.patrols) * i
        ? Math.ceil(wounded)
        : Math.floor(wounded);
    if (Math.rand(0, global.portal.fortress.threat) >= Math.rand(0, 999)) {
      ...
      if (global.race["blood_thirst"]) {
        global.race["blood_thirst_count"] += Math.rand(
          0,
          Math.ceil(demons / 10)
        );
      ...
      if (Math.rand(0, odds) === 0) {
        ...
        dead += casualties(
          Math.round(demons * (1 + Math.random() * 3)),
          0,
          true,
          patrol_report
        );
        ...
        if (
          ...
          global.race.ocularPowerConfig.ds += Math.round(
            killed * traits.ocular_power.vars()[1]
          );
        }
      } else {
          ...
          for (let j = 0; j < chances; j++) {
            if (Math.rand(0, gem_chance) === 0) {
              ...
              if (!global.resource.Soul_Gem.display) {
                ...
                messageQueue(loc("portal_first_gem"), "info", false, [
                  "progress",
                  "hell",
                ]);
              }
  ...
  let garrison_size = global.portal.fortress.nocrew
    ? global.civic.garrison.workers - global.civic.garrison.crew
    : global.civic.garrison.workers;
  ...
  if (
    ...
    global.portal.fortress.patrols = Math.floor(
      global.portal.fortress.garrison / global.portal.fortress.patrol_size
    );
  ...
  if (dead > 0 && global.portal.fortress.notify === "Yes") {
    if (revive > 0) {
      messageQueue(
        loc("fortress_patrol_casualties_revive", [dead, revive]),
        false,
        false,
        ["hell"]
      );
    } else {
      messageQueue(loc("fortress_patrol_casualties", [dead]), false, false, [
        "hell",
      ]);
    }
  }
  ...
  if (
    global.portal.fortress.siege <= 900 &&
    global.portal.fortress.garrison > 0 &&
    1 > Math.rand(0, global.portal.fortress.siege)
  ) {
    let siege_report = {
      destroyed: false,
      damage: 0,
    ...
    if (destroyed) {
      ...
      messageQueue(loc("fortress_sieged", [killed, damage]), false, false, [
        "hell",
      ]);
    }
  ...
  if (global.portal.fortress.threat < 10000) {
    ...
    let demon_spawn = Math.rand(
      Math.round(10 * influx),
      Math.round(50 * influx)
    );
  ...
  // Surveyor threats
  if (
    global.civic.hell_surveyor.display &&
    global.civic.hell_surveyor.workers > 0
  ) {
    let divisor = 1000;
      ...
      if (dead > 0) {
        ...
        if (global.portal.fortress.s_ntfy === "Yes") {
          if (dead === 1) {
            ...
            messageQueue(loc("fortress_eviscerated", [dead]), false, false, [
              "hell",
            ]);
          }
        }
    ...
    if (global.civic.hell_surveyor.workers > 0 && drone_kills > 0) {
      ...
      for (let i = 0; i < global.civic.hell_surveyor.workers; i++) {
        ...
        let max_search_chance = Math.round(
          drone_kills_left / (global.civic.hell_surveyor.workers - i)
        );
  ...
  if (
    global.stats.dkills >= 1000000 &&
    global.tech["gateway"] &&
    !global.tech["hell_pit"]
  ) {
    global.tech["hell_pit"] = 1;
    global.settings.portal.pit = true;
    messageQueue(loc("portal_hell_pit_found"), "info", false, [
      "progress",
      "hell",
    ]);
    renderFortress();
  ...
  if (global.tech["hell_pit"]) {
    if (forgeOperating && global.tech.hell_pit >= 5 && p_on["soul_attractor"]) {
      ...
      let souls =
        p_on["soul_attractor"] * Math.rand(40 + attract, 120 + attract);
    ...
    if (
      ...
      global.tech.asphodel >= 2 &&
      support_on["ectoplasm_processor"]
    ) {
      let attract = global.blood["attract"] ? global.blood.attract * 5 : 0;
      let souls =
        global.civic.ghost_trapper.workers *
        Math.rand(150 + attract, 250 + attract);
      if (
    ...
    if (forgeOperating && global.tech["hell_gun"] && p_on["gun_emplacement"]) {
      ...
      for (let i = 0; i < p_on["gun_emplacement"]; i++) {
        ...
        let kills =
          global.tech.hell_gun >= 2 ? Math.rand(35, 75) : Math.rand(20, 40);
        gunKills += kills;
    ...
    if (forgeOperating) {
      ...
      if (
        ...
        global.race.ocularPowerConfig.ds += Math.round(
          forgeKills * traits.ocular_power.vars()[1]
        );
      }
      let forge_base =
        global.stats.achieve["technophobe"] &&
  ...
  if (
    global.eden.hasOwnProperty("soul_compactor") &&
  ...
  Object.keys(day_report.stats).forEach(function (stat) {
    if (["kills", "gems"].includes(stat)) {
      Object.keys(day_report.stats[stat]).forEach(function (subStat) {
        ...
        global.portal.observe.stats.total[stat][subStat] +=
          day_report.stats[stat][subStat];
        global.portal.observe.stats.period[stat][subStat] +=
          day_report.stats[stat][subStat];
      });
    } else {
  ...
  Object.keys(global.portal.observe.graphs).forEach(function (id) {
    if (!!document.getElementById(global.portal.observe.graphs[id].chartID)) {
      ...
      hell_graphs[id].data.forEach(function (dataPoint) {
        newData.push(
          dataPoint.length === 3
            ? global.portal.observe.stats[dataPoint[0]][dataPoint[1]][
                dataPoint[2]
              ]
            : global.portal.observe.stats[dataPoint[0]][dataPoint[1]]
        );
...
export function hellSupression(area, val, wiki) {
  ...
  switch (area) {
    case "ruins": {
      let guard_posts_on = wiki
        ? global.portal?.guard_post?.on ?? 0
        : p_on["guard_post"];
      let army = val || jobScale(guard_posts_on);
      let arc =
        (wiki ? global.portal?.arcology?.on ?? 0 : p_on["arcology"]) * 75;
    ...
    case "gate": {
      ...
      let turret =
        (wiki ? global.portal?.gate_turret?.on ?? 0 : p_on["gate_turret"]) *
        100;
      if (global.race["holy"]) {
...
export function mechCost(size, infernal) {
  ...
  switch (size) {
    case "small":
      {
        let baseCost =
          global.blood["prepared"] && global.blood.prepared >= 2
            ? 50000
            : 75000;
    ...
    case "collector":
      {
        let baseCost =
          global.blood["prepared"] && global.blood.prepared >= 2 ? 8000 : 10000;
        cost = infernal ? baseCost * 2.5 : baseCost;
        soul = 1;
      }
      break;
...
export function drawMechLab() {
  if (
    !global.settings.tabLoad &&
    (global.settings.civTabs !== 2 || global.settings.govTabs !== 4)
  ) {
    return;
  }
  ...
  if (global.portal.hasOwnProperty("mechbay") && global.settings.showMechLab) {
    ...
    let title = $(
      `<div><span class="has-text-caution">${loc(
        `portal_mech_assembly`
      )}</span> - <span>{{ b.size | slabel }} {{ b.chassis | clabel }}</span></div>`
    );
    assemble.append(title);

    title.append(
      ` | <span><span class="has-text-warning">${loc(
        "portal_mech_bay_space"
      )}</span>: {{ m.bay }} / {{ m.max }}</span>`
    );
    title.append(
      ` | <span><span class="has-text-warning">${loc(
        "portal_mech_sup_avail"
      )}</span>: {{ p.supply | round }} / {{ p.sup_max }}</span>`
    );

    let infernal =
      global.blood["prepared"] && global.blood.prepared >= 3
        ? `<b-checkbox class="patrol" v-model="b.infernal">${loc(
            "portal_mech_infernal"
          )} (${loc("portal_mech_infernal_effect", [25])})</b-checkbox>`
        : ``;
    assemble.append(
      `<div><span class="has-text-warning">${loc(
        `portal_mech_space`
      )}</span> <span class="has-text-danger">{{ b.size | bay }}</span> | <span class="has-text-warning">${loc(
        `portal_mech_cost`
      )}</span> <span class="has-text-danger">{{ b.size | price }}</span> | <span class="has-text-warning">${loc(
        `portal_mech_soul`,
        [global.resource.Soul_Gem.name]
      )}</span> <span class="has-text-danger">{{ b.size | soul }}</span>${infernal}</div>`
    );
    ...
    let sizes = ``;
    ["small", "medium", "large", "titan", "collector"].forEach(function (
      size,
      idx
    ) {
      sizes += `<b-dropdown-item aria-role="listitem" v-on:click="setSize('${size}')" class="size r0 a${idx}" data-val="${size}">${loc(
        `portal_mech_size_${size}`
      )}</b-dropdown-item>`;
    });
    ...
    let chassis = ``;
    ["wheel", "tread", "biped", "quad", "spider", "hover"].forEach(function (
      val,
      idx
    ) {
      chassis += `<b-dropdown-item aria-role="listitem" v-on:click="setType('${val}')" class="chassis r0 a${idx}" data-val="${val}">${loc(
        `portal_mech_chassis_${val}`
      )}</b-dropdown-item>`;
    });
    ...
    for (let i = 0; i < 4; i++) {
      ...
      [
        ...
        weapons += `<b-dropdown-item aria-role="listitem" v-on:click="setWep('${val}',${i})" class="weapon r${i} a${idx}" data-val="${val}">${loc(
          `portal_mech_weapon_${val}`
        )}</b-dropdown-item>`;
      });
      ...
      options.append(`<b-dropdown :triggers="['hover', 'click']" aria-role="list" v-show="vis(${i})">
                <button class="button is-info" slot="trigger">
                    <span>${loc(
                      `portal_mech_weapon`
                    )}: {{ b.hardpoint[${i}] || 'laser' | wlabel }}</span>
                    <b-icon icon="menu-down"></b-icon>
                </button>${weapons}
            </b-dropdown>`);
    }
    ...
    for (let i = 0; i < e_cap; i++) {
      ...
      options.append(`<b-dropdown :triggers="['hover', 'click']" aria-role="list" v-show="eVis(${i})">
                <button class="button is-info" slot="trigger">
                    <span>${loc(
                      `portal_mech_equipment`
                    )}: {{ b.equip[${i}] || 'shields' | equipment }}</span>
                    <b-icon icon="menu-down"></b-icon>
                </button>${equip}
            </b-dropdown>`);
    }
    ...
    assemble.append(
      `<div class="mechAssemble"><button class="button is-info" slot="trigger" v-on:click="build()"><span>${loc(
        "portal_mech_construct"
      )}</span></button></div>`
    );
    ...
    vBind({
      ...
      methods: {
        build() {
          let costs = mechCost(
            global.portal.mechbay.blueprint.size,
            global.portal.mechbay.blueprint.infernal
          );
        ...
        setSize(s) {
          ...
          if (s === "collector") {
            ...
            if (global.portal.mechbay.blueprint.hardpoint.length === 1) {
              global.portal.mechbay.blueprint.hardpoint.push(
                global.portal.mechbay.blueprint.hardpoint.includes("laser")
                  ? "plasma"
                  : "laser"
              );
            }
          ...
          switch (s) {
            case "small":
              ...
              global.portal.mechbay.blueprint.equip.length = global.blood[
                "prepared"
              ]
                ? 1
                : 0;
        ...
        vis(hp) {
          ...
          if (
            hp === 0 ||
            (global.portal.mechbay.blueprint.size === "large" && hp < 2) ||
            global.portal.mechbay.blueprint.size === "titan"
          ) {
            return true;
          }
          return false;
...
function drawMechs() {
  ...
  list.append(`
      <div v-for="(mech, index) of mechs" :key="index" class="mechRow" :class="index < active ? '' : 'inactive-row' ">
        ...
        <span class="has-text-caution">{{ mech.infernal ? "${loc(
          "portal_mech_infernal"
        )} " : "" }}{{ mech | size }} {{ mech | chassis }}</span>
    ...
    methods: {
      scrap(id) {
        if (global.portal.mechbay.mechs[id]) {
          let costs = mechCost(
            global.portal.mechbay.mechs[id].size,
            global.portal.mechbay.mechs[id].infernal
          );
  ...
  $(`#mechList .scrap`).each(function (i, node) {
    popover(
      ...
      function () {
        let costs = mechCost(
          global.portal.mechbay.mechs[i].size,
          global.portal.mechbay.mechs[i].infernal
        );
        return loc(`portal_mech_scrap_refund`, [
          Math.floor(costs.c / 3),
          Math.floor(costs.s / 2),
        ]);
      },
      {
        elm: node,
...
function dragMechList() {
  ...
  Sortable.create(el, {
    onEnd(e) {
      ...
      e.from.insertBefore(
        e.item,
        items[e.oldIndex + (e.oldIndex > e.newIndex)]
      );
...
export function genSpireFloor() {
  ...
  if (global.portal.spire.count >= 10) {
    ...
    if (global.portal.spire.count >= 25 && global.portal.spire.count <= 100) {
      ...
      global.portal.spire.count > 100 &&
      global.portal.spire.count <= 250
    ) {
      assignValidStatus(effects[Math.floor(seededRandom(0, effects.length))]);
      ...
      global.portal.spire.count > 250 &&
      global.portal.spire.count <= 1000
    ) {
      assignValidStatus(effects[Math.floor(seededRandom(0, effects.length))]);
      assignValidStatus(effects[Math.floor(seededRandom(0, effects.length))]);
  ...
  let mobs = Object.keys(monsters).filter(function (k) {
    ...
    if (
      exclude.some((i) =>
        Object.keys(global.portal.spire.status).includes(i)
      ) ||
      exclude.includes(global.portal.spire.type)
    ) {
      return false;
    }
    return true;
...
function assignValidStatus(effect) {
  if (
    global.portal.spire.status["freeze"] ||
    global.portal.spire.status["hot"]
  ) {
    if (effect !== "freeze" && effect !== "hot") {
      global.portal.spire.status[effect] = true;
    }
  } else if (
    global.portal.spire.status["rain"] ||
    global.portal.spire.status["hail"]
  ) {
...
function terrainRating(mech, rating, effects) {
  ...
  if (mech.size !== "small" && rating < 1) {
    rating +=
      (effects.includes("fog") || effects.includes("dark") ? 0.005 : 0.01) *
    ...
    case "tar":
      {
        if (mech.chassis !== "quad") {
          rating =
            mech.chassis === "tread" || mech.chassis === "wheel" ? 0.5 : 0.75;
        }
      }
      break;
    case "steam":
    ...
    case "wheel":
      {
        switch (terrain) {
          case "sand":
            terrainFactor = ["small", "medium"].includes(mech.size)
              ? 0.9
              : 0.85;
            break;
          case "swamp":
...
export function mechRating(mech, boss) {
  ...
  if (
    mech.hasOwnProperty("infernal") &&
    mech.infernal &&
    global.blood["prepared"] &&
    global.blood.prepared >= 3
  ) {
    rating *= 1.25;
  }
  ...
  if (boss) {
    if (
      global.stats.achieve["gladiator"] &&
    ...
    if (
      global.stats.spire.hasOwnProperty(affix) &&
      global.stats.spire[affix].hasOwnProperty("dlstr")
    ) {
...
function drawHellAnalysis() {
  ...
  let totalAnal = $(
    `<div id="hellAnalysisTotal" class="analysisColumn"></div>`
  );
  let partialAnal = $(
    `<div id="hellAnalysisPeriod" class="analysisColumn"></div>`
  );
  ...
  bd_settings.append(`
        ...
        <div>
            <b-checkbox v-model="s.expanded">${loc(
              "hell_analysis_expanded"
            )}</b-checkbox>
            <b-checkbox v-model="s.average">${loc(
              "hell_analysis_average"
            )}</b-checkbox>
            <b-checkbox v-show="r.hyper || r.slow" v-model="s.hyperSlow">${loc(
              "hell_analysis_hyperSlow"
            )}</b-checkbox>
        </div>
        <div>
            <b-radio v-model="s.display" native-value="game_days">${loc(
              "hell_analysis_time_game_days"
            )}</b-radio>
            <b-radio v-model="s.display" native-value="seconds">${loc(
              "hell_analysis_time_seconds"
            )}</b-radio>
            <b-radio v-model="s.display" native-value="minutes">${loc(
              "hell_analysis_time_minutes"
            )}</b-radio>
            <b-radio v-model="s.display" native-value="hours">${loc(
              "hell_analysis_time_hours"
            )}</b-radio>
            <b-radio v-model="s.display" native-value="days">${loc(
              "hell_analysis_time_days"
            )}</b-radio>
        </div>
    `);
  ...
  let calcAverage = function (num, gameDays, units) {
    ...
    return loc("hell_analysis_time_average", [
      num,
      loc(`hell_analysis_time_${units}_abbr`),
    ]);
  };
  ...
  let drawStats = function (id, type) {
    ...
    elem.append(`
            <div><h2 class="has-text-warning">${loc(
              "hell_analysis_" + type
            )}</h2>${
      type === "period"
        ? '<h2 id="resetHellObservation" class="text-button has-text-danger" @click="resetObservations()">{{ | resetLabel }}</h2>'
        : ""
    }</div>
            <div><h2 class="has-text-alert">{{ st.${type}.start | startLabel }}</h2></div>
            <div><h2>{{ st.${type}.days, s.display | time }}</h2></div>
            <div><h2>{{ st.${type}.kills, 'kills', s.average | genericMulti }}</h2><h2 class="text-button has-text-advanced" aria-label="${loc(
      "hell_analysis_toggle_bd",
      [loc("hell_analysis_toggle_bd_kills")]
    )}" @click="toggleDropdown('dropKills')">{{ s.dropKills | dropdownLabel }}</h2></div>
            ...
            <div v-show="sg.display"><h2>{{ st.${type}.gems, 'gems', s.average | genericMulti }}</h2><h2 class="text-button has-text-advanced" aria-label="${loc(
      "hell_analysis_toggle_bd",
      [loc("resource_Soul_Gem_name")]
    )}" @click="toggleDropdown('dropGems')">{{ s.dropGems | dropdownLabel }}</h2></div>
    ...
    vBind({
      ...
      methods: {
        resetObservations() {
          Object.keys(global.portal.observe.stats.period).forEach(function (
            stat
          ) {
            if (["kills", "gems"].includes(stat)) {
              Object.keys(global.portal.observe.stats.period[stat]).forEach(
                function (subStat) {
                  global.portal.observe.stats.period[stat][subStat] = 0;
                }
              );
            } else if (stat === "start") {
              global.portal.observe.stats.period.start = {
        ...
        toggleDropdown(type) {
          global.portal.observe.settings[type] =
            !global.portal.observe.settings[type];
        },
      },
      filters: {
        generic(num, name, average) {
          if (!average) {
            let val = sizeApproximation(
              num,
              5,
              global.portal.observe.settings.expanded
            );
            return loc("hell_analysis_number_display", [
              loc(`hell_analysis_${name}`),
              val,
            ]);
          }
          return loc("hell_analysis_number_display", [
            ...
            calcAverage(
              num,
              global.portal.observe.stats[type].days,
              global.portal.observe.settings.display
            ),
          ]);
        },
        genericSub(num, name, average) {
          if (!average) {
            ...
            return (
              "ᄂ" +
              loc("hell_analysis_number_display", [
                loc(`hell_analysis_${name}`),
                val,
              ])
            );
        ...
        time(days, units) {
          if (
            units !== "game_days" &&
            global.portal.observe.settings.hyperSlow
          ) {
            if (global.race["slow"]) {
          ...
          let formattedTime = sizeApproximation(
            days,
            global.portal.observe.settings.expanded ? 8 : 5,
            global.portal.observe.settings.expanded
          );
          return loc("hell_analysis_time", [
            loc(`hell_analysis_time_${units}`),
            formattedTime,
          ]);
        },
        resetLabel() {
  ...
  stats.append(graphs);
  graphs.append(
    `<div><h2 id="hellGraphCreator" class="text-button has-text-success" @click="createGraph()">${loc(
      "hell_graph_create"
    )}</h2></div>`
  );
      ...
      createGraph() {
        ...
        let checkExist = setInterval(function () {
          if ($("#modalBox").length > 0) {
            ...
            var body = $(
              '<div id="specialModal" class="modalBody vscroll"></div>'
            );
            ...
            creator.append(`
                            <div><h2 class="has-text-warning">${loc(
                              "hell_graph_name"
                            )}</h2> <b-input v-model="s.name" :input="nameUpdate(s.name)"></b-input></div>
                        `);
            creator.append(`
                            <div>
                                <div>
                                    <h2 class="has-text-warning">${loc(
                                      "hell_graph_type"
                                    )}</h2>
                                </div>
                                <div>
                                    <b-radio v-model="s.chartType" native-value="pie" @click.native="dataOptions('pie')">${loc(
                                      "hell_graph_pie"
                                    )}</b-radio>
                                </div>
                            </div>
            ...
            Object.keys(global.portal.observe.stats).forEach(function (
              dataSet
            ) {
              ["kills", "gems"].forEach(function (group) {
                dataRegion.append(`<div>
                                    <b-radio v-show="${
                                      group === "gems" ? "sg.display && " : ""
                                    }s.showGroups" v-model="s.radioFake" native-value="${dataSet}${group}" @click.native="setData('${dataSet}','${group}')">${loc("hell_graph_datapoint", [loc(`hell_analysis_${dataSet}`), loc(`hell_analysis_${group}`)])}</b-radio>
                                </div>`);
              });
            });
            ...
            creator.append(`
                            <div>
                                ...
                                <div>
                                    <button class="button" @click="createGraph()">${loc(
                                      "hell_graph_create"
                                    )}</button>
                                </div>
                            </div>
              ...
              methods: {
                ...
                createGraph() {
                  ...
                  switch (settings.chartType) {
                    case "pie":
                      Object.keys(
                        global.portal.observe.stats[settings.data[0]][
                          settings.data[1]
                        ]
                      ).forEach(function (dataPoint) {
                        graphLabels.push(
                          loc(`hell_analysis_${settings.data[1]}_${dataPoint}`)
                        );
                        graphData.push([
                          settings.data[0],
                          settings.data[1],
                  ...
                  let graphID = newGraph(
                    settings.chartName,
                    settings.chartType,
                    graphLabels,
                    graphData,
                    { title: settings.chartName }
                  );
...
function drawGraph(info, graphInfo) {
  ...
  let chartCont = $(
    `<div id="graph-${id}-container" class="graphContainer"></div>`
  );
  ...
  switch (graphInfo.type) {
    case "pie":
      hell_graphs[id].graph = drawPieChart(
        newChart,
        graphInfo.labels,
        graphInfo.data,
        graphInfo.settings
...
function drawPieChart(info, labels, data, settings) {
  ...
  data.forEach(function (dataPath) {
    drawData.push(
      dataPath.length === 3
        ? global.portal.observe.stats[dataPath[0]][dataPath[1]][dataPath[2]]
        : global.portal.observe.stats[dataPath[0]][dataPath[1]]
    );
  });
...
function drawHellReports() {
  ...
  reportListSection.append(`<div id="hellReportLogTitle"><h2 class="has-text-info">${loc(
    "hell_report_log"
  )}</h2><span class="refresh" @click="updateList()" aria-label="${loc(
    "hell_report_log_refresh_aria"
  )}">
  ...
  reports.append(
    $(
      `<div id="hellReportDisplay" class="reportDisplay is-vertical vscroll"></div>`
    )
  );
  ...
  if (Object.keys(hell_reports).length) {
    ...
    recentDay.day = Object.keys(
      hell_reports[`year-${recentDay.year}`]
    )[0].split("-")[1];
  }
  ...
  let updateList = function (startYear, startDay) {
    if (purgeReports(true)) {
      ...
      startDay = Object.keys(hell_reports[`year-${recentDay.year}`])[0].split(
        "-"
      )[1];
    }
    for (startYear; startYear < global.city.calendar.year; startYear++) {
      for (startDay; startDay <= orbitLength(); startDay++) {
        let gemString = "";
        let gemCount =
          hell_reports[`year-${startYear}`][`day-${startDay}`].foundGems;
        if (gemCount) {
          gemString = `<span class="has-text-advanced" aria-label="${loc(
            `hell_report_log_soul_gem_aria`
          )}">${
            gemCount >= 5 ? `&#9830x${gemCount}` : "&#9830".repeat(gemCount)
          }</span>`;
        }
        list =
          `
                    <div class="text-button"><span @click="reportLoad('${startYear}','${startDay}')">${
            loc("year") + " " + startYear + " | " + loc("day") + " " + startDay
          }</span>${gemString}</div>
                ` + list;
      }
      startDay = 1;
  ...
  let loadReport = function (year, day) {
    ...
    statsBar.append(
      `<div><h2 class="has-text-info">${loc("hell_report_log_stats", [
        year,
        day,
      ])}</h2></div>`
    );
    statsBar.append(`<div>
            ...
            <h2 v-show="g.display">${loc("hell_report_log_stats_gems", [
              gems,
            ])}</h2>
            <h2>${loc("hell_report_log_stats_wounded", [
              curr_report.stats.wounded,
            ])}</h2>
            <h2>${loc("hell_report_log_stats_died", [
              curr_report.stats.died,
            ])}</h2>
        </div>`);

    info.append(
      `<div><h2 class="has-text-info">${loc("hell_report_log_report", [
        year,
        day,
      ])}</h2></div>`
    );
    info.append(
      `<p class="has-text-danger">${loc("hell_report_log_start", [
        curr_report.start,
      ])}</p>`
    );
    ...
    if (curr_report.soul_attractors) {
      info.append(
        `<p>${loc("hell_report_log_soul_attractors", [
          curr_report.soul_attractors,
        ])}</p>`
      );
    }
    if (curr_report.ghost_trappers) {
      info.append(
        `<p>${loc("hell_report_log_ghost_trappers", [
          curr_report.ghost_trappers,
        ])}</p>`
      );
    }
    if (curr_report.soul_forge) {
      let displayText = $(`<p></p>`);
      displayText.append(
        `<span>${loc("hell_report_log_soul_forge", [
          curr_report.soul_forge.kills,
        ])}</span>`
      );
      if (curr_report.soul_forge.gem) {
        displayText.append(
          `<span class="has-text-success">${loc("hell_report_log_soul_find", [
            global.resource.Soul_Gem.name,
            1,
          ])}</span>`
        );
      ...
      if (curr_report.soul_forge.gem_craft) {
        displayText.append(
          `<span class="has-text-success">${loc("hell_report_log_soul_craft", [
            curr_report.soul_forge.corrupt
              ? loc("resource_Corrupt_Gem_name")
              : global.resource.Soul_Gem.name,
          ])}</span>`
        );
      }
    ...
    if (curr_report.stats.gems.compactor) {
      ...
      if (curr_report.stats.gems.compactor) {
        displayText.append(
          `<span class="has-text-success">${loc("hell_report_log_compactor", [
            curr_report.stats.gems.compactor,
            global.resource.Soul_Gem.name,
          ])}</span>`
        );
      }
    ...
    if (curr_report.drones) {
      Object.keys(curr_report.drones).forEach(function (num) {
        ...
        let name = loc("hell_report_log_obj_counter", [
          loc("portal_war_drone_title"),
          num,
        ]);
        if (drone.encounter) {
          info.append(
            `<p>${loc("hell_report_log_encounter", [name, drone.kills])}</p>`
          );
        } else {
          info.append(
            `<p class="has-text-warning">${loc(
              "hell_report_log_encounter_fail",
              [name]
            )}</p>`
          );
        }
    ...
    if (curr_report.patrols) {
      Object.keys(curr_report.patrols).forEach(function (num) {
        ...
        let name = loc("hell_report_log_obj_counter", [
          loc("hell_report_log_patrol"),
          num,
        ]);
        name = patrol.droid
          ? loc("hell_report_log_patrol_droid", [name])
          : name;
        if (patrol.encounter) {
          let displayText = $(`<p></p>`);
          if (patrol.ambush) {
            displayText.append(
              `<span class="has-text-warning">${loc(
                "hell_report_log_patrol_ambush",
                [name, patrol.kills]
              )}</span>`
            );
          } else {
            displayText.append(
              `<span>${loc("hell_report_log_encounter", [
                name,
                patrol.kills,
              ])}</span>`
            );
          ...
          if (patrol.wounded) {
            displayText.append(
              `<span class="has-text-danger">${
                patrol.wounded > 1
                  ? loc("hell_report_log_patrol_wounded_plural", [
                      patrol.wounded,
                    ])
                  : loc("hell_report_log_patrol_wounded")
              }</span>`
            );
          }
          if (patrol.died) {
            displayText.append(
              `<span class="has-text-danger">${
                patrol.died > 1
                  ? loc("hell_report_log_patrol_killed_plural", [patrol.died])
                  : loc("hell_report_log_patrol_killed")
              }</span>`
            );
          }
          if (patrol.gem > 0) {
            displayText.append(
              `<span class="has-text-success">${loc(
                "hell_report_log_soul_find",
                [global.resource.Soul_Gem.name, patrol.gem]
              )}</span>`
            );
          }
          info.append(displayText);
    ...
    if (curr_report.surveyor_finds) {
      Object.keys(curr_report.surveyor_finds).forEach(function (num) {
        ...
        let name = loc("hell_report_log_obj_counter", [
          loc("job_hell_surveyor"),
          num,
        ]);

        let displayText = $(`<p></p>`);
        displayText.append(
          `<span>${loc("hell_report_log_search", [
            name,
            surveyor.bodies,
          ])}</span>`
        );
        if (surveyor.gem > 0) {
          displayText.append(
            `<span class="has-text-success">${loc(
              "hell_report_log_soul_search",
              [global.resource.Soul_Gem.name, surveyor.gem]
            )}</span>`
          );
        }
        info.append(displayText);
    ...
    if (curr_report.revived) {
      info.append(
        `<p>${
          curr_report.revived > 1
            ? loc("hell_report_log_revived_plural", [curr_report.revived])
    ...
    if (curr_report.patrols_lost) {
      info.append(
        `<p class="has-text-danger">${loc("hell_report_log_patrols_lost", [
          curr_report.patrols_lost,
        ])}</p>`
      );
    }
    if (curr_report.siege) {
      if (curr_report.siege.destroyed) {
        info.append(
          `<p class="has-text-danger">${loc("hell_report_log_siege", [
            curr_report.siege.surveyors,
            curr_report.siege.soldiers,
            curr_report.siege.kills,
          ])}</p>`
        );
      } else {
        info.append(
          `<p class="has-text-warning">${loc("hell_report_log_siege_fail", [
            curr_report.siege.damage,
            curr_report.siege.kills,
          ])}</p>`
        );
      }
    ...
    if (curr_report.demons) {
      info.append(
        `<p class="has-text-danger">${loc("hell_report_log_demons", [
          curr_report.demons,
        ])}</p>`
      );
    }
    if (curr_report.surveyors) {
      info.append(
        `<p class="has-text-danger">${
          curr_report.surveyors > 1
            ? loc("hell_report_log_surveyors_plural", [curr_report.surveyors])
            : loc("hell_report_log_surveyors")
        }</p>`
      );
    }
    if (curr_report.gun_emplacements) {
      Object.keys(curr_report.gun_emplacements).forEach(function (num) {
        ...
        let name = loc("hell_report_log_obj_counter", [
          loc("portal_gun_emplacement_title"),
          num,
        ]);
        displayText.append(
          $(
            `<span>${loc("hell_report_log_misc_kills", [
              name,
              gun.kills,
              loc("portal_pit_name"),
            ])}</span>`
...
function purgeReports(refresh) {
  if (!!!document.getElementById(`hellReportList`) || refresh) {
    ...
    let approx =
      (Object.keys(hell_reports).length - 1) * orbitLength() +
      global.city.calendar.day;

    if (approx > threshold) {
      let firstYear = Object.keys(
        hell_reports[Object.keys(hell_reports)[0]]
      ).length;
