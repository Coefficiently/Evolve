...
const spaceProjects = {
  spc_home: {
        ...
        if (payCosts($(this)[0])) {
          if (global.race["truepath"]) {
            ...
            if (Math.floor(seededRandom(0, sabotage)) !== 0) {
              messageQueue(
                loc("space_home_test_launch_action_fail"),
                "danger",
                false,
                ["progress"]
              );
          ...
          messageQueue(loc("space_home_test_launch_action"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    satellite: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("satellite", offset, 72000, 1.22);
        },
        Knowledge(offset) {
          return spaceCostMultiplier("satellite", offset, 28000, 1.22);
        },
        Oil(offset, wiki) {
          return spaceCostMultiplier(
            "satellite",
            offset,
            fuel_adjust(3200, false, wiki),
            1.22
          );
        },
        Alloy(offset) {
          return spaceCostMultiplier("satellite", offset, 8000, 1.22);
        },
      },
      effect() {
        let knowledge =
        ...
        if (
          ...
          let ratio =
            global.tech["particles"] && global.tech["particles"] >= 3 ? 5 : 10;
          knowledge *= global.tech["supercollider"] / ratio + 1;
        }
        let label = global.race["cataclysm"]
          ? loc("space_moon_observatory_title")
          : global.race["orbit_decayed"]
          ? loc("city_university")
          : wardenLabel();
        let amount = global.race["cataclysm"]
          ? 25
          : global.race["orbit_decayed"]
          ? 12
          : 4;
        let synergy = `<div>${loc("space_home_satellite_effect2", [
          label,
          amount,
        ])}</div>`;
        return `<div>${loc("plus_max_resource", [
          knowledge,
          loc("resource_Knowledge_name"),
        ])}</div>${synergy}<div>${loc("space_home_satellite_effect3", [
          global.civic.scientist
            ? global.civic.scientist.name
            : loc("job_scientist"),
        ])}</div>`;
      },
      action() {
    ...
    gps: {
      ...
      desc() {
        if (
          global.space.hasOwnProperty("gps") &&
          global.space["gps"].count < 4
        ) {
          return `<div>${loc(
            "space_home_gps_desc"
          )}</div><div class="has-text-special">${loc(
            "space_home_gps_desc_req"
          )}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("gps", offset, 75000, 1.18);
        },
        Knowledge(offset) {
          return spaceCostMultiplier("gps", offset, 50000, 1.18);
        },
        Copper(offset) {
        ...
        Oil(offset, wiki) {
          return spaceCostMultiplier(
            "gps",
            offset,
            fuel_adjust(3500, false, wiki),
            1.18
          );
        },
        Titanium(offset) {
          return spaceCostMultiplier("gps", offset, 8000, 1.18);
        },
      },
      effect(wiki) {
        let count =
          (wiki?.count ?? 0) +
          (global.space.hasOwnProperty("gps") ? global.space["gps"].count : 0);
        if (count < 4) {
          return loc("space_home_gps_effect_req");
        } else {
          return `<div>${loc("space_home_gps_effect")}</div><div>${loc(
            "space_home_gps_effect2",
            [2]
          )}</div>`;
        }
      },
    ...
    propellant_depot: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("propellant_depot", offset, 55000, 1.35);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("propellant_depot", offset, 22000, 1.35);
        },
        Oil(offset, wiki) {
          return spaceCostMultiplier(
            "propellant_depot",
            offset,
            fuel_adjust(5500, false, wiki),
            1.35
          );
        },
      },
      effect() {
        let oil =
          spatialReasoning(1250) * (global.tech["world_control"] ? 1.5 : 1);
        if (global.resource["Helium_3"].display) {
          let helium =
            spatialReasoning(1000) * (global.tech["world_control"] ? 1.5 : 1);
          return `<div>${loc("plus_max_resource", [
            oil,
            loc("resource_Oil_name"),
          ])}</div><div>${loc("plus_max_resource", [
            helium,
            loc("resource_Helium_3_name"),
          ])}</div>`;
        }
        return `<div>${loc("plus_max_resource", [
          oil,
    ...
    nav_beacon: {
      ...
      title() {
        return global.race["orbit_decayed"]
          ? loc("space_home_broadcast_beacon_title")
          : loc("space_home_nav_beacon_title");
      },
      desc: `<div>${loc(
        "space_home_nav_beacon_desc"
      )}</div><div class="has-text-special">${loc("requires_power")}</div>`,
      reqs: { luna: 2 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("nav_beacon", offset, 75000, 1.32);
        },
        Copper(offset) {
          return spaceCostMultiplier("nav_beacon", offset, 38000, 1.32);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("nav_beacon", offset, 44000, 1.32);
        },
        Oil(offset, wiki) {
          return spaceCostMultiplier(
            "nav_beacon",
            offset,
            fuel_adjust(12500, false, wiki),
            1.32
          );
        },
        Iridium(offset) {
      ...
      powerBalancer() {
        return global.tech["luna"] && global.tech["luna"] >= 3
          ? [
              {
                s:
                  global.space.moon_base.s_max - global.space.moon_base.support,
              },
              {
                s:
                  global.space.spaceport.s_max - global.space.spaceport.support,
              },
            ]
          : [
              {
      ...
      effect() {
        ...
        if (global.race["orbit_decayed"] && global.tech["broadcast"]) {
          orbitEffect = `<div class="has-text-caution">${loc(
            "space_red_vr_center_effect1",
            [global.tech["broadcast"] / 2]
          )}</div>`;
        }
        let effect1 = global.race["orbit_decayed"]
          ? ""
          : `<div>${loc("space_home_nav_beacon_effect1")}</div>`;
        let effect3 =
          global.tech["luna"] >= 3
            ? `<div>${loc("space_red_spaceport_effect1", [
                planetName().red,
                1,
              ])}</div>`
            : "";
        return `${effect1}${effect3}${orbitEffect}<div class="has-text-caution">${loc(
          "space_home_nav_beacon_effect2",
          [$(this)[0].powered()]
        )}</div>`;
      },
      action() {
  ...
  spc_moon: {
    ...
    moon_mission: {
      ...
      cost: {
        Oil(offset, wiki) {
          return +fuel_adjust(12000, false, wiki).toFixed(0);
        },
      },
      effect: loc("space_moon_mission_effect"),
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("space_moon_mission_action"), "info", false, [
            "progress",
          ]);
    ...
    moon_base: {
      ...
      desc() {
        return `<div>${loc(
          "space_moon_base_desc"
        )}</div><div class="has-text-special">${loc("requires_power_combo", [
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("moon_base", offset, 22000, 1.32);
        },
        Cement(offset) {
          return spaceCostMultiplier("moon_base", offset, 18000, 1.32);
        },
        Alloy(offset) {
      ...
      effect(wiki) {
        ...
        let oil = +fuel_adjust($(this)[0].support_fuel().a, true, wiki).toFixed(
          2
        );
        return `<div>${loc("space_moon_base_effect1")}</div><div>${loc(
          "plus_max_resource",
          [iridium, loc("resource_Iridium_name")]
        )}</div><div class="has-text-caution">${loc("space_moon_base_effect3", [
          oil,
          $(this)[0].powered(),
        ])}</div>`;
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (
            ...
            global.race.orbit_decay = global.stats.days + 2500;
            messageQueue(
              loc("evo_challenge_orbit_decayed_accelerated", [
    ...
    iridium_mine: {
      ...
      desc: `<div>${loc(
        "space_moon_iridium_mine_desc"
      )}</div><div class="has-text-special">${loc("space_support", [
        loc("space_moon_info_name"),
      ])}</div>`,
      reqs: { space: 3, luna: 1 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("iridium_mine", offset, 42000, 1.35);
        },
        Lumber(offset) {
          return spaceCostMultiplier("iridium_mine", offset, 9000, 1.35);
        },
        Titanium(offset) {
      ...
      effect() {
        ...
        if (global.race["truepath"]) {
          if (global.civic.foreign.gov3.hstl < 10) {
            rival = `<div class="has-text-success">${loc("space_rival_ally", [
              +(values.g * 100).toFixed(1),
            ])}</div>`;
          } else if (global.civic.foreign.gov3.hstl > 60) {
            rival = `<div class="has-text-danger">${loc("space_rival_war", [
              +(values.g * 100).toFixed(1),
            ])}</div>`;
          }
        }
        let cat_coal = global.race["cataclysm"]
          ? `<div>${loc("produce", [
              +production("iridium_mine", "coal").toFixed(2),
              global.resource.Coal.name,
            ])}</div>`
          : ``;
        let cat_uran = global.race["cataclysm"]
          ? `<div>${loc("produce", [
              +(production("iridium_mine", "coal") / 48).toFixed(3),
              global.resource.Uranium.name,
            ])}</div>`
          : ``;
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("space_moon_info_name"),
        ])}</div><div>${loc("space_moon_iridium_mine_effect", [
          iridium,
        ])}</div>${rival}${cat_coal}${cat_uran}`;
      },
      s_type: "moon",
      support() {
        return -1;
    ...
    helium_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("helium_mine", offset, 38000, 1.35);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("helium_mine", offset, 9000, 1.35);
        },
        Steel(offset) {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("space_moon_info_name"),
        ])}</div><div>${loc("space_moon_helium_mine_effect", [
          helium,
        ])}</div>${rival}<div>${loc("plus_max_resource", [
          storage,
          loc("resource_Helium_3_name"),
        ])}</div>`;
      },
    ...
    observatory: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("observatory", offset, 200000, 1.28);
        },
        Knowledge(offset) {
          return spaceCostMultiplier("observatory", offset, 69000, 1.28);
        },
        Stone(offset) {
          return spaceCostMultiplier("observatory", offset, 125000, 1.28);
        },
        Iron(offset) {
          return spaceCostMultiplier("observatory", offset, 65000, 1.28);
        },
        Iridium(offset) {
      ...
      effect() {
        ...
        let synergy = global.race["cataclysm"]
          ? `<div>${loc("space_moon_observatory_cata_effect", [25])}</div>`
          : `<div>${loc("space_moon_observatory_effect", [5])}</div>`;
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("space_moon_info_name"),
        ])}</div>${prof}<div>${loc("plus_max_resource", [
          gain,
          loc("resource_Knowledge_name"),
        ])}</div>${synergy}`;
      },
  ...
  spc_red: {
    ...
    red_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_red_mission_action", [planetName().red]),
            "info",
            false,
            ["progress"]
          );
    ...
    spaceport: {
      ...
      desc() {
        return `<div>${loc(
          "space_red_spaceport_desc"
        )}</div><div class="has-text-special">${loc("requires_power_space", [
          global.resource.Food.name,
        ])}</div>`;
      },
      reqs: { space: 4 },
      cost: {
      ...
      effect(wiki) {
        let helium = +fuel_adjust(
          $(this)[0].support_fuel().a,
          true,
          wiki
        ).toFixed(2);
        let bank = ``;
        if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
          ...
          bank = `<div>${loc("plus_max_resource", [
            `\$${vault}`,
            loc("resource_Money_name"),
          ])}</div>`;
        }
        return `<div>${loc("space_red_spaceport_effect1", [
          planetName().red,
          $(this)[0].support(),
        ])}</div>${bank}<div class="has-text-caution">${loc(
          "space_red_spaceport_effect2",
          [helium, $(this)[0].powered()]
        )}</div><div class="has-text-caution">${loc("spend", [
      ...
      support() {
        let support =
          global.race["cataclysm"] || global.race["orbit_decayed"] ? 4 : 3;
        if (
          global.stats.achieve["iron_will"] &&
          global.stats.achieve.iron_will.l >= 4
        ) {
          support++;
        }
        return support;
      ...
      powerBalancer() {
        return [
          { s: global.space.spaceport.s_max - global.space.spaceport.support },
        ];
      },
      refresh: true,
      action() {
    ...
    red_tower: {
      ...
      desc() {
        return `<div>${loc(
          "space_red_tower_desc"
        )}</div><div class="has-text-special">${loc("requires_power")}</div>`;
      },
      reqs: { mars: 3 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("red_tower", offset, 225000, 1.28);
        },
        Iron(offset) {
          return spaceCostMultiplier("red_tower", offset, 22000, 1.28);
        },
        Cement(offset) {
          return spaceCostMultiplier("red_tower", offset, 15000, 1.28);
        },
        Alloy(offset) {
          return spaceCostMultiplier("red_tower", offset, 8000, 1.28);
        },
      },
      effect() {
        return `<div>${loc("space_red_spaceport_effect1", [
          planetName().red,
          global.race["cataclysm"] || global.race["fasting"] ? 2 : 1,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
      powered() {
    ...
    terraformer: {
      ...
      desc(wiki) {
        if (
          !global.space.hasOwnProperty("terraformer") ||
          global.space.terraformer.count < 100 ||
          wiki
        ) {
          return (
            `<div>${loc(
              "space_terraformer"
            )}</div><div class="has-text-special">${loc("requires_segments", [
              100,
            ])}</div>` +
            (global.space.hasOwnProperty("terraformer") &&
            global.space.terraformer.count >= 100
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("terraformer")
              ? global.space.terraformer.count
              : 0) <
            100
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          ...
          return `<div>${loc(
            "space_terraformer_effect"
          )}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          )}</div>`;
        } else {
    ...
    atmo_terraformer: {
      ...
      powered(wiki) {
        return powerCostMod(
          (wiki ? wiki.truepath : global.race["truepath"]) ? 500 : 5000
        );
      },
      postPower(o) {
        if (o) {
      ...
      effect(wiki) {
        ...
        let power_label =
          power > 0
            ? `<div class="has-text-caution">${loc("minus_power", [
                power,
              ])}</div>`
            : "";
        return `<div>${loc(
          "space_terraformer_effect2"
        )}</div>${reward}${power_label}`;
      },
      action() {
        return false;
      },
    ...
    living_quarters: {
      ...
      desc() {
        return `<div>${loc(
          "space_red_living_quarters_desc"
        )}</div><div class="has-text-special">${loc("space_support", [
          planetName().red,
        ])}</div>`;
      },
      ...
      effect(wiki) {
        ...
        if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
          ...
          safe = `<div>${loc("plus_max_resource", [
            `\$${vault}`,
            loc("resource_Money_name"),
          ])}</div>`;
        }
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div>${safe}<div>${loc("plus_max_resource", [
          jobScale(1),
          global.race["truepath"]
            ? loc("job_colonist_tp", [planetName().red])
            : loc("colonist"),
        ])}</div><div>${loc("plus_max_resource", [
          gain,
          loc("citizen"),
      ...
      citizens(wiki) {
        let gain =
          global.race["cataclysm"] || global.race["orbit_decayed"] ? 2 : 1;
        let biodome_count = wiki
          ? global.space?.biodome?.on ?? 0
          : support_on["biodome"];
        if (biodome_count) {
    ...
    pylon: {
      ...
      reqs: { magic: 2 },
      condition() {
        return global.race["cataclysm"] || global.race["orbit_decayed"]
          ? true
          : false;
      },
      cost: {
    ...
    vr_center: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("vr_center", offset, 380000, 1.25);
        },
        Copper(offset) {
          return spaceCostMultiplier("vr_center", offset, 55000, 1.25);
        },
        Stanene(offset) {
          return spaceCostMultiplier("vr_center", offset, 100000, 1.25);
        },
        Soul_Gem(offset) {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div><div>${loc("space_red_vr_center_effect1", [
          morale,
        ])}</div><div>${loc("space_red_vr_center_effect2", [2])}</div>`;
      },
      s_type: "red",
      support() {
        return -1;
    ...
    garage: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("garage", offset, 75000, 1.28);
        },
        Iron(offset) {
          return spaceCostMultiplier("garage", offset, 12000, 1.28);
        },
        Brick(offset) {
          return spaceCostMultiplier("garage", offset, 3000, 1.28);
        },
        Sheet_Metal(offset) {
      ...
      multiplier(h) {
        let multiplier =
          global.tech["particles"] >= 4
            ? 1 + global.tech["supercollider"] / 20
            : 1;
        if (
        ...
        multiplier *= global.stats.achieve["blackhole"]
          ? 1 + global.stats.achieve.blackhole.l * 0.05
          : 1;
        if (h) {
          return global.tech["shelving"] && global.tech.shelving >= 2
            ? multiplier * 3
            : multiplier;
        }
        return multiplier;
      ...
      effect() {
        ...
        let crate =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<span>${loc("plus_max_resource", [
                containers,
                loc("resource_Crates_name"),
              ])}</span>`
            : ``;
    ...
    red_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("red_mine", offset, 50000, 1.32);
        },
        Lumber(offset) {
          return spaceCostMultiplier("red_mine", offset, 65000, 1.32);
        },
        Iron(offset) {
          return spaceCostMultiplier("red_mine", offset, 33000, 1.32);
        },
      },
      effect() {
        ...
        let decayed = global.race["orbit_decayed"]
          ? `<div>${loc("plus_max_resource", [
              jobScale(1),
              loc(`job_miner`),
            ])}</div><div>${loc("plus_max_resource", [
              jobScale(1),
        ...
        let cat_stone =
          (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
          !global.race["sappy"]
            ? `<div>${loc("space_red_mine_effect", [
                +production("red_mine", "stone").toFixed(2),
        ...
        let cat_asbestos =
          ...
          global.race["smoldering"]
            ? `<div>${loc("space_red_mine_effect", [
                +production("red_mine", "asbestos").toFixed(2),
        ...
        let cat_alum =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<div>${loc("space_red_mine_effect", [
                +production("red_mine", "aluminium").toFixed(2),
                global.resource.Aluminium.name,
              ])}</div>`
            : ``;
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div>${decayed}<div>${loc("space_red_mine_effect", [
          copper,
          global.resource.Copper.name,
        ])}</div><div>${loc("space_red_mine_effect", [
          titanium,
          global.resource.Titanium.name,
        ])}</div>${rival}${cat_asbestos}${cat_stone}${cat_alum}`;
      },
      s_type: "red",
      support() {
        return -1;
    ...
    fabrication: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("fabrication", offset, 90000, 1.32);
        },
        Copper(offset) {
          return spaceCostMultiplier("fabrication", offset, 25000, 1.32);
        },
        Cement(offset) {
          return spaceCostMultiplier("fabrication", offset, 12000, 1.32);
        },
        Wrought_Iron(offset) {
      ...
      effect() {
        let c_worker =
          global.race["cataclysm"] && !global.race["flier"]
            ? `<div>${loc("plus_max_resource", [
                jobScale(1),
            ...
            : ``;
        let fab =
          global.race["cataclysm"] || global.race["orbit_decayed"] ? 5 : 2;
        if (global.race["high_pop"]) {
          fab = highPopAdjust(fab);
        }
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div><div>${loc("space_red_fabrication_effect1", [
          jobScale(1),
        ])}</div>${c_worker}<div>${loc("space_red_fabrication_effect2", [
          fab,
        ])}</div>`;
      },
      s_type: "red",
    ...
    red_factory: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("red_factory", offset, 75000, 1.32);
        },
        Brick(offset) {
          return spaceCostMultiplier("red_factory", offset, 10000, 1.32);
        },
        Coal(offset) {
      ...
      effect(wiki) {
        ...
        desc =
          desc +
          `<div class="has-text-caution">${loc("space_red_factory_effect3", [
            helium,
            $(this)[0].powered(),
          ])}</div>`;
        return desc;
    ...
    biodome: {
      ...
      title() {
        if (global.race["artifical"]) {
          ...
          return global.race["soul_eater"]
            ? loc("space_red_asphodel_title")
            : loc("space_red_biodome_title");
        }
      },
      desc() {
        let desc;
        if (global.race["artifical"]) {
          ...
          if (global.race["carnivore"]) {
            ...
            desc = `<div>${loc("space_red_biodome_desc", [
              planetName().red,
            ])}</div>`;
          }
        }
        return `<div>${desc}</div><div class="has-text-special">${loc(
          "space_support",
          [planetName().red]
        )}</div>`;
      },
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("biodome", offset, 45000, 1.28);
        },
        Lumber(offset) {
          return spaceCostMultiplier("biodome", offset, 65000, 1.28);
        },
        Brick(offset) {
          return spaceCostMultiplier("biodome", offset, 1000, 1.28);
        },
        Nanite(offset) {
          return global.race["deconstructor"]
            ? spaceCostMultiplier("biodome", offset, 75, 1.28)
            : 0;
        },
      },
      effect() {
        ...
        let cat_fd =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<div>${loc("produce", [
                +production("biodome", "cat_food").toFixed(2),
                global.resource.Food.name,
              ])}</div>`
            : ``;
        let cat_wd =
          ...
          !global.race["smoldering"]
            ? `<div>${loc("space_red_mine_effect", [
                +production("biodome", "lumber").toFixed(2),
                global.resource.Lumber.name,
              ])}</div>`
            : ``;
        let pop = global.tech.mars >= 6 ? 0.1 : 0.05;
        let fLabel = global.race["artifical"]
          ? loc("city_transmitter_effect", [spatialReasoning(500)])
          : loc("plus_max_resource", [
              spatialReasoning(100),
              loc("resource_Food_name"),
            ]);
        let sig_cap =
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div>${cat_fd}<div>${loc("space_red_biodome_effect", [
          food,
          global.resource.Food.name,
        ])}</div><div>${loc("space_red_biodome_effect2", [
          +jobScale(pop).toFixed(2),
        ])}</div>${cat_wd}${sig_cap}`;
      },
      s_type: "red",
      support() {
      ...
      flair() {
        if (global.race["artifical"]) {
          ...
          return global.race["soul_eater"]
            ? loc("space_red_asphodel_flair")
            : global.race["carnivore"]
            ? loc("space_red_biodome_flair_carn")
            : loc("space_red_biodome_flair");
        }
      },
    },
    red_university: {
      ...
      cost: {
        Money(offset) {
          return (
            spaceCostMultiplier("university", offset, 900, 1.5, "city") - 500
          );
        },
        Lumber(offset) {
          return (
            spaceCostMultiplier("university", offset, 500, 1.36, "city") - 200
          );
        },
        Stone(offset) {
          return (
            spaceCostMultiplier("university", offset, 750, 1.36, "city") - 350
          );
        },
        Crystal(offset) {
          return global.race.universe === "magic"
            ? spaceCostMultiplier("university", offset, 5, 1.36, "city")
            : 0;
        },
      },
      wiki: false,
    ...
    exotic_lab: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("exotic_lab", offset, 750000, 1.28);
        },
        Steel(offset) {
          return spaceCostMultiplier("exotic_lab", offset, 100000, 1.28);
        },
        Mythril(offset) {
          return spaceCostMultiplier("exotic_lab", offset, 1000, 1.28);
        },
        Elerium(offset) {
      ...
      effect(wiki) {
        ...
        let num_mass_driver_on = wiki
          ? global.city?.mass_driver?.on ?? 0
          : p_on["mass_driver"];
        if (global.tech.mass >= 2 && num_mass_driver_on > 0) {
          sci += highPopAdjust(
            num_mass_driver_on * global.civic.scientist.workers
          );
        }
        ...
        let lab = "";
        if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
          scientist = `<div>${loc("city_wardenclyffe_effect1", [
            jobScale(1),
            global.civic.scientist.name,
          ])}</div>`;
          sci *=
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          planetName().red,
        ])}</div>${scientist}${lab}<div>${loc("space_red_exotic_lab_effect1", [
          +sci.toFixed(0),
        ])}</div><div>${loc("plus_max_resource", [
          elerium,
          loc("resource_Elerium_name"),
        ])}</div>`;
      },
      s_type: "red",
      support() {
      ...
      flair() {
        return `<div>${loc("space_red_exotic_lab_flair1")}</div><div>${loc(
          "space_red_exotic_lab_flair2"
        )}</div>`;
      },
    },
    ziggurat: {
      ...
      desc() {
        let entity =
          global.race.old_gods !== "none"
            ? races[global.race.old_gods.toLowerCase()].entity
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("ziggurat", offset, 600000, 1.28);
        },
        Stone(offset) {
          return spaceCostMultiplier("ziggurat", offset, 250000, 1.28);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("ziggurat", offset, 70000, 1.28);
        },
        Mythril(offset) {
      ...
      effect(wiki) {
        ...
        let num_exo_labs_on = wiki
          ? global.space?.exotic_lab?.on ?? 0
          : support_on["exotic_lab"];
        if (
          global.tech["ancient_deify"] &&
          global.tech["ancient_deify"] >= 2 &&
          num_exo_labs_on
        ) {
        ...
        let zvar = global.race["truepath"]
          ? [bonus, races[global.race.species].home]
          : [bonus];
        let desc = `<div>${loc(
          global.race["truepath"]
            ? "space_red_ziggurat_effect_tp"
            : "space_red_ziggurat_effect",
          zvar
        )}</div>`;
        if (global.tech["ancient_study"] && global.tech["ancient_study"] >= 2) {
          desc =
            desc + `<div>${loc("interstellar_laboratory_effect", [3])}</div>`;
        }
        if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
    ...
    space_barracks: {
      ...
      desc() {
        return `<div>${loc(
          "space_red_space_barracks_desc"
        )}</div><div class="has-text-special">${loc(
          "space_red_space_barracks_desc_req"
        )}</div>`;
      },
      reqs: { marines: 1 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("space_barracks", offset, 350000, 1.28);
        },
        Alloy(offset) {
          return spaceCostMultiplier("space_barracks", offset, 65000, 1.28);
        },
        Iridium(offset) {
      ...
      effect(wiki) {
        let train = global.race["orbit_decayed"]
          ? actions.city.boot_camp.effect()
          : "";
        let oil = +fuel_adjust(2, true, wiki).toFixed(2);
        let soldiers = $(this)[0].soldiers();
        let food = global.race["cataclysm"]
          ? ``
          : `<div class="has-text-caution">${loc(
              "space_red_space_barracks_effect3",
              [global.resource.Food.name]
            )}</div>`;
        return `<div>${loc("plus_max_soldiers", [
          soldiers,
        ])}</div>${train}<div class="has-text-caution">${loc(
  ...
  spc_hell: {
    ...
    hell_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_hell_mission_action", [planetName().hell]),
            "info",
            false,
            ["progress"]
          );
    ...
    geothermal: {
      ...
      desc() {
        return `<div>${loc(
          "space_hell_geothermal_desc"
        )}</div><div class="has-text-special">${loc(
          "space_hell_geothermal_desc_req"
        )}</div>`;
      },
      reqs: { hell: 1 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("geothermal", offset, 38000, 1.35);
        },
        Steel(offset) {
          return spaceCostMultiplier("geothermal", offset, 15000, 1.35);
        },
        Polymer(offset) {
      ...
      effect(wiki) {
        ...
        let smelter =
          num_smelters > 0
            ? `<div>${loc("interstellar_stellar_forge_effect3", [
                num_smelters,
              ])}</div>`
            : ``;
        return `${smelter}<span>${loc("space_dwarf_reactor_effect1", [
          -$(this)[0].powered(),
        ])}</span>, <span class="has-text-caution">${loc(
      ...
      powered() {
        ...
        if (
          global.stats.achieve["failed_history"] &&
          global.stats.achieve.failed_history.l >= 5
        ) {
          power -= 2;
        }
    ...
    hell_smelter: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("hell_smelter", offset, 250000, 1.24);
        },
        Adamantite(offset) {
          return spaceCostMultiplier("hell_smelter", offset, 15000, 1.24);
        },
      },
      effect() {
        return `<div>${loc("interstellar_stellar_forge_effect3", [
          $(this)[0].smelting(),
        ])}</div>`;
      },
      special: true,
    ...
    spc_casino: {
      ...
      condition() {
        return global.race["cataclysm"] ||
          (global.stats.achieve["iron_will"] &&
            global.stats.achieve.iron_will.l >= 5)
          ? true
          : false;
      },
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "spc_casino",
            offset,
            traitCostMod("untrustworthy", 400000),
            1.35
          );
        },
        Furs(offset) {
          return spaceCostMultiplier(
            "spc_casino",
            offset,
            traitCostMod("untrustworthy", 75000),
            1.35
          );
        },
        Cement(offset) {
          return spaceCostMultiplier(
            "spc_casino",
            offset,
            traitCostMod("untrustworthy", 100000),
            1.35
          );
        },
        Plywood(offset) {
          return spaceCostMultiplier(
            "spc_casino",
            offset,
            traitCostMod("untrustworthy", 20000),
            1.35
          );
        },
      },
  ...
  spc_sun: {
    ...
    sun_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(15000, false, wiki).toFixed(0);
        },
      },
      effect() {
    ...
    swarm_control: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("swarm_control", offset, 100000, 1.3);
        },
        Knowledge(offset) {
          return spaceCostMultiplier("swarm_control", offset, 60000, 1.3);
        },
        Alloy(offset) {
      ...
      effect(wiki) {
        return loc("space_sun_swarm_control_effect1", [
          $(this)[0].support(wiki),
        ]);
      },
      support(wiki) {
        return global.tech["swarm"] && global.tech["swarm"] >= 2
          ? global.tech["high_tech"] >= 11
            ? 11 + Math.round(get_qlevel(wiki))
            : 12
          : 10;
      },
      action() {
    ...
    swarm_satellite: {
      ...
      desc() {
        return `<div>${loc(
          "space_sun_swarm_satellite_desc"
        )}</div><div class="has-text-special">${loc(
          "space_sun_swarm_satellite_desc_req"
        )}</div>`;
      },
      reqs: { solar: 3 },
      cost: {
        ...
        Helium_3(offset, wiki) {
          return spaceCostMultiplier(
            "swarm_satellite",
            offset,
            swarm_adjust(fuel_adjust(50, false, wiki), wiki),
            1.1
          );
        },
      },
      effect() {
        ...
        return `<span>${loc("space_dwarf_reactor_effect1", [
          powerModifier(solar),
        ])}</span>, <span class="has-text-caution">${loc(
          "space_sun_swarm_satellite_effect1",
          [1]
        )}</span>`;
      },
      support() {
  ...
  spc_gas: {
    info: {
      ...
      desc() {
        return loc("space_gas_info_desc", [
          planetName().gas,
    ...
    gas_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_gas_mission_action", [planetName().gas]),
            "info",
            false,
            ["progress"]
          );
    ...
    gas_mining: {
      ...
      desc() {
        return `<div>${loc(
          "space_gas_mining_desc"
        )}</div><div class="has-text-special">${loc("requires_power")}</div>`;
      },
      reqs: { gas_giant: 1 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("gas_mining", offset, 250000, 1.32);
        },
        Uranium(offset) {
          return spaceCostMultiplier("gas_mining", offset, 500, 1.32);
        },
        Alloy(offset) {
          return spaceCostMultiplier("gas_mining", offset, 10000, 1.32);
        },
        Helium_3(offset, wiki) {
          return spaceCostMultiplier(
            "gas_mining",
            offset,
            fuel_adjust(2500, false, wiki),
            1.32
          );
        },
        Mythril(offset) {
      ...
      effect() {
        ...
        return `<div>${loc("space_gas_mining_effect1", [
          helium,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    gas_storage: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("gas_storage", offset, 125000, 1.32);
        },
        Iridium(offset) {
          return spaceCostMultiplier("gas_storage", offset, 3000, 1.32);
        },
        Sheet_Metal(offset) {
          return spaceCostMultiplier("gas_storage", offset, 2000, 1.32);
        },
        Helium_3(offset, wiki) {
          return spaceCostMultiplier(
            "gas_storage",
            offset,
            fuel_adjust(1000, false, wiki),
            1.32
          );
        },
      },
      effect() {
        let oil =
          spatialReasoning(3500) * (global.tech["world_control"] ? 1.5 : 1);
        let helium =
          spatialReasoning(2500) * (global.tech["world_control"] ? 1.5 : 1);
        let uranium =
          spatialReasoning(1000) * (global.tech["world_control"] ? 1.5 : 1);
        return `<div>${loc("plus_max_resource", [
          oil,
    ...
    star_dock: {
      ...
      desc() {
        return `<div>${loc(
          "space_gas_star_dock_title"
        )}</div><div class="has-text-special">${loc(
          "space_gas_star_dock_desc_req"
        )}</div>`;
      },
      reqs: { genesis: 3 },
      queue_complete() {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("star_dock")
              ? global.space.star_dock.count
              : 0) ===
            0
            ? 1500000
            : 0;
        ...
        Helium_3(offset, wiki) {
          return (offset || 0) +
            ...
            ? Math.round(
                fuel_adjust(
                  global.race["gravity_well"] ? 25000 : 10000,
                  false,
                  wiki
                )
              )
  ...
  spc_gas_moon: {
    info: {
      ...
      desc() {
        return loc("space_gas_moon_info_desc", [
          planetName().gas_moon,
          planetName().gas,
        ]);
      },
    ...
    gas_moon_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(30000, false, wiki).toFixed(0);
        },
      },
      effect() {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_gas_moon_mission_action", [planetName().gas_moon]),
            "info",
            false,
            ["progress"]
          );
    ...
    outpost: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("outpost", offset, 666000, 1.3);
        },
        Titanium(offset) {
          return spaceCostMultiplier("outpost", offset, 18000, 1.3);
        },
        Iridium(offset) {
      ...
      effect(wiki) {
        ...
        let oil = +fuel_adjust(2, true, wiki).toFixed(2);
        return `<div>${loc("space_gas_moon_outpost_effect1", [
          neutronium,
        ])}</div><div>${loc("plus_max_resource", [
          max,
          loc("resource_Neutronium_name"),
        ])}</div><div class="has-text-caution">${loc(
    ...
    drone: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("drone", offset, 250000, 1.3);
        },
        Steel(offset) {
          return spaceCostMultiplier("drone", offset, 20000, 1.3);
        },
        Neutronium(offset) {
        ...
        Nano_Tube(offset) {
          return spaceCostMultiplier("drone", offset, 45000, 1.3);
        },
      },
      effect() {
        let value =
          global.stats.achieve["iron_will"] &&
          global.stats.achieve.iron_will.l >= 3
            ? 12
            : 6;
    ...
    oil_extractor: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("oil_extractor", offset, 666000, 1.3);
        },
        Polymer(offset) {
        ...
        Wrought_Iron(offset) {
          return spaceCostMultiplier("oil_extractor", offset, 5000, 1.3);
        },
      },
      effect() {
        let oil = +production("oil_extractor").toFixed(2);

        let desc = `<div>${loc("space_gas_moon_oil_extractor_effect1", [
          oil,
        ])}</div>`;
        if (global.race["blubber"] && global.city.hasOwnProperty("oil_well")) {
          let maxDead =
            ...
            (global.space["oil_extractor"]
              ...
              : 0);
          desc += `<div>${loc("city_oil_well_bodies", [
            +global.city.oil_well.dead.toFixed(1),
            50 * maxDead,
          ])}</div>`;
          desc += `<div>${loc("city_oil_well_consume", [
            traits.blubber.vars()[0],
          ])}</div>`;
        }
        desc += `<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
        return desc;
      },
  ...
  spc_belt: {
    info: {
      ...
      desc() {
        return global.space["space_station"] &&
          global.space.space_station.count > 0
          ? `<div>${loc("space_belt_info_desc", [
              planetName().red,
              planetName().gas,
            ])}</div><div class="has-text-special">${loc(
              "space_belt_info_desc2"
            )}</div>`
    ...
    belt_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(25000, false, wiki).toFixed(0);
        },
      },
      effect() {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("space_belt_mission_action"), "info", false, [
            "progress",
          ]);
    ...
    space_station: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("space_station", offset, 250000, 1.3);
        },
        Iron(offset) {
          return spaceCostMultiplier("space_station", offset, 85000, 1.3);
        },
        Polymer(offset) {
          return spaceCostMultiplier("space_station", offset, 18000, 1.3);
        },
        Iridium(offset) {
      ...
      effect(wiki) {
        ...
        if (global.race["fasting"]) {
          return `<div>${loc("plus_max_resource", [
            jobScale(3),
            loc("job_space_miner"),
          ])}</div>${elerium}<div class="has-text-caution">${loc(
            "space_belt_station_effect5",
            [helium, $(this)[0].powered()]
          )}</div>`;
        }
        return `<div>${loc("plus_max_resource", [
          ...
          "space_belt_station_effect4",
          [food, $(this)[0].powered(), global.resource.Food.name]
        )}</div>`;
      },
      support() {
        return jobScale(3);
    ...
    elerium_ship: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("elerium_ship", offset, 500000, 1.3);
        },
        Uranium(offset) {
          return spaceCostMultiplier("elerium_ship", offset, 2500, 1.3);
        },
        Titanium(offset) {
          return spaceCostMultiplier("elerium_ship", offset, 10000, 1.3);
        },
        Mythril(offset) {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc(
          "space_belt_elerium_ship_effect1",
          [jobScale(2)]
        )}</div><div>${loc("space_belt_elerium_ship_effect2", [
          elerium,
        ])}</div>`;
      },
      s_type: "belt",
    ...
    iridium_ship: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("iridium_ship", offset, 120000, 1.3);
        },
        Uranium(offset) {
          return spaceCostMultiplier("iridium_ship", offset, 1000, 1.3);
        },
        Alloy(offset) {
          return spaceCostMultiplier("iridium_ship", offset, 48000, 1.3);
        },
        Iridium(offset) {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc(
          "space_belt_iridium_ship_effect1",
          [jobScale(1)]
        )}</div><div>${loc("space_belt_iridium_ship_effect2", [
          iridium,
        ])}</div>`;
      },
      s_type: "belt",
    ...
    iron_ship: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("iron_ship", offset, 80000, 1.3);
        },
        Steel(offset) {
          return spaceCostMultiplier("iron_ship", offset, 42000, 1.3);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("iron_ship", offset, 38000, 1.3);
        },
        Polymer(offset) {
          return spaceCostMultiplier("iron_ship", offset, 16000, 1.3);
        },
        Helium_3(offset, wiki) {
          return spaceCostMultiplier(
            "iron_ship",
            offset,
            fuel_adjust(1200, false, wiki),
            1.3
          );
        },
      },
      effect() {
        ...
        if (global.tech["solar"] && global.tech["solar"] >= 5) {
          return `<div class="has-text-caution">${loc(
            "space_belt_iron_ship_effect1",
            [jobScale(1)]
          )}</div><div>${loc("space_belt_iron_ship_effect2", [
            iron,
          ])}</div><div>${loc("space_belt_iron_ship_effect3")}</div>`;
        } else {
          return `<div class="has-text-caution">${loc(
            "space_belt_iron_ship_effect1",
            [jobScale(1)]
          )}</div><div>${loc("space_belt_iron_ship_effect2", [iron])}</div>`;
        }
      },
      s_type: "belt",
      support() {
  ...
  spc_dwarf: {
    ...
    dwarf_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(45000, false, wiki).toFixed(0);
        },
      },
      effect() {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_dwarf_mission_action", [planetName().dwarf]),
            "info",
            false,
            ["progress"]
          );
    ...
    elerium_contain: {
      ...
      desc() {
        return `<div>${loc(
          "space_dwarf_elerium_contain_title"
        )}</div><div class="has-text-special">${loc("requires_power")}</div>`;
      },
      reqs: { dwarf: 1 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("elerium_contain", offset, 800000, 1.28);
        },
        Cement(offset) {
          return spaceCostMultiplier("elerium_contain", offset, 120000, 1.28);
        },
        Iridium(offset) {
          return spaceCostMultiplier("elerium_contain", offset, 50000, 1.28);
        },
        Neutronium(offset) {
    ...
    e_reactor: {
      ...
      desc() {
        return `<div>${loc(
          "space_dwarf_reactor_title"
        )}</div><div class="has-text-special">${loc(
          "space_dwarf_reactor_desc_req"
        )}</div>`;
      },
      reqs: { elerium: 2 },
      cost: {
        Money(offset) {
          return spaceCostMultiplier("e_reactor", offset, 1250000, 1.28);
        },
        Steel(offset) {
          return spaceCostMultiplier("e_reactor", offset, 350000, 1.28);
        },
        Neutronium(offset) {
      ...
      effect() {
        ...
        return `<div>${loc("space_dwarf_reactor_effect1", [
          power,
        ])}</div><div  class="has-text-caution">${loc(
          "space_dwarf_reactor_effect2",
          [elerium]
        )}</div>`;
      },
      powered() {
    ...
    world_collider: {
      ...
      desc(wiki) {
        if (
          !global.space.hasOwnProperty("world_collider") ||
          global.space.world_collider.count < 1859 ||
          wiki
        ) {
          return (
            `<div>${loc(
              "space_dwarf_collider_desc"
            )}</div><div class="has-text-special">${loc(
              "space_dwarf_collider_desc_req"
            )}</div>` +
            (global.space.hasOwnProperty("world_collider") &&
            global.space.world_collider.count >= 1859
              ? `<div class="has-text-special">${loc("requires_power")}</div>`
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("world_collider")
              ? global.space.world_collider.count
              : 0) <
            1859
            ? 25000
            : 0;
    ...
    world_controller: {
      ...
      effect(wiki) {
        ...
        return `<div>${loc("plus_max_resource", [
          ...
          "space_dwarf_controller_effect3"
        )}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
    ...
    mass_relay: {
      ...
      desc(wiki) {
        if (
          !global.space.hasOwnProperty("mass_relay") ||
          global.space.mass_relay.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "space_dwarf_mass_relay_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        }
    ...
    m_relay: {
      ...
      effect() {
        ...
        return `<div>${loc("space_dwarf_mass_relay_effect2", [
          planetName().dwarf,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div><div>${loc("space_dwarf_mass_relay_charged", [charge])}</div>`;
      },
      action() {
        return false;
      },
...
const interstellarProjects = {
  int_alpha: {
    info: {
      ...
      desc() {
        return global.tech["alpha"]
          ? loc("interstellar_alpha_desc2", [races[global.race.species].home])
          : loc("interstellar_alpha_desc1", [races[global.race.species].home]);
      },
      support: "starport",
    },
    alpha_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(40000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("interstellar_alpha_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    starport: {
      ...
      desc() {
        return `<div>${loc(
          "interstellar_alpha_starport_desc"
        )}</div><div class="has-text-special">${loc("requires_power_space", [
          global.resource.Food.name,
        ])}</div>`;
      },
      ...
      effect() {
        ...
        let food = 100;
        return `<div>${loc("interstellar_alpha_starport_effect1", [
          $(this)[0].support(),
        ])}</div><div class="has-text-caution">${loc(
          ...
          [food, global.resource.Food.name]
        )}</div>`;
      },
      support() {
        return 5;
        ...
        return [
          {
            s:
              ...
              global.interstellar.starport.support,
          },
        ];
      },
      refresh: true,
    ...
    habitat: {
      ...
      cost: {
        ...
        Adamantite(offset) {
          return global.race["fasting"] &&
            global.interstellar.habitat &&
            global.interstellar.habitat.count < 5
            ? 0
            : spaceCostMultiplier(
                "habitat",
                offset,
      ...
      effect() {
        ...
        return `<div>${loc("interstellar_alpha_starport_effect1", [
          $(this)[0].support(),
        ])}</div><div><span>${loc("plus_max_citizens", [
          citizens,
        ])}</span>, <span class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</span></div>`;
      },
      support() {
    ...
    mining_droid: {
      ...
      desc: `<div>${loc(
        "interstellar_mining_droid_title"
      )}</div><div class="has-text-special">${loc("space_support", [
        loc("interstellar_alpha_name"),
      ])}</div>`,
      reqs: { alpha: 2 },
      cost: {
        Money(offset) {
      ...
      effect() {
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_alpha_name"),
        ])}</div><div>${loc("interstellar_mining_droid_effect")}</div>`;
      },
      s_type: "alpha",
      support() {
        return -1;
    ...
    processing: {
      ...
      effect(wiki) {
        let bonus = 12;
        let num_citadels_on = wiki
          ? global.interstellar?.citadel?.on ?? 0
          : p_on["citadel"];
        if (
          global.tech["ai_core"] &&
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_alpha_name"),
        ])}</div><div>${loc("interstellar_processing_effect", [bonus])}</div>`;
      },
      s_type: "alpha",
      support() {
        return -1;
    ...
    fusion: {
      ...
      desc() {
        return `<div>${loc(
          "interstellar_fusion_title"
        )}</div><div class="has-text-special">${loc(
          "requires_power_support_combo",
          [loc("interstellar_alpha_name"), global.resource.Deuterium.name]
        )}</div>`;
      },
      reqs: { fusion: 1 },
      cost: {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_alpha_name"),
        ])}</div><div><span>${loc("space_dwarf_reactor_effect1", [
          -$(this)[0].powered(),
        ])}</span>, <span class="has-text-caution">${loc(
      ...
      effect(wiki) {
        ...
        if (
          ...
          know *=
            1 +
            num_tech_scavs_on *
              +(piracy("gxy_alien2", false, false, wiki) * 0.75).toFixed(1);
        }
        if (global.tech["science"] >= 21) {
          know *= 1.45;
        }
        ...
        if (global.tech.science >= 16) {
          sci = `<div>${loc("city_wardenclyffe_effect1", [
            jobScale(1),
            global.civic.scientist.name,
          ])}</div>`;
        }
        let desc = `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_alpha_name"),
        ])}</div>${sci}<div>${loc("city_max_knowledge", [know])}</div>`;
        if (global.tech["science"] >= 13) {
          desc =
            desc + `<div>${loc("interstellar_laboratory_effect", [5])}</div>`;
        }
    ...
    exchange: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "exchange",
            offset,
            traitCostMod("untrustworthy", 680000),
            1.28,
            "interstellar"
          );
        },
        Stone(offset) {
          return spaceCostMultiplier(
            "exchange",
            offset,
            traitCostMod("untrustworthy", 115000),
            1.28,
            "interstellar"
          );
        },
        Adamantite(offset) {
          return spaceCostMultiplier(
            "exchange",
            offset,
            traitCostMod("untrustworthy", 55000),
            1.28,
            "interstellar"
          );
        },
        Graphene(offset) {
          return spaceCostMultiplier(
            "exchange",
            offset,
            traitCostMod("untrustworthy", 78000),
            1.28,
            "interstellar"
          );
        },
      ...
      effect(wiki) {
        ...
        let vault = spatialReasoning(
          global.city["bank"] ? (b_vault * banks) / 18 : 0
        );
    ...
    g_factory: {
      ...
      effect() {
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_alpha_name"),
        ])}</div><div>${loc("interstellar_g_factory_effect")}</div>`;
      },
      s_type: "alpha",
      support() {
        return -1;
    ...
    int_factory: {
      ...
      desc() {
        return `<div>${loc(
          ...
          global.resource.Deuterium.name,
        ])}</div>`;
      },
      reqs: { alpha: 4 },
      cost: {
      ...
      effect() {
        ...
        return `<div>${loc("interstellar_int_factory_effect")}</div><div>${loc(
          "city_crafted_mats",
          [10]
        )}</div><div class="has-text-caution"><span>${loc(
          "interstellar_fusion_effect",
          [deuterium]
        )}</span> <span>${loc("minus_power", [
          $(this)[0].powered(),
        ])}</span></div>`;
      },
      powered() {
    ...
    luxury_condo: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "luxury_condo",
            offset,
            traitCostMod("untrustworthy", 25000000),
            1.25,
            "interstellar"
          );
        },
        ...
        Stanene(offset) {
          return spaceCostMultiplier(
            "luxury_condo",
            offset,
            traitCostMod("untrustworthy", 230000),
            1.25,
            "interstellar"
          );
        },
        Orichalcum(offset) {
          return spaceCostMultiplier(
            "luxury_condo",
            offset,
            traitCostMod("untrustworthy", 65000),
            1.25,
            "interstellar"
          );
        },
        Nanoweave(offset) {
          return spaceCostMultiplier(
            "luxury_condo",
            offset,
            traitCostMod("untrustworthy", 12500),
            1.25,
            "interstellar"
          );
        },
      ...
      effect() {
        ...
        return `<div><span>${loc("plus_max_citizens", [
          citizens,
        ])}</span>, <span class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</span></div><div>${loc("plus_max_resource", [
          `\$${safe.toLocaleString()}`,
          loc("resource_Money_name"),
        ])}</div>`;
      },
    ...
    zoo: {
      ...
      desc() {
        return `<div>${loc(
          ...
          "requires_power_support_combo",
          [loc("interstellar_alpha_name"), global.resource.Food.name]
        )}</div>`;
      },
      reqs: { zoo: 1 },
      cost: {
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          ...
          morale,
        ])}</div><div>${loc("space_red_vr_center_effect2", [
          max,
        ])}</div><div class="has-text-caution">${loc(
          "interstellar_alpha_starport_effect3",
          [food, global.resource.Food.name]
  ...
  int_proxima: {
    info: {
      ...
      desc() {
        return global.tech["proxima"]
          ? loc("interstellar_proxima_desc2")
          : loc("interstellar_proxima_desc1");
      },
    },
    proxima_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(42000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("interstellar_proxima_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    xfer_station: {
      ...
      desc() {
        return `<div>${loc(
          "interstellar_xfer_station_desc"
        )}</div><div class="has-text-special">${loc("requires_power_combo", [
          global.resource.Uranium.name,
        ])}</div>`;
      },
      ...
      effect() {
        ...
        if (global.resource.Deuterium.display) {
          det = `<div>${loc("plus_max_resource", [
            spatialReasoning(2000),
            loc("resource_Deuterium_name"),
          ])}</div>`;
        }
        return `<div>${loc("interstellar_alpha_starport_effect1", [
          $(this)[0].support(),
          ...
          uranium,
          loc("resource_Uranium_name"),
        ])}</div>${det}<div class="has-text-caution">${loc(
          "city_fission_power_effect",
          [fuel]
        )}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    cargo_yard: {
      ...
      effect(wiki) {
        ...
        let desc = `<div>${loc("plus_max_resource", [
          containers,
          loc("resource_Crates_name"),
        ])}</div><div>${loc("plus_max_resource", [
          containers,
        ...
        desc =
          ...
          `<div>${loc("plus_max_resource", [
            ...
            infernite,
            loc("resource_Infernite_name"),
          ])}</div>`;
        if (global.tech["storage"] >= 7) {
          let boost = +get_qlevel(wiki).toFixed(3);
          desc =
            desc +
            `<div>${loc("interstellar_cargo_yard_effect", [boost])}</div>`;
        }
        return desc;
      },
      action() {
      ...
      cost: {
        ...
        Deuterium(offset) {
          return spaceCostMultiplier(
            "cruiser",
            offset,
            +int_fuel_adjust(1500).toFixed(0),
            1.28,
            "interstellar"
          );
        },
    ...
    dyson: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("dyson") ||
          global.interstellar.dyson.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_dyson_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      condition() {
        return global.interstellar.dyson.count >= 100 && global.tech["dyson"]
          ? false
          : true;
      },
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("dyson")
              ? global.interstellar.dyson.count
              : 0) <
            100
            ? 250000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          let power =
            count > 0
              ? `<div>${loc("space_dwarf_reactor_effect1", [
                  powerModifier(count * 1.25),
                ])}</div>`
              : ``;
          let remain = 100 - count;
          return `<div>${loc(
            "interstellar_dyson_effect"
          )}</div>${power}<div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          )}</div>`;
        } else {
    ...
    dyson_sphere: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("dyson_sphere") ||
          global.interstellar.dyson_sphere.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_dyson_sphere_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      condition() {
        return global.interstellar.dyson.count >= 100 &&
          global.tech["dyson"] &&
          global.tech.dyson === 1
          ? true
          : false;
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("dyson_sphere")
              ? global.interstellar.dyson_sphere.count
              : 0) <
            100
            ? 5000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          ...
          let remain = 100 - count;
          return `<div>${loc(
            "interstellar_dyson_sphere_effect"
          )}</div><div>${loc("space_dwarf_reactor_effect1", [
            powerModifier(power),
          ])}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          ...
          return loc("interstellar_dyson_sphere_complete", [
            powerModifier(750),
          ]);
        }
      },
    ...
    orichalcum_sphere: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("orichalcum_sphere") ||
          global.interstellar.orichalcum_sphere.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_orichalcum_sphere_desc"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      condition() {
        return global.interstellar.dyson_sphere.count >= 100 &&
          global.tech["dyson"] &&
          global.tech.dyson === 2
          ? true
          : false;
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("orichalcum_sphere")
              ? global.interstellar.orichalcum_sphere.count
              : 0) <
            100
            ? 25000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          ...
          let remain = 100 - count;
          return `<div>${loc(
            "interstellar_orichalcum_sphere_effect"
          )}</div><div>${loc("space_dwarf_reactor_effect1", [
            powerModifier(power),
          ])}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
    ...
    elysanite_sphere: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("elysanite_sphere") ||
          global.interstellar.elysanite_sphere.count < 1000 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_elysanite_sphere_desc"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            1000,
          ])}</div>`;
        } else {
      ...
      queue_size: 50,
      queue_complete() {
        return 1000 - global.interstellar.elysanite_sphere.count;
      },
      condition() {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("elysanite_sphere")
              ? global.interstellar.elysanite_sphere.count
              : 0) <
            1000
            ? 1000000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 1000) {
          ...
          return `<div>${loc(
            "interstellar_elysanite_sphere_effect"
          )}</div><div>${loc("space_dwarf_reactor_effect1", [
            powerModifier(power),
          ])}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
  ...
  int_nebula: {
    info: {
      ...
      desc() {
        return global.tech["nebula"]
          ? loc("interstellar_nebula_desc2")
          : loc("interstellar_nebula_desc1");
      },
      support: "nexus",
    },
    nebula_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(55000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("interstellar_nebula_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    nexus: {
      ...
      effect() {
        ...
        return `<div>${loc("interstellar_nexus_effect1", [
          $(this)[0].support(),
        ])}</div><div>${loc("plus_max_resource", [
          oil,
          loc("resource_Oil_name"),
        ...
        return [
          {
            s:
              ...
              global.interstellar.nexus.support,
          },
        ];
      },
      refresh: true,
    ...
    harvester: {
      ...
      effect() {
        ...
        let ram = global.tech["ram_scoop"]
          ? `<div>${loc("interstellar_harvester_effect", [deuterium])}</div>`
          : "";
        return `<div class="has-text-caution">${loc("space_used_support", [
          loc("interstellar_nebula_name"),
        ])}</div><div>${loc("space_gas_mining_effect1", [helium])}</div>${ram}`;
      },
      s_type: "nebula",
      support() {
        return -1;
    ...
    elerium_prospector: {
      ...
      desc: `<div>${loc(
        "interstellar_elerium_prospector_title"
      )}</div><div class="has-text-special">${loc("space_support", [
        loc("interstellar_nebula_name"),
      ])}</div>`,
      reqs: { nebula: 3 },
  ...
  int_neutron: {
    info: {
      ...
      desc() {
        return global.tech["neutron"]
          ? loc("interstellar_neutron_desc2", [races[global.race.species].home])
          : loc("interstellar_neutron_desc1");
      },
    },
    neutron_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(60000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(10000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("interstellar_neutron_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    neutron_miner: {
      ...
      desc() {
        return `<div>${loc(
          "interstellar_neutron_miner_desc"
        )}</div><div class="has-text-special">${loc("requires_power_combo", [
          global.resource.Helium_3.name,
        ])}</div>`;
      },
    ...
    citadel: {
      ...
      effect(wiki) {
        ...
        let desc = `<div class="has-text-warning">${loc(
          "interstellar_citadel_stat",
          [+quantum_lv.toFixed(1)]
        )}</div><div>${loc("interstellar_citadel_effect", [5])}</div>`;
        if (global.tech["ai_core"]) {
          let cement = +(quantum_lv / 1.75).toFixed(1);
          if (!global.race["flier"]) {
            desc =
              desc +
              `<div>${loc("interstellar_citadel_effect2", [cement])}</div>`;
          }
          if (global.tech["ai_core"] >= 2) {
            desc =
              desc + `<div>${loc("interstellar_citadel_effect3", [2])}</div>`;
          }
          if (global.tech["ai_core"] >= 3) {
            let graph = +(quantum_lv / 5).toFixed(1);
            desc =
              desc +
              `<div>${loc("interstellar_citadel_effect4", [graph])}</div>`;
          }
          if (global.tech["ai_core"] >= 4) {
            desc =
              desc + `<div>${loc("interstellar_citadel_effect5", [1])}</div>`;
          }
        }
        return `${desc}<div class="has-text-caution">${loc(
          "interstellar_citadel_power",
          [$(this)[0].powered(wiki), powerCostMod(2.5)]
        )}</div>`;
      },
      powered(wiki) {
    ...
    stellar_forge: {
      ...
      desc: `<div>${loc(
        "interstellar_stellar_forge_title"
      )}</div><div class="has-text-special">${loc("requires_power")}</div>`,
      reqs: { star_forge: 1 },
      cost: {
        Money(offset) {
      ...
      effect() {
        let desc = `<div>${loc("city_foundry_effect1", [
          jobScale(2),
        ])}</div><div>${loc("interstellar_stellar_forge_effect", [
          10,
        ])}</div><div>${loc("interstellar_stellar_forge_effect2", [5])}</div>`;
        let num_smelters = $(this)[0].smelting();
        if (num_smelters > 0) {
          desc += `<div>${loc("interstellar_stellar_forge_effect3", [
            num_smelters,
          ])}</div>`;
        }
        return `${desc}<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
      powered() {
  ...
  int_blackhole: {
      ...
      desc() {
        ...
        if (global.tech["blackhole"] >= 5) {
          ...
          if (global.tech["roid_eject"]) {
            mass +=
              0.225 *
              global.tech["roid_eject"] *
              (1 + global.tech["roid_eject"] / 12);
          }
          if (global.tech["whitehole"]) {
            ...
            return `<div>${loc("interstellar_blackhole_desc4", [
              home,
              mass,
              exotic,
            ])}</div><div class="has-text-advanced">${loc(
              "interstellar_blackhole_desc5",
              [gains.plasmid, gains.phage, gains.dark, plasmidType]
            )}</div>`;
          } else {
            return global.interstellar.stellar_engine.exotic > 0
              ? loc("interstellar_blackhole_desc4", [home, mass, exotic])
              : loc("interstellar_blackhole_desc3", [home, mass]);
          }
        } else {
          return global.tech["blackhole"]
            ? loc("interstellar_blackhole_desc2", [home])
            : loc("interstellar_blackhole_desc1", [home]);
        }
      },
    },
    blackhole_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(75000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(25000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("interstellar_blackhole_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    far_reach: {
      ...
      effect() {
        return `<div>${loc("interstellar_far_reach_effect", [
          1,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    stellar_engine: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("stellar_engine") ||
          global.interstellar.stellar_engine.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_stellar_engine"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("stellar_engine")
              ? global.interstellar.stellar_engine.count
              : 0) <
            100
            ? 500000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          let remain = 100 - count;
          return `<div>${loc(
            "interstellar_stellar_engine_effect"
          )}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          )}</div>`;
          ...
          let waves =
            global.tech["gravity"] && global.tech["gravity"] >= 2 ? 13.5 : 7.5;
          let r_mass = global.interstellar["stellar_engine"]
          ...
          let gWell =
            1 +
            (global.stats.achieve["escape_velocity"] &&
            global.stats.achieve.escape_velocity["h"]
              ? global.stats.achieve.escape_velocity["h"] * 0.02
              : 0);
          let output =
            powerModifier(
              (
          ...
          if (output > 10000) {
            ...
            if (output > 20000) {
              output = 20000 + (output - 20000) ** 0.95;
            }
            if (output > 30000) {
              output = 30000 + (output - 30000) ** 0.925;
            }
          }
          if (global.tech["blackhole"] >= 5) {
            let exotic = +global.interstellar.stellar_engine.exotic.toFixed(10);
            let blackhole =
              global.interstellar.stellar_engine.exotic > 0
                ? loc("interstellar_stellar_engine_effect3", [r_mass, exotic])
                : loc("interstellar_stellar_engine_effect2", [r_mass]);
            return `<div>${loc("interstellar_stellar_engine_complete", [
              +output.toFixed(2),
            ])}</div><div>${blackhole}</div>`;
          } else {
            return loc("interstellar_stellar_engine_complete", [
              +output.toFixed(2),
            ]);
          }
        }
    ...
    mass_ejector: {
      ...
      effect(wiki) {
        ...
        if (
          global.race.universe !== "magic" &&
          (wiki || global.stats.blackhole)
        ) {
          let exoticEjectDone =
            global.interstellar?.stellar_engine?.exotic ?? 0;
          let exoticEjectNeeded = (0.025 - exoticEjectDone) * 1e10;
          let exoticEjectRate =
            (global.interstellar?.mass_ejector?.Elerium ?? 0) *
            atomic_mass["Elerium"];
          exoticEjectRate +=
            (global.interstellar?.mass_ejector?.Infernite ?? 0) *
            atomic_mass["Infernite"];

          if (exoticEjectNeeded <= 0) {
            desc += `<div class="has-text-danger-pulse">${loc(
              "interstellar_mass_ejector_reached"
            )}</div>`;
          } else if (exoticEjectRate <= 0) {
            desc += `<div class="has-text-danger">${loc(
              "interstellar_mass_ejector_timer",
              [loc("time_never")]
            )}</div>`;
          } else {
            let timeReq = timeFormat(
              Math.round(exoticEjectNeeded / exoticEjectRate)
            );
            desc += `<div class="has-text-caution">${loc(
              "interstellar_mass_ejector_timer",
              [timeReq]
            )}</div>`;
          }
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (global.interstellar.mass_ejector.count === 1) {
            messageQueue(loc("interstellar_mass_ejector_msg"), "info", false, [
              "progress",
            ]);
    ...
    wormhole_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(150000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(75000).toFixed(0);
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("interstellar_wormhole_mission_result"),
            "info",
            false,
            ["progress"]
          );
    ...
    stargate: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("stargate") ||
          global.interstellar.stargate.count < 200 ||
          wiki
        ) {
          return (
            `<div>${loc(
              "interstellar_stargate"
            )}</div><div class="has-text-special">${loc("requires_segments", [
              200,
            ])}</div>` +
            (global.interstellar.hasOwnProperty("stargate") &&
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("stargate")
              ? global.interstellar.stargate.count
              : 0) <
            200
            ? 1000000
            : 0;
  ...
  int_sirius: {
    info: {
      name() {
        return global.tech.ascension >= 3
          ? loc("interstellar_sirius_b_name")
          : loc("interstellar_sirius_name");
      },
      desc() {
        return global.tech.ascension >= 3
          ? loc("interstellar_sirius_b_desc")
          : loc("interstellar_sirius_desc", [races[global.race.species].home]);
      },
    },
    sirius_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(480000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(225000).toFixed(0);
        },
      },
      effect() {
        return loc("interstellar_sirius_mission_effect", [
          flib("name"),
          races[global.race.species].home,
        ]);
      },
    ...
    space_elevator: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("space_elevator") ||
          global.interstellar.space_elevator.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_space_elevator"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("space_elevator")
              ? global.interstellar.space_elevator.count
              : 0) <
            100
            ? 20000000
            : 0;
      ...
      effect(wiki) {
        let effectText = `<div>${loc(
          "interstellar_space_elevator_effect"
        )}</div>`;
        let count =
          (wiki?.count ?? 0) +
          (global.interstellar.hasOwnProperty("space_elevator")
        ...
        if (count < 100) {
          let remain = 100 - count;
          effectText += `<div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
          )}</div>`;
        }
    ...
    gravity_dome: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("gravity_dome") ||
          global.interstellar.gravity_dome.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "interstellar_gravity_dome"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("gravity_dome")
              ? global.interstellar.gravity_dome.count
              : 0) <
            100
            ? 35000000
            : 0;
      ...
      effect(wiki) {
        let effectText = `<div>${loc("interstellar_gravity_dome_effect", [
          races[global.race.species].home,
        ])}</div>`;
        let count =
          (wiki?.count ?? 0) +
    ...
    ascension_machine: {
      ...
      desc(wiki) {
        if (
          !global.interstellar.hasOwnProperty("ascension_machine") ||
          global.interstellar.ascension_machine.count < 100 ||
          wiki
        ) {
          return (
            `<div>${loc(
      ...
      reqs: { ascension: 6 },
      condition() {
        return global.interstellar.ascension_machine.count >= 100
          ? false
          : true;
      },
      queue_size: 5,
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.interstellar.hasOwnProperty("ascension_machine")
              ? global.interstellar.ascension_machine.count
              : 0) <
            100
            ? 75000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          let remain = 100 - count;
          return `<div>${loc("interstellar_ascension_machine_effect", [
            flib("name"),
          ])}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
            [remain]
    ...
    ascension_trigger: {
      ...
      heatSink() {
        ...
        if (
          ...
          global.stats.achieve.technophobe.l >= 2
        ) {
        ...
        let power = Math.round(
          powerCostMod(10000) -
            heatsink *
              (global.interstellar.hasOwnProperty("thermal_collector")
                ? global.interstellar.thermal_collector.count
                : 0)
        );
        return power;
      },
      special() {
        return global.tech["science"] && global.tech.science >= 24
          ? true
          : false;
      },
      sAction() {
      ...
      effect() {
        if (
          ...
          let desc = `<div>${loc(`eden_ascension_machine_effect1`, [
            loc(`eden_encampment_title`),
            +omniscience.toFixed(0),
            global.resource.Omniscience.name,
          ])}</div>`;
          if (heatSink > 0) {
            let stabilizer = heatSink / 175;
            desc += `<div>${loc(`eden_ascension_machine_effect2`, [
              loc(`eden_stabilizer_title`),
              +stabilizer.toFixed(2),
            ])}</div>`;

            let ghost = heatSink / 125;
            desc += `<div>${loc(`eden_ascension_machine_effect2`, [
              loc(`job_ghost_trapper`),
              +ghost.toFixed(2),
            ])}</div>`;
          }
          ...
          let power_label =
            power > 0
              ...
              : "";
          return `<div>${loc(
            "interstellar_ascension_trigger_effect"
          )}</div>${reward}${power_label}`;
        }
      },
      action() {
...
export function astrialProjection() {
  ...
  let plasmidType =
    global.race.universe === "antimatter"
      ? loc("resource_AntiPlasmid_plural_name")
      : loc("resource_Plasmid_plural_name");
  return `<div class="has-text-advanced">${loc(
    "interstellar_ascension_trigger_effect2",
    [gains.plasmid, plasmidType]
  )}</div><div class="has-text-advanced">${loc(
    "interstellar_ascension_trigger_effect2",
    [gains.phage, loc("resource_Phage_name")]
  )}</div><div class="has-text-advanced">${loc(
    "interstellar_ascension_trigger_effect2",
    [gains.harmony, loc("resource_Harmony_name")]
  )}</div><div>${loc("interstellar_ascension_trigger_effect3")}</div>`;
}
...
function terraformProjection() {
  ...
  return `<div class="has-text-advanced">${loc(
    ...
    "interstellar_ascension_trigger_effect2",
    [gains.harmony, loc("resource_Harmony_name")]
  )}</div><div>${loc("space_terraformer_effect3")}</div>`;
}
...
const galaxyProjects = {
  gxy_gateway: {
    ...
    gateway_mission: {
      ...
      cost: {
        Helium_3() {
          return +int_fuel_adjust(212000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(110000).toFixed(0);
        },
      },
    ...
    starbase: {
      ...
      cost: {
        ...
        Elerium(offset) {
          return spaceCostMultiplier("starbase", offset, 1000, 1.25, "galaxy");
        },
        Mythril(offset) {
          return spaceCostMultiplier("starbase", offset, 90000, 1.25, "galaxy");
        },
        Graphene(offset) {
          return spaceCostMultiplier(
            "starbase",
      ...
      effect(wiki) {
        ...
        let soldiers = $(this)[0].soldiers();
        return `<div class="has-text-advanced">${loc(
          "galaxy_defense_platform_effect",
          [25]
        )}</div><div>${loc("galaxy_gateway_support", [
          $(this)[0].support(),
        ])}</div><div>${loc("plus_max_soldiers", [
          soldiers,
        ])}</div><div class="has-text-caution">${loc(
          "interstellar_alpha_starport_effect2",
          [helium, $(this)[0].powered(wiki)]
        )}</div><div class="has-text-caution">${loc(
          "interstellar_alpha_starport_effect3",
          [food, global.resource.Food.name]
        )}</div>`;
      ...
      powerBalancer() {
        return [
          { s: global.galaxy.starbase.s_max - global.galaxy.starbase.support },
        ];
      },
      refresh: true,
      action() {
    ...
    ship_dock: {
      ...
      effect(wiki) {
        if (global.race["fasting"]) {
          return `<div>${loc("galaxy_ship_dock_effect_fasting", [
            0.1,
          ])}</div><div class="has-text-caution">${loc("minus_power", [
            $(this)[0].powered(wiki),
          ])}</div>`;
        }
        return `<div>${loc("galaxy_ship_dock_effect", [
          0.25,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(wiki),
        ])}</div>`;
      ...
      powerBalancer() {
        if (global.race["fasting"]) {
          return [
            {
              s:
                global.galaxy.gateway_station.s_max -
                global.galaxy.gateway_station.support,
            },
          ];
        }
        return [
    ...
    bolognium_ship: {
      ...
      desc() {
        return `<div>${loc(
          "galaxy_bolognium_ship_desc"
        )}</div><div class="has-text-special">${loc("galaxy_starbase_support", [
          loc("resource_Helium_3_name"),
        ])}</div>`;
      },
      reqs: { gateway: 3 },
      ...
      effect() {
        ...
        return `<div>${loc("gain", [
          bolognium,
          loc("resource_Bolognium_name"),
        ])}</div><div class="has-text-caution">${loc(
          "galaxy_starbase_civ_crew",
          [$(this)[0].ship.civ()]
        )}</div><div class="has-text-caution">${loc(
          "galaxy_gateway_used_support",
          [-$(this)[0].support()]
        )}</div><div class="has-text-caution">${loc("spend", [
          helium,
          global.resource.Helium_3.name,
    ...
    scout_ship: {
      ...
      effect() {
        let helium = +int_fuel_adjust($(this)[0].ship.helium).toFixed(2);
        let sensors =
          global.tech.science >= 17
            ? `<div>${loc("galaxy_scout_ship_effect2", [25])}</div>`
            : "";
        return `<div class="has-text-advanced">${loc("galaxy_ship_rating", [
          $(this)[0].ship.rating(),
        ])}</div><div>${loc(
          "galaxy_scout_ship_effect"
        )}</div>${sensors}<div class="has-text-caution">${loc(
          "galaxy_starbase_civ_crew",
          [$(this)[0].ship.civ()]
        )}</div><div class="has-text-caution">${loc(
          "galaxy_starbase_mil_crew",
          [$(this)[0].ship.mil()]
        )}</div><div class="has-text-caution">${loc(
          "galaxy_gateway_used_support",
          [-$(this)[0].support()]
      ...
      ship: {
        civ() {
          return global.race["grenadier"]
            ? 0
            : global.race["high_pop"]
            ? traits.high_pop.vars()[0] * 1
            : 1;
        },
        mil() {
          let base = global.race["high_pop"]
            ? traits.high_pop.vars()[0] * 1
            : 1;
    ...
    corvette_ship: {
      ...
      ship: {
        ...
        mil() {
          ...
          return global.race["high_pop"]
            ? traits.high_pop.vars()[0] * base
            : base;
        },
        helium: 10,
        rating() {
  ...
  gxy_stargate: {
    info: {
      ...
      desc() {
        return global.tech["piracy"]
          ? loc("galaxy_stargate_desc_alt")
          : loc("galaxy_stargate_desc");
      },
      control() {
        return {
          name: flib("name"),
    ...
    gateway_station: {
      ...
      effect(wiki) {
        ...
        if (global.tech["gateway"] && global.tech["gateway"] >= 2) {
          gateway = `<div>${loc("galaxy_gateway_support", [
            $(this)[0].support(),
          ])}</div>`;
        }
        return `${gateway}<div>${loc("plus_max_resource", [
          helium,
          loc("resource_Helium_3_name"),
        ])}</div><div>${loc("plus_max_resource", [
          deuterium,
      ...
      powerBalancer() {
        return global.galaxy.hasOwnProperty("starbase")
          ? [
              {
                s:
                  global.galaxy.starbase.s_max - global.galaxy.starbase.support,
              },
            ]
          : false;
      },
    ...
    telemetry_beacon: {
      ...
      effect(wiki) {
        ...
        if (global.tech.science >= 17) {
          let num_scout_ship_on = wiki
            ? global.galaxy.scout_ship.on
            : gal_on["scout_ship"];
          base += num_scout_ship_on * 25;
        }
        let num_telemetry_on = wiki
          ? global.galaxy?.telemetry_beacon?.on ?? 0
        ...
        return `${gateway}<div>${loc("galaxy_telemetry_beacon_effect1", [
          base,
        ])}</div><div>${loc("galaxy_telemetry_beacon_effect2", [
          know,
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(wiki),
        ])}</div>`;
      ...
      postPower(o) {
        let powered = o
          ? p_on["telemetry_beacon"] + keyMultiplier()
          : p_on["telemetry_beacon"] - keyMultiplier();
    ...
    gateway_depot: {
      ...
      effect(wiki) {
        ...
        let uranium = sizeApproximation(
          +spatialReasoning(3000 * multiplier).toFixed(0),
          1
        );
        let nano = sizeApproximation(
          +spatialReasoning(250000 * multiplier).toFixed(0),
          1
        );
        let neutronium = sizeApproximation(
          +spatialReasoning(9001 * multiplier).toFixed(0),
          1
        );
        let infernite = sizeApproximation(
          +spatialReasoning(6660 * multiplier).toFixed(0),
          1
        );
        let desc = '<div class="aTable">';
        desc =
          desc +
          `<span>${loc("plus_max_crates", [containers])}</span><span>${loc(
            "plus_max_containers",
            [containers]
          )}</span>`;
        desc =
        ...
        desc =
          ...
          `<span>${loc("plus_max_resource", [
            infernite,
            global.resource.Infernite.name,
          ])}</span>`;
        desc = desc + "</div>";
        return `${desc}<div>${loc("galaxy_gateway_depot_effect", [
          elerium,
          loc("resource_Elerium_name"),
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(wiki),
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          let multiplier = gatewayStorage();
          global["resource"]["Uranium"].max += spatialReasoning(
            3000 * multiplier
          );
          global["resource"]["Nano_Tube"].max += spatialReasoning(
            250000 * multiplier
          );
          global["resource"]["Neutronium"].max += spatialReasoning(
            9001 * multiplier
          );
          global["resource"]["Infernite"].max += spatialReasoning(
            6660 * multiplier
          );
          powerOnNewStruct($(this)[0]);
          return true;
  ...
  gxy_gorddon: {
    ...
    gorddon_mission: {
      ...
      cost: {
        ...
        Helium_3() {
          return +int_fuel_adjust(230000).toFixed(0);
        },
        Deuterium() {
          return +int_fuel_adjust(125000).toFixed(0);
        },
      },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("galaxy_gorddon_mission_result", [
              s1desc,
              s1name,
              s2desc,
              s2name,
    ...
    embassy: {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.galaxy.hasOwnProperty("embassy")
              ? global.galaxy.embassy.count
              : 0) <
            1
            ? 30000000
            : 0;
      ...
      effect(wiki) {
        ...
        if (global.tech.xeno >= 11) {
          housing = `<div>${loc("plus_max_citizens", [
            $(this)[0].citizens(),
          ])}</div>`;
        }
        let foodDesc = "";
        if (!global.race["fasting"]) {
          foodDesc = `<div class="has-text-caution">${loc(
            "interstellar_alpha_starport_effect3",
            [food, global.resource.Food.name]
          )}</div>`;
        }
        return `<div>${loc("galaxy_embassy_effect", [
          races[
            global.galaxy.hasOwnProperty("alien1")
              ? global.galaxy.alien1.id
              : global.race.species
          ].name,
        ])}</div>${housing}${foodDesc}<div class="has-text-caution">${loc(
          "minus_power",
          [$(this)[0].powered(wiki)]
        )}</div>`;
      },
      ...
      action() {
        if (global.galaxy.embassy.count < 1 && payCosts($(this)[0])) {
          ...
          if (global.tech["xeno"] === 4) {
            ...
            messageQueue(
              loc("galaxy_embassy_complete", [
                races[global.galaxy.alien1.id].name,
                races[global.galaxy.alien2.id].name,
              ]),
              "info",
              false,
              ["progress"]
    ...
    dormitory: {
      ...
      effect() {
        return `<div class="has-text-caution">${loc(`requires_res`, [
          loc("galaxy_embassy"),
        ])}</div><div>${loc("plus_max_citizens", [
          $(this)[0].citizens(),
        ])}</div><div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    symposium: {
      ...
      cost: {
        ...
        Food(offset) {
          return global.race["ravenous"]
            ? 0
            : spaceCostMultiplier(
                "symposium",
                offset,
                global.race["artifical"] ? 45000 : 125000,
                1.25,
                "galaxy"
              );
        },
      ...
      effect(wiki) {
        ...
        if (global.tech.xeno >= 7) {
          leave = `<div>${loc("galaxy_symposium_effect3", [
            +highPopAdjust(300 * pirate).toFixed(2),
          ])}</div>`;
        }
        return `<div class="has-text-caution">${loc(`requires_res`, [
          loc("galaxy_embassy"),
        ])}</div><div>${loc("galaxy_symposium_effect", [
          (1750 * pirate).toFixed(0),
        ])}</div><div>${loc("galaxy_symposium_effect2", [
          (650 * pirate).toFixed(0),
        ])}</div>${leave}<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
      },
      powered() {
    ...
    freighter: {
      ...
      desc() {
        return `<div>${loc(
          "galaxy_freighter"
        )}</div><div class="has-text-special">${loc("galaxy_crew_fuel", [
          loc("resource_Helium_3_name"),
        ])}</div>`;
      },
      reqs: { xeno: 5 },
      cost: {
        ...
        Uranium(offset) {
          return spaceCostMultiplier("freighter", offset, 10000, 1.2, "galaxy");
        },
        Adamantite(offset) {
          return spaceCostMultiplier(
            "freighter",
        ...
        Bolognium(offset) {
          return spaceCostMultiplier("freighter", offset, 66000, 1.2, "galaxy");
        },
      },
      effect() {
        let helium = +int_fuel_adjust($(this)[0].ship.helium).toFixed(2);
        ...
        return `<div class="has-text-caution">${loc(`requires_res`, [
          loc("galaxy_embassy"),
        ])}</div><div>${loc("galaxy_freighter_effect", [
          2,
          races[
            global.galaxy.hasOwnProperty("alien1")
              ? global.galaxy.alien1.id
  ...
  gxy_alien1: {
    ...
    consulate: {
      ...
      desc() {
        return loc("galaxy_consulate_desc", [
          races[
            global.galaxy.hasOwnProperty("alien1")
              ? global.galaxy.alien1.id
              : global.race.species
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.galaxy.hasOwnProperty("consulate")
              ? global.galaxy.consulate.count
              : 0) <
            1
            ? 90000000
            : 0;
    ...
    resort: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "resort",
            offset,
            traitCostMod("untrustworthy", 33000000),
            1.25,
            "galaxy"
          );
        },
        ...
        Furs(offset) {
          return spaceCostMultiplier(
            "resort",
            offset,
            traitCostMod("untrustworthy", 10000000),
            1.25,
            "galaxy"
          );
        },
        Oil(offset) {
          return spaceCostMultiplier(
            "resort",
            offset,
            traitCostMod("untrustworthy", int_fuel_adjust(125000)),
            1.25,
            "galaxy"
          );
        },
      ...
      effect() {
        let money = spatialReasoning(
          global.tech["world_control"] ? 1875000 : 1500000
        );
        let joy = global.race["joyless"]
          ? ""
          : `<div>${loc("plus_max_resource", [
              jobScale(2),
              loc(`job_entertainer`),
        ...
        let desc = `<div>${loc("plus_max_resource", [
          ...
          loc("resource_Money_name"),
        ])}</div>${joy}<div>${loc("space_red_vr_center_effect2", [2])}</div>`;
        return (
          desc +
          `<div class="has-text-caution">${loc("minus_power", [
            $(this)[0].powered(),
    ...
    vitreloy_plant: {
      ...
      desc() {
        return `<div>${loc(
          "galaxy_vitreloy_plant"
        )}</div><div class="has-text-special">${loc(
          "galaxy_vitreloy_plant_desc"
        )}</div>`;
      },
      reqs: { xeno: 10 },
      cost: {
      ...
      effect() {
        ...
        let stanene = 100;
        let cash = 50000;
        return `<div>${loc("galaxy_vitreloy_plant_effect", [
          vitreloy,
        ])}</div><div class="has-text-caution">${loc(
  ...
  gxy_alien2: {
    info: {
      name() {
        return loc("galaxy_alien", [
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
              : global.race.species
          ].solar.red,
        ]);
      },
    ...
    alien2_mission: {
      ...
      title() {
        return loc("galaxy_alien2_mission", [
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
              : global.race.species
      ...
      desc() {
        return loc("galaxy_alien2_mission_desc", [
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
              : global.race.species
      ...
      cost: {
        Custom() {
          if (
            global.galaxy.hasOwnProperty("defense") &&
            global.galaxy.defense.hasOwnProperty("gxy_alien2")
          ) {
            let total = 0;
            Object.keys(global.galaxy.defense.gxy_alien2).forEach(function (
              ship
            ) {
              total +=
                galaxyProjects.gxy_gateway[ship].ship.rating() *
                global.galaxy.defense.gxy_alien2[ship];
            });
            return {
              label: loc(`galaxy_fleet_rating`, [
                `<span${
                  total < 400 ? ` class="has-text-danger"` : ""
                }>400</span>`,
              ]),
          ...
          return {
            label: loc(`galaxy_fleet_rating`, [
              `<span class="has-text-danger">400</span>`,
            ]),
            met: false,
          };
        },
      ...
      effect() {
        ...
        let odds =
          total >= 650
            ? `<span class="has-text-success">${loc(
                `galaxy_piracy_low`
              )}</span>`
            : `<span class="has-text-warning">${loc(
                `galaxy_piracy_avg`
              )}</span>`;
        return `<div>${loc("galaxy_alien2_mission_effect2", [
          total,
        ])}</div><div>${loc("galaxy_alien2_mission_effect3", [
          odds,
        ])}</div><div class="has-text-caution">${loc(
          "galaxy_alien2_mission_effect",
          [
            races[
              global.galaxy.hasOwnProperty("alien2")
                ? global.galaxy.alien2.id
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (total >= 400) {
            messageQueue(
              loc("galaxy_alien2_mission_result2", [
                races[global.galaxy.alien2.id].solar.red,
              ]),
              "info",
              false,
              ["progress"]
            ...
            if (total < 650) {
              ...
              Object.keys(global.galaxy.defense.gxy_alien2).forEach(function (
                ...
                for (
                  let i = 0;
                  i < global.galaxy.defense.gxy_alien2[ship];
                  i++
                ) {
                  if (wreck > 0) {
              ...
              messageQueue(
                loc("galaxy_chthonian_mission_result_losses", [
                  loss.map((v) => loc(`galaxy_${v}`)).join(", "),
                ]),
                "danger",
                false,
                ["progress"]
              ...
              for (let i = 0; i < loss.length; i++) {
                ...
                global.galaxy[ship].crew -=
                  galaxyProjects.gxy_gateway[ship].ship.civ();
                global.galaxy[ship].mil -=
                  galaxyProjects.gxy_gateway[ship].ship.mil();
    ...
    foothold: {
      ...
      cost: {
        ...
        Bolognium(offset) {
          return spaceCostMultiplier("foothold", offset, 50000, 1.25, "galaxy");
        },
      },
      effect(wiki) {
        let elerium = 2.5;
        return `<div class="has-text-advanced">${loc(
          "galaxy_defense_platform_effect",
          [50]
        )}</div><div>${loc("galaxy_foothold_effect", [
          $(this)[0].support(),
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
      ...
      powerBalancer() {
        return [
          { s: global.galaxy.foothold.s_max - global.galaxy.foothold.support },
        ];
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    armed_miner: {
      ...
      effect() {
        ...
        return `<div class="has-text-advanced">${loc("galaxy_ship_rating", [
          ...
          adamantite,
          loc("resource_Adamantite_name"),
        ])}</div><div>${loc("gain", [
          iridium,
          loc("resource_Iridium_name"),
        ])}</div><div class="has-text-caution">${loc("galaxy_alien2_support", [
          $(this)[0].support(),
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
    ...
    ore_processor: {
      ...
      effect() {
        return `<div>${loc("galaxy_ore_processor_effect", [
          10,
        ])}</div><div class="has-text-caution">${loc("galaxy_alien2_support", [
          $(this)[0].support(),
          races[
    ...
    scavenger: {
      ...
      effect(wiki) {
        ...
        let helium = +int_fuel_adjust($(this)[0].ship.helium).toFixed(2);
        let boost = global.race["cataclysm"]
          ? `<div>${loc("galaxy_scavenger_effect2_cata", [
              +(pirate * 100 * 0.75).toFixed(1),
            ])}</div>`
          : `<div>${loc("galaxy_scavenger_effect2", [
              +((pirate * 100) / 4).toFixed(1),
            ])}</div>`;
        return `<div>${loc("galaxy_scavenger_effect", [
          know,
        ])}</div>${boost}<div class="has-text-caution">${loc(
          "galaxy_alien2_support",
          [
            $(this)[0].support(),
            races[
              global.galaxy.hasOwnProperty("alien2")
  ...
  gxy_chthonian: {
    info: {
      ...
      desc() {
        return loc("galaxy_chthonian_desc", [
          races[
            global.galaxy.hasOwnProperty("alien2")
              ? global.galaxy.alien2.id
              : global.race.species
    ...
    chthonian_mission: {
      ...
      cost: {
        Custom() {
          if (
            global.galaxy.hasOwnProperty("defense") &&
            global.galaxy.defense.hasOwnProperty("gxy_chthonian")
          ) {
            let total = 0;
            Object.keys(global.galaxy.defense.gxy_chthonian).forEach(function (
              ship
            ) {
              total +=
                galaxyProjects.gxy_gateway[ship].ship.rating() *
            ...
            return {
              label: loc(`galaxy_fleet_rating`, [
                `<span${
                  total < 1250 ? ` class="has-text-danger"` : ``
                }>1250</span>`,
              ]),
          ...
          return {
            label: loc(`galaxy_fleet_rating`, [
              `<span class="has-text-danger">1250</span>`,
            ]),
            met: false,
          };
        },
      ...
      effect() {
        ...
        let odds =
          total >= 4500
            ...
            : `<span class="has-text-danger">${loc(
                `galaxy_piracy_high`
              )}</span>`;
        return `<div>${loc("galaxy_alien2_mission_effect2", [
          total,
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          if (total >= 1250) {
            ...
            let loss = [];
            messageQueue(
              loc("galaxy_chthonian_mission_result"),
              "info",
              false,
              ["progress"]
            );
            ...
            Object.keys(global.galaxy.defense.gxy_chthonian).forEach(function (
              ...
              for (
                let i = 0;
                i < global.galaxy.defense.gxy_chthonian[ship];
                i++
              ) {
                if (wreck > 0) {
                  wreck -= galaxyProjects.gxy_gateway[ship].ship.rating();
    ...
    raider: {
      ...
      cost: {
        ...
        Titanium(offset) {
          return spaceCostMultiplier("raider", offset, 1250000, 1.25, "galaxy");
        },
        Bolognium(offset) {
          return spaceCostMultiplier("raider", offset, 600000, 1.25, "galaxy");
        },
        Vitreloy(offset) {
          return spaceCostMultiplier("raider", offset, 125000, 1.25, "galaxy");
        },
        Stanene(offset) {
          return spaceCostMultiplier("raider", offset, 825000, 1.25, "galaxy");
        },
      },
      effect() {
        let helium = +int_fuel_adjust($(this)[0].ship.helium).toFixed(2);
        ...
        return `<div class="has-text-caution">${loc(`requires_res`, [
          loc("galaxy_starbase"),
        ])}</div><div class="has-text-advanced">${loc("galaxy_ship_rating", [
          $(this)[0].ship.rating(),
        ])}</div><div>${loc("gain", [
          deuterium,
          loc("resource_Deuterium_name"),
          ...
          vitreloy,
          loc("resource_Vitreloy_name"),
        ])}</div><div>${loc("gain", [
          polymer,
          loc("resource_Polymer_name"),
        ])}</div><div>${loc("gain", [
          neutronium,
          loc("resource_Neutronium_name"),
        ])}</div><div class="has-text-caution">${loc(
  ...
  if (global.tech["piracy"] && !global.race["truepath"]) {
    ...
    switch (region) {
      case "gxy_stargate":
        pirate =
          0.1 *
          (global.race["instinct"]
            ? global.tech.piracy * 0.9
            : global.tech.piracy);
        pillage = 0.5;
        break;
      case "gxy_gateway":
    ...
    let num_def_plat_on = wiki
      ? global.galaxy.defense_platform.on
      : p_on["defense_platform"];
    ...
    if (region === "gxy_alien2" && num_foothold_on) {
      ...
      let num_armed_miner_on = wiki
        ? global.galaxy.armed_miner.on
        : gal_on["armed_miner"];
      if (num_armed_miner_on) {
        armada +=
          num_armed_miner_on *
          galaxyProjects.gxy_alien2.armed_miner.ship.rating();
      }
    }

    if (region === "gxy_chthonian") {
      let num_minelayer_on = wiki
        ? global.galaxy.minelayer.on
        : gal_on["minelayer"];
      if (num_minelayer_on) {
        armada +=
          num_minelayer_on *
          galaxyProjects.gxy_chthonian.minelayer.ship.rating();
      }
      let num_raider_on = wiki ? global.galaxy.raider.on : gal_on["raider"];
      if (num_raider_on) {
        armada +=
          num_raider_on * galaxyProjects.gxy_chthonian.raider.ship.rating();
      }
    }
    ...
    if (region !== "gxy_stargate") {
      ...
      return (
        ...
        (rating ? 1 : piracy("gxy_stargate", false, false, wiki))
      );
    } else {
      let patrol = armada > pirate ? pirate : armada;
...
function xeno_race() {
  ...
  let list = Object.keys(races).filter(function (r) {
    return (
      !["demonic", "eldritch"].includes(races[r].type) && !skip.includes(r)
    );
  });
...
export function incrementStruct(c_action, sector) {
  ...
  if (!global[sector][struct]) {
    global[sector][struct] =
      typeof c_action === "object"
        ? c_action.struct().d
...
export function checkRequirements(action_set, region, action) {
  ...
  if (
    action_set[region][action].hasOwnProperty("path") &&
    !action_set[region][action].path.includes(path)
  ) {
    return false;
  }
  var isMet = true;
  Object.keys(action_set[region][action].reqs).forEach(function (req) {
    if (
      !global.tech[req] ||
      global.tech[req] < action_set[region][action].reqs[req]
    ) {
      isMet = false;
    }
  });
  if (
    isMet &&
    action_set[region][action].grant &&
    global.tech[action_set[region][action].grant[0]] &&
    global.tech[action_set[region][action].grant[0]] >=
      action_set[region][action].grant[1]
  ) {
    isMet = false;
  }
  return isMet;
...
function space(zone) {
  ...
  if (!global.settings.tabLoad) {
    if (
      ...
      ![1, 5].includes(global.settings.spaceTabs) ||
      (global.settings.civTabs === 1 &&
        global.settings.spaceTabs === 1 &&
        zone !== "inner") ||
      (global.settings.spaceTabs === 5 && zone !== "outer")
    ) {
      return;
    }
  }
  ...
  parent.append(
    $(
      `<h2 class="is-sr-only">${loc(
        zone === "inner" ? "tab_space" : "tab_outer_space"
      )}</h2>`
    )
  );
  ...
  regionOrder.forEach(function (region) {
    ...
    if (global.settings.space[`${show}`]) {
      ...
      let name =
        typeof spaceProjects[region].info.name === "string"
          ? spaceProjects[region].info.name
          : spaceProjects[region].info.name();
      let noHome =
        global.race["orbit_decayed"] || global.race["cataclysm"] ? true : false;
      ...
      if ((noHome && region !== "spc_home") || !noHome) {
        if (spaceProjects[region].info["support"]) {
          ...
          parent.append(
            `<div id="${region}" class="space"><div id="sr${region}"><h3 class="name has-text-warning">${name}</h3> <span v-show="s_max">{{ support }}/{{ s_max }}</span></div></div>`
          );
          vBind({
            el: `#sr${region}`,
          ...
          parent.append(
            `<div id="${region}" class="space"><div><h3 class="name has-text-warning">${name}</h3></div></div>`
          );
        }

        if (
          global.race["truepath"] &&
          spaceProjects[region].info.hasOwnProperty("syndicate") &&
          spaceProjects[region].info.syndicate() &&
          ...
          $(`#${region}synd`).append(
            `<span class="syndThreat has-text-caution">${loc(
              "space_syndicate"
            )} <span class="has-text-danger" v-html="threat('${region}')"></span></span>`
          );
          $(`#${region}synd`).append(
            `<span class="syndThreat has-text-caution">${loc(
              "space_scan_effectiveness"
            )} <span class="has-text-warning" v-html="scan('${region}')"></span></span>`
          );
          $(`#${region}synd`).append(
            `<span v-show="overkill('${region}')" class="syndThreat has-text-caution">${loc(
              "space_overkill"
            )} <span class="has-text-warning" v-html="overkill('${region}')"></span></span>`
          );
          vBind({
            el: `#${region}synd`,
      ...
      popover(
        region,
        function () {
          return typeof spaceProjects[region].info.desc === "string"
            ? spaceProjects[region].info.desc
            : spaceProjects[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
...
function deepSpace() {
  if (
    ...
    (global.settings.civTabs !== 1 || global.settings.spaceTabs !== 2)
  ) {
    return;
  }
  ...
  Object.keys(interstellarProjects).forEach(function (region) {
    ...
    if (global.settings.space[`${show}`]) {
      let name =
        typeof interstellarProjects[region].info.name === "string"
          ? interstellarProjects[region].info.name
          : interstellarProjects[region].info.name();
      ...
      popover(
        region,
        function () {
          return typeof interstellarProjects[region].info.desc === "string"
            ? interstellarProjects[region].info.desc
            : interstellarProjects[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
      ...
      Object.keys(interstellarProjects[region]).forEach(function (tech) {
        if (
          tech !== "info" &&
          checkRequirements(interstellarProjects, region, tech)
        ) {
...
function galaxySpace() {
  if (
    !global.settings.tabLoad &&
    (global.settings.civTabs !== 1 || global.settings.spaceTabs !== 3)
  ) {
    return;
  }
  let parent = $("#galaxy");
  ...
  Object.keys(galaxyProjects).forEach(function (region) {
    ...
    if (
      global.galaxy["defense"] &&
      !global.galaxy.defense.hasOwnProperty(region)
    ) {
      global.galaxy.defense[region] = {};
    }
    if (global.settings.space[`${show}`]) {
      let name =
        typeof galaxyProjects[region].info.name === "string"
          ? galaxyProjects[region].info.name
          : galaxyProjects[region].info.name();
      ...
      if (global.tech["xeno"] && global.tech["xeno"] >= 3) {
        regionContent.append(
          `<span class="regionControl has-text-${
            galaxyProjects[region].info.control().color
          }">{{ r.control().name }}</span>`
        );
      }
      ...
      if (galaxyProjects[region].info["support"]) {
        ...
        if (global.galaxy[support]) {
          ...
          regionContent.append(
            `<span class="regionSupport" v-show="s.s_max">{{ s.support }}/{{ s.s_max%1 ? s.s_max.toFixed(2) : s.s_max }}</span>`
          );
      ...
      if (global.tech["piracy"]) {
        regionContent.append(
          `<div><span class="has-text-caution pirate">${loc(
            "galaxy_piracy_threat",
            [races[global.galaxy.alien2.id].name]
          )}</span><span :class="threat('${region}')">{{ '${region}' | pirate }}</span><span class="sep">|</span><span class="has-text-warning">${loc(
            "galaxy_armada"
          )}</span>: <span class="has-text-success">{{ '${region}' | defense }}</span></div>`
        );
      }
      ...
      popover(
        region,
        function () {
          return typeof galaxyProjects[region].info.desc === "string"
            ? galaxyProjects[region].info.desc
            : galaxyProjects[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
      ...
      popover(
        region,
        function () {
          return loc("galaxy_control", [
            galaxyProjects[region].info.control().name,
            name,
          ]);
        },
        {
      ...
      Object.keys(galaxyProjects[region]).forEach(function (tech) {
        if (
          tech !== "info" &&
          checkRequirements(galaxyProjects, region, tech)
        ) {
...
function armada(parent, id) {
  if (global.tech["piracy"] && !global.race["truepath"]) {
    let header = $(
      `<div id="h${id}" class="armHead"><h3 class="has-text-warning">${loc(
        "galaxy_armada"
      )}</h3></div>`
    );
    parent.append(header);

    let soldier_title = global.tech["world_control"]
      ? loc("civics_garrison_peacekeepers")
      : loc("civics_garrison_soldiers");
    header.append($(`<span>|</span>`));
    header.append(
      $(
        `<span class="has-text-caution"><span class="soldier">${soldier_title}</span> <span>{{ g.workers | stationed }} / {{ g.max | s_max }}</span></span>`
      )
    );
    header.append($(`<span>|</span>`));
    header.append(
      $(
        `<span class="has-text-caution"><span class="crew1">${loc(
          "job_crew_mil"
        )}</span> <span>{{ g.crew }}</span></span>`
      )
    );
    header.append($(`<span>|</span>`));
    header.append(
      $(
        `<span class="has-text-success"><span class="crew2">${loc(
          "job_crew_civ"
        )}</span> <span>{{ c.workers }} / {{ c.max }}</span></span>`
      )
    );
    ...
    for (let i = 0; i < ships.length; i++) {
      if (global.galaxy.hasOwnProperty(ships[i])) {
        let ship = $(
          `<span id="armada${ships[i]}" class="ship has-text-advanced">${
            galaxyProjects.gxy_gateway[ships[i]].title
          }</span>`
        );
    ...
    cols[0].append(
      $(
        `<span id="armadagateway" class="has-text-danger">${galaxyProjects.gxy_gateway.info.name}</span>`
      )
    );
    ...
    Object.keys(global.galaxy.defense).forEach(function (area) {
      ...
      if (global.settings.space[r] && r !== "gateway") {
        let region = $(
          `<span id="armada${r}" class="has-text-caution">${
            typeof galaxyProjects[area].info.name === "string"
              ? galaxyProjects[area].info.name
              : galaxyProjects[area].info.name()
          }</span>`
        );
        ...
        for (let i = 0; i < ships.length; i++) {
          if (global.galaxy.hasOwnProperty(ships[i])) {
            ...
            let sub = $(
              `<span role="button" aria-label="remove ${ships[i]}" class="sub has-text-danger" @click="sub('${area}','${ships[i]}')"><span>&laquo;</span></span>`
            );
            let count = $(
              `<span class="current">{{ ${r}.${ships[i]} }}</span>`
            );
            let add = $(
              `<span role="button" aria-label="add ${ships[i]}" class="add has-text-success" @click="add('${area}','${ships[i]}')"><span>&raquo;</span></span>`
            );
            cols[i + 1].append(ship);
            ship.append(sub);
            ship.append(count);
    ...
    Object.keys(global.galaxy.defense).forEach(function (area) {
      ...
      if (global.settings.space[r]) {
        popover(`armada${r}`, function () {
          return `<div>${
            typeof galaxyProjects[area].info.desc === "string"
              ? galaxyProjects[area].info.desc
              : galaxyProjects[area].info.desc()
          }</div>`;
        });
      }
    ...
    for (let i = 0; i < ships.length; i++) {
      if (global.galaxy.hasOwnProperty(ships[i])) {
        popover(`armada${ships[i]}`, function (obj) {
          actionDesc(
            obj.popper,
            galaxyProjects.gxy_gateway[ships[i]],
            global.galaxy[ships[i]]
          );
          return undefined;
        });
      }
...
export function iron_adjust(res, wiki) {
  let num_iron_ship_on = wiki
    ? global.space?.iron_ship?.on ?? 0
...
export function genPlanets() {
  ...
  if (
    ...
    global.custom.hasOwnProperty("planet")
  ) {
  ...
  if (global.race["geck"] && global.race.geck > 0) {
    let geck = $(
      `<div id="geck" class="geck"><span class="has-text-caution">${loc(
        `gecks_remaining`
      )}</span>: <span class="has-text-warning">{{ geck }}</span></div>`
    );
...
export function setUniverse() {
  ...
  for (let i = 0; i < universes.length; i++) {
    ...
    let element = $(
      `<a class="button is-dark" v-on:click="action"><span class="aTitle">${universe_types[universe].name}</span></a>`
    );
...
export function ascendLab(hybrid, wiki) {
  if (!wiki && !global.race["noexport"]) {
    ...
    if (!global["sim"]) {
      save.setItem(
        "evolveBak",
        LZString.compressToUTF16(JSON.stringify(global))
      );
    }
    ...
    if (hybrid) {
      ...
      if (
        ...
        global.stats.achieve.what_is_best["e"]
      ) {
  ...
  let wikiVars = {
    ascended: {},
    technophobe:
      global.stats.achieve["technophobe"] && global.stats.achieve.technophobe.l
        ? global.stats.achieve.technophobe.l
        : 0,
  };
  ...
  let labStatus = `<div><h3 class="has-text-danger">${loc(
    "genelab_title"
  )}</h3> - <span class="has-text-warning">${loc(
    "genelab_genes"
  )} {{ g.genes }}</span> - <span class="has-text-warning">${loc(
    "trait_untapped_name"
  )}: {{ g.genes | untapped }}</span> - <span class="has-text-caution">${loc(
    "genelab_neg"
  )} {{ td.neg }}/10</span></div>`;
  ...
  if (wiki) {
    ...
    Object.keys(universe_types).forEach(function (uni) {
      ...
      ascended_levels.append(`
                <div class="calcInput"><span>${loc(
                  "universe_" + uni
                )}</span> <b-numberinput :input="val('${uni}')" min="0" max="5" v-model="w.ascended.${uni}" :controls="false"></b-numberinput></div>
            `);
    });
    lab.append(`
            <div class="has-text-caution">${loc(
              "achieve_technophobe_name"
            )}</div>
            <div>
  ...
  let name = $(
    `<div class="fields"><div class="name">${loc(
      "genelab_name"
    )} <b-input v-model="g.name" maxlength="20"></b-input></div><div class="entity">${loc(
      "genelab_entity"
    )} <b-input v-model="g.entity" maxlength="40"></b-input></div><div class="name">${loc(
      "genelab_home"
    )} <b-input v-model="g.home" maxlength="20"></b-input></div> <div>${loc(
      "genelab_desc"
    )} <b-input v-model="g.desc" maxlength="255"></b-input></div></div>`
  );
  lab.append(name);
  ...
  let planets = $(`<div class="fields">
        <div class="name">${loc(
          "genelab_red"
        )} <b-input v-model="g.red" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_hell"
        )} <b-input v-model="g.hell" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_gas"
        )} <b-input v-model="g.gas" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_gas_moon"
        )} <b-input v-model="g.gas_moon" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_dwarf"
        )} <b-input v-model="g.dwarf" maxlength="20"></b-input></div></div>`);
  lab.append(planets);

  let tpPlanets = $(`<div class="fields">
        <div class="name">${loc(
          "genelab_titan"
        )} <b-input v-model="g.titan" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_enceladus"
        )} <b-input v-model="g.enceladus" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_triton"
        )} <b-input v-model="g.triton" maxlength="20"></b-input></div>
        <div class="name">${loc(
          "genelab_eris"
        )} <b-input v-model="g.eris" maxlength="20"></b-input></div></div>`);
  lab.append(tpPlanets);

  let genes = $(`<div class="sequence"></div>`);
  lab.append(genes);

  let fanatic = `<div id="geneLabFanatic"><button class="button" @click="fanatic()">${loc(
    `tech_fanaticism`
  )}</button><div class="has-text-special">{{ g.fanaticism | fanaticism }}</div></div>`;

  let dGenus = false;
  let genus = `<div class="genus_selection"><div class="has-text-caution">${loc(
    "genelab_genus"
  )}</div><template><section>`;
  Object.keys(genus_traits).forEach(function (type) {
    if (
      wiki ||
      (global.stats.achieve[`genus_${type}`] &&
      ...
      genus =
        genus +
        `<div class="field ${type}"><b-radio v-model="g.genus" native-value="${type}">${loc(
          `genelab_genus_${type}`
        )}</b-radio></div>`;
    }
  });
  ...
  let trait_list = `<div class="trait_selection"><div class="has-text-warning">${loc(
    "genelab_traits"
  )}</div><template><section>`;
  let negative = "";
  let unlockedTraits = {};
  Object.keys(races).forEach(function (race) {
    ...
    if (
      ...
      if (
        races[race].hasOwnProperty("traits") &&
        !["junker", "sludge", "ultra_sludge"].includes(race)
      ) {
    ...
    .forEach(function (trait) {
      if (traits.hasOwnProperty(trait) && traits[trait].type === "major") {
        if (traits[trait].val >= 0) {
          trait_list =
            trait_list +
            `<div class="field t${trait}"><b-checkbox :disabled="allowed('${trait}')" :input="geneEdit()" v-model="g.traitlist" native-value="${trait}"><span class="has-text-success">${loc(
              `trait_${trait}_name`
            )}</span> (<span class="has-text-advanced">{{ '${trait}' | cost }}</span>)</b-checkbox></div>`;
        } else {
          negative =
            negative +
            `<div class="field t${trait}"><b-checkbox :disabled="allowed('${trait}')" :input="geneEdit()" v-model="g.traitlist" native-value="${trait}"><span class="has-text-danger">${loc(
              `trait_${trait}_name`
            )}</span> (<span class="has-text-caution">{{ '${trait}' | cost }}</span>)</b-checkbox></div>`;
        }
      }
    });
  ...
  let buttons = `
        <div class="reset">
            <button class="button" @click="reset()">${loc(
              "genelab_reset"
            )}</button>
        </div>
        <hr>
        ...
        <div class="importExport">
            <button class="button" @click="customImport">${loc(
              "genelab_import"
            )}</button>
            <input type="file" class="fileImport" id="customFile" accept=".txt">
            <button class="button right" @click="customExport">${loc(
              "genelab_export"
            )}</button>
        </div>
        <div class="importExport">
  ...
  if (!wiki) {
    buttons += `
            <div class="create">
                <button class="button" @click="setRace()">${loc(
                  "genelab_create"
                )}</button>
            </div>
        `;
  ...
  let genome =
    global.hasOwnProperty("custom") && global.custom.hasOwnProperty(slot)
      ? {
          ...
          fanaticism:
            global.custom[slot].hasOwnProperty("fanaticism") &&
            global.custom[slot].fanaticism
              ? global.custom[slot].fanaticism
              : false,
        }
      : {
  ...
  for (let i = genome.traitlist.length - 1; i >= 0; i--) {
    if (
      !traits.hasOwnProperty(genome.traitlist[i]) ||
      traits[genome.traitlist[i]].type !== "major"
    ) {
  ...
  vBind({
    ...
    methods: {
      ...
      setRace() {
        if (
          genome.fanaticism &&
          !genome.traitlist.includes(genome.fanaticism)
        ) {
          return false;
        }
        if (
          calcGenomeScore(genome) >= 0 &&
          genome.name.length > 0 &&
          genome.desc.length > 0 &&
          genome.entity.length > 0 &&
          genome.home.length > 0 &&
          genome.red.length > 0 &&
          genome.hell.length > 0 &&
          genome.gas.length > 0 &&
          genome.gas_moon.length > 0 &&
          genome.dwarf.length > 0
        ) {
          let neg_traits = 0;
          for (let i = 0; i < genome.traitlist.length; i++) {
            if (traits[genome.traitlist[i]].val < 0) {
      ...
      allowed(t) {
        if (
          ...
          ["deconstructor", "imitation"].includes(t)
        ) {
      ...
      fanatic() {
        ...
        var checkExist = setInterval(function () {
          if ($("#modalBox").length > 0) {
            ...
            $("#modalBox").append(
              $(
                `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc(
                  `genelab_fanatic_set`
                )}</p>`
              )
            );
            ...
            genome.traitlist.forEach(function (trait) {
              if (trait !== "imitation") {
                traits += `<div class="field ${trait}"><b-radio v-model="fanaticism" native-value="${trait}">${loc(
                  `trait_${trait}_name`
                )}</b-radio></div>`;
              }
            });
        ...
        if (file) {
          ...
          reader.onload = function (evt) {
            ...
            Object.keys(genome).forEach(function (type) {
              if (
                importCustom[type] &&
                typeof genome[type] !== typeof importCustom[type]
              ) {
                formatError = true;
                return;
              }
            ...
            [
              ...
              if (
                !importCustom[field] &&
                ["titan", "enceladus", "triton", "eris"].includes(field)
              ) {
            ...
            if (
              !wiki &&
              !(
                global.stats.achieve[`genus_${genome.genus}`] &&
                global.stats.achieve[`genus_${genome.genus}`].l > 0
              )
            ) {
              genome.genus = dGenus;
            }
            ...
            for (let i = 0; i < genome.traitlist.length; i++) {
              if (
                traits.hasOwnProperty(genome.traitlist[i]) &&
                traits[genome.traitlist[i]].type === "major" &&
                unlockedTraits[genome.traitlist[i]] &&
                !fixTraitlist.includes(genome.traitlist[i])
              ) {
                fixTraitlist.push(genome.traitlist[i]);
              }
            }
            (genome.fanaticism = importCustom.hasOwnProperty("fanaticism")
              ? importCustom.fanaticism
              : false),
      ...
      customExport() {
        ...
        downloadToFile(
          JSON.stringify(genome, null, 4),
          `evolve-custom-${genome.name}.txt`,
          "text/plain"
        );
      },
    },
    filters: {
      ...
      untapped(genes) {
        ...
        let num =
          genes > 0
            ? +((genes / (genes + 20) / 10 + 0.00024) * 100).toFixed(3)
            : 0;
        return `+${num}%`;
      },
      fanaticism(trait) {
  ...
  Object.keys(genus_traits).forEach(function (type) {
    if (
      ...
      popover(
        `celestialLabgenusSelection${type}`,
        function () {
          let desc = $(
            `<div><div>${loc(`genelab_genus_${type}_desc`)}</div></div>`
          );
...
export function terraformLab(wiki) {
  ...
  let wikiVars = {
    ascended: {},
    lamentis:
      global.stats.achieve["lamentis"] && global.stats.achieve.lamentis.l
        ? global.stats.achieve.lamentis.l
        : 0,
  };
  ...
  lab.append(
    `<div><h3 class="has-text-danger">${loc(
      "planetlab_title"
    )}</h3> - <span class="has-text-warning">${loc(
      "planetlab_points"
    )} {{ p.pts }}</span></div>`
  ...
  let dBiome = false;
  let biome = `<div class="genus_selection"><div class="has-text-caution">${loc(
    "wiki_planet_biome"
  )}</div><template><section>`;
  Object.keys(biomes).forEach(function (type) {
    if (
      wiki ||
      (global.stats.achieve[`biome_${type}`] &&
        global.stats.achieve[`biome_${type}`].l > 0)
    ) {
      if (!dBiome) {
        dBiome = type;
      }
      biome =
        biome +
        `<div class="field ${type}"><b-radio v-model="p.biome" native-value="${type}">${biomes[type].label}</b-radio></div>`;
    }
  });
  biome = biome + `</section></template></div>`;
  pBiome.append($(biome));

  let trait_list = `<div class="planet_selection"><div class="has-text-warning">${loc(
    "wiki_planet_trait"
  )}</div><template><section>`;
  Object.keys(planetTraits).forEach(function (trait) {
    if (
      wiki ||
      (global.stats.achieve[`atmo_${trait}`] &&
        global.stats.achieve[`atmo_${trait}`].l > 0)
    ) {
      trait_list =
        trait_list +
        `<div class="field t${trait}"><b-checkbox :input="pEdit()" v-model="p.traitlist" native-value="${trait}"><span class="has-text-success">${planetTraits[trait].label}</span></b-checkbox></div>`;
    }
  });
  ...
  let geo_list = `<div class="res_selection"><div class="has-text-warning">${loc(
    "planetlab_res"
  )}</div><template><section>`;
  geoList.forEach(function (res) {
    ...
    geo_list =
      geo_list +
      `<div class="field t${res}"><div>${global.resource[res].name}</div><div>`;
    geo_list =
      geo_list +
      `<span role="button" aria-label="export ${res}" class="sub has-text-danger" @click="less('${res}')"><span>-</span></span>`;
    geo_list =
      geo_list +
      `<span class="current" v-html="$options.filters.res('${res}')"></span>`;
    geo_list =
      geo_list +
      `<span role="button" aria-label="import ${res}" class="add has-text-success" @click="more('${res}')"><span>+</span></span>`;
  ...
  if (!wiki) {
    buttons += `
            <div class="create">
                <button class="button" @click="setPlanet()">${loc(
                  "planetlab_create"
                )}</button>
            </div>
        `;
...
function terraformScore(planet, wiki) {
  let pts =
    (planet.biome === "eden" ? 0 : 10) +
    (global.stats.achieve["lamentis"]
      ? global.stats.achieve.lamentis.l * 10
      : 0);
  if (global.race["truepath"]) {
    pts *= 2;
  }
