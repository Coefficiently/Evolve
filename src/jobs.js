...
export const job_desc = {
  ...
  hunter: function (servant) {
    ...
    if (global.race["artifical"]) {
      desc = global.race["soul_eater"]
        ? loc("job_art_demon_hunter_desc", [
            global.resource.Furs.name,
            global.resource.Lumber.name,
          ])
        : loc("job_art_hunter_desc", [global.resource.Furs.name]);
    } else if (global.race["soul_eater"] && global.race.species !== "wendigo") {
      desc = loc(
        global.race["evil"]
          ? "job_evil_hunter_desc"
          : "job_not_evil_hunter_desc",
        [
    ...
    if (global.civic.d_job === "hunter" && !servant) {
      desc =
        ...
        " " +
        loc("job_default", [
          global.race["unfathomable"] ? loc("job_raider") : loc("job_hunter"),
        ]);
    }
    return desc;
  },
  ...
  farmer: function (servant) {
    ...
    let desc =
      global.race["high_pop"] && !servant
        ? loc("job_farmer_desc_hp", [
            farmer,
            global.resource.Food.name,
            jobScale(1),
            farmhand,
            jobScale(1) * global.city.farm.count,
          ])
        : loc("job_farmer_desc", [
            farmer,
            global.resource.Food.name,
    ...
    if (
      ...
      let desc =
        global.race.species === "wendigo"
          ? loc("job_reclaimer_desc2", [bone])
          : loc("job_reclaimer_desc", [bone, flesh]);
      ...
      let multiplier =
        (global.tech["axe"] && global.tech["axe"] > 0
      ...
      let desc = loc("job_lumberjack_desc", [
        gain,
        global.resource.Lumber.name,
      ]);
      if (global.civic.d_job === "lumberjack" && !servant) {
      ...
      if (hallowed.active) {
        desc =
          desc +
          ` <span class="has-text-special">${loc(
            "events_halloween_lumberjack"
          )}</span> `;
      }
      return desc;
    }
  ...
  quarry_worker: function (servant) {
    let workers =
      servant && global.race["servants"]
        ? global.race.servants.jobs.quarry_worker
    ...
    let desc = global.resource.Aluminium.display
      ? loc("job_quarry_worker_desc2", [
          gain,
          global.resource.Stone.name,
          global.resource.Aluminium.name,
        ])
      : loc("job_quarry_worker_desc1", [gain, global.resource.Stone.name]);
    if (global.race["smoldering"]) {
      desc =
        desc +
        " " +
        loc("job_quarry_worker_smoldering", [global.resource.Chrysotile.name]);
    }
  ...
  crystal_miner: function (servant) {
    let workers =
      servant && global.race["servants"]
        ? global.race.servants.jobs.crystal_miner
    ...
    let gain = +(impact * multiplier).toFixed(2);
    let desc = loc("job_crystal_miner_desc", [
      gain,
  ...
  scavenger: function (servant) {
    ...
    let desc = loc("job_scavenger_desc", [
  ...
  miner: function () {
    if (global.tech["mining"] >= 3) {
      return global.race["sappy"] && global.tech["alumina"]
        ? loc("job_miner_desc2_amber")
        : loc("job_miner_desc2");
    } else {
  ...
  cement_worker: function () {
    ...
    let worker_impact = +workerScale(
      global.civic.cement_worker.impact,
      "cement_worker"
    ).toFixed(2);
    let impact =
      global.tech["cement"] >= 4 ? (global.tech.cement >= 7 ? 1.45 : 1.2) : 1;
    let cement_multiplier = racialTrait(
    ...
    return global.race["sappy"]
      ? loc("job_cement_worker_amber_desc", [gain])
      : loc("job_cement_worker_desc", [gain, unit_price]);
  },
  banker: function () {
    let interest =
      +workerScale(global.civic.banker.impact, "banker").toFixed(2) * 100;
  ...
  entertainer: function () {
    ...
    return global.tech["superstar"]
      ? loc("job_entertainer_desc2", [morale, mcap])
      : loc("job_entertainer_desc", [+morale.toFixed(2)]);
  },
  priest: function () {
    let desc = ``;
    if (
  ...
  professor: function () {
    ...
    let impact = +(global.race["studious"]
  ...
  scientist: function () {
    let impact = +workerScale(
      global.civic.scientist.impact,
      "scientist"
    ).toFixed(2);
    ...
    if (global.tech["science"] >= 6 && global.city["wardenclyffe"]) {
      impact *=
        1 +
        global.civic.professor.workers * global.city["wardenclyffe"].on * 0.01;
    }
    ...
    return global.race.universe === "magic"
      ? loc("job_wizard_desc", [
          impact,
          +(0.025 * darkEffect("magic")).toFixed(4),
        ])
      : loc("job_scientist_desc", [impact]);
  },
  colonist() {
    return loc(
      global.race["truepath"] ? "job_colonist_desc_tp" : "job_colonist_desc",
      [planetName().red]
    );
  },
  titan_colonist() {
  ...
  archaeologist() {
    ...
    let supress =
      (armyRating(global.portal.guard_post.on, "hellArmy", 0) + arc) / 5000;
  ...
  ghost_trapper() {
    ...
    return loc("job_ghost_trapper_desc", [
  ...
  elysium_miner() {
    ...
    if (global.tech["elysium"] && global.tech.elysium >= 12) {
      desc += ` ${loc("eden_restaurant_effect", [
        0.15,
    ...
    $("#civics").append(
      $(
        `<h2 class="is-sr-only">${loc(
          "civics_jobs"
        )}</h2><div class="tile is-child jobList"><div id="sshifter" class="tile sshifter"></div><div id="jobs" class="tile is-child"></div><div id="foundry" class="tile is-child"></div><div id="servants" class="tile is-child"></div><div id="skilledServants" class="tile is-child"></div></div>`
      )
    );
  }
...
export function workerScale(num, job) {
  ...
  if (
    (global.race["swift"] || global.race["living_tool"]) &&
    [
      "hunter",
      "forager",
      "farmer",
...
function loadJob(job, define, impact, stress, color) {
  ...
  if (!global.civic[job]["assigned"]) {
    global.civic[job]["assigned"] =
      job === "craftsman" ? 0 : global.civic[job].workers;
  }
  ...
  var civ_container = $(
    `<div id="${id}" v-show="showJob('${job}')" class="job"></div>`
  );
  var controls = servant
    ? $(`<div class="controls"></div>`)
    : $(`<div v-show="!isDefault('${job}')" class="controls"></div>`);
  if (!color || job === "unemployed") {
    ...
    let job_label = $(
      `<div class="job_label"><h3 class="has-text-${color}">{{ civic.${job}.name }}</h3><span :class="level('${job}')">{{ civic.${job}.workers | adjust('${job}') }} / {{ civic.${job}.max | adjust('${job}') }}</span></div>`
    );
    civ_container.append(job_label);
  }
  ...
  if (job !== "crew") {
    var sub = $(
      `<span role="button" aria-label="${loc("remove")} ${
        global["civic"][job].name
      }" class="sub has-text-danger" @click="sub"><span>&laquo;</span></span>`
    );
    var add = $(
      `<span role="button" aria-label="${loc("add")} ${
        global["civic"][job].name
      }" class="add has-text-success" @click="add"><span>&raquo;</span></span>`
    );
    controls.append(sub);
    controls.append(add);
  }
  ...
  if (servant) {
    vBind({
      ...
      methods: {
        showJob(j) {
          return (
            global.civic[j].display ||
            (j === "scavenger" && global.race.servants.force_scavenger)
          );
        },
        add() {
    ...
    vBind({
      ...
      filters: {
        ...
        event(c) {
          if (
            (job === "unemployed" && global.civic.unemployed.display) ||
...
export function loadServants() {
  ...
  if (
    ...
    Object.keys(global.race.servants.jobs).length > 0
  ) {
    var servants = $(
      `<div id="servantList" class="job"><div class="foundry job_label"><h3 class="serveHeader has-text-warning">${loc(
        "civics_servants"
      )}</h3><span :class="level()">{{ s.used }} / {{ s.max }}</span></div></div>`
    );
      ...
      methods: {
        level() {
          if (global.race.servants.used === 0) {
            ...
            global.race.servants.used <=
            global.race.servants.max / 3
          ) {
            return "count has-text-caution";
          } else if (
            global.race.servants.used <=
            global.race.servants.max * 0.66
          ) {
            return "count has-text-warning";
          } else if (global.race.servants.used < global.race.servants.max) {
            return "count has-text-info";
...
export function limitCraftsmen(res) {
  ...
  if (
    ...
    global.city.foundry.hasOwnProperty(res) &&
    cap < global.city.foundry[res]
  ) {
    ...
    tmp_vars["craftsman_cap"].hasOwnProperty(res) &&
    cap != tmp_vars["craftsman_cap"][res]
  ) {
    refresh = true;
  }
...
export function farmerValue(farm, servant) {
  ...
  if (farm) {
    farming +=
      global.tech["agriculture"] && global.tech.agriculture >= 2 ? 1.15 : 0.65;
  }
    ...
    farming *=
      ...
      traits.living_tool.vars()[0] *
        (global.tech["science"] && global.tech.science > 0
          ? global.tech.science / 5
          : 0);
  } else {
    farming *=
      1 + (global.tech["hoe"] && global.tech.hoe > 0 ? global.tech.hoe / 3 : 0);
  }
  ...
  farming *= global.city.ptrait.includes("trashed")
  ...
  farming *= global.race["low_light"]
    ? 1 - traits.low_light.vars()[0] / 100
    : 1;
  return farming;
}
...
export function loadFoundry(servants) {
  ...
  if (
    ...
    let track = servants
      ...
      : `{{ f.crafting }} / {{ c.max }}`;
    let foundry = $(
      `<div class="job"><div class="foundry job_label"><h3 class="has-text-warning">${loc(
        servants ? "civics_skilled_servants" : "craftsman_assigned"
      )}</h3><span :class="level()">${track}</span></div></div>`
    );
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (global.resource[res].display || (summer && res === "Thermite")) {
        ...
        if (
          res === "Scarletite" &&
          global.portal.hasOwnProperty("hell_forge")
        ) {
          job_label = $(
          ...
          res === "Quantium" &&
          (global.space.hasOwnProperty("zero_g_lab") ||
            global.tauceti.hasOwnProperty("infectious_disease_lab"))
        ) {
          job_label = $(
            `<div id="craft${res}" class="job_label"><h3 class="has-text-danger">${name}</h3><span class="count">{{ f.${res} }} / {{ e.on | maxQuantium }}</span></div>`
          );
        } else {
          let tracker = servants ? `{{ s.sjobs.${res} }}` : `{{ f.${res} }}`;
          let id = servants ? `scraft${res}` : `craft${res}`;
          job_label = $(
            `<div id="${id}" class="job_label"><h3 class="has-text-danger">${name}</h3><span class="count">${tracker}</span></div>`
          );
        }
        ...
        let sub = $(
          `<span role="button" aria-label="remove ${res} craftsman" class="sub has-text-danger" @click="sub('${res}')"><span>&laquo;</span></span>`
        );
        let add = $(
          `<span role="button" aria-label="add ${res} craftsman" class="add has-text-success" @click="add('${res}')"><span>&raquo;</span></span>`
        );
    ...
    vBind({
      ...
      methods: {
        ...
        level() {
          let workers = servants
            ? global.race.servants.sused
            : global.civic.craftsman.workers;
          let max = servants
            ? global.race.servants.smax
      ...
      if (global.resource[res].display || (summer && res === "Thermite")) {
        let extra = function () {
          ...
          for (let i = 0; i < craft_cost[res].length; i++) {
            ...
            total.append(
              $(
                `<div>${loc("craftsman_hover_cost", [
                  cost,
                  global.resource[craft_cost[res][i].r].name,
                ])}<div>`
              )
            );
          }
