...
export function teamster(v) {
  if (
    ...
    global.race["teamster"] &&
    global.race.teamster > 0
  ) {
    let cap = teamsterCap();
    if (cap < 1) {
      cap = 1;
    ...
    let teamster =
      global.civic.teamster.workers > cap ? cap : global.civic.teamster.workers;
    v *= teamster / cap;
  }
  return v;
}
  ...
  switch (id) {
    ...
    case "titan_mine": {
      switch (val) {
        case "adamantite": {
          ...
          return (
            (base *
              (global.space["titan_mine"]
                ? global.space.titan_mine.ratio
                : 50)) /
            100
          );
        }
    ...
    case "mining_ship": {
      if (global.tauceti["patrol_ship"]) {
        let patrol = 1;
        if (
          global.tauceti.patrol_ship.support > global.tauceti.patrol_ship.s_max
        ) {
          patrol = flib(
            "curve",
            global.tauceti.patrol_ship.s_max /
              global.tauceti.patrol_ship.support,
            1.4
          );
        }
        return (
          (global.tech["tau_ore_mining"] && global.tech.tau_ore_mining >= 2
            ? 12
            : 10) * patrol
        );
      }
    ...
    case "psychic_boost": {
      if (
        ...
        global.race.psychicPowers.boost.r === val &&
        global.race.psychicPowers.hasOwnProperty("boostTime")
      ) {
        let boost = 0;
    ...
    case "psychic_cash": {
      if (
        ...
        global.race["psychic"] &&
        global.race["psychicPowers"] &&
        global.race.psychicPowers.hasOwnProperty("cash")
      ) {
        let boost = 0;
