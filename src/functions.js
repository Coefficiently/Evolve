...
export function popover(id, content, opts) {
    ...
    $(opts.elm).on(
      ...
      function () {
        ...
        let classes = opts["classes"]
          ? opts["classes"]
          : `has-background-light has-text-dark pop-desc`;
        var popper = $(
          `<div id="popper" class="popper${wide} ${classes}" data-id="${id}"></div>`
        );
        if (opts["attach"]) {
        ...
        if (content) {
          popper.append(
            typeof content === "function"
              ? content({ this: this, popper: popper })
              : content
          );
        }
        ...
        if (
          ...
          $(popper).append(
            `<span class="pyro"><span class="before"></span><span class="after"></span></span>`
          );
        }
      }
    );
  ...
  if (opts["unbind"]) {
    if (
      ...
      $(opts.elm).on(
        opts["bind_mouse_enter"] ? "mouseleave" : "mouseout",
        function () {
          clearPopper();
          if (opts.hasOwnProperty("out") && typeof opts["out"] === "function") {
            opts["out"]({ this: this, popper: $(`#popper`), id: `popper` });
...
export function addATime(currentTimestamp) {
  ...
  if (
    exceededATimeThreshold(currentTimestamp) ||
    (global.stats.hasOwnProperty("current") && global.settings.at > 0)
  ) {
    ...
    if (timeDiff >= 120000) {
      ...
      global.settings.at += Math.floor(
        ((2 / 3) * timeDiff * timers.timeAccelerationFactor) / gameDayDuration
      );
    }
...
export function exceededATimeThreshold(currentTimestamp) {
  return (
    global.stats.hasOwnProperty("current") &&
    currentTimestamp - global.stats.current >= 120000
  );
}
...
window.importGame = function importGame(data, utf16) {
  ...
  if (
    saveState &&
    "evolution" in saveState &&
    "settings" in saveState &&
    "stats" in saveState &&
    "plasmid" in saveState.stats
  ) {
    if (webWorker.w) {
    ...
    if (saveState.hasOwnProperty("tech") && utf16) {
      if (
        saveState.tech.hasOwnProperty("whitehole") &&
        saveState.tech.whitehole >= 4
      ) {
      ...
      if (
        saveState.tech.hasOwnProperty("quaked") &&
        saveState.tech.quaked === 2
      ) {
      ...
      if (
        saveState.tech.hasOwnProperty("corrupted_ai") &&
        saveState.tech.corrupted_ai === 3
      ) {
    ...
    save.setItem(
      "evolved",
      LZString.compressToUTF16(JSON.stringify(saveState))
    );
...
export function powerGrid(type, reset) {
  ...
  switch (type) {
    ...
    case "moon":
      power_structs = [
        "spc_moon:helium_mine",
        "spc_moon:iridium_mine",
        "spc_moon:observatory",
      ];
      break;
    case "red":
      power_structs = [
        "spc_red:living_quarters",
        "spc_red:exotic_lab",
        "spc_red:red_mine",
        "spc_red:fabrication",
        "spc_red:biodome",
    ...
    case "belt":
      power_structs = [
        "spc_belt:elerium_ship",
        "spc_belt:iridium_ship",
        "spc_belt:iron_ship",
      ];
      break;
    case "alpha":
      power_structs = [
        "int_alpha:fusion",
        "int_alpha:mining_droid",
        "int_alpha:processing",
        "int_alpha:laboratory",
        "int_alpha:g_factory",
        "int_alpha:exchange",
        "int_alpha:zoo",
      ];
    ...
    case "gateway":
      power_structs = [
        "gxy_gateway:bolognium_ship",
        "gxy_gateway:dreadnought",
        "gxy_gateway:cruiser_ship",
        "gxy_gateway:frigate_ship",
        "gxy_gateway:corvette_ship",
        "gxy_gateway:scout_ship",
      ];
      break;
    case "alien2":
      power_structs = [
        "gxy_alien2:armed_miner",
        "gxy_alien2:ore_processor",
        "gxy_alien2:scavenger",
      ];
      break;
    case "lake":
    ...
    case "spire":
      power_structs = [
        "prtl_spire:port",
        "prtl_spire:base_camp",
        "prtl_spire:mechbay",
      ];
      break;
    case "titan":
      power_structs = [
        "spc_titan:titan_quarters",
        "spc_titan:titan_mine",
        "spc_titan:g_factory",
        "spc_titan:decoder",
      ];
      break;
    ...
    case "tau_red":
      power_structs = [
        "tau_red:womling_village",
        "tau_red:womling_farm",
        "tau_red:overseer",
        "tau_red:womling_mine",
        "tau_red:womling_fun",
        "tau_red:womling_lab",
      ];
      break;
    ...
    case "asphodel":
      power_structs = [
        "eden_asphodel:soul_engine",
        "eden_asphodel:bunker",
        "eden_asphodel:asphodel_harvester",
        "eden_asphodel:ectoplasm_processor",
        "eden_asphodel:research_station",
        "eden_asphodel:bliss_den",
      ];
      break;
  }
...
export function messageQueue(msg, color, dnr, tags, reload) {
  ...
  if (tags.includes(message_logs.view)) {
    ...
    if (
      $("#msgQueueLog").children().length >
      global.settings.msgFilters[message_logs.view].max
    ) {
...
export function removeFromRQueue(tech_trees) {
  for (let i = global.r_queue.queue.length - 1; i >= 0; i--) {
    if (
      tech_trees.includes(actions.tech[global.r_queue.queue[i].type].grant[0])
    ) {
...
export function calcQueueMax() {
  let max_queue =
    global.tech["queue"] >= 2 ? (global.tech["queue"] >= 3 ? 8 : 5) : 3;
  if (
    global.stats.feat["journeyman"] &&
    global.stats.feat["journeyman"] >= 2 &&
    global.stats.achieve["seeder"] &&
    global.stats.achieve.seeder.l >= 2
  ) {
    let rank = Math.min(
...
export function buildQueue() {
  ...
  $("#buildQueue").append(
    $(`
        <h2 class="has-text-success">${loc(
          "building_queue"
        )} ({{ | used_q }}/{{ max }})</h2>
        <span id="pausequeue" class="${
          global.queue.pause ? "pause" : "play"
        }" role="button" @click="pauseQueue()" :aria-label="pausedesc()"></span>
    `)
  );
  ...
  queue.append(
    $(
      `<li v-for="(item, index) in queue"><a v-bind:id="setID(index)" class="has-text-warning queued" v-bind:class="{ 'qany': item.qa }" @click="remove(index)"><span v-bind:class="setData(index,'res')" v-bind="setData(index,'data')">{{ item.label }}{{ item.q | count }}</span> [<span v-bind:class="{ 'has-text-danger': item.cna, 'has-text-success': !item.cna }">{{ item.time | time }}{{ item.t_max | max_t(item.time) }}</span>]</a></li>`
    )
  );
...
function attachQueuePopovers() {
  for (let i = 0; i < global.queue.queue.length; i++) {
    ...
    let isWide =
      struct.s[0].substring(0, 4) !== "arpa" && struct.a["wide"] ? true : false;

    popover(
      id,
      function (obj) {
        let b_res = global.queue.queue[i].hasOwnProperty("bres")
          ? global.queue.queue[i].bres
          : false;
        if (struct.s[0].substring(0, 4) === "arpa") {
          obj.popper.append(arpaProjectCosts(100, struct.a));
        } else if (
          struct.s[0].substring(0, 2) === "tp" &&
          struct.s[1].substring(0, 4) === "ship"
        ) {
...
export function costMultiplier(structure, offset, base, mutiplier, cat) {
  ...
  if (global.race["small"]) {
    mutiplier -= traits.small.vars()[0];
  } else if (global.race["large"]) {
    mutiplier += traits.large.vars()[0];
  }
  if (global.race["compact"]) {
    mutiplier -= traits.compact.vars()[0];
  }
  if (
    global.race["tunneler"] &&
    (structure === "mine" || structure === "coal_mine")
  ) {
    mutiplier -= traits.tunneler.vars()[0];
  }
  if (
    global.tech["housing_reduction"] &&
    (structure === "basic_housing" || structure === "cottage")
  ) {
  ...
  var count =
    structure === "citizen"
      ...
      : global[cat][structure]
      ? global[cat][structure].count
      : 0;
  if (offset) {
    count += offset;
  }
...
export function spaceCostMultiplier(
  action,
  offset,
  base,
  mutiplier,
  ...
  if (global.race["small"]) {
    mutiplier -= traits.small.vars()[1];
  }
  if (global.race["compact"]) {
    mutiplier -= traits.compact.vars()[1];
  }
  ...
  var count =
    action === "citizen"
      ? global["resource"][global.race.species].amount
      : global[sector][action]
      ? global[sector][action].count
      : 0;
    ...
    switch (global.race.universe) {
      case "heavy":
        if (global.stats.achieve.ascended.hasOwnProperty("h")) {
          boost =
            global.stats.achieve.ascended.h * global.prestige.Harmony.count;
        }
        break;
      case "antimatter":
        if (global.stats.achieve.ascended.hasOwnProperty("a")) {
          boost =
            global.stats.achieve.ascended.a * global.prestige.Harmony.count;
        }
        break;
      case "evil":
        if (global.stats.achieve.ascended.hasOwnProperty("e")) {
          boost =
            global.stats.achieve.ascended.e * global.prestige.Harmony.count;
        }
        break;
      case "micro":
        if (global.stats.achieve.ascended.hasOwnProperty("m")) {
          boost =
            global.stats.achieve.ascended.m * global.prestige.Harmony.count;
        }
        break;
      case "magic":
        if (global.stats.achieve.ascended.hasOwnProperty("mg")) {
          boost =
            global.stats.achieve.ascended.mg * global.prestige.Harmony.count;
        }
        break;
      default:
        if (global.stats.achieve.ascended.hasOwnProperty("l")) {
          boost =
            global.stats.achieve.ascended.l * global.prestige.Harmony.count;
        }
        break;
    }
    if (boost > 0) {
...
export function timeCheck(c_action, track, detailed, reqMet) {
  ...
  if (c_action.cost) {
    ...
    let costs = c_action["doNotAdjustCost"]
      ? c_action.cost
      : adjustCosts(c_action, offset);
    ...
    let hasTrash = false;
    if (
      global.interstellar.hasOwnProperty("mass_ejector") &&
    ...
    Object.keys(costs).forEach(function (res) {
      if (
        time >= 0 &&
        !global.prestige.hasOwnProperty(res) &&
        !["Morale", "HellArmy", "Structs", "Bool", "Army", "Troops"].includes(
          res
        )
      ) {
        var testCost = offset
          ? Number(costs[res](offset))
          : Number(costs[res]());
        if (testCost > 0) {
          ...
          let res_have =
            res === "Supply"
              ...
              : Number(global.resource[f_res].amount);
          let res_max =
            res === "Supply"
              ? global.portal.purifier.sup_max
              : global.resource[f_res].max;
          let res_diff =
            res === "Supply"
              ? global.portal.purifier.diff
              : global.resource[f_res].diff;
          ...
          if (hasTrash && global.interstellar.mass_ejector[res]) {
            ...
            if (global.race.governor.config.trash.hasOwnProperty(res)) {
              res_diff -= Math.min(
                global.race.governor.config.trash[res].v,
          ...
          if (testCost > res_have) {
            if (res_diff > 0) {
              ...
              if (
                (shorted[bottleneck] && shorted[f_res] > shorted[bottleneck]) ||
                !shorted[bottleneck]
              ) {
                bottleneck = f_res;
              }
...
export function arpaTimeCheck(project, remain, track, detailed) {
      ...
      if (allRemainingSegmentsCost > 0) {
        ...
        if (allRemainingSegmentsCost > res_have) {
          if (res_diff > 0) {
            ...
            if (
              (shorted[bottleneck] && shorted[res] > shorted[bottleneck]) ||
              !shorted[bottleneck]
            ) {
              bottleneck = res;
            }
  ...
  return detailed
    ? { t: allRemainingSegmentsTime, r: bottleneck, s: shorted }
    : allRemainingSegmentsTime;
}
...
export function darkEffect(universe, flag, info, inputs) {
  ...
  let dark =
    inputs.dark !== undefined ? inputs.dark : global.prestige.Dark.count;
  let harmony =
    inputs.harmony !== undefined
      ? inputs.harmony
      : global.prestige.Harmony.count;
  let sludge =
    inputs.sludge !== undefined
      ? inputs.sludge
      : global.stats.achieve["extinct_sludge"] &&
        global.stats.achieve.extinct_sludge[universeAffix(universe)]
      ? global.stats.achieve.extinct_sludge[universeAffix(universe)]
      : 0;
...
export function masteryType(universe, detailed, unmodified) {
  if (global.genes["challenge"] && global.genes.challenge >= 2) {
    ...
    if (!unmodified) {
      ...
      if (
        global.genes.challenge >= 5 &&
        global.race.hasOwnProperty("mastery")
      ) {
    ...
    return detailed
      ? { g: m_mastery, u: u_mastery, m: m_mastery + u_mastery }
      : m_mastery + u_mastery;
  }
...
export function calcPrestige(type, inputs) {
  ...
  if (inputs.synth === undefined)
    inputs.synth = races[global.race.species].type === "synthetic";
  ...
  if (inputs.cit === undefined) {
    let garrisoned = global.civic.hasOwnProperty("garrison")
    ...
    if (global.race["high_pop"]) {
      pop =
        ...
        ) + Math.round(garrisoned / traits.high_pop.vars()[0]);
    } else {
    ...
    if (inputs.high_pop) {
      pop =
        Math.round(inputs.cit / traits.high_pop.vars(inputs.high_pop)[0]) +
        Math.round(inputs.sol / traits.high_pop.vars(inputs.high_pop)[0]);
    } else {
  ...
  if (challenge !== undefined) {
    plasmid_cap = Math.floor(
      plasmid_cap * (1 + (challenge + (inputs.tp ? 1 : 0)) / 8)
    );
  } else {
    plasmid_cap = Math.floor(
      plasmid_cap * (1 + (alevel() - (global.race["truepath"] ? 0 : 1)) / 8)
    );
  }
  ...
  if (inputs.plas === undefined) {
    ...
    if (!inputs.rawPlasmids && gains.plasmid > plasmid_cap) {
      ...
      overflow = Math.floor(
        (overflow / (overflow + plasmid_cap)) * plasmid_cap
      );
  ...
  gains.phage =
    gains.plasmid > 0
      ? challenge_multiplier(
          Math.floor(Math.log2(gains.plasmid) * Math.E * phage_mult),
          type,
          false,
          inputs
        )
  ...
  if (type === "bigbang") {
    ...
    let mana =
      inputs.mana !== undefined ? inputs.mana : global.resource.Mana.gen;
  ...
  if (["ascend", "descend", "terraform", "apotheosis"].includes(type)) {
    ...
    if (type === "ascend" || type === "terraform") {
      ...
      if (spire !== undefined) {
        ...
        spire = global.portal.hasOwnProperty("spire")
...
function truthAdjust(costs, c_action, offset, wiki) {
  if (
    (wiki ? wiki.truepath : global.race["truepath"]) &&
    (!c_action.hasOwnProperty("path") || !c_action.path.includes("truepath"))
  ) {
    var newCosts = {};
    Object.keys(costs).forEach(function (res) {
      if (res === "Money") {
...
function scienceAdjust(costs, offset, wiki) {
  ...
  if (
    (global.race["smart"] || global.race["dumb"] || pragVal || fathom > 0) &&
    costs["Knowledge"]
  ) {
    var newCosts = {};
    Object.keys(costs).forEach(function (res) {
...
function smolderAdjust(costs, offset, wiki) {
  if (global.race["smoldering"]) {
    ...
    if (
      !newCosts.hasOwnProperty("Chrysotile") &&
      costs.hasOwnProperty("Money") &&
      ...
      if (res === "Stone" && !costs["Cement"]) {
        ...
        if (costs["Stone"]) {
          newCosts["Stone"] = function () {
            return (
              Math.round(
                costs["Stone"](offset, wiki) * adjustRate +
                  costs[res](offset, wiki) * 1.8 * adjustRate
              ) || 0
            );
          };
        } else {
...
function craftAdjust(costs, offset, wiki) {
  ...
  if (
    (global.race["hollow_bones"] || fathom > 0) &&
    (costs["Plywood"] ||
      costs["Brick"] ||
      costs["Wrought_Iron"] ||
      costs["Sheet_Metal"] ||
...
function lMatAdjust(costs, c_action, offset, wiki) {
  if (global.race["living_materials"]) {
    ...
    Object.keys(costs).forEach(function (res) {
      if (
        path &&
        global[path[1]].hasOwnProperty(path[0]) &&
        global[path[1]][path[0]].hasOwnProperty("l_m") &&
        (["Lumber", "Furs", "Plywood"].includes(res) ||
          (res === "Stone" && global.race["sappy"]))
      ) {
        newCosts[res] = function () {
          return Math.round(
            costs[res](offset, wiki) *
              traits.living_materials.vars()[0] **
                (global[path[1]][path[0]].l_m / 25)
          );
        };
      } else {
        newCosts[res] = function () {
  ...
  if (global.race["heavy"]) {
    ...
    Object.keys(costs).forEach(function (res) {
      if (res === "Stone" || res === "Cement" || res === "Wrought_Iron") {
        newCosts[res] = function () {
          return Math.round(
            costs[res](offset, wiki) * (1 + traits.heavy.vars()[1] / 100)
          );
        };
      } else {
        newCosts[res] = function () {
...
export function drawIcon(icon, size, shade, id, inject) {
  ...
  inject = inject || "";
  return `<span ${inject}${select}class="flair drawnIcon"><svg class="star${shade}" version="1.1" x="0px" y="0px" width="${size}px" height="${size}px" viewBox="${svgViewBox(
    icon
  )}" xml:space="preserve">${svgIcons(icon)}</svg></span>`;
}
...
export function easterEgg(num, size) {
  ...
  if (
    easter.active &&
    !global.special.egg[date.getFullYear()][`egg${num}`] &&
    $(`#egg${num}`).length === 0
  ) {
    return drawIcon(
      "egg",
    ...
    if (!global.special.egg[year][`egg${id}`]) {
      ...
      if (id <= 12) {
        if (global.race.universe === "antimatter") {
          ...
          messageQueue(
            loc("city_egg_msg", [9, loc("resource_AntiPlasmid_plural_name")]),
            "success",
            false,
            ["events"]
          );
          ...
          messageQueue(
            loc("city_egg_msg", [9, loc("resource_Plasmid_plural_name")]),
            "success",
            false,
            ["events"]
          );
        ...
        messageQueue(
          loc("city_egg_msg", [4, loc("resource_Phage_name")]),
          "success",
          false,
          ["events"]
        );
...
export function trickOrTreat(num, size, trick) {
  ...
  if (
    halloween.active &&
    !global.special.trick[year][`${tot}${num}`] &&
    $(`#${tot}${num}`).length === 0
  ) {
    let label = trick ? `Ghost` : `Candy Corn`;
    return drawIcon(
      trick ? "ghost" : "candycorn",
      size ? size : 16,
      2,
      `${tot}${num}`,
  ...
  $(`#${tot}${id}`).click(function () {
    if (!global.special.trick[year][`${tot}${id}`]) {
      ...
      if (trick) {
        ...
        messageQueue(
          loc("city_ghost_msg", [2, loc("resource_Phage_name")]),
          "success",
          false,
          ["events"]
        );
        ...
        if (global.race.universe === "antimatter") {
          ...
          messageQueue(
            loc("city_trick_msg", [12, loc("resource_Plasmid_plural_name")]),
            "success",
            false,
            ["events"]
          );
...
function single_emblem(achieve, size, icon, iconName, fool, uAffix) {
  return global.stats.achieve[achieve] &&
    (fool
      ? global.stats.achieve[achieve][uAffix] - 1
      : global.stats.achieve[achieve][uAffix]) > 0
    ? `<p class="flair" title="${sLevel(
        global.stats.achieve[achieve][uAffix]
      )} ${iconName}"><svg class="star${
        fool
          ? global.stats.achieve[achieve][uAffix] - 1
          : global.stats.achieve[achieve][uAffix]
      }" version="1.1" x="0px" y="0px" width="${size}px" height="${size}px" viewBox="${svgViewBox(
        icon
      )}" xml:space="preserve">${svgIcons(
        icon
      )}</svg><span class="is-sr-only">${sLevel(
        global.stats.achieve[achieve][uAffix]
      )} ${iconName}</span></p>`
    : "";
}
...
export function format_emblem(achieve, size, baseIcon, fool, universe) {
  ...
  if (!universe) {
    ...
    emblem =
      emblem +
      single_emblem(achieve, size, "evil", loc("universe_evil"), fool, "e");
    emblem =
      emblem +
      single_emblem(achieve, size, "heavy", loc("universe_heavy"), fool, "h");
    emblem =
      emblem +
      single_emblem(achieve, size, "micro", loc("universe_micro"), fool, "m");
    emblem =
      emblem +
      single_emblem(achieve, size, "magic", loc("universe_magic"), fool, "mg");
  } else {
    switch (universe) {
      case "standard":
        emblem =
  ...
  switch (event) {
    ...
    case "summer": {
      ...
      if (
        !global.settings.boring &&
        date.getMonth() === 5 &&
        [20, 21, 22].includes(date.getDate())
      ) {
        if (
          global.city.hasOwnProperty("foundry") &&
          !global.city.foundry.hasOwnProperty("Thermite")
        ) {
    ...
    case "firework": {
      ...
      if (
        ...
        global.city.hasOwnProperty("firework") ||
        global.space.hasOwnProperty("firework")
      ) {
...
export function getEaster() {
  ...
  if (
    global.special.egg.hasOwnProperty(year) &&
    !global.special.egg[year].hasOwnProperty("egg13")
  ) {
  ...
  if (
    global.special.egg.hasOwnProperty(year) &&
    !global.special.egg[year].hasOwnProperty("egg16")
  ) {
  ...
  easter.endDate[1] += 10;
  if (
    (easter.endDate[0] === 2 && easter.endDate[1] > 31) ||
    (easter.endDate[0] === 3 && easter.endDate[1] > 30)
  ) {
  ...
  easter.hintDate[1] += 1;
  if (
    (easter.hintDate[0] === 2 && easter.hintDate[1] > 31) ||
    (easter.hintDate[0] === 3 && easter.hintDate[1] > 30)
  ) {
  ...
  if (
    (easter.solveDate[0] === 2 && easter.solveDate[1] > 31) ||
    (easter.solveDate[0] === 3 && easter.solveDate[1] > 30)
  ) {
  ...
  const isAfterBeginning =
    cur_month > easter.date[0] ||
    (cur_month === easter.date[0] && cur_day >= easter.date[1]);
  const isBeforeEnd =
    cur_month < easter.endDate[0] ||
    (cur_month === easter.endDate[0] && cur_day <= easter.endDate[1]);
  if (isAfterBeginning && isBeforeEnd) {
    easter.active = true;
    if (
      cur_month >= easter.hintDate[0] &&
      cur_day >= easter.hintDate[1] &&
      cur_month <= easter.endDate[0] &&
      cur_day <= easter.endDate[1]
    ) {
      easter.hint = true;
    }
    if (
      cur_month >= easter.solveDate[0] &&
      cur_day >= easter.solveDate[1] &&
      cur_month <= easter.endDate[0] &&
      cur_day <= easter.endDate[1]
    ) {
      easter.solve = true;
...
export function getHalloween() {
  ...
  let end = new Date(
    `${halloween.endDate[0] + 1}/${halloween.endDate[1]}/${year}`
  );

  if (date >= start && date <= end) {
    halloween.active = true;
    let hint = new Date(
      `${halloween.hintDate[0] + 1}/${halloween.hintDate[1]}/${year}`
    );
    if (date >= hint && date <= end) {
      halloween.hint = true;
    }
    let sol = new Date(
      `${halloween.solveDate[0] + 1}/${halloween.solveDate[1]}/${year}`
    );
...
export function shrineBonusActive() {
  return (
    global.race["magnificent"] &&
    global.city.hasOwnProperty("shrine") &&
    global.city.shrine.count > 0
  );
}
  ...
  if (shrineBonusActive()) {
    switch (type) {
      case "metal":
        ...
        if (
          (global.city.calendar.moon >= 7 && global.city.calendar.moon < 14) ||
      ...
      case "know":
        ...
        if (
          (global.city.calendar.moon > 14 && global.city.calendar.moon <= 21) ||
          global.city.calendar.moon === 14
        ) {
      ...
      case "morale":
        ...
        if (
          (global.city.calendar.moon > 0 && global.city.calendar.moon <= 7) ||
          global.city.calendar.moon === 14
        ) {
...
function getTraitVals(trait, rank, species) {
  let vals = traits[trait].hasOwnProperty("vars")
    ? traits[trait].vars(rank)
    : [];
  if (valAdjust.hasOwnProperty(trait)) {
    ...
    switch (traits.elemental.vars(rank)[0]) {
      case "electric":
        vals = [
          loc(`element_electric`),
          traits.elemental.vars(rank)[1],
          traits.elemental.vars(rank)[5],
        ];
        break;
      case "acid":
        vals = [
          loc(`element_acid`),
          traits.elemental.vars(rank)[2],
          traits.elemental.vars(rank)[5],
        ];
        break;
      case "fire":
...
const traitExtra = {
  ...
  heavy: [
    loc(`wiki_trait_effect_heavy_ex1`, [
      rName("Stone"),
      rName("Cement"),
      rName("Wrought_Iron"),
    ]),
  ],
  sniper: [loc(`wiki_trait_effect_sniper_ex1`)],
  hooved: [
    function (opts) {
      return loc(`wiki_trait_effect_hooved_ex1`, [
        hoovedRename(false, opts.species),
      ]);
    },
    loc(`wiki_trait_effect_hooved_ex2`, [
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Lumber")
          ? global.resource.Lumber.name
          : loc("resource_Lumber_name")
      }</span>`,
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Copper")
          ? global.resource.Copper.name
          : loc("resource_Copper_name")
      }</span>`,
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Iron")
          ? global.resource.Iron.name
          : loc("resource_Iron_name")
      }</span>`,
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Steel")
          ? global.resource.Steel.name
          : loc("resource_Steel_name")
      }</span>`,
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Adamantite")
          ? global.resource.Adamantite.name
          : loc("resource_Adamantite_name")
      }</span>`,
      `<span class="has-text-warning">${
        global.resource.hasOwnProperty("Orichalcum")
    ...
    function (opts) {
      return loc(`wiki_trait_effect_hooved_ex4`, [
        `<span class="has-text-warning">${5}</span>`,
        hoovedRename(false, opts.species),
      ]);
    },
  ...
  high_pop: [loc(`wiki_trait_effect_high_pop_ex1`)],
  flier: [loc(`wiki_trait_effect_flier_ex1`)],
  unfathomable: [
...
function rName(r) {
  let res =
    global.hasOwnProperty("resource") && global.resource.hasOwnProperty(r)
      ? global.resource[r].name
      : loc(`resource_${r}_name`);
...
export function getTraitDesc(info, trait, opts) {
  ...
  if (tpage && ["genus", "major"].includes(traits[trait].type)) {
    rank = `<span><span role="button" @click="down()">&laquo;</span><span class="has-text-warning">${loc(
      `wiki_trait_rank`
    )} {{ rank }}</span><span role="button" @click="up()">&raquo;</span></span>`;
  }
  if (wiki) {
    info.append(
      `<div class="type"><h2 class="has-text-warning">${traitName}</h2>${rank}</div>`
    );
  }
  if (wiki) {
    if (tpage && traits[trait].hasOwnProperty("val")) {
      info.append(
        `<div class="type has-text-caution">${loc(
          `wiki_trait_${traits[trait].type}`
        )}<span>${loc(`wiki_trait_value`, [traits[trait].val])}</span></div>`
      );
    } else {
      info.append(
        `<div class="type has-text-caution">${loc(
  ...
  if (fanatic && wiki) {
    info.append(
      `<div class="has-text-danger">${loc(`wiki_trait_fanaticism`, [
        fanatic,
      ])}</div>`
    );
  }
  ...
  if (tpage && ["genus", "major"].includes(traits[trait].type)) {
    info.append(
      `<div class="has-text-${color} effect" v-html="getTraitDesc(rank)"></div>`
    );
  } else {
    if (
      wiki ||
      ...
      if (trait === "elemental") {
        trait_desc = loc(
          `wiki_trait_effect_${trait}_${traits.elemental.vars()[0]}`,
          getTraitVals(trait, trank, species)
        );
      } else {
        trait_desc = loc(
          `wiki_trait_effect_${trait}`,
          getTraitVals(trait, trank, species)
        );
      }
