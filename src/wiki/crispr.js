...
export function crisprPage(content) {
  content.append(
    `<div class="header has-text-warning">${loc("wiki_arpa_crispr")}</div>`
  );
...
Object.keys(genePool).forEach(function (gene) {
  ...
  let text =
    typeof genePool[gene].title === "string"
      ? genePool[gene].title
      : genePool[gene].title();
...
function geneDesc(info, gene) {
  let owned =
    ...
    global.genes[genePool[gene].grant[0]] >= genePool[gene].grant[1]
      ? true
      : false;
  ...
  info.append(
    `<div class="type"><h2 class="has-text-warning">${
      ...
      `wiki_arpa_crispr_${genePool[gene].grant[0]}`
    )}: ${genePool[gene].grant[1]}</span></div>`
  );
  ...
  if (Object.keys(genePool[gene].reqs).length > 0) {
    ...
    Object.keys(genePool[gene].reqs).forEach(function (req) {
      let color =
        global.genes[req] && global.genes[req] >= genePool[gene].reqs[req]
          ? "success"
          : "danger";
      reqs.append(
        `${comma ? `, ` : ``}<span><a href="wiki.html#crispr-prestige-${
          crisprTrees[req][genePool[gene].reqs[req]].name
        }" class="has-text-${color}" target="_blank">${loc(
          `wiki_arpa_crispr_${req}`
        )} ${genePool[gene].reqs[req]}</a></span>`
      );
      comma = true;
    });
  }
  if (specialRequirements.hasOwnProperty(gene)) {
    ...
    Object.keys(specialRequirements[gene]).forEach(function (req) {
      ...
      let text = specialRequirements[gene][req].link
        ? `<a href="${specialRequirements[gene][req].link}" class="has-text-${color}" target="_blank">${specialRequirements[gene][req].title}</a>`
        : specialRequirements[gene][req].title;
      specialReq.append(
        `${comma ? `, ` : ``}<span class="has-text-${color}">${text}</span>`
      );
