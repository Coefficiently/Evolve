...
function indexPage(page) {
  if (pages[page][1]) {
    ...
    for (let i = 0; i < v.length; i++)
      index[pages[page][2]].push([
        v[i].innerText,
        v[i].getAttribute("href"),
        pages[page][3],
      ]);
    fakecontent.empty();
  } else {
    if (pages[page][0] === faqPage) {
      ...
      for (let i = 0; i < v.length; i++)
        index["gameplay"].push([
          v[i].innerText,
          `wiki.html#basics-gameplay-${v[i].innerText
            .toLowerCase()
            .replaceAll(" ", "_")}`,
          "basics",
        ]);
      ...
      for (let i = 0; i < changeList.length; i++) {
        index["changelog"].push([
          ...
          `wiki.html#version-changelog-${changeList[i].version.replaceAll(
            ".",
            "_"
          )}`,
          changeList[i].version,
...
function indexWiki() {
  ...
  content.attr("id", "");
  content.append(
    `<span id="content" class="temp-indexer" style="display: none"></span>`
  );
  fakecontent = $("#content");
  earlyExit = false;
        ...
        if (v[2] !== undefined) {
          if (!headers.includes(v[2])) {
            ...
            if ((v[2].match(/\./gi) || []).length > 1) {
              element.append(
                `<div class="infoBox" id="changelog-entry-${v[2].replaceAll(
                  ".",
                  "_"
                )}"><h1>v${v[2]}</h1></div>`
              );
              ...
              element.append(
                `<div class="infoBox" id="${v[2]}"><h1>${loc(
                  "wiki_menu_" +
                    (v[2].includes("tp_")
                      ? v[2].substring(3)
                      : v[2].includes("tech_")
          ...
          if ((v[2].match(/\./gi) || []).length > 1) {
            $(`#changelog-entry-${v[2].replaceAll(".", "_")}`).append(
              `<p><a href="${v[1]}" class="has-text-red" target="_blank">${v[0]}</a></p>`
            );
          } else {
            $(`#${v[2]}`).append(
              `<p><a href="${v[1]}" class="has-text-red" target="_blank">${v[0]}</a></p>`
...
export function search() {
    ...
    `
        <div class="infoBox">
            ...
            <div class="control is-clearfix">
                <input id="searchInput" class="input" placeholder="${loc(
                  "wiki_search_regex_support"
                )}"type="text"/>
            </div>
        </div>
        `
  ...
  for (const k of Object.keys(sections)) {
    content.append(
      `<div class="infoBox"><h1 style="padding-bottom: 5px">${loc(
        "wiki_menu_" + k
      )}</h1><div id="${"searchResult-" + k}"></div></div>`
    );
  }
