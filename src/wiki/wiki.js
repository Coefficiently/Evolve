...
function initPage() {
  ...
  let menuItems = [
    ...
    {
      key: "species",
      submenu: [{ key: "races" }, { key: "traits" }, { key: "custom" }],
    },
    {
      key: "structures",
      submenu: [
  ...
  let wikiMenu = `<template><b-menu class="sticky has-text-caution"><b-menu-list label="${loc(
    "wiki_menu_evolve"
  )}">`;
  ...
  if (window.location.hash) {
    ...
    if (hash.length > 1) {
      hash.length > 2
        ? menuDispatch(hash[1], hash[0], hash[2])
...
async function menuDispatch(main, sub, frag) {
  if (window.location.hash === "#search" && main !== "search") {
    const until = (condition) => {
      const poll = (resolve) =>
        condition() ? resolve() : setTimeout((_) => poll(resolve), 16);
...
function buiildMenu(items, set, parent) {
  ...
  for (let i = 0; i < items.length; i++) {
    if (items[i].hasOwnProperty("submenu")) {
      let active =
        (!hash && set && i === 0) ||
        (hash && hash.length > 1 && hash[1] === items[i].key)
          ? ` :active="true" expanded`
          : "";
      menu =
        menu +
        `<b-menu-item${active}><template slot="label" slot-scope="props">${loc(
          `wiki_menu_${items[i].key}`
        )}</template>`;
      menu = menu + buiildMenu(items[i].submenu, false, items[i].key);
      menu = menu + `</b-menu-item>`;
    } else {
      let active =
        (!hash && set && i === 0) || (hash && hash[0] === items[i].key)
          ? ` :active="true"`
          : "";
      let args = parent
        ? `'${parent}','${items[i].key}'`
      ...
      menu =
        menu +
        `<b-menu-item${active} label="${loc(
          `wiki_menu_${items[i].key}`
        )}" @click="loadPage(${args})"></b-menu-item>`;
    }
  }
  return menu;
}
...
function mainPage() {
  ...
  let contribute = `<span class="has-text-caution">${[
    "Beorseder",
    "Rodrigodd",
    "Volch",
    "Condoslime",
    ...
    .join('</span>, <span class="has-text-caution">')
    .replace(/, ([^,]*)$/, `, & $1`)}</span>`;

  let version = global["beta"]
    ? `beta v${global.version}.${global.beta}`
    : "v" + global.version;
  content.append(
    `<div class="title has-text-warning">${loc(
      `wiki_main_title`
    )} - ${version}</div>`
  );
  content.append(
    `<div class="paragraph has-text-advanced">${loc(`wiki_main_author`, [
      "Demagorddon",
    ])}</div>`
  );
  content.append(
    `<div class="paragraph has-text-danger">${loc(`wiki_main_spoiler`)}</div>`
  );
  content.append(`<div class="paragraph">${loc(`wiki_main_blurb`)}</div>`);
  content.append(
    `<div class="paragraph has-text-warning">${loc(`wiki_main_contribution`, [
      contribute,
    ])}</div>`
  );
  ...
  list.append(
    `<li><a href="https://wooledge.org/~greg/evolve/guide.html" target="_blank">${loc(
      `wiki_resources_begin_guide`
    )}</a> ${loc(`wiki_resources_by`, ["GreyCat"])}</li>`
  );
  list.append(
    `<li><a href="https://karsen777.github.io/" target="_blank">${loc(
      `wiki_resources_tracker`
    )}</a> ${loc(`wiki_resources_by`, ["Karsen777"])}</li>`
  );
