...
const extraInformation = {
  ...
  monument: [
    ...
    loc("wiki_arpa_project_monument_exceptions"),
  ],
  railway: [
    loc(
      "wiki_arpa_project_railway_homeless",
      [3, 7, 11, 17, 25, 38, 59, 100, 225]
    ),
  ],
...
export function projectsPage(content) {
  content.append(
    `<div class="header has-text-warning">${loc("wiki_arpa_projects")}</div>`
  );
  ...
  Object.keys(arpaProjects).forEach(function (project) {
    ...
    sideMenu(
      ...
      typeof arpaProjects[project].title === "string"
        ...
        : arpaProjects[project].title(true)
    );
  });
}
...
function projectDesc(info, project) {
  ...
  stats.append(
    `<div class="effect">${
      typeof arpaProjects[project].effect === "string"
        ? arpaProjects[project].effect
        : arpaProjects[project].effect()
    }</div>`
  );
...
function addCosts(parent, key) {
  ...
  let costs = $(
    `<div class="cost"><h2 class="has-text-warning">${loc(
      "wiki_calc_cost"
    )}</h2></div>`
  );
  let costCreep = $(
  ...
  Object.keys(arpaAdjustCosts(cost)).forEach(function (res) {
    ...
    costs.append(
      $(
        `<div class="has-text-success" v-show="r.${res}.vis">${label}{{ r.${res}.cost }}</div>`
      )
    );
    costCreep.append(
      $(
        `<div class="has-text-success" v-show="r.${res}.vis">{{ r.${res}.creep }}</div>`
      )
    );
  });
  ...
  let updateCostCreep = function () {
    if (key !== "launch_facility") {
      ...
      Object.keys(resources).forEach(function (res) {
        if (upper[res]) {
          resources[res].creep = +(
            upper[res](100, inputs.extra) / lower[res](99, inputs.extra)
          ).toFixed(4);
        }
      });
    }
  ...
  calcInputs += `
                <div class="calcInput"><b-checkbox class="patrol" :input="update()" v-model="i.extra.creative">${loc(
                  "trait_creative_name"
                )}</b-checkbox></div>
            </div>
...
function monumentExtra() {
  ...
  let desc = `<div>${loc("wiki_arpa_project_monument", [
    monuments.length,
    monuments.join(", "),
  ])}</div>`;
  for (let i = 0; i < monuments.length; i++) {
    desc =
      desc +
      `<div>${loc("wiki_arpa_project_monument_type", [
