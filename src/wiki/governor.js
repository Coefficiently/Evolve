...
export function governPage(content) {
  ...
  {
    let govern = infoBoxBuilder(mainContent, {
      name: "intro",
      template: "governor",
      label: loc("governor"),
      paragraphs: 4,
    ...
    Object.keys(gmen).forEach(function (gov) {
      infoBoxBuilder(govern, {
        name: gov,
        template: "government",
        label: loc(`governor_${gov}`),
        paragraphs: 2,
        break: [2],
        h_level: 3,
        text: {
              ...
              Object.keys(gmen[gov].traits).forEach(function (t) {
                desc +=
                  (gov_traits[t].hasOwnProperty("effect")
                    ? gov_traits[t].effect(b, { isWiki: true })
                    : "") + " ";
              });
              return desc;
            },
        ...
        h_extra: `<div><b-checkbox v-model="b">${loc(
          gov === "bureaucrat"
            ? `arpa_genepool_civil_service_title`
...

  {
...

    {
      ...
      infoBoxBuilder(govern, {
        name: task,
        template: "government",
        label: loc(`gov_task_${task}`, [loc(`resource_Slave_name`)]),
        paragraphs: 2,
        break: [2],
        h_level: 3,
        text: {
      ...
      subSideMenu(
        ...
        `governor-gameplay`,
        task,
        loc(`gov_task_${task}`, [loc(`resource_Slave_name`)])
      );
    }
...

    {
      ...
      infoBoxBuilder(govern, {
        ...
        para_data: {
          1: [
            loc(`city_${hoovedRename(true)}`, [hoovedRename(false)]),
            hoovedRename(false),
          ],
      ...
      infoBoxBuilder(govern, {
        ...
        data_link: {
          4: [
            ...
            "wiki.html#hell-structures-spire",
          ],
        },
      });
      subSideMenu("add", `governor-gameplay`, task, loc(`gov_task_${task}`));
