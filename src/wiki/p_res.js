...
export function pResPage(content) {
  ...
  //Anti-Plasmids
  section = infoBoxBuilder(mainContent, {
    name: "antiplasmids",
    template: "p_res",
    paragraphs: 5,
    h_level: 2,
  ...
  //Dark Energy
  let dark = infoBoxBuilder(mainContent, {
    name: "dark",
    template: "p_res",
    paragraphs: 1,
    h_level: 2,
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    para_data: {
      1: [
        loc("wiki_resets_matrix"),
        loc("wiki_resets_retired"),
        loc("wiki_resets_eden"),
      ],
      5: [loc("achieve_overlord_name")],
    },
    data_link: {
      1: [
        "wiki.html#resets-prestige-matrix",
        "wiki.html#resets-prestige-retired",
        "wiki.html#resets-prestige-eden",
      ],
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    data_link: {
      ...
      6: [
        "wiki.html#perks-prestige-pathfinder",
        "wiki.html#perks-prestige-overlord",
      ],
    },
  });
  sideMenu(
    ...
    `resources-prestige`,
    "skilled_servants",
    loc("wiki_p_res_skilled_servants")
  );
}
...
export function prestigeCalc(info, resource, extraType, resetType) {
  ...
  calc.append(
    `<h2 class="has-text-caution">${loc("wiki_calc_gains", [title])}</h2>`
  );
  ...
  if (resource === "plasmid") {
    equation += `
            <div>
                <h3 class="has-text-caution">${loc(
                  "wiki_calc_plasmid_cap_total",
                  [
                    prestigeType === "anti"
  ...
  variables.append(`
        <div>
            <div class="calcInput" v-show="i.cit.use"><span>${loc(
              "wiki_calc_citizens"
            )}</span> <b-numberinput :input="val('cit')" min="0" v-model="i.cit.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.sol.use"><span>${loc(
              "wiki_calc_soldiers"
            )}</span> <b-numberinput :input="val('sol')" min="0" v-model="i.sol.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.know.use"><span>${loc(
              "wiki_calc_knowledge"
            )}</span> <b-numberinput :input="val('know')" min="0" v-model="i.know.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.plas.use"><span>${loc(
              "resource_Plasmid_plural_name"
            )}</span> <b-numberinput :input="val('plas')" min="0" v-model="i.plas.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.mass.use"><span>${loc(
              "wiki_calc_mass"
            )}</span> <b-numberinput :input="val('mass')" min="0" v-model="i.mass.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.exotic.use"><span>${loc(
              "wiki_calc_exotic"
            )}</span> <b-numberinput :input="val('exotic')" min="0" v-model="i.exotic.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.mana.use"><span>${loc(
              "wiki_calc_mana"
            )}</span> <b-numberinput :input="val('mana')" min="0" v-model="i.mana.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.floor.use"><span>${loc(
              "wiki_calc_floor"
            )}</span> <b-numberinput :input="val('floor')" min="0" v-model="i.floor.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.genes.use"><span>${loc(
              "wiki_calc_genes"
            )}</span> <b-numberinput :input="val('genes')" min="0" max="4" v-model="i.genes.val" :controls="false"></b-numberinput></div>
            <div class="calcInput" v-show="i.reset.use"><span>${loc(
              "wiki_calc_prestige"
            ...
            <div class="calcInput" v-show="i.uni.use"><span>${loc(
              "wiki_calc_universe"
            )}</span> <b-dropdown hoverable>
                <button class="button is-primary" slot="trigger">
            ...
            <div class="calcInput" v-show="i.high_pop.use"><span>${loc(
              "trait_high_pop_name"
            )}</span> <b-dropdown hoverable>
                <button class="button is-primary" slot="trigger">
            ...
            <div class="calcInput" v-show="i.micro.use"><b-checkbox class="patrol" v-model="i.micro.val">${loc(
              "universe_micro"
            )}</b-checkbox></div>
            <div class="calcInput" v-show="i.synth.use"><b-checkbox class="patrol" v-model="i.synth.val">${loc(
              "wiki_calc_synth"
            )}</b-checkbox></div>
            <div class="calcInput" v-show="i.tp.use"><b-checkbox class="patrol" v-model="i.tp.val" :disabled="!i.tp.enabled">${loc(
              "wiki_calc_tp"
            )}</b-checkbox></div>
        </div>
    ...
    methods: {
      ...
      importInputs() {
        ...
        if (inputs.tp.use) {
          if (
            inputs.reset.val &&
            ["mad", "bioseed", "terraform"].includes(inputs.reset.val)
          ) {
    ...
    filters: {
      generic(num, type) {
        return num !== undefined
          ? num
          : type === "plasmid_gains_raw" || type === "plasmid_cap_total"
          ? loc("wiki_calc_" + type, [
              prestigeType === "anti"
                ? loc("resource_AntiPlasmid_name")
                : loc("resource_Plasmid_name"),
      ...
      plasmidTotalCalc() {
        ...
        if (plasExtra.totalVis) {
          return (
            plasExtra.capVal +
            Math.floor(
              (plasExtra.overflow / (plasExtra.overflow + plasExtra.capVal)) *
                plasExtra.capVal
            )
          );
        }
      ...
      challenge(num) {
        return num !== undefined
          ? challenge_multiplier(1, "mad", 2, {
              genes: num,
              uni: "standard",
              tp: false,
            })
          : loc("wiki_calc_challenge_multi");
      },
      universe(type) {
        if (!type) {
        ...
        return +(
          challenge_multiplier(1, inputs.reset.val, 2, {
            genes: genes,
            uni: type,
          }) /
          challenge_multiplier(1, "mad", 2, { genes: genes, uni: "standard" })
        ).toFixed(2);
      },
      calc() {
        let show = true;
        ...
        if (showEval.vis) {
          ...
          if (resource === "plasmid") {
            ...
            plasExtra.overflow =
              plasExtra.capVal !== undefined && gains > plasExtra.capVal
                ? gains - plasExtra.capVal
                : undefined;
            plasExtra.overflowVis =
...
function plasProdCalc(info, type) {
  ...
  formula.append(`
        ...
        <div>
            <span>(ln(</span><span v-show="s.bleed.vis">(</span><span>{{ i.plas.val, t | generic }} </span><span v-show="s.bleed.vis">/ {{ | bleedDiv }}) </span><span>+ 50) - 3.91202) / 2.888</span><span v-show="${
              type === "anti"
            }"> / 3</span><span v-show="s.underResult.vis"> = {{ false | calcUnder }} = +{{ true | calcUnder }}%</span>
        </div>
        <div>
            <span>${loc("wiki_calc_over_cap")}</span>
        </div>
        <div>
            <span>((((ln({{ i.phage.val, 'phage' | generic }} + 300) - 3.91202)) / 2.888) + ((ln(</span><span v-show="s.bleed.vis">(</span><span>{{ i.plas.val, t | generic }} </span><span v-show="s.bleed.vis">/ {{ | bleedDiv }}) </span><span>+ 1 - ({{ i.phage.val, 'phage' | generic }} + 250)) / ln2 / 250)))</span><span v-show="${
              type === "anti"
            }"> / 3</span><span v-show="s.overResult.vis"> = {{ false | calcOver }} = +{{ true | calcOver }}%</span>
        </div>
    `);

  variables.append(`
        <div>
            <div class="calcInput"><span>${
              type === "plasmid"
                ? loc("resource_Plasmid_plural_name")
                : loc("resource_AntiPlasmid_plural_name")
            }</span> <b-numberinput :input="val('plas')" min="0" v-model="i.plas.val" :controls="false"></b-numberinput></div>
            <div class="calcInput"><span>${loc(
              `resource_Phage_name`
            )}</span> <b-numberinput :input="val('phage')" min="0" v-model="i.phage.val" :controls="false"></b-numberinput></div>
            <div class="calcInput"><b-checkbox class="patrol" :input="bleed()" v-model="i.antimatter.val">${loc(
              "universe_antimatter"
            )}</b-checkbox></div>
        </div>
        <div class="calcButton">
            <button class="button" @click="resetInputs()" :click="bleed()">${loc(
              "wiki_calc_reset"
            )}</button>
            <button class="button" @click="importInputs()" :click="bleed()">${loc(
              "wiki_calc_import"
            )}</button>
        </div>
    `);
  ...
  vBind({
    ...
    methods: {
      ...
      bleed() {
        if (
          (inputs.antimatter.val && type === "plasmid") ||
          (!inputs.antimatter.val && type === "anti")
        ) {
          show.bleed.vis = true;
        } else {
    ...
    filters: {
      ...
      calcUnder(percent) {
        ...
        if (
          inputs.plas.val === undefined ||
          effective > 250 + inputs.phage.val
        ) {
          vis = false;
        }
      ...
      calcOver(percent) {
        ...
        if (show.overResult.vis) {
          let bonus =
            +(Math.log(250 + inputs.phage.val + 50) - 3.91202).toFixed(5) /
              2.888 +
            Math.log(effective + 1 - (250 + inputs.phage.val)) / Math.LN2 / 250;
          if (type === "anti") {
            bonus /= 3;
          }
...
function storeBonusCalc(info, type) {
  ...
  variables.append(`
        <div>
            ...
            <div class="calcInput"><span>${loc("wiki_tech_special_crispr", [
              loc("wiki_arpa_crispr_store"),
            ])}</span> <b-dropdown hoverable>
                <button class="button is-primary" slot="trigger">
            ...
            <div class="calcInput" v-show="i.antimatter.use"><b-checkbox class="patrol" :input="bleed()" v-model="i.antimatter.val">${loc(
              "universe_antimatter"
            )}</b-checkbox></div>
        </div>
        <div class="calcButton">
  ...
  vBind({
    ...
    filters: {
      ...
      storeLabel(num) {
        if (num === undefined) {
          return loc("wiki_tech_special_crispr", [
            loc("wiki_arpa_crispr_store"),
          ]);
        }
        if (!num) {
      ...
      calc(percent) {
        ...
        show.result.vis =
          inputs.res.val !== undefined && inputs.store.val !== undefined;

        if (show.result.vis) {
          if (
            inputs.store.val === 0 ||
            (type === "phage" && inputs.store.val < 4)
          ) {
            show.result.val = 0;
          } else {
...
function darkBonusCalc(info) {
  ...
  formula.append(`
        <div v-show="s.standard.vis">
            <div>
                <span>${loc("wiki_calc_bonuses", [
                  loc("wiki_calc_dark_standard"),
                ])}:</span>
            </div>
            <div>
        ...
        <div v-show="s.micro.vis">
            <div>
                <span>${loc("wiki_calc_creep_reduction", [
                  loc("wiki_calc_home"),
                ])} (${loc("wiki_calc_cap", [0.06])}):</span>
            </div>
            <div>
            ...
            <div>
                <span>${loc("wiki_calc_creep_reduction", [
                  loc("wiki_calc_not_home"),
                ])} (${loc("wiki_calc_cap", [0.04])}):</span>
            </div>
            <div>
        ...
        <div v-show="s.heavy.vis">
            ...
            <div>
                <span>${loc("wiki_calc_dark_heavy", [
                  loc("wiki_tech_req_or", [
                    loc("wiki_menu_interstellar"),
                    loc("wiki_menu_intergalactic"),
                  ]),
                ])}:</span>
            </div>
            <div>
  ...
  variables.append(`
        <div>
            <div class="calcInput"><span>${loc(
              "wiki_p_res_dark"
            )}</span> <b-numberinput :input="val('dark')" min="0" v-model="i.dark.val" :controls="false"></b-numberinput></div>
            <div class="calcInput"><span>${loc(
              "wiki_p_res_harmony"
            )}</span> <b-numberinput :input="val('harmony')" min="0" v-model="i.harmony.val" :controls="false"></b-numberinput></div>
            <div class="calcInput"><span>${loc(
              "wiki_calc_sludge_level"
            )}</span> <b-numberinput :input="val('sludge')" min="0" max="5" v-model="i.sludge.val" :controls="false"></b-numberinput></div>
        </div>
        <div class="calcButton">
            <button class="button" @click="resetInputs()">${loc(
              "wiki_calc_reset"
  ...
  vBind({
    ...
    methods: {
      ...
      importInputs() {
        ...
        inputs.sludge.val =
          ...
          global.stats.achieve["extinct_sludge"][universeAffix(inputs.uni.val)]
            ? global.stats.achieve["extinct_sludge"][
                universeAffix(inputs.uni.val)
              ]
            : 0;
      },
    },
    filters: {
      ...
      calc(uni, percent, valNum) {
        ...
        show[uni].result =
          inputs.dark.val !== undefined && inputs.harmony.val !== undefined;
        ...
        if (show[uni].result) {
          switch (uni) {
            ...
            case "magic":
              show[uni].val = +(
                darkEffect(uni, false, true, {
                  dark: inputs.dark.val,
                  harmony: inputs.harmony.val,
                  sludge: inputs.sludge.val,
                }) - 1
              ).toFixed(6);
              return show[uni].val;
            case "micro":
              show[uni][`val${valNum}`] = darkEffect(
                uni,
                valNum === 1 ? true : false,
                true,
                {
            ...
            case "heavy":
              let bonus = darkEffect(uni, false, true, {
                dark: inputs.dark.val,
                harmony: inputs.harmony.val,
                sludge: inputs.sludge.val,
              });
...
function harmonyCreepCalc(info) {
  ...
  calc.append(
    `<h2 class="has-text-caution">${loc("wiki_calc_creep_reduction", [
  ...
  variables.append(`
        <div>
            ...
            <div class="calcInput"><span>${loc(
              "wiki_calc_ascended_level"
            )}</span> <b-numberinput :input="val('ascended')" min="0" max="5" v-model="i.ascended.val" :controls="false"></b-numberinput></div>
        </div>
        <div class="calcButton">
            <button class="button" @click="resetInputs()">${loc(
              "wiki_calc_reset"
  ...
  vBind({
    ...
    methods: {
      ...
      importInputs() {
        ...
        inputs.ascended.val =
          ...
          global.stats.achieve["ascended"][
            universeAffix(global.race.universe || "standard")
          ]
            ? global.stats.achieve["ascended"][
                universeAffix(global.race.universe || "standard")
              ]
    ...
    filters: {
      ...
      calc() {
        show.result.vis =
          inputs.harmony.val !== undefined && inputs.ascended.val !== undefined;

        if (show.result.vis) {
          show.result.val = +(
            (Math.log(50 + inputs.harmony.val * inputs.ascended.val) -
              3.912023005428146) *
            0.01
          ).toFixed(5);
...
function coresQuantumCalc(info) {
  ...
  calc.append(
    `<h2 class="has-text-caution">${loc("wiki_calc_core_quantum")}</h2>`
  );
