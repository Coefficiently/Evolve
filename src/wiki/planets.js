...
export function planetsPage(content) {
  ...
  let intro = headerBoxBuilder(content, {
    name: "planet",
    template: "planet",
    paragraphs: 4,
    full: true,
  ...
  let planetInfo = infoForFeature(
    biomes,
    $(
      `<div class="listSide"><h3 class="header has-text-caution">${loc(
        "wiki_planet_biome"
      )}</h3></div>`
    )
  );
  let planetTraitsInfo = infoForFeature(
    planetTraits,
    $(
      `<div class="listSide"><h3 class="header has-text-caution">${loc(
        "wiki_planet_trait"
...
function infoForFeature(planetFeatures, content) {
  Object.keys(planetFeatures).forEach(function (planetFeatureName) {
    ...
    info.append(
      `<div class="type"><h4 class="has-text-caution">${planetFeature.label}</h4></div>`
    );
    ...
    if (planetFeature["vars"] && planetFeature["wiki"]) {
      for (let i = 0; i < planetFeature.vars().length; i++) {
        let type =
          planetFeature.wiki[i] === "%"
            ? "percent"
            : planetFeature.wiki[i] === "-%"
            ? "inverted"
            : planetFeature.wiki[i] === "-A"
            ? "inverted-decimal"
            : "decimal";
        modifiers.append(
          $(
            `<div class="has-text-label">${loc(
              `wiki_planet_${planetFeatureName}${i}`,
              [formatBonusNumber(planetFeature.vars()[i], type)]
            )}</div>`
          )
        );
      }
      ...
      extraInfo[planetFeatureName].forEach(function (label) {
        if (typeof label === "object") {
          info.append(
            $(
              `<div class="has-text-warning">${loc(
                `wiki_planet_${planetFeatureName}_${label.l}`,
                label.v
              )}</div>`
            )
          );
          ...
          info.append(
            $(
              `<div class="has-text-warning">${loc(
                `wiki_planet_${planetFeatureName}_${label}`
              )}</div>`
            )
          );
        }
...
export function formatBonusNumber(num, style) {
  ...
  let modResText =
    (modRes >= 0 ? "+" : "") +
    modRes.toLocaleString(global.settings.locale, {
      style: style,
