...
Object.keys(evolutionPath).forEach(function (key) {
  evolutionPath[key] = evolutionPath[key]
    .map(function (s) {
      return typeof actions.evolution[s].title === "function"
        ? actions.evolution[s].title()
        : actions.evolution[s].title;
    })
    .join(" -> ");
});
...
export function racesPage(content) {
  ...
  Object.keys(races).forEach(function (race) {
    ...
    typeList.forEach(function (gType) {
      genusListing += `<span id="genus${race}${gType}" class="has-text-caution">${loc(
        `genelab_genus_${gType}`
      )}</span>`;
    });

    info.append(
      `<div class="type"><h2 class="has-text-warning">${races[race].name}</h2><span>${genusListing}</span></div>`
    );
    info.append(
      `<div class="desc">${
    ...
    (typeList.includes("carnivore") && typeList.includes("herbivore")
      ? ["omnivore"]
      : typeList
    ).forEach(function (gType) {
      Object.keys(genus_traits[gType])
        .sort()
        .forEach(function (trait) {
          let id = `raceTrait${race}${trait}`;
          let color = races[race].fanaticism === trait ? "danger" : "caution";
          genes.append(
            `<span class="has-text-${color}" id="${id}">${traitSkin(
              "name",
              trait,
              race
            )}<span>`
    ...
    Object.keys(races[race].traits)
      .sort()
      .forEach(function (trait) {
        if (
          (hallowed.active && race === "tortoisan" && trait === "slow") ||
          (race === "unicorn" && trait === "rainbow")
        ) {
          return;
        }
        let id = `raceTrait${race}${trait}`;
    ...
    typeList.forEach(function (gType) {
      popover(
        `genus${race}${gType}`,
        $(
          `<div>${loc(`genelab_genus_${gType}_desc`)}<br><br>${
            evolutionPath[gType]
          }</div>`
        ),
    ...
    for (let i = 0; i < traitList.length; i++) {
      ...
      getTraitDesc(desc, traitList[i].t, {
        fanatic:
          traitList[i].t === races[race].fanaticism ? races[race].name : false,
        trank: traitList[i].r,
        wiki: true,
        species: race,
      });
  ...
  list
    .sort((a, b) => (races[a].name < races[b].name ? -1 : 1))
  ...
  switch (race) {
    ...
    case "entish":
      return date.getMonth() === 11 && date.getDate() >= 17
        ...
        : [];
    case "human":
      return hallowed.active
        ? [
            { t: "anthropophagite", r: 1 },
            { t: "cannibalize", r: 2 },
            { t: "infectious", r: 3 },
          ]
        : [];
...
export function traitsPage(content) {
  ...
  for (let i = 0; i < types.length; i++) {
    Object.keys(traits)
      .sort((a, b) => traitSkin("name", a).localeCompare(traitSkin("name", b)))
      .forEach(function (trait) {
        if (types[i].includes(traits[trait].type)) {
          let info = $(
            `<div id="${traits[trait].type}_${trait}" class="infoBox"></div>`
          );
          content.append(info);
          ...
          sideMenu(
            "add",
            `traits-species`,
            `${traits[trait].type}_${trait}`,
            traitSkin("name", trait)
          );
        }
      });
  }
