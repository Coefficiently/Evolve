...
export function infoBoxBuilder(parent, args, box) {
  ...
  if (!args.hasOwnProperty("paragraphs")) {
    args["paragraphs"] = 0;
  }
  if (!args.hasOwnProperty("text")) {
    args["text"] = {};
  }
  if (!args.hasOwnProperty("rawtext")) {
    args["rawtext"] = {};
  }
  if (!args.hasOwnProperty("para_data")) {
    args["para_data"] = {};
  }
  if (!args.hasOwnProperty("data_color")) {
    args["data_color"] = {};
  }
  if (!args.hasOwnProperty("data_link")) {
    args["data_link"] = {};
  }
  if (!args.hasOwnProperty("h_level")) {
    args["h_level"] = 3;
  }
  if (!args.hasOwnProperty("h_extra")) {
    args["h_extra"] = false;
  }
  if (!args.hasOwnProperty("header")) {
    args["header"] = false;
  }
  ...
  if (!args.hasOwnProperty("default_color")) {
    args["default_color"] = "warning";
  }
  if (!args.hasOwnProperty("examples")) {
    args["examples"] = false;
  }
  if (!args.hasOwnProperty("vue")) {
    args["vue"] = false;
  }
  ...
  if (box) {
    ...
    info = $(
      `<div${args["vue"] ? ` id="${args.name}InfoBox"` : ``} class="infoBox${
        args.full ? ` wide` : ``
      }${args["pclass"] ? ` ${args["pclass"]}` : ""}"></div>`
    );
    if (args["h_level"] && args.h_extra) {
      info.append(
        `<div class="flexInfo"><h${args.h_level} id="${
          args.name
        }" class="header has-text-${args.header ? "caution" : "warning"}">${
          args["label"]
            ? args["label"]
            : loc(`wiki_${args.template}_${args.name}`)
        }</h${args.h_level}>${args.h_extra}</div>`
      );
    } else if (args["h_level"]) {
      info.append(
        `<h${args.h_level} id="${args.name}" class="header has-text-${
          args.header ? "caution" : "warning"
        }">${
          args["label"]
            ? args["label"]
            : loc(`wiki_${args.template}_${args.name}`)
  ...
  if (args.break) {
    for (let i = 0; i < args.break.length; i++) {
      let end =
        i + 1 === args.break.length ? args.paragraphs : args.break[i + 1] - 1;
  ...
  ranges.forEach(function (range) {
    ...
    for (let i = range.s; i <= range.e; i++) {
      if (
        (args.text[i] || args.rawtext[i] || args.para_data[i]) &&
        Array.isArray(args.para_data[i])
      ) {
        ...
        if (args.data_link[i] && Array.isArray(args.data_link[i])) {
          for (let j = 0; j < args.data_link[i].length; j++) {
            if (args.data_link[i][j] && args.data_link[i][j] !== "plain") {
              inputs[
                j
              ] = `<a href="${args.data_link[i][j]}" target="_blank">${inputs[j]}</a>`;
            }
          }
        }
        let color_list =
          args.data_color[i] && Array.isArray(args.data_color[i])
            ? args.data_color[i]
            : args.para_data[i].map((x) => args.default_color);
        for (let j = 0; j < color_list.length; j++) {
          if (color_list[j] !== "plain") {
            inputs[
              j
            ] = `<span class="has-text-${color_list[j]}">${inputs[j]}</span>`;
          }
        }
        let string = args.rawtext[i]
          ? args.rawtext[i]
          : loc(
              args.text[i]
                ? args.text[i]
                : `wiki_${args.template}_${args.name}_para${i}`,
              inputs
            );
  ...
  if (args.examples) {
    info.append(
      $(`<div class="para"><span>${loc(`wiki_examples`)}</span></div>`)
    );
...
export function actionDesc(info, c_action, extended, isStruct) {
  ...
  if (extended) {
    info.append(
      `<div class="type"><h2 class="has-text-warning">${title}</h2><span class="has-text-caution">${extended}</span></div>`
    );
  } else if (!isStruct) {
    let owned =
      global.tech[c_action.grant[0]] &&
      global.tech[c_action.grant[0]] >= c_action.grant[1];
    info.append(
      `<div class="type"><h2 class="has-text-warning">${title}</h2>${
        owned
          ? `<span class="is-sr-only">${loc("wiki_arpa_purchased")}</span>`
          : ``
      }<span class="has-text-${owned ? `success` : `caution`}">${loc(
        `wiki_tech_tree_${c_action.grant[0]}`
      )}: ${c_action.grant[1]}</span></div>`
    );
  } else {
    info.append(
      `<div class="type"><h2 class="has-text-warning">${title}</h2></div>`
    );
  }

  let desc =
    typeof c_action.desc === "string" ? c_action.desc : c_action.desc(true);
  if (desc !== title) {
  ...
  if (c_action.hasOwnProperty("effect")) {
    if (isStruct) {
      let effect =
        typeof c_action.effect === "string" ? c_action.effect : false;
      if (effect !== false) {
      ...
      let effect =
        typeof c_action.effect === "string"
          ? c_action.effect
          : c_action.effect(true);
      if (effect !== false) {
        stats.append(`<div class="effect">${effect}</div>`);
        hasEffect = true;
      }
  ...
  if (c_action.hasOwnProperty("cost")) {
    ...
    let cost = hasEffect
      ? $(
          `<div class="cost right"${
            isStruct ? ' v-show="i.costVis"' : ""
          }></div>`
        )
      : $(`<div class="cost"${isStruct ? ' v-show="i.costVis"' : ""}></div>`);
    let costCreep = ``;
    if (isStruct) {
      cost.append(
        $(`<h2 class="has-text-warning">${loc("wiki_calc_cost")}</h2>`)
      );
      costCreep = $(
        `<div class="cost right" v-show="i.creepVis"><h2 class="has-text-warning">${loc(
          "wiki_calc_cost_creep"
        )}</h2></div>`
      );
    }
    let render = false;
    ...
    let addCost = function (res, res_cost, label, color, structBypass) {
      if (isStruct) {
        cost.append(
          $(
            `<div class="${color}" v-show="r.${res}.vis">${label}{{ r.${res}.cost }}</div>`
          )
        );
        costCreep.append(
          $(
            `<div class="${color}" v-show="r.${res}.vis">{{ r.${res}.creep }}</div>`
          )
        );
        render = true;
      } else if (res_cost > 0) {
      ...
      if (res === "Structs") {
        ...
        Object.keys(structs).forEach(function (region) {
          Object.keys(structs[region]).forEach(function (struct) {
            let res_cost = structs[region][struct].hasOwnProperty("on")
            ...
            if (isStruct) {
              costCreep.append(
                $(`<div class="${color}">${loc("wiki_calc_none")}</div>`)
              );
            }
            render = true;
          });
...
export function createRevealSection(info, id, type, insert) {
  ...
  info.append(reveal);
  reveal.append(
    `<span role="button" id="${id}${type}Button" class="has-text-info reveal" @click="show()">{{ vis | label }}</span>`
  );
  let section = $(
    `<div id="${id}${type}Section" style="display: none;"></div>`
  );
  ...
  vBind({
    ...
    filters: {
      label(vis) {
        return vis
          ? loc(`wiki_reveal_hide`, [insert])
          : loc(`wiki_reveal_show`, [insert]);
      },
    },
  });
...
export function createCalcSection(info, id, type, insert) {
  ...
  vBind({
    ...
    filters: {
      label(vis) {
        return vis
          ? loc(`wiki_calc_hide`, [insert])
          : loc(`wiki_calc_show`, [insert]);
      },
    },
  });
