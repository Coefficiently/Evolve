...
function perkDesc(content, perk) {
  ...
  if (perkList[perk].hasOwnProperty("group")) {
    let gperk = $(
      `<div><div class="has-text-warning">${perkList[perk].name}</div></div>`
    );
    perkList[perk].group.forEach(function (subperk) {
      gperk.append(
        $(
          `<div class="perk has-text-${
            subperk.active() ? `success` : `danger`
          }">${subperk.desc(true)}</div>`
        )
      );
    });
    ...
    perkbox.append(
      $(
        `<div class="has-text-warning">${
          perkList[perk].name
        }</div><div class="has-text-${
          perkList[perk].active() ? `success` : `danger`
        }">${perkList[perk].desc(true)}</div>`
      )
    );
  }
