...
export function govPage(content) {
  ...
  let govs = {
    ...
    democracy: {
      ...
      para_data: {
        ...
        6: [
          loc("job_farmer"),
          loc("job_lumberjack"),
          loc("job_quarry_worker"),
          loc("job_miner"),
          loc("job_crystal_miner"),
          loc("job_coal_miner"),
    ...
    socialist: {
      ...
      vue: {
        filters: {
          ...
          line3(b, g) {
            return b ? (g ? 15 : 12) : g ? 12 : 10;
          },
          line5(b, g) {
            return b ? (g ? 5 : 15) : g ? 10 : 20;
          },
          line6a(b, g) {
            return b ? (g ? 52 : 47) : 42;
          },
    ...
    corpocracy: {
      ...
      vue: {
        filters: {
          line3(b, g) {
            return b ? (g ? 250 : 220) : g ? 220 : 200;
          },
          line4(b, g) {
            return b ? (g ? 190 : 160) : g ? 175 : 150;
          },
          line5(b, g) {
            return b ? (g ? 125 : 110) : g ? 110 : 100;
          },
          line7(b, g) {
            return b ? (g ? 40 : 35) : 30;
          },
  ...
  Object.keys(govs).forEach(function (gov) {
    infoBoxBuilder(mainContent, {
      name: gov,
      template: "government",
      label: loc(`govern_${gov}`),
      paragraphs: govs[gov].paragraphs,
      break: govs[gov].break,
      h_level: 2,
      text: govs[gov].hasOwnProperty("text")
        ? govs[gov].text
        : {
            1: `govern_${gov}_desc`,
          },
      para_data: govs[gov].hasOwnProperty("para_data")
        ? govs[gov].para_data
        : {},
      para_color: govs[gov].hasOwnProperty("para_color")
        ? govs[gov].para_color
        : {},
      vue: {
      ...
      h_extra: govs[gov]?.vue
        ? `<div><b-checkbox v-model="g">${loc(
            `governor_bureaucrat`
          )}</b-checkbox><b-checkbox v-model="b">${loc(
            `arpa_genepool_bureaucratic_efficiency_title`
          )}</b-checkbox></div>`
        : false,
    });
...
export function govBoost(rank) {
  ...
  return global?.genes?.hasOwnProperty("governor") &&
    global?.genes?.governor >= rank
    ? true
    : false;
}
