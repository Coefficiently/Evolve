...
const extraInformation = {
  ...
  starDock: {
    geck: [loc(`wiki_structure_stardock_geck`)],
  },
  interstellar: {},
  intergalactic: {},
  hell: {},
...
const calcInfo = {
  include: {
    // Things that are not one-offs (only used for prehistoric since most are one-offs).
    prehistoric: [
      "membrane",
      "organelles",
      "nucleus",
  ...
  exclude: {
    // Things that are one-offs, disappearing after they're clicked. Automatically excludes creep. Does not add a calculator.
    planetary: [
      "food",
      "lumber",
      "stone",
  ...
  excludeCreep: {
    // Things that aren't one-offs, but also don't have conventional cost creep.
    planetary: ["horseshoe"],
    space: ["horseshoe"],
  ...
  max: {
    // Structures that can have a max to the number of them that you can get. Things with a max of 1 that are included here as opposed to in the exclude section are things that linger around after being purchased, usually having a changing text after being bought.
    prehistoric: {},
    planetary: {
      s_alter: 1,
      banquet: 5,
  ...
  count: {
    // Structures that have "count" values that aren't tracked in the building itself. Here you calculate the count that building would have from the save provided.
    planetary: {
      horseshoe: global.race["shoecnt"] ? global.race["shoecnt"] : 0,
      assembly: global.resource[global.race.species]
  ...
  creepCalc: {
    // Because the cost creep is reverse engineered, buildings with very low cost creep can calculation discrepencies by using the base offset of 100. Here you set higher amounts for those specific buildings to use with the calculation to get a more accurate result.
    planetary: {
      assembly: 1000,
    },
    space: {
...
function addCalcInputs(parent, key, section, region, path) {
  let hasMax =
    calcInfo.max[section] && calcInfo.max[section][key]
      ? calcInfo.max[section][key]
      : false;
  let inputs = {
    owned: 0,
    costVis: false,
  ...
  switch (section) {
    case "prehistoric":
      ...
      inputs.real_owned = global.evolution[key]
        ? global.evolution[key].count
        : 0;
      break;
    case "planetary":
    ...
    case "interstellar":
      ...
      inputs.real_owned = global.interstellar[key]
        ? global.interstellar[key].count
        : 0;
      break;
    case "intergalactic":
    ...
    if (cost) {
      ...
      Object.keys(resources).forEach(function (res) {
        if (res === "Custom") {
          ...
          let new_cost = new_costs[res]
            ? new_costs[res](inputs.owned - inputs.real_owned, inputs.extra)
            : 0;
  ...
  let updateCostCreep = function () {
    ...
    if (
      ...
      !hasMax &&
      !(
        calcInfo.excludeCreep[section] &&
        calcInfo.excludeCreep[section].includes(key)
      ) &&
      section !== "prehistoric"
    ) {
      let high =
        calcInfo.creepCalc[section] && calcInfo.creepCalc[section][key]
          ? calcInfo.creepCalc[section][key]
          : 100;
      let low = high - 1;
      ...
      Object.keys(resources).forEach(function (res) {
        if (upper[res]) {
          resources[res].creep = +(
            upper[res](high, inputs.extra) / lower[res](low, inputs.extra)
          ).toFixed(5);
  ...
  if (
    (calcInfo.include[section] && calcInfo.include[section].includes(key)) ||
    (calcInfo.exclude[section] && !calcInfo.exclude[section].includes(key))
  ) {
    updateCostCreep();
...
function prehistoricPage(content, path) {
  ...
  Object.keys(actions.evolution).forEach(function (action) {
    if (
      actions.evolution[action].hasOwnProperty("title") &&
      (action !== "custom" || global.hasOwnProperty("custom")) &&
      (!actions.evolution[action].hasOwnProperty("wiki") ||
        actions.evolution[action].wiki)
    ) {
      ...
      sideMenu(
        ...
        `prehistoric-${affix}`,
        id[1],
        typeof actions.evolution[action].title === "function"
          ? actions.evolution[action].title()
          : actions.evolution[action].title
      );
    }
  });
...
function planetaryPage(content, path) {
  ...
  Object.keys(actions.city).forEach(function (action) {
    if (
      (!actions.city[action].hasOwnProperty("wiki") ||
        actions.city[action].wiki) &&
      (!actions.city[action].hasOwnProperty("path") ||
        actions.city[action].path.includes(path))
    ) {
      ...
      sideMenu(
        ...
        `planetary-${affix}`,
        id[1],
        typeof actions.city[action].title === "function"
          ? actions.city[action].title()
          : actions.city[action].title
      );
    }
  });
...
function spacePage(content, path) {
  ...
  Object.keys(actions.space).forEach(function (region) {
    let name =
      typeof actions.space[region].info.name === "string"
        ? actions.space[region].info.name
        : actions.space[region].info.name();
    let desc =
      typeof actions.space[region].info.desc === "string"
        ? actions.space[region].info.desc
        : actions.space[region].info.desc();

    Object.keys(actions.space[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.space[region][struct].hasOwnProperty("wiki") ||
          actions.space[region][struct].wiki) &&
        (!actions.space[region][struct].hasOwnProperty("path") ||
          actions.space[region][struct].path.includes(path))
      ) {
        ...
        actionDesc(
          info,
          actions.space[region][struct],
          `<span id="pop${actions.space[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          `space-${affix}`,
          id[1],
          typeof actions.space[region][struct].title === "function"
            ? actions.space[region][struct].title()
            : actions.space[region][struct].title
        );
        popover(
          `pop${actions.space[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
  ...
  Object.keys(actions.starDock).forEach(function (struct) {
    if (
      struct !== "info" &&
      (!actions.starDock[struct].hasOwnProperty("wiki") ||
        actions.starDock[struct].wiki) &&
      (!actions.starDock[struct].hasOwnProperty("path") ||
        actions.starDock[struct].path.includes(path))
    ) {
      ...
      actionDesc(
        info,
        actions.starDock[struct],
        `<span id="pop${actions.starDock[struct].id}">${loc(
      ...
      sideMenu(
        ...
        `space-${affix}`,
        id[1],
        typeof actions.starDock[struct].title === "function"
          ? actions.starDock[struct].title()
          : actions.starDock[struct].title
      );
      popover(
        `pop${actions.starDock[struct].id}`,
        $(`<div>${loc(`space_gas_star_dock_wiki`)}</div>`)
      );
    }
  });
}
...
function interstellarPage(content) {
  Object.keys(actions.interstellar).forEach(function (region) {
    let name =
      typeof actions.interstellar[region].info.name === "string"
        ? actions.interstellar[region].info.name
        : actions.interstellar[region].info.name();
    let desc =
      typeof actions.interstellar[region].info.desc === "string"
        ? actions.interstellar[region].info.desc
        : actions.interstellar[region].info.desc();

    Object.keys(actions.interstellar[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.interstellar[region][struct].hasOwnProperty("wiki") ||
          actions.interstellar[region][struct].wiki)
      ) {
        ...
        actionDesc(
          info,
          actions.interstellar[region][struct],
          `<span id="pop${actions.interstellar[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          id[1],
          typeof actions.interstellar[region][struct].title === "function"
            ? actions.interstellar[region][struct].title()
            : actions.interstellar[region][struct].title
        );
        popover(
          `pop${actions.interstellar[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
...
function intergalacticPage(content) {
  Object.keys(actions.galaxy).forEach(function (region) {
    let name =
      typeof actions.galaxy[region].info.name === "string"
        ? actions.galaxy[region].info.name
        : actions.galaxy[region].info.name();
    let desc =
      typeof actions.galaxy[region].info.desc === "string"
        ? actions.galaxy[region].info.desc
        : actions.galaxy[region].info.desc();

    Object.keys(actions.galaxy[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.galaxy[region][struct].hasOwnProperty("wiki") ||
          actions.galaxy[region][struct].wiki)
      ) {
        ...
        actionDesc(
          info,
          actions.galaxy[region][struct],
          `<span id="pop${actions.galaxy[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          id[1],
          typeof actions.galaxy[region][struct].title === "function"
            ? actions.galaxy[region][struct].title()
            : actions.galaxy[region][struct].title
        );
        popover(
          `pop${actions.galaxy[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
...
function hellPage(content) {
  Object.keys(actions.portal).forEach(function (region) {
    let name =
      typeof actions.portal[region].info.name === "string"
        ? actions.portal[region].info.name
        : actions.portal[region].info.name();
    let desc =
      typeof actions.portal[region].info.desc === "string"
        ? actions.portal[region].info.desc
        : actions.portal[region].info.desc();

    Object.keys(actions.portal[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.portal[region][struct].hasOwnProperty("wiki") ||
          actions.portal[region][struct].wiki)
      ) {
        ...
        actionDesc(
          info,
          actions.portal[region][struct],
          `<span id="pop${actions.portal[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          `hell-structures`,
          id[1],
          typeof actions.portal[region][struct].title === "function"
            ? actions.portal[region][struct].title()
            : actions.portal[region][struct].title
        );
        popover(
          `pop${actions.portal[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
...
function edenPage(content) {
  Object.keys(actions.eden).forEach(function (region) {
    let name =
      typeof actions.eden[region].info.name === "string"
        ? actions.eden[region].info.name
        : actions.eden[region].info.name();
    let desc =
      typeof actions.eden[region].info.desc === "string"
        ? actions.eden[region].info.desc
        : actions.eden[region].info.desc();

    Object.keys(actions.eden[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.eden[region][struct].hasOwnProperty("wiki") ||
          actions.eden[region][struct].wiki)
      ) {
        ...
        actionDesc(
          info,
          actions.eden[region][struct],
          `<span id="pop${actions.eden[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          `eden-structures`,
          id[1],
          typeof actions.eden[region][struct].title === "function"
            ? actions.eden[region][struct].title()
            : actions.eden[region][struct].title
        );
        popover(
          `pop${actions.eden[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
...
function taucetiPage(content) {
  Object.keys(actions.tauceti).forEach(function (region) {
    let name =
      typeof actions.tauceti[region].info.name === "string"
        ? actions.tauceti[region].info.name
        : actions.tauceti[region].info.name();
    let desc =
      typeof actions.tauceti[region].info.desc === "string"
        ? actions.tauceti[region].info.desc
        : actions.tauceti[region].info.desc();

    Object.keys(actions.tauceti[region]).forEach(function (struct) {
      if (
        struct !== "info" &&
        (!actions.tauceti[region][struct].hasOwnProperty("wiki") ||
          actions.tauceti[region][struct].wiki)
      ) {
        ...
        actionDesc(
          info,
          actions.tauceti[region][struct],
          `<span id="pop${actions.tauceti[region][struct].id}">${name}</span>`,
          true
        );
        ...
        sideMenu(
          ...
          `tauceti-structures`,
          id[1],
          typeof actions.tauceti[region][struct].title === "function"
            ? actions.tauceti[region][struct].title()
            : actions.tauceti[region][struct].title
        );
        popover(
          `pop${actions.tauceti[region][struct].id}`,
          $(`<div>${desc}</div>`)
        );
      }
    });
