...
export function resetsPage(content) {
  ...
  let reset_labels = resets.map(
    (x) => `<span class="has-text-caution">${loc(`wiki_resets_${x}`)}</span>`
  );
  ...
  infoBoxBuilder(mainContent, {
    ...
    para_data: {
      1: [
        resets.length,
        reset_labels.slice(0, -1).join(", ") +
          `, & ${reset_labels[reset_labels.length - 1]}`,
      ],
    },
  ...
  let universe_labels = universes.map(
    (x) => `<span class="has-text-caution">${loc(`universe_${x}`)}</span>`
  );
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    para_data: {
      2: [
        loc("wiki_p_res_plasmids"),
        loc("wiki_p_res_phage"),
        loc("wiki_p_res_dark"),
      ],
      3: [loc("tech_dist_stellar_engine")],
      4: [
        loc("tech_mass_ejector"),
        loc("wiki_resets_blackhole_exotic"),
        0.025,
      ...
      10: [
        universes.length,
        universe_labels.slice(0, -1).join(", ") +
          `, ${loc("or")} ${universe_labels[universe_labels.length - 1]}`,
      ],
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    para_data: {
      2: [
        loc("wiki_p_res_plasmids"),
        loc("wiki_p_res_phage"),
        loc("wiki_p_res_harmony"),
      ],
      3: [
        loc("interstellar_ascension_machine"),
        "10,000",
        loc("interstellar_thermal_collector"),
      ],
      4: [100, 25],
      7: [`+2%`],
      8: [`+2`, `+5%`, `+10%`],
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    data_link: {
      ...
      5: [
        "#space-tp_structures-ai_colonist",
        "#space-tp_structures-decoder",
        "#space-tp_structures-shock_trooper",
        "#space-tp_structures-tank",
      ],
    },
  });
  ...
  section = infoBoxBuilder(mainContent, {
    ...
    data_link: {
      2: ["wiki.html#challenges-gameplay-scenarios_truepath"],
      3: [
        "wiki.html#tauceti-structures-matrioshka_brain",
        "wiki.html#tauceti-structures-ignition_device",
      ],
    },
  });
