...
export const actions = {
  ...
  city: {
    ...
    s_alter: {
      ...
      effect() {
        ...
        if (
          ...
          desc =
            desc +
            `<div>${loc("city_s_alter_rage", [
              traits.cannibalize.vars()[0],
              timeFormat(global.city.s_alter.rage),
            ])}</div>`;
        }
        if (
          global.city.hasOwnProperty("s_alter") &&
          ...
          desc =
            desc +
            `<div>${loc("city_s_alter_regen", [
              traits.cannibalize.vars()[0],
              timeFormat(global.city.s_alter.regen),
            ])}</div>`;
        }
        if (
          global.city.hasOwnProperty("s_alter") &&
          ...
          desc =
            desc +
            `<div>${loc("city_s_alter_mind", [
              traits.cannibalize.vars()[0],
              timeFormat(global.city.s_alter.mind),
            ])}</div>`;
        }
        if (
          global.city.hasOwnProperty("s_alter") &&
          ...
          desc =
            desc +
            `<div>${loc("city_s_alter_mine", [
              traits.cannibalize.vars()[0],
              timeFormat(global.city.s_alter.mine),
            ])}</div>`;
        }
        if (
          global.city.hasOwnProperty("s_alter") &&
          ...
          desc =
            desc +
            `<div>${loc("city_s_alter_harvest", [
              traits.cannibalize.vars()[0],
              timeFormat(global.city.s_alter.harvest),
              jobType,
            ])}</div>`;
        }
        return desc;
    ...
    cottage: {
      ...
      effect() {
        ...
        if (global.tech["home_safe"]) {
          ...
          return `<div>${loc("plus_max_citizens", [pop])}</div><div>${loc(
            "plus_max_resource",
            [`\$${safe.toLocaleString()}`, loc("resource_Money_name")]
          )}</div>`;
        } else {
    ...
    apartment: {
      ...
      desc() {
        return `<div>${loc("city_apartment_desc", [
          $(this)[0].citizens(),
      ...
      effect() {
        ...
        if (global.tech["home_safe"]) {
          ...
          return `<div>${loc("plus_max_citizens", [
            pop,
          ])}. <span class="has-text-caution">${loc("minus_power", [
            $(this)[0].powered(),
      ...
      effect() {
        ...
        return global.race["carnivore"] && !global.race["artifical"]
          ? `<div>${loc("plus_max_resource", [
              pop,
              loc("citizen"),
            ])}</div><div>${loc("city_lodge_effect", [5])}</div>`
          : loc("plus_max_resource", [pop, loc("citizen")]);
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    smokehouse: {
      ...
      effect() {
        ...
        ])}</div><div>${loc("city_smokehouse_effect", [10])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    soul_well: {
      ...
      effect() {
        ...
        let production = global.race["ghostly"]
          ...
          : 2;
        return `<div>${loc("city_soul_well_effect", [
          production,
        ])}</div><div>${loc("plus_max_resource", [
          souls,
          loc("resource_Souls_name"),
        ])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    transmitter: {
      ...
      effect() {
        ...
        ])}</div><div>${loc("city_transmitter_effect", [
          sig_cap,
    ...
    farm: {
      ...
      effect() {
        ...
        return global.tech["farm"]
          ? `<div>${loc("city_farm_effect")}</div><div>${loc(
              "plus_max_resource",
              [pop, loc("citizen")]
            )}</div>`
        ...
        let wood =
          global.race["kindling_kindred"] || global.race["smoldering"]
            ...
            : `<div class="has-text-caution">${loc(
                "city_compost_heap_effect2",
                [0.5, global.resource.Lumber.name]
              )}</div>`;
        return `<div>${loc("city_compost_heap_effect", [
          generated,
        ])}</div><div>${loc("city_compost_heap_effect3", [
          store,
        ])}</div>${wood}`;
      },
      switchable() {
    ...
    mill: {
      ...
      powered() {
        return global.race["environmentalist"] ? -1.5 : -1;
      },
      power_reqs: { agriculture: 6 },
      effect() {
        if (global.tech["agriculture"] >= 6) {
          return `<span class="has-text-success">${loc("city_on")}</span> ${loc(
            "city_mill_effect1"
          )} <span class="has-text-danger">${loc("city_off")}</span> ${loc(
            "city_mill_effect2"
          )}`;
        } else {
          return false;
    ...
    hospital: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("city_hospital_effect", [
          healing,
        ])}</div>${clinic}`;
      ...
      effect() {
        ...
        let effect =
          global.tech["spy"] && global.tech["spy"] >= 3
            ? `<div>${loc("city_boot_camp_effect", [rate])}</div><div>${loc(
                "city_boot_camp_effect2",
                [10]
              )}</div>`
            : `<div>${loc("city_boot_camp_effect", [rate])}</div>`;
        if (global.race["artifical"] && !global.race["orbit_decayed"]) {
          let repair = global.tech["medic"] || 1;
          effect += `<div>${loc("city_boot_camp_art_effect", [
            repair * 5,
          ])}</div>`;
        }
        if (global.race["artifical"] && global.race.hasOwnProperty("vax")) {
          effect += `<div>${loc("tau_home_disease_lab_vax", [
            +global.race.vax.toFixed(2),
          ])}</div>`;
        }
        return effect;
    ...
    storage_yard: {
      ...
      effect() {
        ...
        if (global.tech["trade"] && global.tech["trade"] >= 3) {
          ...
          ])}</div><div>${loc("city_trade_effect", [1])}</div>`;
        } else {
    ...
    graveyard: {
      ...
      effect() {
        ...
        return `<div>${loc("city_graveyard_effect", [8])}</div><div>${loc(
          "plus_max_resource",
          [lum, global.resource.Lumber.name]
        )}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    sawmill: {
      ...
      effect() {
        ...
        if (global.city.powered) {
          desc =
            desc +
            `<div class="has-text-caution">${loc("city_sawmill_effect3", [
              4,
              $(this)[0].powered(),
            ])}</div>`;
        }
    ...
    rock_quarry: {
      ...
      effect() {
        ...
        if (global.tech["mine_conveyor"]) {
          return `<div>${loc("city_rock_quarry_effect1", [2])}</div><div>${loc(
            "plus_max_resource",
            [stone, global.resource.Stone.name]
          )}</div>${asbestos}<div class="has-text-caution">${loc(
            "city_rock_quarry_effect2",
            [4, $(this)[0].powered()]
          )}</div>`;
        } else {
          return `<div>${loc("city_rock_quarry_effect1", [2])}</div><div>${loc(
      ...
      effect() {
        if (global.tech["cement"] >= 5) {
          ...
          return `<div>${loc("plus_max_resource", [
            ...
            "city_cement_plant_effect2",
            [$(this)[0].powered(), screws]
          )}</div>`;
        } else {
          return loc("plus_max_resource", [
            jobScale(2),
    ...
    smelter: {
      ...
      flair: `<div>${loc("city_smelter_flair1")}<div></div>${loc(
        "city_smelter_flair2"
      )}</div>`,
    },
    metal_refinery: {
      ...
      effect() {
        ...
        if (global.tech["alumina"] >= 2) {
          return `<span>${loc(label, [
            6,
          ])}</span> <span class="has-text-caution">${loc(
            "city_metal_refinery_effect2",
            [6, 12, $(this)[0].powered()]
          )}</span>`;
        } else {
          return loc(label, [6]);
      ...
      effect() {
        if (global.tech["mine_conveyor"]) {
          return `<div>${loc("plus_max_resource", [
            jobScale(1),
            loc(`job_miner`),
          ])}</div><div class="has-text-caution">${loc("city_mine_effect2", [
            $(this)[0].powered(),
            5,
          ])}</div>`;
        } else {
    ...
    oil_depot: {
      ...
      effect() {
        ...
        let effect = `<div>${loc("plus_max_resource", [
          oil,
    ...
    trade: {
      ...
      effect() {
        let routes =
          global.race["xenophobic"] || global.race["nomadic"]
            ? global.tech.trade
            : global.tech.trade + 1;
        if (global.tech["trade"] && global.tech["trade"] >= 3) {
          routes--;
    ...
    wharf: {
      ...
      effect() {
        ...
        return `<div>${loc("city_trade_effect", [2])}</div><div>${loc(
          "city_wharf_effect"
        )}</div><div>${loc("plus_max_crates", [containers])}</div><div>${loc(
          "plus_max_containers",
          [containers]
        )}</div>`;
      },
    ...
    tourist_center: {
      ...
      effect(wiki) {
        ...
        let desc = `<div class="has-text-caution">${loc(
          "city_tourist_center_effect1",
          [global.resource.Food.name]
        )}</div>`;
        desc += `<div>${loc("city_tourist_center_effect2", [
          amp,
          actions.city.amphitheatre.title(),
        ])}</div>`;
        desc += `<div>${loc("city_tourist_center_effect2", [
          cas,
          structName("casino"),
        ...
        if (
          ...
          desc += `<div>${loc(`city_tourist_center_effect2`, [
            (global.civic.govern.type === "corpocracy" ? 6 : 3) * xeno,
            loc("city_trade"),
          ])}</div>`;
        }
        ...
        if (piousVal) {
          desc += `<div>${loc(`city_tourist_center_effect2`, [
            (global.civic.govern.type === "corpocracy"
              ? piousVal * 2
              : piousVal) * xeno,
            structName("temple"),
          ])}</div>`;
    ...
    amphitheatre: {
      ...
      effect() {
        ...
        return `<div>${loc("plus_max_resource", [
          jobScale(athVal2 ? athVal2 : 1),
          loc(`job_entertainer`),
        ])}</div><div>${loc("city_max_morale", [athVal1 ? athVal1 : 1])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    shrine: {
      ...
      effect() {
        ...
        let desc = `<div class="has-text-special">${loc(
          "city_shrine_effect"
        )}</div>`;
        ...
        if (global.city["shrine"] && metal.active) {
          desc =
            desc +
            `<div>${loc("city_shrine_metal", [
              +((metal.mult - 1) * 100).toFixed(1),
            ])}</div>`;
        }
        if (global.city["shrine"] && know.active) {
          desc =
            desc +
            `<div>${loc("city_shrine_know", [
              (+know.add.toFixed(1)).toLocaleString(),
            ])}</div>`;
          desc =
            desc +
            `<div>${loc("city_shrine_know2", [
              +((know.mult - 1) * 100).toFixed(1),
            ])}</div>`;
        }
        if (global.city["shrine"] && tax.active) {
          desc =
            desc +
            `<div>${loc("city_shrine_tax", [
              +((tax.mult - 1) * 100).toFixed(1),
            ])}</div>`;
        }
        return desc;
      },
    ...
    meditation: {
      ...
      effect() {
        ...
        return `<div>${loc(`city_meditation_effect`, [
          traits.calm.vars()[0],
        ])}</div><div class="has-text-special">${loc(
          `city_meditation_effect2`,
          [2]
        )}</div><div class="has-text-special">${loc(`city_meditation_effect3`, [
          1,
        ])}</div><div>${loc(`city_meditation_effect4`, [
          `${(zen * 100).toFixed(2)}%`,
        ])}</div>`;
      },
      action() {
    ...
    banquet: {
      ...
      cost: {
        Money(offset) {
          const count =
            (offset ? offset : 0) +
            (global.city["banquet"] ? global.city["banquet"].count : 0);
      ...
      effect(wiki) {
        ...
        desc += `<div>${loc(`city_banquet_effect1`, [
          sizeApproximation(
            ((count >= 5 ? 1.02 : 1.022) ** strength - 1) * 100
          ),
        ])}</div>`;
        if (count >= 1) {
          desc += `<div>${loc(`city_banquet_effect2`, [
            (strength ** 0.75).toFixed(2),
          ])}</div>`;
        }
        if (count >= 2) {
          desc += `<div>${loc(`city_banquet_effect3`, [
            (strength ** 0.65).toFixed(2),
          ])}</div>`;
        }
        if (count >= 3) {
          desc += `<div>${loc(`city_banquet_effect4`, [
            (strength ** 0.65).toFixed(2),
          ])}</div>`;
        }
        if (count >= 4) {
          desc += `<div>${loc(`city_banquet_effect5`, [
            (strength ** 0.75).toFixed(2),
          ])}</div>`;
        }
        return desc;
    ...
    university: {
      ...
      effect(wiki) {
        ...
        return `<div>${loc("city_university_effect", [
          jobScale(1),
        ])}</div><div>${loc("city_max_knowledge", [
          gain.toLocaleString(),
        ])}</div>`;
      },
      knowVal(wiki) {
    ...
    library: {
      ...
      effect() {
        ...
        return `<div>${loc("city_max_knowledge", [
          gain.toLocaleString(),
        ])}</div><div>${loc("city_library_effect", [know])}</div>`;
      },
      action() {
        if (payCosts($(this)[0])) {
          let gain = 125;
    ...
    wardenclyffe: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("city_wardenclyffe_effect1", [
          jobScale(1),
        ...
        if (global.city.powered) {
          ...
          if (global.tech["broadcast"]) {
            ...
            desc =
              desc +
              `<div class="has-text-caution">${loc(
                "city_wardenclyffe_effect3",
                [$(this)[0].powered(), pgain.toLocaleString(), morale]
              )}</div>`;
          } else {
            desc =
              desc +
              `<div class="has-text-caution">${loc(
                "city_wardenclyffe_effect2",
                [$(this)[0].powered(), pgain.toLocaleString()]
              )}</div>`;
          }
          if (global.race["artifical"]) {
            desc =
              desc +
              `<div class="has-text-caution">${loc("city_transmitter_effect", [
                spatialReasoning(250),
              ])}</div`;
          }
        }
        return desc;
      ...
      flair() {
        return global.race.universe === "magic"
          ...
          : `<div>${loc("city_wardenclyffe_flair1")}</div><div>${loc(
              "city_wardenclyffe_flair2"
            )}</div>`;
      },
    },
    biolab: {
      ...
      effect(wiki) {
        ...
        gain = +gain.toFixed(0);
        return `<span>${loc("city_max_knowledge", [
          gain.toLocaleString(),
    ...
    coal_power: {
      ...
      desc() {
        return global.race["environmentalist"]
          ...
          : `<div>${loc(
              global.race.universe === "magic"
                ? "city_mana_engine_desc"
                : "city_coal_power_desc"
      ...
      effect() {
        ...
        return global.race["environmentalist"]
          ? `+${power}MW`
          : `<span>+${power}MW.</span> <span class="has-text-caution">${loc(
              global.race.universe === "magic"
                ? "city_mana_engine_effect"
      ...
      powered() {
        ...
        if (global.race["environmentalist"]) {
          if (
            ...
            power =
              global.stats.achieve["dissipated"].l >= 5 ? base - 2 : base - 1;
          } else {
    ...
    fission_power: {
      ...
      effect() {
        ...
        return `<span>+${-$(
          this
        )[0].powered()}MW.</span> <span class="has-text-caution">${loc(
    ...
    mass_driver: {
      ...
      effect() {
        let exo =
          global.tech.mass >= 2
            ? `<div>${loc("city_mass_driver_effect2", [
                1,
                global.civic.scientist.name,
              ])}</div>`
            : "";
        return `${exo}<span>${loc("city_mass_driver_effect", [
          global.race["truepath"] ? 6 : 5,
          flib("name"),
        ])}</span> <span class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</span>`;
  ...
  starDock: {
    ...
    seeder: {
      ...
      desc() {
        ...
        if (global.starDock["seeder"] && global.starDock.seeder.count >= 100) {
          return `<div>${label}</div><div class="has-text-special">${loc(
            "star_dock_seeder_desc2"
          )}</div>`;
        } else {
          return `<div>${label}</div><div class="has-text-special">${loc(
            "star_dock_seeder_desc1"
          )}</div>`;
        }
      },
      reqs: { genesis: 5 },
      ...
      effect(wiki) {
        ...
        return `<div>${
          global.race["cataclysm"]
            ? loc("star_dock_exodus_effect")
            : loc("star_dock_seeder_effect")
        }</div><div class="has-text-special">${remain}</div>`;
      },
      action() {
    ...
    prep_ship: {
      ...
      desc() {
        ...
        return `<div>${label}</div><div class="has-text-danger">${loc(
          "star_dock_genesis_desc2"
        )}</div>`;
      },
      reqs: { genesis: 6 },
      queue_complete() {
        ...
        effect() {
          ...
          let goal =
            global.resource.Thermite.amount < thermite
              ? `<div class="has-text-warning">${loc("city_bonfire_effect3", [
                  thermite.toLocaleString(),
                ])}</div><div class="has-text-caution">${loc(
                  "city_bonfire_effect4",
                  [
                    (+global.resource.Thermite.amount.toFixed(
                      0
                    )).toLocaleString(),
                    thermite.toLocaleString(),
                  ]
              ...
              : ``;
          return `<div>${loc(`city_bonfire_effect`, [
            global.resource.Thermite.diff,
          ])}</div><div>${loc(`city_bonfire_effect2`, [
            +morale.toFixed(1),
          ])}</div>${goal}`;
        },
        action() {
    ...
    case "assembly": {
      ...
      let action = {
        ...
        cost: {
          ...
          Copper(offset) {
            return global.race["deconstructor"]
              ...
              ? costMultiplier("citizen", offset, assemblyCostAdjust(50), 1.01)
              : 0;
          },
          Aluminium(offset) {
            return global.race["deconstructor"]
        ...
        effect() {
          ...
          return `<div>${loc("city_assembly_effect", [
    ...
    case "nanite_factory": {
      let action = {
        ...
        effect() {
          ...
          return `<div>${loc("city_nanite_factory_effect", [
        ...
        effect() {
          ...
          if (
            ...
            let cattleCap = global.city.hasOwnProperty("captive_housing")
              ...
              : 0;
            desc += `<div>${loc(`city_captive_housing_cattle`, [
              cattle,
              cattleCap,
            ])}</div>`;
          }
          ...
          if (global.city.hasOwnProperty("surfaceDwellers")) {
            for (let i = 0; i < global.city.surfaceDwellers.length; i++) {
              ...
              desc += `<div>${loc(`city_captive_housing_broken`, [
                races[r].name,
                mindbreak,
              ])}</div>`;
              desc += `<div>${loc(`city_captive_housing_untrained`, [
                races[r].name,
                jailed,
              ])}</div>`;
            }
          ...
          let raceCap = global.city.hasOwnProperty("captive_housing")
            ...
            : 0;
          desc += `<div>${loc(`city_captive_housing_capacity`, [
            usedCap,
            raceCap,
          ])}</div>`;
          if (global.tech["unfathomable"] && global.tech.unfathomable >= 2) {
            desc += `<div>${loc(`plus_max_resource`, [
              1,
              loc("job_torturer"),
            ])}</div>`;
          }
    ...
    (actions.evolution[challenge] = {
      ...
      desc() {
        ...
        if (global.race.universe === "micro") {
          desc =
            desc +
            `<div class="has-text-danger">${loc(
              "evo_challenge_micro_warn"
            )}</div>`;
        }
...
actions.evolution["bunker"] = {
  ...
  desc() {
    return `<div>${loc("evo_bunker")}</div><div class="has-text-special">${loc(
      "evo_challenge"
    )}</div>`;
  },
  reqs: { evo: 6 },
...
function challengeEffect(c) {
  switch (c) {
    ...
    case "orbit_decay": {
      if (calc_mastery() >= 100) {
        return `<div>${loc("evo_challenge_orbit_decay_effect", [
          5000,
        ])}</div><div class="has-text-caution">${loc(
          "evo_challenge_scenario_failwarn"
        )}</div>`;
      } else {
        return `<div>${loc("evo_challenge_orbit_decay_effect", [
          5000,
    ...
    case "junker": {
      return global.city.biome === "hellscape" &&
        global.race.universe !== "evil"
        ? `<div>${loc(
            "evo_challenge_junker_effect"
          )}</div><div class="has-text-special">${loc("evo_warn_unwise")}</div>`
        : loc("evo_challenge_junker_effect");
    }
    case "cataclysm": {
      if (calc_mastery() >= 50) {
        return `<div>${loc(
          "evo_challenge_cataclysm_effect"
        )}</div><div class="has-text-caution">${loc(
          "evo_challenge_cataclysm_warn"
        )}</div>`;
      } else {
        return `<div>${loc(
          "evo_challenge_cataclysm_effect"
    ...
    desc =
      ...
      `<div>${
        global.race["truepath"]
          ? loc("city_temple_effect_tp", [2, 25])
          : loc("city_temple_effect4")
      }</div>`;
  }
  return desc;
}
...
export function casinoEffect() {
  ...
  let banker =
    global.race["orbit_decayed"] || global.tech["isolation"]
      ? `<div>${loc("plus_max_resource", [
          jobScale(1),
          loc("banker_name"),
        ])}</div>`
  ...
  let desc = `<div>${loc("plus_max_resource", [
    ...
    loc("resource_Money_name"),
  ])}</div>${joy}${banker}<div>${loc("city_max_morale", [1])}</div>`;
  let cash =
    Math.log2(1 + global.resource[global.race.species].amount) *
    (global.race["gambler"] ? 2.5 + global.race["gambler"] / 10 : 2.5);
  ...
  city_categories.forEach(function (category) {
    ...
    if (global.settings["cLabels"]) {
      ...
      $(`<div id="city-dist-${category}" class="city"></div>`)
        .appendTo("#city")
        .append(
          `<div><h3 class="name has-text-warning">${loc(
            `city_dist_${category}`
          )}</h3></div>`
        );
...
export function setAction(c_action, action, type, old, prediction) {
  ...
  let parent =
    c_action["highlight"] && c_action.highlight()
      ? $(`<div id="${id}" class="action hl"${reqs}></div>`)
      : $(`<div id="${id}" class="action"${reqs}></div>`);
  ...
  if (old) {
    ...
    let active = c_action["highlight"]
      ...
      : "";
    element = $(
      `<a class="button is-dark${cst}${clss}"${data} v-on:click="action"><span class="aTitle" v-html="$options.filters.title(title)"></span>${active}</a><a role="button" v-on:click="describe" class="is-sr-only">{{ title }} description</a>`
    );
  }
  ...
  if (
    ...
    let special =
      $(`<div class="special" role="button" v-bind:title="title | options" @click="trigModal"><svg version="1.1" x="0px" y="0px" width="12px" height="12px" viewBox="340 140 280 279.416" enable-background="new 340 140 280 279.416" xml:space="preserve">
  ...
  if (c_action["on"] || c_action["off"]) {
    if (c_action["on"]) {
      let powerOn = $(
        `<span class="on" title="ON" v-html="$options.filters.val('on')"></span>`
      );
      parent.append(powerOn);
    }
    if (c_action["off"]) {
      let powerOff = $(
        `<span class="off" title="OFF" v-html="$options.filters.val('off')"></span>`
      );
    ...
    if (
      ...
      (c_action["switchable"] && c_action.switchable())
    ) {
      let powerOn = $(
        `<span role="button" :aria-label="on_label()" class="on" @click="power_on" title="ON" v-html="$options.filters.p_on(act.on,'${c_action.id}')"></span>`
      );
      let powerOff = $(
        `<span role="button" :aria-label="off_label()" class="off" @click="power_off" title="OFF" v-html="$options.filters.p_off(act.on,'${c_action.id}')"></span>`
      );
      parent.append(powerOn);
      parent.append(powerOff);
    }
  ...
  if (
    ...
    element.append(
      $(
        `<div class="repair"><progress class="progress" :value="repair()" :max="repairMax()"></progress></div>`
      )
    );
  }
  if (old) {
...
export function setPlanet(opt) {
  ...
  var element = $(
    `<a class="button is-dark" v-on:click="action"><span class="aTitle">${title}</span></a>`
  );
  parent.append(element);
...
export function planetGeology(geology) {
  ...
  if (Object.keys(geology).length > 0) {
    ...
    for (let key in geology) {
      if (key !== 0) {
        if (geology[key] > 0) {
          let res_val = `<div class="has-text-advanced pGeo">${loc(
            `resource_${key}_name`
          )}`;
          if (numShow > 0) {
            res_val += `: <span class="has-text-success">+${Math.round(
              (geology[key] + 1) * 100 - 100
            )}%</span>`;
            numShow--;
          } else {
            res_val += `: <span class="has-text-success">${loc(
              "bonus"
            )}</span>`;
          }
          res_val += `</div>`;
          ...
          let res_val = `<div class="has-text-caution pGeo">${loc(
            `resource_${key}_name`
          )}`;
          if (numShow > 0) {
            res_val += `: <span class="has-text-danger">${Math.round(
              (geology[key] + 1) * 100 - 100
            )}%</span>`;
            numShow--;
          } else {
...
export function actionDesc(parent, c_action, obj, old, action, a_type, bres) {
  ...
  if (
    ...
    touch = $(
      `<a id="touchButton" class="button is-dark touchButton">${
        c_action.hasOwnProperty("touchlabel")
          ? c_action.touchlabel
          : loc("construct")
      }</a>`
    );
  ...
  if (c_action["category"] && type === "tech" && !old) {
    parent.append(
      $(
        `<div class="has-text-flair">${loc("tech_dist_category")}: ${loc(
          `tech_dist_${c_action.category}`
        )}</div>`
      )
    );
  }
  ...
  if (c_action["reqs"]) {
    ...
    if (reqList.length > 0) {
      ...
      parent.append(
        $(
          `<div class="has-text-caution">${loc("requires_tech", [
            listing,
          ])}</div>`
        )
      );
  ...
  if (
    ...
    if (obj && obj["time"]) {
      ...
      parent.append(
        $(
          `<div class="flair has-text-advanced">${loc("action_ready", [
            time,
          ])}</div>`
        )
      );
...
export function checkCosts(costs) {
  ...
  Object.keys(costs).forEach(function (res) {
    if (res === "Custom") {
      ...
      let f_res = res === "Species" ? global.race.species : res;
      let fail_max =
        global.resource[f_res].max >= 0 && testCost > global.resource[f_res].max
          ? true
          : false;
      if (
        testCost >
          Number(global.resource[f_res].amount) + global.resource[f_res].diff ||
        fail_max
      ) {
        test = false;
        return;
...
function challengeGeneHeader() {
  ...
  challenge.append(
    $(
      `<div class="divider has-text-warning"><h2 class="has-text-danger">${loc(
        "evo_challenge_genes"
      )}</h2></div>`
    )
  );
  ...
  if (global.genes["challenge"] && global.genes["challenge"] >= 2) {
    challenge.append(
      $(
        `<div class="has-text-advanced">${loc(
          "evo_challenge_genes_mastery"
        )}</div>`
      )
    );
  }
...
function exitSimulation() {
  ...
  challenge.append(
    $(
      `<div class="has-text-advanced">${loc(
        "evo_challenge_simulation_desc"
      )}</div>`
    )
  );
  challenge.append(
    $(
      `<button class="button simButton" @click="exitsim()">${loc(
        `evo_challenge_end_sim`
      )}</button>`
    )
  );
    ...
    config.append(
      $(
        `<div><span class="has-text-warning">${loc(
          `resource_${res}_name`
        )}</span><input type="number" min="0" class="input" v-model="${res}"></div>`
      )
    );
  });
...
export function orbitDecayed() {
  if (
    ...
    ["forager", "farmer", "lumberjack", "quarry_worker"].forEach(function (
      job
    ) {
...
export function evoProgress() {
  clearElement($("#evolution .evolving"), true);
  let progress = $(
    `<div class="evolving"><progress class="progress" value="${global.evolution.final}" max="100">${global.evolution.final}%</progress></div>`
  );
...
function evoExtraState(race) {
  if (
    (race === "synth" ||
      (race === "custom" &&
        global.custom.race0.traits.includes("imitation"))) &&
...
function sentience() {
  ...
  if (global.race["cataclysm"]) {
    ...
    messageQueue(
      loc("sentience", [
        loc("genelab_genus_" + global.race.maintype),
        races[global.race.species].entity,
        flib("name"),
      ]),
      "info",
...
export function resQueue() {
  ...
  $("#resQueue").append(
    $(`
        ...
        <span id="pauserqueue" class="${
          global.r_queue.pause ? "pause" : "play"
        }" role="button" @click="pauseRQueue()" :aria-label="pausedesc()"></span>
    `)
  );
  ...
  queue.append(
    $(
      `<li v-for="(item, index) in queue"><a v-bind:id="setID(index)" class="queued" v-bind:class="{ 'qany': item.qa }" @click="remove(index)"><span class="has-text-warning">{{ item.label }}</span> [<span v-bind:class="{ 'has-text-danger': item.cna, 'has-text-success': !item.cna && item.req, 'has-text-caution': !item.req && !item.cna }">{{ item.time | time }}</span>]</a></li>`
    )
  );
