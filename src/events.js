...
export const events = {
    ...
    effect(wiki) {
      ...
      if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
        if (global.space.hasOwnProperty("living_quarters")) {
          ...
          at_risk += Math.round(
            num_lq_on * actions.space.spc_red.living_quarters.citizens()
          );
        }
        ...
        if (global.city.hasOwnProperty("basic_housing")) {
          at_risk +=
            ...
            actions.city.basic_housing.citizens();
        }
        if (global.city.hasOwnProperty("cottage")) {
          at_risk +=
            global.city.cottage.count * actions.city.cottage.citizens();
        }
        if (global.city.hasOwnProperty("apartment")) {
          let num_apartment_on = wiki
      ...
      if (global.city.biome !== "oceanic") {
        ...
        if (global.city.biome === "forest") {
          ...
          global.city.biome === "desert" ||
          global.city.biome === "volcanic"
        ) {
          time /= 2;
        }
      ...
      return loc(
        global.city.biome === "oceanic" ? "event_flare2" : "event_flare",
        [planet, loss.toLocaleString()]
      );
    },
  },
  raid: {
    ...
    condition() {
      if (
        checkControlling(`gov0`) &&
        checkControlling(`gov1`) &&
        checkControlling(`gov2`)
      ) {
        return false;
      }
      return !global.race["truepath"] &&
        !global.race["cataclysm"] &&
        (global.civic.foreign.gov0.hstl > 60 ||
          global.civic.foreign.gov1.hstl > 60 ||
          global.civic.foreign.gov2.hstl > 60)
        ? true
        : false;
    },
    effect() {
      ...
      let injured =
        global.civic.garrison.wounded > garrisonSize()
          ? garrisonSize()
          : global.civic.garrison.wounded;
      ...
      if (army > enemy) {
        return loc("event_raid1", [
          killed.toLocaleString(),
          wounded.toLocaleString(),
        ]);
      } else {
  ...
  siege: {
    ...
    condition() {
      ...
      return !global.race["truepath"] &&
        global.civic.foreign.gov0.hstl > 80 &&
        global.civic.foreign.gov1.hstl > 80 &&
        global.civic.foreign.gov2.hstl > 80
        ? true
        : false;
    },
    effect() {
      ...
      let eAdv = global.tech["high_tech"] ? global.tech["high_tech"] + 1 : 1;
      let enemy =
        (global.civic.foreign.gov0.mil +
          global.civic.foreign.gov1.mil +
          global.civic.foreign.gov2.mil) *
        eAdv;
  ...
  pillage3: {
    ...
    condition() {
      return global.race["truepath"] &&
        !global.tech["isolation"] &&
        global.tech["rival"] &&
        global.civic.foreign.gov3.hstl > 60
        ? true
        : false;
    },
    effect() {
  ...
  witch_hunt_crusade: {
    ...
    type: "major",
    condition() {
      return global.race["witch_hunter"] && global.resource.Sus.amount >= 100
        ? true
        : false;
    },
    effect() {
  ...
  doom: {
    ...
    type: "major",
    condition() {
      return global.space["space_barracks"] &&
        global.space.space_barracks.on > 0
        ? true
        : false;
    },
    effect() {
      ...
      return loc(global.race["evil"] ? "event_doom_alt" : "event_doom", [
  ...
  spy: {
    ...
    condition() {
      ...
      for (let i = 0; i < 3; i++) {
        if (
          global.civic.foreign[`gov${i}`].spy > 0 &&
  ...
  klepto: {
    ...
    effect() {
      ...
      if (
        ...
        maxRoll > (global.resource[res].max * traits.rogue.vars()[0]) / 100
      ) {
        maxRoll = Math.round(
          (global.resource[res].max * traits.rogue.vars()[0]) / 100
        );
      }
      ...
      return res === "Money"
        ? loc("event_klepto", [gain])
        : loc("event_klepto", [gain, global.resource[res].name]);
    },
  },
  chicken_feast: {
    reqs: {
    ...
    effect() {
      ...
      blubberFill(dead);
      return loc("event_chicken", [
        loc(`event_chicken_eaten${Math.floor(seededRandom(0, 10))}`),
        dead,
        loc(`event_chicken_seasoning${Math.floor(seededRandom(0, 10))}`),
      ]);
    },
  },
  brawl: {
    ...
    effect() {
      let dead = Math.floor(
        seededRandom(1, jobScale(traits.aggressive.vars()[0] + 1))
      );
      if (dead > global.civic.garrison.workers) {
        dead = global.civic.garrison.workers;
      }
      soldierDeath(dead);
      return loc("event_brawl_s", [
        loc(`event_brawl${Math.floor(seededRandom(0, 10))}`),
        dead,
      ]);
    },
  },
  ...
  witch_hunt: {
    ...
    type: "minor",
    condition() {
      return global.race["witch_hunter"] &&
  ...
  chicken: {
    ...
    effect() {
      ...
      blubberFill(1);
      return loc("event_chicken", [
        loc(`event_chicken_eaten${Math.rand(0, 10)}`),
        1,
        loc(`event_chicken_seasoning${Math.floor(seededRandom(0, 10))}`),
      ]);
    },
  ...
  fight: {
    ...
    effect() {
      let dead = Math.floor(
        seededRandom(1, jobScale(traits.aggressive.vars()[1] + 1))
      );
      if (dead > global.resource[global.race.species].amount) {
        dead = global.resource[global.race.species].amount;
      }
  ...
  heatwave: {
    ...
    condition() {
      ...
      if (
        ...
        ["tundra", "taiga", "eden"].includes(global.city.biome)
      ) {
        return false;
      }
  ...
  coldsnap: {
    ...
    condition() {
      ...
      if (
        ...
        ["ashland", "volcanic", "eden"].includes(global.city.biome)
      ) {
        return false;
      }
  ...
  contest: {
    ...
    effect() {
      ...
      return loc("event_contest", [
        loc(`event_contest_place${place}`),
        loc(`event_contest_type${contest}`),
      ]);
    },
  },
  ...
  pet: {
    ...
    effect() {
      if (global.race["pet"]) {
        ...
        return loc(`event_${global.race.pet.type}_interaction${interaction}`, [
        ...
        return loc(`event_pet_${global.race.pet.type}`, [
          loc(`event_${global.race.pet.type}_name${global.race.pet.name}`),
        ]);
      }
    },
...
function pillaged(gov, serious) {
  ...
  let eAdv = global.tech["high_tech"] ? global.tech["high_tech"] + 1 : 1;
  let enemy =
    (gov === "witchhunt" ? 1000 : global.civic.foreign[gov].mil) *
    (1 + Math.floor(seededRandom(0, 10) - 5) / 10) *
    eAdv;
  ...
  let enemy_name =
    gov === "witchhunt"
      ? loc(`witch_hunter_crusade`)
      : loc(`civics_gov${global.civic.foreign[gov].name.s0}`, [
          global.civic.foreign[gov].name.s1,
        ]);

  if (army > enemy) {
    return loc("event_pillaged1", [
      enemy_name,
      killed.toLocaleString(),
      wounded.toLocaleString(),
    ]);
    ...
    targets.forEach(function (res) {
      if (
        ...
        let loss = Math.rand(
          1,
          Math.round(global.resource[res].amount / limiter)
        );
        ...
        if (res === "Money") {
          ...
          stolen.push(
            `${sizeApproximation(loss)} ${global.resource[res].name}`
          );
        }
      }
    });
    return loc("event_pillaged2", [
      enemy_name,
      killed.toLocaleString(),
      wounded.toLocaleString(),
      stolen.join(", "),
...
export function eventList(type) {
  ...
  Object.keys(events).forEach(function (event) {
    ...
    if (
      (type === "major" && global.event.l === event) ||
      (type === "minor" && global.m_event.l === event)
    ) {
      isOk = false;
    }
    if (events[event]["reqs"]) {
      Object.keys(events[event].reqs).forEach(function (req) {
        switch (req) {
          ...
          case "resource":
            if (
              !global.resource[events[event].reqs[req]] ||
              !global.resource[events[event].reqs[req]].display
            ) {
              isOk = false;
            }
            break;
