...
export function setWeather() {
  ...
  if (global.city.calendar.weather === 0) {
    if (global.city.calendar.temp === 0) {
      ...
      weather =
        global.city.calendar.wind === 0 ? "wi-rain" : "wi-storm-showers";
    }
  ...
  switch (global.city.calendar.weather) {
    case 0:
      if (global.city.calendar.temp === 0) {
        ...
        return global.city.calendar.wind === 1
          ? loc("thunderstorm")
          : loc("rain");
      }
    case 1:
...
export function astroVal(sign) {
  ...
  switch (sign) {
    case "aries": // Combat Rating
      return [
        boosted ? Math.round(12 * multiplier) : Math.round(10 * multiplier),
      ];
    case "taurus": // Unification Bonus
      return [boosted ? 3 * multiplier : 2 * multiplier];
    case "gemini": // Knowledge
      return [
        boosted ? Math.round(30 * multiplier) : Math.round(20 * multiplier),
      ];
    case "cancer": // Soldier Healing
      return [
        boosted ? Math.round(8 * multiplier) : Math.round(5 * multiplier),
      ];
    case "leo": // Power
      return [boosted ? 5 * multiplier : 4 * multiplier];
    case "virgo": // Food Bonus
      return [
        boosted ? Math.round(20 * multiplier) : Math.round(15 * multiplier),
      ];
    case "libra": // Pop growth rate
      return [
        boosted ? Math.round(40 * multiplier) : Math.round(25 * multiplier),
      ];
    case "scorpio": // Cheaper and more effective spies
      return boosted
        ? [Math.round(20 * multiplier), 2]
        : [Math.round(12 * multiplier), 1];
    case "sagittarius": // Entertainer Morale
      return [boosted ? 6 * multiplier : 5 * multiplier];
    case "capricorn": // Trade gains
      return [
        boosted ? Math.round(20 * multiplier) : Math.round(10 * multiplier),
      ];
...
export function astrologySign() {
  ...
  if (
    ...
    (date.getMonth() === 9 && date.getDate() <= 22)
  ) {
    return "libra";
  } else if (
...
function astrologyDescription() {
  ...
  let desc = `<div>${loc(`sign_description`, [
    loc(`sign_${sign}`),
    loc(`sign_${sign}_desc`),
  ])}</div>`;
