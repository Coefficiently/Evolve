...
export function defineGovernment(define) {
  ...
  if (
    ...
    (global.settings.civTabs !== 2 || global.settings.govTabs !== 0)
  ) {
    return;
  }
  ...
  var tabs =
    $(`<b-tabs class="resTabs govTabs2" v-show="vis()" v-model="s.govTabs2" :animated="s.animated">
...
export function defineGarrison() {
  ...
  if (
    !global.settings.tabLoad &&
    (global.settings.civTabs !== 2 || global.settings.govTabs !== 3)
  ) {
    return;
  }
...
export function govTitle(id) {
  ...
  return loc(`civics_gov${global.civic.foreign[`gov${id}`].name.s0}`, [
    global.civic.foreign[`gov${id}`].name.s1,
  ]);
}
...
const government_desc = function () {
  return {
    ...
    oligarchy:
      global.tech["high_tech"] && global.tech["high_tech"] >= 12
        ? loc("govern_oligarchy_effect_alt", [govEffect.oligarchy()[1]])
        : loc("govern_oligarchy_effect", [
            govEffect.oligarchy()[0],
            govEffect.oligarchy()[1],
          ]),
    ...
    technocracy:
      global.tech["high_tech"] && global.tech["high_tech"] >= 16
        ? loc("govern_technocracy_effect_alt", [
            govEffect.technocracy()[0],
            govEffect.technocracy()[2],
          ])
        : loc("govern_technocracy_effect", govEffect.technocracy()),
    federation: loc("govern_federation_effect", [
      govEffect.federation()[0],
      govEffect.federation()[1],
    ]),
    federation_alt: loc("govern_federation_effect_alt", [
      25,
      govEffect.federation()[2],
      govEffect.federation()[1],
    ]),
...
export const govEffect = {
  autocracy() {
    let stress =
      global.tech["high_tech"] && global.tech["high_tech"] >= 2
  ...
  corpocracy() {
    ...
    let morale =
      global.tech["high_tech"] && global.tech["high_tech"] >= 12 ? 5 : 10;
    let factory =
      global.tech["high_tech"] && global.tech["high_tech"] >= 16 ? 40 : 30;
    if (global.genes.hasOwnProperty("governor") && global.genes.governor >= 3) {
  ...
  dictator() {
    ...
    let production =
      global.tech["high_tech"] && global.tech["high_tech"] >= 12 ? 12 : 10;
    let materials =
      global.tech["high_tech"] && global.tech["high_tech"] >= 16 ? 6 : 4;
    if (global.genes.hasOwnProperty("governor") && global.genes.governor >= 3) {
...
function government(govern) {
  ...
  var type = $(
    `<div>${loc(
      "civics_government_type"
    )} <span id="govLabel" class="has-text-warning">{{ type | govern }}</span></div>`
  );
  gov.append(type);
  ...
  var change = $(
    `<span class="change inline"><button class="button" @click="trigModal" :disabled="rev > 0">{{ type | set }}</button></span>`
  );
  ...
  vBind({
    ...
    methods: {
      ...
      force() {
        return global.civic.govern.rev > 0
          ? loc("civics_force_rev_desc")
          : loc("civics_force_rev_desc2");
      },
      vis() {
        return global.tech["govern"] ? true : false;
      },
  ...
  popover("govLabel", function () {
    ...
    return $(
      `<div>${govDescription(
        global.civic.govern.type
      )}</div><div class="has-text-advanced">${
        government_desc()[effect_type]
      }</div>`
    );
  });
  ...
  popover(
    ...
    function () {
      return global.civic.govern.rev > 0
        ? loc("civics_change_desc", [global.civic.govern.rev])
...
function drawGovModal() {
  ...
  if (global.tech["govern"]) {
    if (global.civic.govern.type !== "autocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="autocracy" @click="setGov('autocracy')">${loc(
            `govern_autocracy`
          )}</button>`
        )
      );
    ...
    if (global.civic.govern.type !== "democracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="democracy" @click="setGov('democracy')">${loc(
            `govern_democracy`
          )}</button>`
        )
      );
    ...
    if (global.civic.govern.type !== "oligarchy") {
      body.append(
        $(
          `<button class="button gap" data-gov="oligarchy" @click="setGov('oligarchy')">${loc(
            `govern_oligarchy`
          )}</button>`
        )
      );
    ...
    if (global.tech["gov_theo"] && global.civic.govern.type !== "theocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="theocracy" @click="setGov('theocracy')">${loc(
            `govern_theocracy`
          )}</button>`
        )
      );
    ...
    if (global.tech["govern"] >= 2 && global.civic.govern.type !== "republic") {
      body.append(
        $(
          `<button class="button gap" data-gov="republic" @click="setGov('republic')">${loc(
            `govern_republic`
          )}</button>`
        )
      );
    ...
    if (global.tech["gov_soc"] && global.civic.govern.type !== "socialist") {
      body.append(
        $(
          `<button class="button gap" data-gov="socialist" @click="setGov('socialist')">${loc(
            `govern_socialist`
          )}</button>`
        )
      );
    ...
    if (global.tech["gov_corp"] && global.civic.govern.type !== "corpocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="corpocracy" @click="setGov('corpocracy')">${loc(
            `govern_corpocracy`
          )}</button>`
        )
      );
    ...
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-gov="technocracy" @click="setGov('technocracy')">${loc(
            `govern_technocracy`
          )}</button>`
        )
      );
    ...
    if (global.tech["gov_fed"] && global.civic.govern.type !== "federation") {
      body.append(
        $(
          `<button class="button gap" data-gov="federation" @click="setGov('federation')">${loc(
            `govern_federation`
          )}</button>`
        )
      );
    ...
    if (global.tech["gov_mage"] && global.civic.govern.type !== "magocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="magocracy" @click="setGov('magocracy')">${loc(
            `govern_magocracy`
          )}</button>`
        )
      );
    ...
    if (
      ...
      global.race.wishStats.gov &&
      global.civic.govern.type !== "dictator"
    ) {
      body.append(
        $(
          `<button class="button gap" data-gov="dictator" @click="setGov('dictator')">${loc(
            `govern_dictator`
          )}</button>`
        )
      );
    ...
    methods: {
      setGov(g) {
        if (global.civic.govern.rev === 0) {
          let drawTechs =
            global.genes["governor"] && global.civic.govern.type === "anarchy";
          ...
          if (global.stats.achieve["anarchist"]) {
            time = Math.round(
              time * (1 - global.stats.achieve["anarchist"].l / 10)
            );
          }
          ...
          if (fathom > 0) {
            time = Math.round(
              time * ((100 - traits.lawless.vars(1)[0] * fathom) / 100)
            );
          }
  ...
  popover(
    ...
    function (obj) {
      ...
      return $(
        `<div>${govDescription(govType)}</div><div class="has-text-advanced">${
          government_desc()[effectType]
        }</div>`
      );
    },
    {
...
export function foreignGov() {
  if (
    ...
    foreign.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc(
          "civics_foreign"
        )}</h2></div>`
      )
    );
    ...
    for (let i = 0; i < govEnd; i++) {
      let gov = $(
        `<div id="gov${i}" class="foreign" v-show="gvis(${i})"><span class="has-text-caution">{{ '${i}' | gov }}</span><span v-if="f${i}.occ" class="has-text-advanced"> - ${loc(
          "civics_garrison_occupy"
        )}</span><span v-else-if="f${i}.anx" class="has-text-advanced"> - ${loc(
          "civics_garrison_annex"
        )}</span></span><span v-else-if="f${i}.buy" class="has-text-advanced"> - ${loc(
          "civics_garrison_purchase"
        )}</span></div>`
      );
      foreign.append(gov);
      ...
      actions.append(
        $(
          `<button :label="battleAssessment(${i})" class="button gaction attack" @click="campaign(${i})"><span v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy">${loc(
            "civics_garrison_attack"
          )}</span><span v-show="f${i}.occ || f${i}.anx || f${i}.buy">${loc(
            "civics_garrison_unoccupy"
          )}</span></button>`
        )
      );
      actions.append(
        $(
          `<span class="tspy inline"><button :label="spyDesc(${i})" v-show="t.spy >= 1 && !f${i}.occ && !f${i}.anx && !f${i}.buy" :disabled="spy_disabled(${i})" class="button gaction" @click="spy(${i})"><span v-show="f${i}.trn === 0">${loc(
            "tech_spy"
          )}: {{ f${i}.spy }}</span><span v-show="f${i}.trn > 0">${loc(
            "civics_train"
          )}: {{ f${i}.trn }}</span></button></span>`
        )
      );
      actions.append(
        $(
          `<span class="sspy inline"><button :label="espDesc()" v-show="t.spy >= 2 && !f${i}.occ && !f${i}.anx && !f${i}.buy && f${i}.spy >= 1" :disabled="f${i}.sab > 0" class="button gaction" @click="trigModal(${i})"><span v-show="f${i}.sab === 0">${loc(
            "tech_espionage"
          )}</span><span v-show="f${i}.sab > 0">{{ f${i}.act | sab }}: {{ f${i}.sab }}</span></button></span>`
        )
      );
      gov.append(actions);
      ...
      gov.append(
        $(
          `<div v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc(
            "civics_gov_mil_rate"
          )}:</span> <span class="glevel">{{ f${i}.mil | military(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 2"> ({{ f${i}.mil }})</span></span></div>`
        )
      );
      gov.append(
        $(
          ...
          )}:</span> <span class="glevel">{{ f${i}.hstl | relation }}<span class="has-text-warning" v-show="f${i}.spy >= 1"> ({{ f${i}.hstl | hate }})</span></span></div>`
        )
      );
      gov.append(
        $(
          `<div v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc(
            "civics_gov_eco_rate"
          )}:</span> <span class="glevel">{{ f${i}.eco | eco(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 3"> ({{ f${i}.eco }})</span></span></div>`
        )
      );
      gov.append(
        $(
          `<div v-show="f${i}.spy >= 2 && !f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc(
            "civics_gov_unrest"
          )}:</span> <span class="glevel">{{ f${i}.unrest | discontent(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 4"> ({{ f${i}.unrest | turmoil }})</span></span></div>`
        )
      );
    }
    ...
    vBind({
      ...
      methods: {
        ...
        spy_disabled(i) {
          return global.civic.foreign[`gov${i}`].trn > 0 ||
            spyCost(i) > global.resource.Money.amount
            ? true
            : false;
        },
        spy(i) {
    ...
    if (global.race["truepath"]) {
      popover(
        `garRivaldesc1`,
        function () {
          return loc(`civics_gov_tp_rival`, [
            govTitle(3),
...
function spyCost(i) {
  let base =
    Math.round(
      global.civic.foreign[`gov${i}`].mil / 2 +
        global.civic.foreign[`gov${i}`].hstl / 2 -
        global.civic.foreign[`gov${i}`].unrest
    ) + 10;
  if (base < 50) {
    base = 50;
  }
      ...
      if (global.tech["spy"] >= 3 && global.city["boot_camp"]) {
        time -=
          (global.race["orbit_decayed"] && global.space["space_barracks"]
            ...
            : global.city["boot_camp"].count) * 10;
        if (time < 10) {
          time = 10;
        }
      }
    ...
    case "influence":
      {
        if (
          global.tech["spy"] &&
          global.tech["spy"] >= 2 &&
          global.civic.foreign[`gov${g}`].spy >= 1 &&
          global.civic.foreign[`gov${g}`].sab === 0
        ) {
          let timer = global.tech["spy"] >= 4 ? 200 : 300;
          if (global.civic.foreign[`gov${g}`].spy === 1) {
            timer *= 1.5;
          } else if (global.civic.foreign[`gov${g}`].spy >= 3) {
            timer -= (global.civic.foreign[`gov${g}`].spy - 2) * 50;
          }
          if (
            global.genes.hasOwnProperty("governor") &&
            global.genes.governor >= 3
          ) {
            timer *= 0.9;
          }
          ...
          if (fathom > 0) {
            timer = Math.round(
              timer * (1 - (traits.befuddle.vars(1)[0] / 100) * fathom)
            );
          }
    ...
    case "sabotage":
      {
        if (
          ...
          if (global.civic.foreign[`gov${g}`].spy >= 2) {
            timer -= (global.civic.foreign[`gov${g}`].spy - 1) * 50;
          }
          if (
            global.genes.hasOwnProperty("governor") &&
            global.genes.governor >= 3
    ...
    case "incite":
      {
        if (g >= 3) {
          ...
          if (global.civic.foreign[`gov${g}`].spy <= 2) {
            timer *= 1.5;
          } else if (global.civic.foreign[`gov${g}`].spy >= 4) {
            timer -= (global.civic.foreign[`gov${g}`].spy - 3) * 100;
          }
          if (
            global.genes.hasOwnProperty("governor") &&
            global.genes.governor >= 3
...
function drawEspModal(gov) {
  ...
  if (
    global.tech["spy"] &&
    global.tech["spy"] >= 2 &&
    global.civic.foreign[`gov${gov}`].spy >= 1
  ) {
    body.append(
      $(
        `<button class="button gap" data-esp="influence" @click="influence('${gov}')">${loc(
          `civics_spy_influence`
        )}</button>`
      )
    );
    body.append(
      $(
        `<button class="button gap" data-esp="sabotage" @click="sabotage('${gov}')">${loc(
          `civics_spy_sabotage`
        )}</button>`
      )
    );
    if (gov < 3) {
      body.append(
        $(
          `<button class="button gap" data-esp="incite" @click="incite('${gov}')">${loc(
            `civics_spy_incite`
          )}</button>`
        )
      );
    ...
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-esp="annex" @click="annex('${gov}')">${loc(
            `civics_spy_annex`
          )}</button>`
        )
      );
    ...
    if (gov < 3 && global.civic.foreign[`gov${gov}`].spy >= 3) {
      body.append(
        $(
          `<button class="button gap" data-esp="purchase" @click="purchase('${gov}')">${loc(
            `civics_spy_purchase`
          )}</button>`
        )
      );
    ...
    methods: {
      ...
      annex(g) {
        ...
        if (
          global.civic.foreign[`gov${gov}`].hstl <= 50 &&
          global.civic.foreign[`gov${gov}`].unrest >= 50 &&
          global.city.morale.current >=
            200 +
              global.civic.foreign[`gov${gov}`].hstl -
              global.civic.foreign[`gov${gov}`].unrest
        ) {
          if (
            global.tech["spy"] &&
            global.tech["spy"] >= 2 &&
      ...
      purchase(g) {
        ...
        if (price <= global.resource.Money.amount) {
          if (
            global.tech["spy"] &&
            global.tech["spy"] >= 2 &&
            global.civic.foreign[`gov${g}`].spy >= 3 &&
            global.civic.foreign[`gov${g}`].sab === 0
          ) {
  ...
  popover(
    ...
    function (obj) {
      ...
      if (
        ...
        (esp === "sabotage" &&
          global.civic.foreign[`gov${gov}`].spy >= 2 &&
          global.civic.foreign[`gov${gov}`].mil === 50) ||
        (esp === "incite" &&
          global.civic.foreign[`gov${gov}`].spy >= 4 &&
          global.civic.foreign[`gov${gov}`].unrest === 100)
      ) {
        warn = `<div class="has-text-danger">${loc(
          `civics_spy_warning`
        )}</div>`;
      }
...
function taxCap(min) {
  let extreme =
    global.tech["currency"] && global.tech.currency >= 5 ? true : false;
  if (min) {
    return (extreme || global.race["terrifying"]) && !global.race["noble"]
      ? 0
      : global.race["noble"]
      ? traits.noble.vars()[0]
      : 10;
...
function taxRates(govern) {
  ...
  var tax_level = $(
    '<span class="current" v-html="$options.filters.tax_level(tax_rate)"></span>'
  );
  var sub = $(
  ...
  vBind({
    ...
    filters: {
      tax_level(rate) {
        ...
        if (
          egg.length > 0 &&
          ((rate === 0 && !global.race["noble"]) ||
            (rate === 10 && global.race["noble"]))
        ) {
          return egg;
...
export function buildGarrison(garrison, full) {
  ...
  if (global.tech["world_control"] && !global.race["truepath"]) {
    garrison.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc(
          "civics_garrison"
        )}</h2> - <span class="has-text-success">${loc(
          "rating"
        )} <span class="defenseRating">{{ g.workers | hell | rating }}</span></div>`
      )
    );
  } else {
    garrison.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc(
          ...
          "rating"
        )} <span class="defenseRating">{{ g.workers | hell | rating }}</span> / <span class="offenseRating">{{ g.raid | rating }}</span></span></div>`
      )
    );
  }
  ...
  if (!global.tech["isolation"]) {
    bunks.append(
      $(
        `<div class="barracks"><span class="soldier">${soldier_title}</span> <span v-html="$options.filters.stationed(g.workers)"></span> / <span>{{ g.max | s_max }}<span></div>`
      )
    );
    bunks.append(
      $(
        `<div class="barracks" v-show="g.crew > 0"><span class="crew">${loc(
          "civics_garrison_crew"
        )}</span> <span>{{ g.crew }}</span></div>`
      )
    );
    bunks.append(
      $(
        `<div class="barracks"><span class="wounded">${loc(
          "civics_garrison_wounded"
        )}</span> <span v-html="$options.filters.wounded(g.wounded)"></span></div>`
      )
    );

    barracks.append(
      $(
        `<div class="hire"><button v-show="g.mercs" class="button first hmerc" @click="hire">${loc(
    ...
    garrison.append(
      $(
        `<div class="training"><span>${loc("civics_garrison_training")} - ${loc(
          "arpa_to_complete"
        )} {{ g.rate, g.progress | trainTime }}${egg8}</span> <progress class="progress" :value="g.progress" max="100">{{ g.progress }}%</progress></div>`
      )
    );
  }
  ...
  if (
    ...
    var tactics = $(
      `<div id="${
        full ? "tactics" : "c_tactics"
      }" v-show="g.display" class="tactics"><span>${loc(
        "civics_garrison_campaign"
      )}</span></div>`
    );
    wrap.append(tactics);

    var strategy = $(
      '<span class="current tactic">{{ g.tactic | tactics }}</span>'
    );
    var last = $(
      '<span role="button" aria-label="easier campaign" class="sub" @click="last">&laquo;</span>'
    );
    var next = $(
      '<span role="button" aria-label="harder campaign" class="add" @click="next">&raquo;</span>'
    );
    tactics.append(last);
    ...
    var battalion = $(
      `<div id="${
        full ? "battalion" : "c_battalion"
      }" v-show="g.display" class="tactics"><span>${loc(
        "civics_garrison_battalion"
      )}</span></div>`
    );
    ...
    var alast = $(
      '<span role="button" aria-label="remove soldiers from campaign" class="sub" @click="aLast">&laquo;</span>'
    );
    var anext = $(
      '<span role="button" aria-label="add soldiers to campaign" class="add" @click="aNext">&raquo;</span>'
    );
    ...
    if (full) {
      if (global.race["truepath"] && global.tech["rival"]) {
        campaign.append(
          $(
            `<div class="launch gov3" v-show="rvis()"><div class="has-text-caution">${govTitle(
              3
            )}</div><button class="button campaign" @click="campaign(3)"><span>${loc(
              "civics_garrison_launch_campaign"
            )}</span></button></div>`
          )
        );
      }
      if (!global.tech["world_control"]) {
        campaign.append(
          $(
            `<div class="launch gov0"><div class="has-text-caution">${govTitle(
              0
            )}</div><button class="button campaign" @click="campaign(0)"><span v-show="!g0.occ && !g0.anx && !g0.buy">${loc(
              "civics_garrison_launch_campaign"
            )}</span><span v-show="g0.occ || g0.anx || g0.buy">${loc(
              "civics_garrison_deoccupy"
            )}</span></button></div>`
          )
        );
        campaign.append(
          $(
            `<div class="launch gov1"><div class="has-text-caution">${govTitle(
              1
            )}</div><button class="button campaign" @click="campaign(1)"><span v-show="!g1.occ && !g1.anx && !g1.buy">${loc(
              "civics_garrison_launch_campaign"
            )}</span><span v-show="g1.occ || g1.anx || g1.buy">${loc(
              "civics_garrison_deoccupy"
            )}</span></button></div>`
          )
        );
        campaign.append(
          $(
            `<div class="launch gov2"><div class="has-text-caution">${govTitle(
              2
            )}</div><button class="button campaign" @click="campaign(2)"><span v-show="!g2.occ && !g2.anx && !g2.buy">${loc(
              "civics_garrison_launch_campaign"
            )}</span><span v-show="g2.occ || g2.anx || g2.buy">${loc(
              "civics_garrison_deoccupy"
            )}</span></button></div>`
          )
        );
  ...
  vBind({
    ...
    filters: {
      ...
      stationed(v) {
        ...
        let trick =
          size === trickNum && !full ? trickOrTreat(2, 14, true) : false;
...
function battleAssessment(gov) {
  if (global.civic.foreign[`gov${gov}`].occ) {
    ...
    global.civic.foreign[`gov${gov}`].buy ||
    global.civic.foreign[`gov${gov}`].anx
  ) {
    return loc("civics_garrison_secede_desc");
  } else if (
    (global.civic.garrison.tactic <= 1 &&
      global.civic.foreign[`gov${gov}`].spy < 1) ||
    (global.civic.garrison.tactic >= 2 &&
      global.civic.garrison.tactic <= 3 &&
      global.civic.foreign[`gov${gov}`].spy < 2) ||
    (global.civic.garrison.tactic === 4 &&
      global.civic.foreign[`gov${gov}`].spy < 3)
  ) {
  ...
  if (army < enemy) {
    return loc("civics_garrison_disadvantage", [
      +((1 - army / enemy) * 100).toFixed(1),
    ]);
  } else {
    return loc("civics_garrison_advantage", [
      +((1 - enemy / army) * 100).toFixed(1),
    ]);
  }
}
...
function war_campaign(gov) {
  if (global.civic.foreign[`gov${gov}`].occ) {
    ...
    global.civic.garrison.max += jobScale(
      global.civic.govern.type === "federation" ? 15 : 20
    );
    global.civic.garrison.workers += jobScale(
      global.civic.govern.type === "federation" ? 15 : 20
    );
    return;
  }
  ...
  if (global.civic.garrison.raid === 0) {
    messageQueue(loc("civics_garrison_campaign_no_soldier"), "warning", false, [
      "combat",
    ]);
    return;
  }
  ...
  switch (global.civic.garrison.tactic) {
    case 0:
      ...
      global.civic.foreign[`gov${gov}`].hstl += Math.floor(
        seededRandom(0, 2, true)
      );
      break;
    case 1:
      enemy = seededRandom(5, 50, true);
      global.civic.foreign[`gov${gov}`].hstl += Math.floor(
        seededRandom(0, 3, true)
      );
      break;
    case 2:
      enemy = seededRandom(25, 100, true);
      global.civic.foreign[`gov${gov}`].hstl += Math.floor(
        seededRandom(1, 5, true)
      );
      break;
    case 3:
      enemy = seededRandom(50, 200, true);
      global.civic.foreign[`gov${gov}`].hstl += Math.floor(
        seededRandom(4, 12, true)
      );
      break;
    case 4:
      enemy = seededRandom(100, 500, true);
      global.civic.foreign[`gov${gov}`].hstl += Math.floor(
        seededRandom(10, 25, true)
      );
      break;
  }
  ...
  if (army > enemy) {
    let deathCap = Math.floor(
      global.civic.garrison.raid / (5 - global.civic.garrison.tactic)
    );
    deathCap += wounded;
    ...
    if (
      ...
      global.race.ocularPowerConfig.ds += Math.round(
        enemy * traits.ocular_power.vars()[1]
      );
    }
    ...
    let basic =
      gov === 3 && global.race["truepath"]
        ? ["Food", "Lumber", "Stone", "Copper", "Iron"]
        : ["Food", "Lumber", "Stone"];
    let common =
      gov === 3 && global.race["truepath"]
        ? ["Aluminium", "Coal", "Cement", "Steel", "Furs"]
        : ["Copper", "Iron", "Aluminium", "Coal"];
    let rare =
      gov === 3 && global.race["truepath"]
        ? ["Titanium", "Oil", "Iridium", "Alloy", "Polymer"]
        : ["Cement", "Steel"];
      ...
      case 0:
        {
          ...
          if (
            ...
            Math.floor(seededRandom(0, 20, true)) === 0
          ) {
            looted.push("Steel");
          }
        }
    ...
    let titanium_low =
      global.race["terrifying"] && gov !== 3 ? traits.terrifying.vars()[0] : 12;
    let titanium_high =
      global.race["terrifying"] && gov !== 3 ? traits.terrifying.vars()[1] : 32;
    ...
    looted.forEach(function (goods) {
      switch (goods) {
        ...
        case "Chrysotile":
          ...
          break;
        case "Titanium":
          gains[goods] += Math.floor(
            seededRandom(titanium_low, titanium_high, true)
          );
          break;
        case "Crystal":
    ...
    payout.forEach(function (res) {
      if (
        gains[res] > 0 &&
        (global.resource[res].display || res === "Steel" || res === "Titanium")
      ) {
        gains[res] = lootModify(gains[res], gov);
        loot =
          loot +
          loc("civics_garrison_quant_res", [
            gains[res],
    ...
    if (global.race["revive"]) {
      switch (global.city.calendar.temp) {
        case 0:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[0]), true)
          );
          break;
        case 1:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[1]), true)
          );
          break;
        case 2:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[2]), true)
          );
          break;
      }
    ...
    if (revive > 0) {
      messageQueue(
        loc("civics_garrison_victorious_revive", [death, revive]),
        "success",
        false,
        ["combat"]
      );
      ...
      messageQueue(
        loc("civics_garrison_victorious", [death]),
        "success",
        false,
        ["combat"]
      );
    ...
    if (global.race["slaver"] && global.city["slave_pen"]) {
      ...
      if (max > global.resource.Slave.amount) {
        let slaves = Math.floor(
          seededRandom(0, global.civic.garrison.tactic + 2, true)
        );
        ...
        if (slaves > 0) {
          ...
          messageQueue(
            loc("civics_garrison_capture", [slaves]),
            "success",
            false,
            ["combat"]
          );
    ...
    if (global.race["infectious"]) {
      ...
      switch (global.civic.garrison.tactic) {
        case 0:
          infected = Math.floor(
            seededRandom(0, traits.infectious.vars()[0], true)
          );
          break;
        case 1:
          infected = Math.floor(
            seededRandom(0, traits.infectious.vars()[1], true)
          );
          break;
        case 2:
          infected = Math.floor(
            seededRandom(0, traits.infectious.vars()[2], true)
          );
          break;
        case 3:
          infected = Math.floor(
            seededRandom(0, traits.infectious.vars()[3], true)
          );
          break;
        case 4:
          infected = Math.floor(
            seededRandom(0, traits.infectious.vars()[4], true)
          );
          break;
      }
      ...
      if (infected > 0) {
        ...
        if (infected === 1) {
          messageQueue(
            loc("civics_garrison_soldier_infected"),
            "special",
            false,
            ["combat"]
          );
          ...
          messageQueue(
            loc("civics_garrison_soldiers_infected", [infected]),
            "special",
            false,
            ["combat"]
          );
    ...
    if (global.race["revive"]) {
      switch (global.city.calendar.temp) {
        case 0:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[3]), true)
          );
          break;
        case 1:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[4]), true)
          );
          break;
        case 2:
          revive = Math.floor(
            seededRandom(0, Math.floor(death / traits.revive.vars()[5]), true)
          );
          break;
      }
      global.civic.garrison.workers += revive;
    ...
    if (revive > 0) {
      messageQueue(
        loc("civics_garrison_defeat_revive", [death, revive]),
        "danger",
        false,
        ["combat"]
      );
    } else {
      messageQueue(loc("civics_garrison_defeat", [death]), "danger", false, [
        "combat",
      ]);
    }
  }
...
export function armorCalc(dead) {
  ...
  if (global.race["high_pop"]) {
    armor += Math.floor(
      seededRandom(0, armor * traits.high_pop.vars()[0], true)
    );
  }
...
export function weaponTechModifer() {
  let weapon_tech = global.tech["military"]
    ? global.tech.military >= 5
      ? global.tech.military - 1
  ...
  if (global.tech["military"] && global.tech.military > 1) {
    ...
    if (fathom > 0) {
      weapon_tech *=
        1 + (traits.sniper.vars(1)[0] / 100) * weapon_tech * fathom;
    }
    weapon_tech += global.tech.military >= 11 ? 2 : 1;
  }
  return weapon_tech;
...
export function armyRating(val, type, wound) {
  ...
  if (typeof wound === "number") {
    ...
    val >
    global.civic.garrison.workers - global.civic.garrison.wounded
  ) {
    wounded =
      val - (global.civic.garrison.workers - global.civic.garrison.wounded);
  }
  ...
  if (global.race["rage"] || rhinoFathom > 0) {
    let rageVal = global.race["rage"]
      ? (wounded * traits.rage.vars()[1]) / 100
      : 0;
    let fathomVal =
      rhinoFathom > 0
        ? ((wounded * traits.rage.vars(1)[1]) / 100) * rhinoFathom
        : 0;
    adjusted_val = val + rageVal + fathomVal;
  }
  let army = global.tech["military"]
    ? adjusted_val * weapon_tech
    : adjusted_val;
  if (type === "army" || type === "hellArmy" || type === "Troops") {
    if (global.race["rage"]) {
      army *=
        1 +
        (traits.rage.vars()[0] / 100) * (global.civic.garrison.wounded || 0);
    }
    if (rhinoFathom > 0) {
      army *=
        1 +
        (traits.rage.vars(1)[0] / 100) *
          rhinoFathom *
          (global.civic.garrison.wounded || 0);
    }
    if (global.race["puny"]) {
    ...
    if (
      global.race["ocular_power"] &&
      global.race["ocularPowerConfig"] &&
      global.race.ocularPowerConfig.d
    ) {
    ...
    if (
      ...
      global.race.psychicPowers.hasOwnProperty("assaultTime")
    ) {
      let boost = 0;
  ...
  if (type === "hunting") {
    ...
    if (
      global.race["ocular_power"] &&
      global.race["ocularPowerConfig"] &&
      global.race.ocularPowerConfig.w
    ) {
...
export function garrisonSize(max, args = {}) {
  ...
  let fortress = global.portal["fortress"]
    ? global.portal.fortress.garrison
    : 0;
  let fob = global.space["fob"] && !args["nofob"] ? global.space.fob.troops : 0;
  let pillbox =
    global.eden["pillbox"] && !args["nopill"] ? global.eden.pillbox.staffed : 0;
  return (
    global.civic.garrison[type] -
    global.civic.garrison.crew -
    fortress -
    fob -
    pillbox
...
function defineMad() {
  ...
  if ($(`#mad`).length === 0) {
    ...
    var mad_command = $(
      '<div id="mad" v-show="display" class="tile is-child"></div>'
    );
    ...
    let altText =
      global.race["hrt"] && ["wolven", "vulpine"].includes(global.race["hrt"])
        ? true
        : false;

    mad.append(
      $(
        `<div class="defcon mdarm"><button class="button arm" @click="arm">${loc(
          altText ? "civics_mad_arm_grenades" : "civics_mad_arm_missiles"
        )}</button></div>`
      )
    );
    mad.append(
      $(
        `<div class="defcon mdlaunch"><button class="button" @click="launch" :disabled="armed">${loc(
          altText ? "civics_mad_launch_grenades" : "civics_mad_launch_missiles"
        )}</button></div>`
      )
    );
    ...
    if (!global.civic.mad.armed) {
      ...
      $("#mad .arm").html(
        loc(
          altText ? "civics_mad_disarm_grenades" : "civics_mad_disarm_missiles"
        )
      );
    }
        ...
        arm() {
          if (global.civic.mad.armed) {
            $("#mad .arm").html(
              loc(
                altText
                  ? "civics_mad_disarm_grenades"
                  : "civics_mad_disarm_missiles"
              )
            );
                  ...
                  switch (k) {
                    case "mdarm":
                      return global.tech["world_control"] &&
                        ...
                        : loc(
                            altText
                              ? "civics_mad_missiles_desc_easter"
                              : "civics_mad_missiles_desc"
                          );
                    case "mdlaunch": {
                      ...
                      let plasmidType =
                        global.race.universe === "antimatter"
                          ? loc("resource_AntiPlasmid_plural_name")
                          : loc("resource_Plasmid_plural_name");
                      return loc("civics_mad_missiles_warning", [
                        gains.plasmid,
                        plasmidType,
                      ]);
                    }
