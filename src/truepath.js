...
const outerTruth = {
  spc_titan: {
      ...
      effect() {
        ...
        let storage =
          global.tech["titan"] && global.tech.titan >= 5
            ...
            : ``;
        return `${support}${water}${storage}<div class="has-text-caution">${loc(
          "minus_power",
          [$(this)[0].powered()]
        )}</div>`;
      },
    ...
    electrolysis: {
      ...
      desc() {
        return `<div>${loc(
          "space_electrolysis_title"
        )}</div><div class="has-text-special">${loc("space_electrolysis_req", [
          global.resource.Water.name,
        ])}</div>`;
      },
      reqs: { titan: 3 },
      ...
      effect(wiki) {
        let support = `<div>+${loc(`galaxy_alien2_support`, [
          $(this)[0].support(wiki),
          planetName().titan,
        ])}</div>`;
        return `${support}<div class="has-text-caution">${loc(
          "space_electrolysis_use",
          [
    ...
    hydrogen_plant: {
      ...
      desc() {
        return `<div>${loc(
          "space_hydrogen_plant_title"
        )}</div><div class="has-text-special">${loc(
          "space_hydrogen_plant_req"
        )}</div>`;
      },
      reqs: { titan_power: 1 },
      path: ["truepath"],
      ...
      effect() {
        return `<span>${loc("space_dwarf_reactor_effect1", [
          ...
          "space_hydrogen_plant_effect",
          [1, loc("space_electrolysis_title")]
        )}</span>`;
      },
      support() {
        return 2;
    ...
    titan_quarters: {
      ...
      desc() {
        return `<div>${loc(
          "interstellar_habitat_title"
        )}</div><div class="has-text-special">${loc("space_habitat_req", [
          planetName().titan,
      ...
      effect() {
        ...
        return `<div class="has-text-caution">${loc("space_used_support", [
          ...
          global.race["truepath"]
            ? loc("job_colonist_tp", [planetName().titan])
            : loc("colonist"),
        ])}</div><div>${loc("plus_max_resource", [
          gain,
          loc("citizen"),
        ])}</div><div class="has-text-caution">${loc(`spend`, [
          $(this)[0].support_fuel()[0].a,
          global.resource[$(this)[0].support_fuel()[0].r].name,
        ])}</div><div class="has-text-caution">${loc(`spend`, [
          $(this)[0].support_fuel()[1].a,
          global.resource[$(this)[0].support_fuel()[1].r].name,
        ])}</div>`;
      },
      s_type: "titan",
      support() {
    ...
    titan_bank: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("titan_bank", offset, 2500000, 1.32);
        },
        Titanium(offset) {
          return spaceCostMultiplier("titan_bank", offset, 380000, 1.32);
        },
        Neutronium(offset) {
          return spaceCostMultiplier("titan_bank", offset, 5000, 1.32);
        },
      },
      effect() {
    ...
    decoder: {
      ...
      desc() {
        return `<div>${loc(
          ...
          [planetName().titan, global.resource.Cipher.name]
        )}</div>`;
      },
      reqs: { titan: 8 },
      path: ["truepath"],
      ...
      effect(wiki) {
        ...
        return (
          desc +
          `<div class="has-text-caution">${loc("spend", [
            cipher,
            global.resource[$(this)[0].support_fuel().r].name,
          ])}</div>`
        );
      },
      s_type: "titan",
    ...
    ai_core2: {
      ...
      desc() {
        return `<div>${loc(
          ...
          global.resource[$(this)[0].p_fuel().r].name,
        ])}</div>`;
      },
      ...
      effect(wiki) {
        ...
        let desc = `<div class="has-text-warning">${loc(
          ...
          [+get_qlevel(wiki).toFixed(1)]
        )}</div>`;
        desc += `<div>${loc("interstellar_citadel_effect", [
          value,
        ])}</div><div>${loc("space_ai_core_effect2", [value])}</div>`;
        if (global.tech["titan_ai_core"] && global.tech.titan_ai_core >= 2) {
          desc += `<div>${loc("space_ai_core_effect3", [50])}</div>`;
        }
        desc += `<div class="has-text-caution">${loc("space_electrolysis_use", [
          $(this)[0].p_fuel().a,
          global.resource[$(this)[0].p_fuel().r].name,
          $(this)[0].powered(),
        ])}</div>`;
        if (global.tech["titan_ai_core"] && global.tech.titan_ai_core >= 3) {
          ...
          desc += `<div class="has-text-advanced">${loc(
            "space_ai_core_effect4",
            [drift]
          )}</div>`;
        }
        return desc;
      ...
      effect() {
        return `<div>${loc("plus_max_resource", [
          ...
          global.race["truepath"]
            ? loc("job_colonist_tp", [planetName().titan])
            : loc("colonist"),
        ])}</div><div>${loc("space_ai_colonist_effect", [
          jobScale(1),
          planetName().titan,
  ...
  spc_enceladus: {
    ...
    zero_g_lab: {
      ...
      desc() {
        return `<div>${loc(
          "tech_zero_g_lab"
        )}</div><div class="has-text-special">${loc("requires_power_support", [
          planetName().enceladus,
        ])}</div>`;
      },
      reqs: { enceladus: 3 },
      ...
      effect() {
        ...
        let desc = `<div class="has-text-caution">${loc("space_used_support", [
          planetName().enceladus,
        ])}</div><div>${loc("city_max_knowledge", [know])}</div>`;
    ...
    munitions_depot: {
      ...
      effect() {
        let containers = 25;
        return `<div>${loc("plus_max_crates", [containers])}</div><div>${loc(
          "plus_max_containers",
          [containers]
        )}</div>`;
      },
  ...
  spc_triton: {
    info: {
      ...
      extra(region) {
        if (global.tech["triton"] && global.tech.triton >= 3) {
          $(`#${region}`).append(
            `<div id="${region}resist" v-show="${region}" class="syndThreat has-text-caution">${loc(
              "space_ground_resist"
            )} <span class="has-text-danger" v-html="threat(enemy,troops)"></span></div>`
          );
          vBind({
            el: `#${region}resist`,
    ...
    fob: {
      ...
      effect(wiki) {
        ...
        desc += `<div class="has-text-warning"><span class="soldier">${loc(
          "civics_garrison_soldiers"
        )}:</span> <span>${troops}</span> / <span>${max_troops}<span></div>`;
        desc += `<div class="has-text-warning"><span class="wounded">${loc(
          "civics_garrison_wounded"
        )}:</span> <span>${
          global.civic["garrison"] ? global.civic.garrison.wounded : 0
        }</span></div>`;
        desc += `<div class="has-text-warning">${loc("space_fob_landed", [
          global.space["fob"] ? global.space.fob.troops : 0,
        ])}</div>`;
        let helium = +fuel_adjust(125, true, wiki).toFixed(2);
        return (
          desc +
          `<div class="has-text-caution">${loc("requires_power_combo_effect", [
            $(this)[0].powered(),
            helium,
            global.resource.Helium_3.name,
          ])}</div>`
    ...
    lander: {
      ...
      effect(wiki) {
        ...
        if (
          ...
          data = `<div>${loc(`space_lander_effect3`, [
            production("lander"),
            global.resource.Cipher.name,
          ])}</div>`;
        }
        return `<div>${loc("space_lander_effect", [
          planetName().triton,
        ])}</div>${data}<div class="has-text-warning">${loc(
          `space_lander_effect2`,
          [jobScale(3)]
        )}</div><div class="has-text-caution">${loc(
  ...
  spc_kuiper: {
    ...
    orichalcum_mine: {
      ...
      desc() {
        return `<div>${loc("space_kuiper_mine", [
  ...
  spc_eris: {
    info: {
      ...
      extra(region) {
        if (global.tech["eris"] && global.tech["eris"] === 1) {
          $(`#${region}`).append(
            `<div id="${region}scanned" v-show="${region}" class="syndThreat has-text-caution">${loc(
              "space_scanned"
            )} <span class="has-text-info">{{ eris_scan }}%</span></div>`
          );
          vBind({
            el: `#${region}scanned`,
            data: global.tech,
    ...
    drone_control: {
      ...
      desc() {
        return `<div>${loc("space_drone_control", [
          planetName().titan,
        ])}</div><div class="has-text-special">${loc("requires_power_combo", [
          global.resource[$(this)[0].p_fuel().r].name,
        ])}</div>`;
      ...
      effect() {
        ...
        let desc = `<div>+${loc(`galaxy_alien2_support`, [
          $(this)[0].support(),
          planetName().eris,
        ])}</div>`;
        return (
    ...
    shock_trooper: {
      ...
      effect() {
        ...
        if (global.space["digsite"] && global.space.digsite.count === 100) {
          desc = `<div>${loc(`space_lander_effect3`, [
            production("shock_trooper"),
            global.resource.Cipher.name,
          ])}</div>`;
        }
...
const tauCetiModules = {
    ...
    ringworld: {
      ...
      effect(wiki) {
        ...
        if (count < 1000) {
          ...
          effectText += `<div class="has-text-special">${loc(
            "space_dwarf_reactor_effect1",
            [global.race["lone_survivor"] ? 100 : 10000]
          )}</div>`;
        }
        return effectText;
      },
  ...
  tau_home: {
    info: {
      ...
      extra(region) {
        if (
          ...
          $(`#${region}`).append(
            `<div id="${region}Mats" v-show="tauShow()" class="syndThreat has-text-warning">${loc(
              "resource_Materials_name"
            )} <span class="has-text-info">{{ amount | round | locale }}</span> / <span class="has-text-info">{{ max | locale }}</span></div>`
          );
          vBind({
            el: `#${region}Mats`,
    ...
    orbital_station: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("space_red_spaceport_effect1", [
        ...
        desc =
          desc +
          `<div>${loc("plus_max_resource", [
            helium.toLocaleString(),
            global.resource.Helium_3.name,
          ])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("spend_power", [
            fuel,
            global.resource[$(this)[0].support_fuel().r].name,
            $(this)[0].powered(),
          ])}</div>`;
      ...
      powered() {
        return powerCostMod(
          global.tech["isolation"] ? (global.race["lone_survivor"] ? 4 : 6) : 30
        );
      },
      refresh: true,
      action() {
    ...
    colony: {
      ...
      desc() {
        return `<div>${loc("tau_home_colony_desc", [
          races[global.race.species].home,
        ])}</div><div class="has-text-special">${loc(
          "requires_power_support_combo",
          [races[global.race.species].home, global.resource.Food.name]
        )}</div>`;
      },
      reqs: { tau_home: 2 },
      path: ["truepath"],
      ...
      effect() {
        ...
        let desc = `<div class="has-text-caution">${loc("tau_new_support", [
          $(this)[0].support(),
          races[global.race.species].home,
        ])}</div>`;
        ...
        desc =
          desc +
          `<div>${loc("tau_home_colony_effect", [
            50,
            races[global.race.species].home,
          ])}</div>`;
        ...
        if (global.tech["isolation"]) {
          ...
          let mVal =
            (gasVal ? gasVal + global.tech.broadcast : global.tech.broadcast) *
            2;
          desc =
      ...
      effect() {
        ...
        return global.race["sappy"]
          ? `<div>${loc("plus_max_resource", [
              pop,
              loc("citizen"),
            ])}</div><div>${loc("city_grove_effect", [2.5])}</div>`
    ...
    cloning_facility: {
      ...
      effect() {
        ...
        if (
          ...
          warn = `<div class="has-text-caution">${loc(
            "city_assembly_effect_warn"
          )}</div>`;
        }
        return `<div>${loc("tau_home_cloning_effect", [
    ...
    tau_farm: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("produce", [
            +production("tau_farm", "food").toFixed(2),
            global.resource.Food.name,
          ])}</div>`;
        if (!global.race["kindling_kindred"] && !global.race["smoldering"]) {
          desc =
            desc +
            `<div>${loc("produce", [
              +production("tau_farm", "lumber").toFixed(2),
      ...
      effect() {
        ...
        if (!tauEnabled()) {
          desc =
            ...
            `<div>${loc("plus_max_resource", [
              1000000,
              loc("resource_Materials_name"),
            ])}</div>`;
          desc =
            desc +
            `<div>${loc("tau_home_mining_pit_effect", [
              global.resource.Materials.name,
            ])}</div>`;
        } else {
          if (global.tech["isolation"]) {
            if (global.race["lone_survivor"]) {
            ...
            desc =
              desc +
              `<div>${loc("tau_home_mining_pit_effect2", [
                global.resource.Bolognium.name,
                global.resource.Adamantite.name,
                global.resource.Stone.name,
              ])}</div>`;
    ...
    alien_outpost: {
      ...
      effect() {
        ...
        if (global.tech["isolation"]) {
          ...
          desc =
            desc +
            `<div>${loc(`space_lander_effect3`, [
              production("alien_outpost"),
              global.resource.Cipher.name,
            ])}</div>`;
        }
        ...
        if (global.race["lone_survivor"]) {
          desc =
            desc +
            `<div>${loc("city_wardenclyffe_effect1", [
              jobScale(1),
              global.civic.professor.name,
            ])}</div>`;
        }
        desc =
          desc +
    ...
    fusion_generator: {
      ...
      desc() {
        return `<div>${loc(
          "tech_fusion_generator"
        )}</div><div class="has-text-special">${loc("requires_res", [
          global.resource.Helium_3.name,
        ])}</div>`;
      },
      reqs: { tau_home: 6 },
      ...
      effect() {
        ...
        let desc = `<div>${loc("space_dwarf_reactor_effect1", [
          -$(this)[0].powered(),
        ])}</div>`;
        if (global.race["lone_survivor"]) {
          desc =
        ...
        if (global.tech["isolation"]) {
          storage =
            ...
            `<span>${loc("plus_max_resource", [
              containers,
              loc("resource_Crates_name"),
            ])}</span><span>${loc("plus_max_resource", [
              containers,
              loc("resource_Containers_name"),
            ])}</span>`;
        }
    ...
    tau_factory: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("tau_home_tau_factory_effect", [
            $(this)[0].manufacturing(),
          ])}</div>`;
        if (global.tech["isolation"]) {
        ...
        desc =
          desc +
          `<div>${loc("city_crafted_mats", [
            global.tech["isolation"] ? 275 : 90,
          ])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [
    ...
    infectious_disease_lab: {
      ...
      desc() {
        return `<div>${$(
          this
        )[0].title()}</div><div class="has-text-special">${loc(
          "requires_power_support",
          [races[global.race.species].home]
        )}</div>`;
      },
      reqs: { disease: 1 },
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("city_max_knowledge", [
            Math.round(sci).toLocaleString(),
          ])}</div>`;
        if (global.tech["isolation"]) {
        ...
        if (global.tech["focus_cure"]) {
          desc =
            desc +
            `<div>${loc("tau_home_disease_lab_cure", [
              +global.tauceti.infectious_disease_lab.cure.toFixed(1),
            ])}</div>`;
          if (global.race.hasOwnProperty("vax")) {
            desc =
              desc +
              `<div>${loc("tau_home_disease_lab_vax", [
                +global.race.vax.toFixed(2),
              ])}</div>`;
          }
        }
        desc =
      ...
      powered() {
        return powerCostMod(
          global.tech["isolation"] ? (global.race["lone_survivor"] ? 2 : 8) : 35
        );
      },
      action() {
        if (payCosts($(this)[0])) {
    ...
    tau_cultural_center: {
      ...
      effect() {
        ...
        let desc = `<div class="has-text-caution">${loc(
          "tau_home_cultureal_effect1",
          [
            $(this)[0].p_fuel().a,
            global.resource[$(this)[0].p_fuel().r].name,
            $(this)[0].title,
        ...
        desc += `<div>${loc("city_tourist_center_effect2", [
          cas,
          structName("casino"),
        ])}</div>`;
        desc += `<div>${loc("city_tourist_center_effect2", [
          mon,
          loc(`arpa_project_monument_title`),
        ])}</div>`;
        desc += `<div>${loc("tau_home_cultureal_effect2", [
          womling,
          loc("tau_red_womlings"),
        ])}</div>`;
        if (global.tech.tau_culture >= 2) {
          desc += `<div>${loc("tau_home_cultureal_effect3", [
            bake,
            loc(
  ...
  tau_red: {
    info: {
      ...
      extra(region) {
        if (global.tech["tau_red"] && global.tech.tau_red >= 5) {
          $(`#${region}`).append(
            `<div id="${region}Womlings" class="syndThreat has-text-warning">${loc(
              "tau_red_womling_prod"
            )} <span class="has-text-info">{{ prod }}%</span></div>`
          );
          vBind({
            el: `#${region}Womlings`,
    ...
    orbital_platform: {
      ...
      powered() {
        return powerCostMod(
          global.tech["isolation"] ? (global.race["lone_survivor"] ? 2 : 3) : 18
        );
      },
      refresh: true,
      action() {
    ...
    jeff: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("tau_red_jeff_effect1", [
          global.tauceti["overseer"] ? global.tauceti.overseer.pop : 0,
        ])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect2", [
            global.tauceti["overseer"] ? global.tauceti.overseer.working : 0,
          ])}</div>`;
        desc = desc + `<div>${loc("tau_red_jeff_effect3", [injured])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect4", [
            global.tauceti["overseer"] ? global.tauceti.overseer.loyal : 0,
          ])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect5", [
            global.tauceti["overseer"] ? global.tauceti.overseer.morale : 0,
          ])}</div>`;
        if (global.tech.hasOwnProperty("womling_tech")) {
          desc =
            desc +
            `<div>${loc("tau_red_jeff_effect6", [
              global.tech.womling_tech,
            ])}</div>`;
        }
        desc =
          desc +
          `<div class="divide-line">${loc("job_farmer")}: ${
        ...
        if (global.tauceti["womling_lab"]) {
          desc =
            desc +
            `<div>${loc("job_scientist")}: ${
              global.tauceti["womling_lab"]
    ...
    overseer: {
      ...
      desc() {
        return `<div>${$(
          this
        )[0].name()}</div><div class="has-text-special">${loc("space_support", [
          planetName().red,
        ])}</div>`;
      },
      name() {
    ...
    womling_village: {
      ...
      effect() {
        ...
        if (global.tech["tau_junksale"]) {
          desc =
            desc +
            `<div>${loc("tau_red_womling_village_effect2", [
              40,
              loc(
                `tau_gas2_alien_station_data4_r${
                  global.race.tau_junk_item || 0
                }`
              ),
            ])}</div>`;
        }
    ...
    womling_farm: {
      ...
      effect() {
        ...
        if (global.tech["isolation"]) {
          desc =
            desc +
            `<div>${loc("tau_red_womling_generate", [
              global.resource.Furs.name,
            ])}</div>`;
        }
        desc =
          desc +
          `<div>${loc("tau_red_womling_farm_effect2", [
            (food / 2) * farmers,
          ])}</div>`;
        return desc;
      },
      ...
      effect() {
        ...
        if (global.tech["isolation"]) {
          if (global.race["lone_survivor"]) {
            ...
            desc =
              desc +
              `<div>${loc("tau_red_womling_mine_effect_b", [
                global.resource.Unobtainium.name,
                global.resource.Uranium.name,
                global.resource.Titanium.name,
              ])}</div>`;
          ...
          desc =
            desc +
            `<div>${loc("tau_red_womling_mine_effect_a", [
              global.resource.Unobtainium.name,
            ])}</div>`;
        }
  ...
  tau_gas: {
    ...
    refueling_station: {
      ...
      desc() {
        return `<div>${loc(
          "tau_gas_refueling_station_title"
        )}</div><div class="has-text-special">${loc("requires_power")}</div>`;
      },
      reqs: { tau_gas: 2 },
      path: ["truepath"],
      ...
      effect() {
        ...
        let desc = `<div>${loc("space_gas_mining_effect1", [
          helium_prod,
        ])}</div>`;
        desc =
          desc +
          `<div>${loc("plus_max_resource", [
            helium_tank.toLocaleString(),
            global.resource.Helium_3.name,
          ])}</div>`;
    ...
    ore_refinery: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("tau_gas_ore_refinery_effect", [
          +ore.toFixed(2),
        ])}</div>`;
        desc =
    ...
    whaling_station: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("tau_gas_whaling_station_effect", [
          +blubber.toFixed(2),
        ])}</div>`;
        desc =
    ...
    womling_station: {
      ...
      effect() {
        ...
        let desc = `<div>${loc("production", [
          prod,
          tauCetiModules.tau_gas.info.name(),
        ])}</div>`;
        if (!global.race["flier"]) {
          desc =
            desc +
  ...
  tau_roid: {
    ...
    patrol_ship: {
      ...
      effect() {
        let fuel = +int_fuel_adjust($(this)[0].support_fuel().a).toFixed(1);
        let desc = `<div>${loc("tau_roid_patrol_ship_security", [
          $(this)[0].support(),
        ])}</div>`;
  ...
  tau_gas2: {
    ...
    alien_station: {
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          effectText += `<div class="has-text-special">${loc(
            "tau_gas2_alien_station_repaired",
            [count]
          )}</div>`;
          return effectText;
        } else {
    ...
    ignite_gas_giant: {
      ...
      effect() {
        let desc = `<div>${loc("tau_gas2_ignite_gas_giant_effect", [
        ...
        if (!global.tech["m_brain"]) {
          desc =
            ...
            `<div class="has-text-warning">${loc(
              "tau_gas2_ignite_gas_giant_effect2",
              [loc("tech_matrioshka_brain")]
            )}</div>`;
        } else {
          desc =
...
function matrixProjection() {
  ...
  let skilled =
    global.stats.retire === global.stats.matrix + 1
      ? `<div class="has-text-advanced">${loc("tau_star_matrix_skilled", [
          1,
        ])}</div>`
      : ``;
...
export function drawShipYard() {
  ...
  if (global.space.hasOwnProperty("shipyard") && global.settings.showShipYard) {
    ...
    shipStats.append(
      `<div class="registry"><span class="has-text-caution">${loc(
        `outer_shipyard_registry`
      )}</span>: <b-input v-model="b.name" maxlength="25" class="nameplate"></b-input></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `crew`
      )}</span> <span v-html="crewText()"></span></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `power`
      )}</span> <span v-html="powerText()"></span></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `firepower`
      )}</span> <span v-html="fireText()"></span></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `outer_shipyard_sensors`
      )}</span> <span v-html="sensorText()"></span></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `speed`
      )}</span> <span v-html="speedText()"></span></div>`
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(
        `outer_shipyard_fuel`
      )}</span> <span v-html="fuelText()"></span></div>`
    );
    ...
    Object.keys(shipConfig).forEach(function (k) {
      ...
      shipConfig[k].forEach(function (v, idx) {
        values += `<b-dropdown-item aria-role="listitem" v-on:click="setVal('${k}','${v}')" class="${k} a${idx}" data-val="${v}" v-show="avail('${k}','${idx}','${v}')">${loc(
          `outer_shipyard_${k}_${v}`
        )}</b-dropdown-item>`;
      });
    ...
    assemble.append(
      `<button class="button is-info" slot="trigger" v-on:click="build()"><span>${loc(
        "outer_shipyard_build"
      )}</span></button>`
    );
    assemble.append(
      `<span><b-checkbox class="patrol" v-model="s.expand" v-on:input="redraw()">${loc(
        "outer_shipyard_fleet_details"
      )}</b-checkbox></span>`
    );
    assemble.append(
      `<span><b-checkbox class="patrol" v-model="s.sort" v-on:input="redraw()">${loc(
        "outer_shipyard_fleet_sort"
      )}</b-checkbox></span>`
    );

    plans.append(assemble);
    assemble.append(
      `<div><span>${loc(`outer_shipyard_park`, [
        planetName().dwarf,
      ])}</span><a href="#" class="solarMap" @click="trigModal">${loc(
        `outer_shipyard_map`
      )}</span></a>`
    );
...
export function TPShipDesc(parent, obj) {
  ...
  var shipPattern = $(
    `<div class="divider">${loc(`outer_shipyard_class_${ship.class}`)} | ${loc(
      `outer_shipyard_engine_${ship.engine}`
    )} | ${loc(`outer_shipyard_weapon_${ship.weapon}`)} | ${loc(
      `outer_shipyard_power_${ship.power}`
    )} | ${loc(`outer_shipyard_sensor_${ship.sensor}`)}</div>`
  );
  parent.append(desc);
  ...
  Object.keys(costs).forEach(function (res) {
    if (costs[res]() > 0) {
      ...
      cost.append(
        $(
          `<div class="${color}" data-${res}="${costs[
            res
          ]()}">${label}${sizeApproximation(costs[res](), 2)}</div>`
        )
      );
    }
  });
  ...
  Object.keys(costs).forEach(function (k) {
    if (k === "Money") {
      $(`#shipYardCosts`).append(
        `<span class="res-${k} has-text-success" data-${k}="${
          costs[k]
        }" data-ok="has-text-success">${
          global.resource[k].name
        }${sizeApproximation(costs[k])}</span>`
      );
    } else {
      $(`#shipYardCosts`).append(
        `<span> | </span><span class="res-${k} has-text-success" data-${k}="${
          costs[k]
        }" data-ok="has-text-success">${
          global.resource[k].name
        } ${sizeApproximation(costs[k])}</span>`
...
function drawShips() {
  ...
  for (let i = 0; i < global.space.shipyard.ships.length; i++) {
    ...
    if (global.space.shipyard.expand) {
      let ship_class = `${loc(`outer_shipyard_engine_${ship.engine}`)} ${loc(
        `outer_shipyard_class_${ship.class}`
      )}`;
      let desc = $(`<div id="shipReg${i}" class="shipRow ship${i}"></div>`);
      let row1 = $(
        `<div class="row1"><span class="name has-text-caution">${
          ship.name
        }</span> <span v-show="scrapAllowed(${i})">| </span><a class="scrap${i}" v-show="scrapAllowed(${i})" @click="scrap(${i})">${loc(
          `outer_shipyard_scrap`
        )}</a> | <span class="has-text-warning">${ship_class}</span> | <span class="has-text-danger">${loc(
          `outer_shipyard_weapon_${ship.weapon}`
        )}</span> | <span class="has-text-warning">${loc(
          `outer_shipyard_power_${ship.power}`
        )}</span> | <span class="has-text-warning">${loc(
          `outer_shipyard_armor_${ship.armor}`
        )}</span> | <span class="has-text-warning">${loc(
          `outer_shipyard_sensor_${ship.sensor}`
        )}</span></div>`
      );
      ...
      row2.append(
        `<span class="has-text-warning">${loc(
          `crew`
        )}</span> <span class="pad" v-html="crewText(${i})"></span>`
      );
      row2.append(
        `<span class="has-text-warning">${loc(
          `firepower`
        )}</span> <span class="pad" v-html="fireText(${i})"></span>`
      );
      row2.append(
        `<span class="has-text-warning">${loc(
          `outer_shipyard_sensors`
        )}</span> <span class="pad" v-html="sensorText(${i})"></span>`
      );
      row2.append(
        `<span class="has-text-warning">${loc(
          `speed`
        )}</span> <span class="pad" v-html="speedText(${i})"></span>`
      );
      row2.append(
        `<span class="has-text-warning">${loc(
          `outer_shipyard_fuel`
        )}</span> <span class="pad" v-bind:class="{ 'has-text-danger': !fueled }" v-html="fuelText(${i})"></span>`
      );
      row2.append(
        `<span class="has-text-warning">${loc(
          `outer_shipyard_hull`
        )}</span> <span class="pad" v-bind:class="hullDamage(${i})" v-html="hullText(${i})"></span>`
      );
          ...
          let name =
            ship.class === "explorer"
              ...
              : typeof spaceRegions[global.space.shipyard.ships[id].location]
                  .info.name === "string"
              ? spaceRegions[global.space.shipyard.ships[id].location].info.name
              : spaceRegions[
