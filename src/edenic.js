...
const edenicModules = {
  eden_asphodel: {
    info: {
      ...
      desc() {
        if (global.tech["asphodel"] && global.tech.asphodel >= 5) {
          return `${loc("eden_asphodel_desc")} ${loc(
            "eden_asphodel_desc_hostile"
          )}`;
        } else {
          return `${loc("eden_asphodel_desc")} ${loc(
            "eden_asphodel_desc_peaceful"
          )}`;
        }
      },
      support: "encampment",
    ...
    survery_meadows: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("eden_survery_meadows_action"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    encampment: {
      ...
      effect() {
        let desc = `<div>${loc("eden_encampment_effect", [
          $(this)[0].support(),
        ])}</div>`;

        if (
          global.tech.hasOwnProperty("asphodel") &&
          global.tech.asphodel >= 1
        ) {
          let powder = spatialReasoning(250);
          desc += `<div>${loc("plus_max_resource", [
            powder,
            loc("resource_Asphodel_Powder_name"),
          ])}</div>`;
        }
        if (
    ...
    mech_station: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("mech_station") ||
          global.eden.mech_station.count < 10 ||
          wiki
        ) {
          return `<div>${loc(
            "eden_mech_station_title"
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("mech_station")
                ? global.eden.mech_station.count
                : 0) <
              10
              ? 6750000000
              : 0;
          }
          return !global.eden.hasOwnProperty("mech_station") ||
            global.eden.mech_station.count < 10
            ? 6750000000
            : 0;
        },
      ...
      effect(wiki) {
        ...
        if (count >= 10) {
          ...
          desc += `<div>${loc("eden_mech_station_mechs", [
            global.eden.mech_station.mechs,
          ])}</div>`;
          desc += `<div>${loc("eden_mech_station_effective", [
            global.eden.mech_station.effect,
          ])}</div>`;
          return desc;
        } else {
          let size = 10;
    ...
    ectoplasm_processor: {
      ...
      desc: `<div>${loc(
        "eden_ectoplasm_processor_title"
      )}</div><div class="has-text-special">${loc("space_support", [
        loc("eden_asphodel_name"),
      ])}</div>`,
      reqs: { asphodel: 2 },
    ...
    research_station: {
      ...
      effect() {
        ...
        desc += `<div>${loc("eden_research_station_effect", [
          highPopAdjust(souls).toFixed(0),
          loc("job_ghost_trapper"),
        ])}</div>`;
    ...
    stabilizer: {
      ...
      effect() {
        ...
        desc += `<div>${loc("eden_stabilizer_effect1", [
          global.resource.Asphodel_Powder.name,
          +stabilize.toFixed(1),
        ])}</div>`;
        desc += `<div>${loc("eden_stabilizer_effect2", [
          global.resource.Asphodel_Powder.name,
          loc("city_shed_title3"),
          15,
        ])}</div>`;
        desc += `<div class="has-text-special">${loc("eden_stabilizer_limit", [
      ...
      action() {
        if (
          global.eden.stabilizer.count < global.eden.warehouse.count &&
          payCosts($(this)[0])
        ) {
    ...
    rune_gate: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("rune_gate") ||
          global.eden.rune_gate.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "eden_rune_gate_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("rune_gate")
                ? global.eden.rune_gate.count
                : 0) <
              100
              ? 1000000000
              : 0;
          }
          return !global.eden.hasOwnProperty("rune_gate") ||
            global.eden.rune_gate.count < 100
            ? 1000000000
            : 0;
        },
    ...
    rune_gate_open: {
      ...
      cost: {},
      effect() {
        return `<div>${loc("eden_rune_gate_open", [
          loc("eden_elysium_name"),
        ])}</div>`;
      },
      action() {
        return false;
    ...
    bunker: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("bunker", offset, 777000000, 1.2, "eden");
        },
        Stone(offset) {
          return spaceCostMultiplier("bunker", offset, 358000000, 1.2, "eden");
        },
        Furs(offset) {
          return spaceCostMultiplier("bunker", offset, 66600000, 1.2, "eden");
        },
        Asphodel_Powder(offset) {
      ...
      effect() {
        ...
        desc += `<div>${loc("plus_max_soldiers", [
          $(this)[0].soldiers(),
        ])}</div>`;
        if (
          global.tech["celestial_warfare"] &&
          global.tech.celestial_warfare >= 4 &&
          (!global.tech["elysium"] || global.tech.elysium < 8)
        ) {
    ...
    bliss_den: {
      ...
      cost: {
        ...
        Asphodel_Powder(offset) {
          return spaceCostMultiplier("bliss_den", offset, 35000, 1.22, "eden");
        },
        Plywood(offset) {
          return spaceCostMultiplier(
            "bliss_den",
    ...
    rectory: {
      ...
      cost: {
        ...
        Copper(offset) {
          return spaceCostMultiplier("rectory", offset, 18200000, 1.24, "eden");
        },
        Brick(offset) {
          return spaceCostMultiplier("rectory", offset, 7500000, 1.24, "eden");
        },
        Soul_Gem(offset) {
      ...
      effect() {
        ...
        desc += `<div>${loc("plus_max_citizens", [
          $(this)[0].citizens(),
        ])}</div>`;
  ...
  eden_elysium: {
    info: {
      ...
      prop() {
        ...
        desc += `<span class="pad"><span class="wounded">${loc(
          "civics_garrison_wounded"
        )}</span> <span>{{ wounded }}</span></span>`;
      ...
      filter(v, type) {
        switch (type) {
          ...
          case "m_use":
            return loc(`civics_garrison_mercenary_cost`, [
              Math.round(mercCost()).toLocaleString(),
            ]);
        }
      },
    },
    survey_fields: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("eden_survey_fields_msg"), "info", false, [
            "progress",
          ]);
    ...
    fortress: {
      ...
      effect() {
        let desc = `<div>${loc("eden_fortress_rating", [
          global.eden["fortress"] ? global.eden.fortress.fortress / 10 : 0,
        ])}</div>`;
        desc += `<div>${loc("eden_fortress_patrols", [
          global.eden["fortress"] ? global.eden.fortress.patrols : 0,
        ])}</div>`;
        desc += `<div>${loc("eden_fortress_detect", [
          global.eden["fortress"] ? global.eden.fortress.detector : 0,
        ])}</div>`;
        desc += `<div>${loc("eden_fortress_armory", [
          global.eden["fortress"] ? global.eden.fortress.armory : 0,
        ])}</div>`;
        return desc;
      },
      action() {
    ...
    siege_fortress: {
      ...
      effect() {
        let desc = `<div class="has-text-warning">${loc(
          `eden_siege_fortress_effect`
        )}</div>`;
        if (
          global.eden.hasOwnProperty("fortress") &&
          global.eden.fortress.hasOwnProperty("siege")
        ) {
          desc += `<div>${loc(`eden_siege_fortress_result`)}</div>`;
          desc += `<div>${loc(`eden_siege_fortress_lost`, [
            global.eden.fortress.siege.loss,
          ])}</div>`;
          desc += `<div>${loc(`eden_siege_fortress_damage`, [
            global.eden.fortress.siege.damage,
          ])}</div>`;
          desc += `<div class="has-text-caution">${loc("eden_fortress_rating", [
            global.eden["fortress"] ? global.eden.fortress.fortress / 10 : 0,
          ])}</div>`;
        }
        return desc;
      ...
      action() {
        ...
        if (remain <= 0) {
          ...
          messageQueue(loc("eden_siege_fortress_fail"), "warning", false, [
            "combat",
          ]);
        } else {
          let dead =
            ...
            remain +
            Math.floor(
              seededRandom(0, jobScale(global.eden.fortress.detector), true)
            );
          ...
          if (global.eden.fortress.fortress <= 0) {
            messageQueue(loc("eden_siege_fortress_fall"), "success", false, [
              "combat",
            ]);
            global.tech.elysium = 4;
          } else {
            messageQueue(
              loc("eden_siege_fortress_success", [damage / 10]),
              "success",
              false,
              ["combat"]
            );
    ...
    raid_supplies: {
      ...
      effect() {
        let desc = `<div class="has-text-warning">${loc(
          `eden_raid_supplies_effect`
        )}</div>`;
        if (
          global.eden.hasOwnProperty("fortress") &&
          global.eden.fortress.hasOwnProperty("raid")
        ) {
          desc += `<div>${loc(`eden_raid_fortress_result`)}</div>`;
          desc += `<div>${loc(`eden_siege_fortress_lost`, [
            global.eden.fortress.raid.loss,
          ])}</div>`;
          desc += `<div>${loc(`eden_siege_fortress_damage`, [
            global.eden.fortress.raid.damage,
          ])}</div>`;
          desc += `<div class="has-text-caution">${loc("eden_fortress_armory", [
            global.eden["fortress"] ? global.eden.fortress.armory : 0,
          ])}</div>`;
        }
        return desc;
      ...
      action() {
        ...
        let remain = Math.ceil(
          jobScale(50 - enemy_pats < 0 ? 0 : 50 - enemy_pats)
        );
        ...
        if (remain <= 0) {
          ...
          global.civic.garrison.protest += armySize;
          soldierDeath(armySize);
          messageQueue(loc("eden_raid_fortress_fail"), "warning", false, [
            "combat",
          ]);
        } else {
          let dead =
            ...
            remain +
            Math.floor(
              seededRandom(0, jobScale(global.eden.fortress.detector / 2), true)
            );
          dead = deadCalc(dead, armySize);
          remain = armySize - dead;
    ...
    ambush_patrol: {
      ...
      effect() {
        let desc = `<div class="has-text-warning">${loc(
          `eden_ambush_patrol_effect`
        )}</div>`;
        if (
          global.eden.hasOwnProperty("fortress") &&
          global.eden.fortress.hasOwnProperty("ambush")
        ) {
          desc += `<div>${loc(`eden_ambush_patrol_result`)}</div>`;
          desc += `<div>${loc(`eden_siege_fortress_lost`, [
            global.eden.fortress.ambush.loss,
          ])}</div>`;
          desc += `<div>${loc(`eden_ambush_patrol_damage`, [
            global.eden.fortress.ambush.damage ? loc("true") : loc("false"),
          ])}</div>`;
          desc += `<div class="has-text-caution">${loc(
            "eden_fortress_patrols",
            [global.eden["fortress"] ? global.eden.fortress.patrols : 0]
          )}</div>`;
        }
        return desc;
      ...
      action() {
        ...
        if (
          armyRating(jobScale(1), "Troops") >
          Math.floor(seededRandom(0, global.eden.fortress.detector * 2, true))
        ) {
          ...
          messageQueue(loc("eden_ambush_patrol_success"), "success", false, [
            "combat",
          ]);
          drawTech();
        } else {
          ...
          global.civic.garrison.protest += armySize;
          soldierDeath(armySize);
          messageQueue(loc("eden_ambush_patrol_fail"), "warning", false, [
            "combat",
          ]);
        }
    ...
    scout_elysium: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("eden_scout_elysium_result"), "info", false, [
            "progress",
          ]);
    ...
    fire_support_base: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("fire_support_base") ||
          global.eden.fire_support_base.count < 100 ||
          wiki
        ) {
          return `<div>${loc(
            "eden_fire_support_base_title"
          )}</div><div class="has-text-special">${loc("requires_segments", [
            100,
          ])}</div>`;
        } else {
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("fire_support_base")
                ? global.eden.fire_support_base.count
                : 0) <
              100
              ? 2500000000
              : 0;
          }
          return !global.eden.hasOwnProperty("fire_support_base") ||
            global.eden.fire_support_base.count < 100
            ? 2500000000
            : 0;
        },
      ...
      effect(wiki) {
        let count =
          ...
          (global.eden.hasOwnProperty("fire_support_base")
            ...
            : 0);
        if (count >= 100) {
          let desc = `<div>${loc("plus_max_soldiers", [
            $(this)[0].soldiers(),
          ])}</div>`;
          if (
            global.tech["elysium"] &&
            global.tech.elysium >= 10 &&
            global.tech.isle === 1
          ) {
            if (global.resource.Elerium.amount >= 250000) {
              desc += `<div class="has-text-success">${loc(
                "eden_fire_support_base_effect"
              )}</div>`;
            } else {
            ...
            desc += `<div class="has-text-caution">${loc(
              "eden_fire_support_base_effect2",
              [sizeApproximation(250000), global.resource.Elerium.name]
            )}</div>`;
          }
          return desc;
        } else {
      ...
      action() {
        if (global.eden.fire_support_base.count < 100 && payCosts($(this)[0])) {
          ...
          if (global.eden["pillbox"] && global.eden.pillbox.staffed > 0) {
            let rating = +(
              Math.round(armyRating(global.eden.pillbox.staffed, "army", 0)) /
              75
            ).toFixed(0);
            if (rating > 100) {
              rating = 100;
          ...
          if (global.eden.fire_support_base.count < 100) {
            messageQueue(
              loc("eden_fire_support_base_counterattack", [
          ...
          if (
            global.eden.enemy_isle.wt === 0 &&
            global.eden.enemy_isle.g === 0 &&
            global.eden.enemy_isle.et === 0
          ) {
            global.tech.isle = 2;
    ...
    sacred_smelter: {
      ...
      effect() {
        let desc = `<div>${loc("interstellar_stellar_forge_effect3", [
          $(this)[0].smelting(),
        ])}</div>`;
    ...
    elerium_containment: {
      ...
      desc() {
        return `<div>${loc("eden_elerium_containment", [
    ...
    pillbox: {
      ...
      cost: {
        ...
        Steel(offset) {
          return spaceCostMultiplier("pillbox", offset, 65000000, 1.26, "eden");
        },
        Nanoweave(offset) {
          return spaceCostMultiplier("pillbox", offset, 38000000, 1.26, "eden");
        },
      },
      effect() {
        let rating = +(
          Math.round(
            armyRating(
              global.eden["pillbox"] && global.eden.pillbox.staffed
                ? global.eden.pillbox.staffed
                : jobScale(10),
              "army",
              0
            )
        ...
        if (global.tech["elysium"] && global.tech.elysium >= 12) {
          desc += `<div>${loc("eden_restaurant_effect", [
            0.35,
            loc(`eden_restaurant_bd`),
          ])}</div>`;
        }
        desc += `<div class="has-text-caution">${loc(
          "portal_guard_post_effect2",
          [jobScale(10), $(this)[0].powered()]
        )}</div>`;
    ...
    restaurant: {
      id: "eden-restaurant",
      title() {
        return global.eden["restaurant"] && global.eden.restaurant.count >= 10
          ? loc("eden_restaurant_bd")
          : loc("eden_restaurant_title");
      },
      desc() {
        return `<div>${loc(
          "eden_restaurant_title"
      ...
      effect() {
        ...
        let desc = `<div>${loc("space_red_vr_center_effect1", [
          morale.toFixed(1),
        ])}</div>`;
        desc += `<div class="has-text-caution">${loc(
          "interstellar_alpha_starport_effect3",
          [sizeApproximation(food), global.resource.Food.name]
        )}</div>`;
        desc += `<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
        ])}</div>`;
    ...
    eternal_bank: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "eternal_bank",
            offset,
            traitCostMod("untrustworthy", 2500000000),
            1.26,
            "eden"
          );
        },
        ...
        Orichalcum(offset) {
          return spaceCostMultiplier(
            "eternal_bank",
            offset,
            traitCostMod("untrustworthy", 12500000),
            1.26,
            "eden"
          );
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          global["resource"]["Money"].max += spatialReasoning(
            bank_vault() * 10
          );
          return true;
        }
    ...
    archive: {
      id: "eden-archive",
      title() {
        return global.eden["archive"] && global.eden.archive.count >= 10
          ? loc("eden_archive_bd")
      ...
      cost: {
        ...
        Nano_Tube(offset) {
          return spaceCostMultiplier("archive", offset, 90000000, 1.26, "eden");
        },
        Asphodel_Powder(offset) {
          return spaceCostMultiplier("archive", offset, 50000, 1.26, "eden");
        },
        Elysanite(offset) {
          return spaceCostMultiplier("archive", offset, 35000000, 1.26, "eden");
        },
        Soul_Gem(offset) {
    ...
    north_pier: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("rune_gate") ||
          global.eden.north_pier.count < 10 ||
          wiki
        ) {
          return `<div>${loc("eden_pier", [
            loc("north"),
          ])}</div><div class="has-text-special">${loc("requires_segments", [
            10,
          ])}</div>`;
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("north_pier")
                ? global.eden.north_pier.count
                : 0) <
              10
              ? 7500000000
              : 0;
          }
          return !global.eden.hasOwnProperty("north_pier") ||
            global.eden.north_pier.count < 10
            ? 7500000000
            : 0;
        },
      ...
      effect(wiki) {
        let count =
          ...
          (global.eden.hasOwnProperty("north_pier")
            ...
            : 0);
        if (count >= 10) {
          let desc = `<div>${loc("eden_pier_effect", [
          ...
          let size = 10;
          let remain = size - count;
          return `<div>${loc("eden_pier_effect", [
            loc("eden_pier", [loc("south")]),
            loc("eden_isle_name"),
          ])}</div><div class="has-text-special">${loc(
            "space_dwarf_collider_effect2",
      ...
      action() {
        if (global.eden.north_pier.count < 10 && payCosts($(this)[0])) {
          ...
          if (
            global.eden.south_pier.count === 10 &&
            global.eden.north_pier.count === 10 &&
            global.tech.isle === 2
          ) {
            global.tech.isle = 3;
    ...
    rushmore: {
      ...
      cost: {
        Money(o, wiki) {
          return global.eden?.rushmore?.count === 0 || wiki ? 55000000000 : 0;
        },
        Stone(o, wiki) {
          return global.eden?.rushmore?.count === 0 || wiki ? 10000000000 : 0;
        },
      },
      queue_complete() {
    ...
    reincarnation: {
      ...
      cost: {
        Money(o) {
          return global.eden?.reincarnation?.count === 0
            ? 35000000000
            : global.eden?.reincarnation?.count === 1
            ? 5000000000
            : 0;
        },
        Aluminium(o) {
          return global.eden?.reincarnation?.count === 0 ? 10000000000 : 0;
        },
        Nano_Tube(o) {
          return global.eden?.reincarnation?.count === 0 ? 2000000000 : 0;
        },
        Asphodel_Powder(o) {
          return global.eden?.reincarnation?.count === 0 ? 750000 : 0;
        },
      },
      queue_complete() {
      ...
      effect() {
        return `<div>${loc("eden_reincarnation_effect", [
          races[global.race.species].name,
        ])}</div>`;
      },
      action() {
    ...
    eden_cement: {
      ...
      effect() {
        let desc = loc("plus_max_resource", [
          jobScale(5),
  ...
  eden_isle: {
    ...
    south_pier: {
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("south_pier")
                ? global.eden.south_pier.count
                : 0) <
              10
              ? 7500000000
              : 0;
          }
          return !global.eden.hasOwnProperty("south_pier") ||
            global.eden.south_pier.count < 10
            ? 7500000000
            : 0;
        },
    ...
    west_tower: {
      id: "eden-west_tower",
      title() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.wt === 0
          ? loc("eden_rampart_ruin", [loc("west")])
          : loc("eden_rampart_title", [loc("west")]);
      },
      desc() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.wt === 0
          ? loc("eden_rampart_ruin", [loc("west")])
      ...
      effect() {
        if (global.eden["enemy_isle"] && global.eden.enemy_isle.wt === 0) {
          ...
          return `<div>${loc("eden_tower_intact", [
            global.eden["enemy_isle"] ? global.eden.enemy_isle.wt : 100,
          ])}</div>`;
        }
      },
      action() {
    ...
    isle_garrison: {
      ...
      title() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.g === 0
          ? loc("eden_garrison_ruin")
          : loc("eden_garrison_title");
      },
      desc() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.g === 0
          ? loc("eden_garrison_ruin")
      ...
      effect() {
        if (global.eden["enemy_isle"] && global.eden.enemy_isle.g === 0) {
          ...
          return `<div>${loc("eden_tower_intact", [
            global.eden["enemy_isle"] ? global.eden.enemy_isle.g : 100,
          ])}</div>`;
        }
      },
      action() {
    ...
    east_tower: {
      id: "eden-east_tower",
      title() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.et === 0
          ? loc("eden_rampart_ruin", [loc("east")])
          : loc("eden_rampart_title", [loc("east")]);
      },
      desc() {
        return global.eden["enemy_isle"] && global.eden.enemy_isle.et === 0
          ? loc("eden_rampart_ruin", [loc("east")])
      ...
      effect() {
        if (global.eden["enemy_isle"] && global.eden.enemy_isle.et === 0) {
          ...
          return `<div>${loc("eden_tower_intact", [
            global.eden["enemy_isle"] ? global.eden.enemy_isle.et : 100,
          ])}</div>`;
        }
      },
      action() {
    ...
    spirit_vacuum: {
      ...
      effect() {
        ...
        if (
          global.eden.hasOwnProperty("palace") &&
          global.eden.palace.rate > 0 &&
          global.eden.palace.energy > 0
        ) {
          desc += `<div>${loc(`eden_spirit_vacuum_time`, [
            timeFormat(global.eden.palace.energy / global.eden.palace.rate),
          ])}</div>`;
        }
        desc += `<div class="has-text-caution">${loc("minus_power", [
          $(this)[0].powered(),
      ...
      powered(wiki) {
        let num_battery = wiki
          ? global.eden?.spirit_battery?.on ?? 0
          : p_on["spirit_battery"] || 0;
    ...
    spirit_battery: {
      ...
      effect() {
        let desc = `<div>${loc("eden_spirit_battery_effect", [
          loc("eden_spirit_vacuum_title"),
          10,
        ])}</div>`;
        if (global.tech["isle"] && global.tech.isle >= 6) {
          desc += `<div>${loc("eden_spirit_battery_effect2", [
            loc("eden_spirit_vacuum_title"),
            8,
          ])}</div>`;
        }
    ...
    soul_compactor: {
      ...
      cost: {
        Money(o, wiki) {
          return global.eden?.soul_compactor?.count === 0 || wiki
            ? 50000000000
            : 0;
        },
        Iron(o, wiki) {
        ...
        Uranium(o, wiki) {
          return global.eden?.soul_compactor?.count === 0 || wiki ? 4000000 : 0;
        },
        Scarletite(o, wiki) {
          return global.eden?.soul_compactor?.count === 0 || wiki
            ? 300000000
      ...
      effect() {
        let desc = `<div>${loc("eden_soul_compactor_effect1", [
          global.eden?.soul_compactor?.energy.toLocaleString() || 0,
        ])}</div>`;
        desc += `<div>${loc("eden_soul_compactor_effect2", [
          (1000000000).toLocaleString(),
        ])}</div>`;
        desc += `<div>${loc("eden_soul_compactor_effect3", [
  ...
  eden_palace: {
    ...
    scout_palace: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("eden_scout_palace_result"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    infuser: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("infuser") ||
          global.eden.infuser.count < 25 ||
          wiki
        ) {
          return `<div>${loc(
            "eden_infuser_title"
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("infuser")
                ? global.eden.infuser.count
                : 0) <
              25
              ? 12000000000
              : 0;
          }
          return !global.eden.hasOwnProperty("infuser") ||
            global.eden.infuser.count < 25
            ? 12000000000
            : 0;
        },
    ...
    apotheosis: {
      ...
      condition() {
        return (
          global.eden.hasOwnProperty("apotheosis") &&
          global.eden.apotheosis.count === 0
        );
      },
      queue_complete() {
    ...
    conduit: {
      ...
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("conduit") ||
          global.eden.conduit.count < 25 ||
          wiki
        ) {
          return `<div>${loc(
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("conduit")
                ? global.eden.conduit.count
                : 0) <
              25
              ? 8000000000
              : 0;
          }
          return !global.eden.hasOwnProperty("conduit") ||
            global.eden.conduit.count < 25
            ? 25000000000
            : 0;
        },
    ...
    tomb: {
      id: "eden-tomb",
      title() {
        return global.eden?.tomb?.count === 10
          ? loc("eden_tomb_sealed")
          : loc("eden_tomb_title");
      },
      desc(wiki) {
        if (
          !global.eden.hasOwnProperty("tomb") ||
          global.eden.tomb.count < 10 ||
          wiki
        ) {
          return `<div>${loc(
            "eden_tomb_title"
      ...
      cost: {
        Money(offset) {
          if (offset) {
            return offset +
              (global.eden.hasOwnProperty("tomb")
                ? global.eden.tomb.count
                : 0) <
              10
              ? 25000000000
              : 0;
          }
          return !global.eden.hasOwnProperty("tomb") ||
            global.eden.tomb.count < 10
            ? 25000000000
            : 0;
        },
      ...
      effect(wiki) {
        let count =
          (wiki?.count ?? 0) +
          (global.eden.hasOwnProperty("tomb") ? global.eden.tomb.count : 0);
        if (count >= 10) {
...
export function renderEdenic() {
  if (
    ...
    (global.settings.civTabs !== 1 || global.settings.spaceTabs !== 7)
  ) {
    return;
  }
  let parent = $("#eden");
  ...
  Object.keys(edenicModules).forEach(function (region) {
    ...
    if (global.settings.eden[`${show}`]) {
      let name =
        typeof edenicModules[region].info.name === "string"
          ? edenicModules[region].info.name
          : edenicModules[region].info.name();
      ...
      popover(
        region,
        function () {
          return typeof edenicModules[region].info.desc === "string"
            ? edenicModules[region].info.desc
            : edenicModules[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
...
export function mechStationEffect() {
  ...
  global.eden.mech_station.effect =
    100 - Math.floor((hostility / rawHostility) * 100);

  if (
    global.eden.mech_station.effect === 100 &&
    global.eden.mech_station.mode === 4
  ) {
    global.eden.mech_station.effect = 110;
  } else if (
    global.eden.mech_station.effect === 100 &&
    global.eden.mech_station.mode === 5
  ) {
