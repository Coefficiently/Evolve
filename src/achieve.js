...
Object.keys(achieve_list).forEach(function (type) {
  achieve_list[type].forEach(
    (achieve) =>
      (achievements[achieve] = {
        name: loc(`achieve_${achieve}_name`),
        desc: descData[achieve]
          ? loc(`achieve_${achieve}_desc`, descData[achieve])
          : loc(`achieve_${achieve}_desc`),
        flair: flairData[achieve]
          ? loc(`achieve_${achieve}_flair`, flairData[achieve])
          : loc(`achieve_${achieve}_flair`),
        type: type,
      })
  );
});
...
export function universeLevel(universe) {
  ...
  Object.keys(achievements).forEach(function (achievement) {
    if (global.stats.achieve[achievement]) {
      lvl +=
        global.stats.achieve[achievement].l > 5
          ? 5
          : global.stats.achieve[achievement].l;
      if (global.stats.achieve[achievement][affix]) {
        ulvl +=
          global.stats.achieve[achievement][affix] > 5
            ? 5
            : global.stats.achieve[achievement][affix];
      }
    }
  });
...
export function unlockAchieve(achievement, small, rank, universe) {
  ...
  if (
    ...
    if (
      global.stats.achieve[achievement] &&
      ...
      messageQueue(
        loc(
          upgrade ? "achieve_unlock_achieve_upgrade" : "achieve_unlock_achieve",
  ...
  if (global.stats.achieve[achievement] && universe !== "l") {
    ...
    if (
      !global.stats.achieve[achievement][u_affix] ||
      (global.stats.achieve[achievement][u_affix] &&
      ...
      if (!unlock) {
        messageQueue(
          loc(
            i_upgrade
              ? "achieve_unlock_achieve_icon_upgrade"
              : "achieve_unlock_achieve_icon",
            [achievements[achievement].name]
          ),
          "special",
          false,
...
export function unlockFeat(feat, small, rank) {
  ...
  if (
    ...
    (global.stats.feat[feat] && global.stats.feat[feat] < rank)
  ) {
    let upgrade = global.stats.feat[feat] ? true : false;
    global.settings.showAchieve = true;
    global.stats.feat[feat] = rank;
    messageQueue(
      loc(upgrade ? "feat_upgraded" : "feat_unlocked", [feats[feat].name]),
      "special",
      false,
      ["achievements"]
    );
...
export function drawAchieve(args) {
  ...
  Object.keys(achievements).forEach(function (achievement) {
    ...
    if (global.stats.achieve[achievement]) {
      ...
      if ((fool && global.stats.achieve[achievement].l > 1) || !fool) {
        achieve.append(
          $(
            `<b-tooltip :label="flair('${achievement}')" position="is-bottom" size="is-small" animated><div class="achievement"><span class="has-text-warning">${achievements[achievement].name}</span><span>${achievements[achievement].desc}</span>${emblem}</div></b-tooltip>`
          )
        );
  ...
  if (fool && !global.stats.feat["fool"]) {
    let thefool = $(
      `<b-tooltip :label="feat('fool')" position="is-bottom" size="is-small" animated><div id="thefool" class="achievement"><span class="has-text-danger">${
        feats.fool.name
      }</span><span>${loc("feat_fool_spoof")}</span></div></b-tooltip>`
    );
    ...
    Object.keys(feats).forEach(function (feat) {
      ...
      if (global.stats.feat[feat]) {
        let star =
          global.stats.feat[feat] > 1
            ? `<p class="flair" title="${sLevel(global.stats.feat[feat])} ${loc(
                baseIcon
              )}"><svg class="star${
                global.stats.feat[feat]
              }" version="1.1" x="0px" y="0px" width="16px" height="16px" viewBox="${svgViewBox(
                baseIcon
              )}" xml:space="preserve">${svgIcons(baseIcon)}</svg></p>`
            : "";
        if (feat === "easter") {
        ...
        achieve.append(
          $(
            `<b-tooltip :label="feat('${feat}')" position="is-bottom" size="is-small" animated><div class="achievement"><span class="has-text-danger">${feats[feat].name}</span><span>${feats[feat].desc}</span>${star}</div></b-tooltip>`
          )
        );
      }
    });
  ...
  achieve.prepend(
    `<div class="has-text-warning">${loc("achieve_draw_achieve_earned", [
      earned,
      total,
    ])}${trick}</div>`
  );
...
export function challengeIcon() {
  ...
  if (a_level > 1 && $("#topBar .planet .flair").length === 0) {
    if (egg.length === 0) {
      ...
      $("#topBar .planet").after(
        `<span class="flair"><svg class="star${a_level}" version="1.1" x="0px" y="0px" width="16px" height="16px" viewBox="${svgViewBox(
          bIcon
        )}" xml:space="preserve">${svgIcons(bIcon)}</svg></span>`
      );
    }
    ...
    if (desc.length > 0) {
      $("#topBar .planetWrap .flair").append(
        $(
          `<div class="is-sr-only"><div>Active Challenge Genes</div>${desc}</div>`
        )
      );
    }
...
export function checkAchievements() {
  ...
  if (
    Math.round(
      Math.log2(global.civic.garrison.protest + global.civic.garrison.fatigue)
    ) >= 8
  ) {
  ...
  if (
    ...
    global.interstellar["stellar_engine"].mass >= 12
  ) {
    unlockAchieve("landfill");
  }
  if (
    global.interstellar["stellar_engine"] &&
    global.interstellar["stellar_engine"].mass >= 100
  ) {
  ...
  if (
    ...
    if (
      stargate === 1 &&
      piracy("gxy_gateway") === 1 &&
      piracy("gxy_gorddon") === 1 &&
      piracy("gxy_alien1") === 1 &&
      piracy("gxy_alien2") === 1 &&
      chthonian === 1
    ) {
  ...
  if (
    global.resource.hasOwnProperty("Money") &&
  ...
  if (
    ...
    global.resource.Money.amount >= 250000000000
  ) {
    unlockAchieve("wheelbarrow");
  }

  if (
    global.civic.hasOwnProperty("govern") &&
    global.galaxy.hasOwnProperty("trade") &&
    global.city.hasOwnProperty("market") &&
    global.galaxy.trade.cur >= 50 &&
    global.city.market.trade >= 750 &&
  ...
  if (global.tech["pillars"]) {
    ...
    Object.keys(global.pillars).forEach(function (race) {
      if (races[race]) {
        if (
          !genus[races[race].type] ||
          global.pillars[race] > genus[races[race].type]
        ) {
  ...
  if (
    ...
    global.tech.hasOwnProperty("hell_spire") &&
    ...
    Object.keys(global.stats.spire).forEach(function (universe) {
      ...
      if (Object.keys(current).length === mobs) {
        unlockAchieve(
          "gladiator",
          false,
          Math.min(...Object.values(current)),
          universe
        );
      }
    });
    if (Object.keys(highest).length === mobs) {
      unlockAchieve(
        "gladiator",
        false,
        Math.min(...Object.values(highest)),
        "l"
      );
    }
  }
  ...
  if (global.race["banana"]) {
    let affix = universeAffix();
    if (
      global.tech.hasOwnProperty("monuments") &&
      global.tech.monuments >= 50
    ) {
  ...
  // Path Finder
  {
    ...
    ["l", uAffix].forEach(function (affix) {
      let rank = 0;
      ["ashanddust", "exodus", "obsolete", "bluepill", "retired"].forEach(
        function (achieve) {
          if (
            global.stats.achieve[achieve] &&
            global.stats.achieve[achieve][affix] &&
            global.stats.achieve[achieve][affix] >= 5
          ) {
            rank++;
          }
        }
  ...
  if (global.race["fasting"]) {
    let affix = universeAffix();
    if (
      global.tech.hasOwnProperty("stock_exchange") &&
      global.tech.stock_exchange >= 80
    ) {
  ...
  if (
    ...
    murder = unlockFeat(
      "friday",
      global.race.universe === "micro" ? true : false
    );
    if (murder) {
  ...
  if (
    ...
    date.getDate() >= 1 &&
    date.getDate() <= 3 &&
    global.stats.feat.hasOwnProperty("fool") &&
    global.stats.feat.fool > 0
  ) {
  ...
  // total achievements feat
  {
    for (let t_level = a_level; t_level >= 1; t_level--) {
      ...
      for (const key of keys) {
        if (
          global.stats.achieve[key] &&
          global.stats.achieve[key].l >= t_level
        ) {
          total++;
        }
      }
      ...
      for (let i = 0; i < 6; i++) {
        if (
          total >= progress[i].c &&
          (!global.stats.feat[progress[i].f] ||
            global.stats.feat[progress[i].f] < t_level)
        ) {
          if (global.race.universe === "micro") {
...
export function checkAdept() {
  let rank = 0;
  ["whitehole", "eviltwin", "canceled", "heavy", "pw_apocalypse"].forEach(
    function (x) {
  ...
  rank = global.stats.feat["adept"]
    ? Math.min(rank, global.stats.feat["adept"])
    : 0;
  return rank;
}
...
function checkBigAchievementUniverse(frag, name, num, level) {
  ...
  switch (global.race.universe) {
    case "evil":
      if (
        typeof global.stats.achieve[name] === "undefined" ||
        typeof global.stats.achieve[name].e === "undefined" ||
        global.stats.achieve[name].e < level
      ) {
        proceed = true;
      }
      break;
    case "antimatter":
      if (
        typeof global.stats.achieve[name] === "undefined" ||
        typeof global.stats.achieve[name].a === "undefined" ||
        global.stats.achieve[name].a < level
      ) {
        proceed = true;
      }
      break;
    case "heavy":
      if (
        typeof global.stats.achieve[name] === "undefined" ||
        typeof global.stats.achieve[name].h === "undefined" ||
        global.stats.achieve[name].h < level
      ) {
        proceed = true;
      }
      break;
    case "micro":
      if (
        typeof global.stats.achieve[name] === "undefined" ||
        typeof global.stats.achieve[name].m === "undefined" ||
        global.stats.achieve[name].m < level
      ) {
        proceed = true;
      }
      break;
    case "magic":
      if (
        typeof global.stats.achieve[name] === "undefined" ||
        typeof global.stats.achieve[name].mg === "undefined" ||
        global.stats.achieve[name].mg < level
      ) {
        proceed = true;
      }
      break;
      ...
      if (key.includes(frag)) {
        switch (global.race.universe) {
          case "evil":
            if (
              global.stats.achieve[key] &&
              global.stats.achieve[key]["e"] &&
              global.stats.achieve[key].e >= level
            ) {
              total++;
            }
            break;
          case "antimatter":
            if (
              global.stats.achieve[key] &&
              global.stats.achieve[key]["a"] &&
              global.stats.achieve[key].a >= level
            ) {
              total++;
            }
            break;
          case "heavy":
            if (
              global.stats.achieve[key] &&
              global.stats.achieve[key]["h"] &&
              global.stats.achieve[key].h >= level
            ) {
              total++;
            }
            break;
          case "micro":
            if (
              global.stats.achieve[key] &&
              global.stats.achieve[key]["m"] &&
              global.stats.achieve[key].m >= level
            ) {
              total++;
            }
            break;
          case "magic":
            if (
              global.stats.achieve[key] &&
              global.stats.achieve[key]["mg"] &&
              global.stats.achieve[key].mg >= level
            ) {
              total++;
            }
            break;
...
export const perkList = {
  mastery: {
    ...
    desc() {
      ...
      Object.keys(universe_types).forEach(function (universe) {
        ...
        if (universe === "standard") {
          desc += `<span class="row"><span class="has-text-caution">${
            universe_types[universe].name
          }</span>: <span>${loc("perks_mastery_general", [
            `<span class="has-text-advanced">${+mastery.g.toFixed(2)}%</span>`,
          ])}</span></span>`;
          ...
          desc += `<span class="row"><span class="has-text-caution">${
            ...
            `<span class="has-text-advanced">${+mastery.g.toFixed(2)}%</span>`,
          ])}, ${loc("perks_mastery_universe", [
            `<span class="has-text-advanced">${+mastery.u.toFixed(2)}%</span>`,
          ])}</span></span>`;
        }
      });
      return desc;
    ...
    active() {
      return global.genes["challenge"] && global.genes["challenge"] >= 2
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_unlocked_title`
        )}</span>`,
      ]),
    ],
  },
  blackhole: {
    ...
    desc(wiki) {
      let bonus = wiki
        ? "5/10/15/20/25"
        : global.stats.achieve["blackhole"]
        ? global.stats.achieve.blackhole.l * 5
        : 5;
      return loc("achieve_perks_blackhole", [bonus]);
    },
    active() {
      return global.stats.achieve["blackhole"] &&
        global.stats.achieve.blackhole.l >= 1
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(
          `achieve_blackhole_name`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(
          `achieve_blackhole_name`
        )}</span>`,
      ]),
  ...
  trade: {
    ...
    desc(wiki) {
      let bonus1 = wiki
        ...
        ? global.stats.achieve.trade.l * 2
        : 2;
      let bonus2 = wiki
        ? "1/2/3/4/5"
        : global.stats.achieve["trade"]
        ? global.stats.achieve.trade.l
        : 1;
      return loc("achieve_perks_trade", [bonus1, bonus2]);
    },
    active() {
      return global.stats.achieve["trade"] && global.stats.achieve.trade.l >= 1
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_trade_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_trade_name`)}</span>`,
      ]),
  ...
  creator: {
    ...
    desc(wiki) {
      let bonus = wiki
        ? "1.5/2/2.5/3/3.5"
        : 1 +
          (global.stats.achieve["creator"]
            ? global.stats.achieve["creator"].l * 0.5
            : 0.5);
      return loc("achieve_perks_creator", [bonus]);
    },
    active() {
      return global.stats.achieve["creator"] &&
        global.stats.achieve.creator.l >= 1
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_creator_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_creator_name`)}</span>`,
      ]),
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["mass_extinction"] &&
      ...
      {
        desc(wiki) {
          let rank = global.stats.achieve["mass_extinction"]
            ? global.stats.achieve.mass_extinction.l
            : 1;
  ...
  doomed: {
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_doomed_name`)}</span>`,
      ]),
    ],
  },
  explorer: {
    ...
    desc(wiki) {
      let bonus = wiki
        ? "1/2/3/4/5"
        : global.stats.achieve["explorer"]
        ? global.stats.achieve["explorer"].l
        : 1;
      return loc("achieve_perks_explorer", [bonus]);
    },
    active() {
      return global.stats.achieve["explorer"] &&
        global.stats.achieve.explorer.l >= 1
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_explorer_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_explorer_name`)}</span>`,
      ]),
  ...
  miners_dream: {
    ...
    desc(wiki) {
      let numGeo = wiki
        ? "1/2/3/5/7"
        : global.stats.achieve["miners_dream"]
    ...
    active() {
      return global.stats.achieve["miners_dream"] &&
        global.stats.achieve.miners_dream.l >= 1
        ? true
        : false;
    },
    notes: [
  ...
  extinct_junker: {
    ...
    active() {
      return global.stats.achieve["extinct_junker"] &&
  ...
  joyless: {
    ...
    desc(wiki) {
      let bonus = wiki
        ? "2/4/6/8/10"
        : global.stats.achieve["joyless"]
        ? global.stats.achieve["joyless"].l * 2
        : 2;
      return loc("achieve_perks_joyless", [bonus]);
    },
    active() {
      return global.stats.achieve["joyless"] &&
        global.stats.achieve.joyless.l >= 1
        ? true
        : false;
    },
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_joyless_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_joyless_name`)}</span>`,
      ]),
  ...
  steelen: {
    ...
    desc(wiki) {
      let bonus = wiki
        ...
        ? global.stats.achieve["steelen"].l * 2
        : 2;
      return loc("achieve_perks_steelen", [bonus]);
    },
    active() {
      return global.stats.achieve["steelen"] &&
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_steelen_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_steelen_name`)}</span>`,
      ]),
  ...
  wheelbarrow: {
    ...
    desc(wiki) {
      let bonus = wiki
        ...
        ? global.stats.achieve["wheelbarrow"].l * 2
        : 2;
      return loc("achieve_perks_wheelbarrow", [bonus]);
    },
    active() {
      return global.stats.achieve["wheelbarrow"] &&
        global.stats.achieve.wheelbarrow.l >= 1
        ? true
        : false;
    },
    notes: [
  ...
  extinct_sludge: {
    ...
    group: [
      {
        desc(wiki) {
          let bonus = wiki
            ...
            ? global.stats.achieve["extinct_sludge"].l * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
            loc(`universe_standard`),
          ]);
        },
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.l >= 1
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "3/6/9/12/15"
            : global.stats.achieve["extinct_sludge"]
            ? global.stats.achieve["extinct_sludge"].h * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
            loc(`universe_heavy`),
        ...
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.h >= 1
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "3/6/9/12/15"
            : global.stats.achieve["extinct_sludge"]
            ? global.stats.achieve["extinct_sludge"].a * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
        ...
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.a >= 1
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "3/6/9/12/15"
            : global.stats.achieve["extinct_sludge"]
            ? global.stats.achieve["extinct_sludge"].e * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
            loc(`universe_evil`),
        ...
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.e >= 1
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "3/6/9/12/15"
            : global.stats.achieve["extinct_sludge"]
            ? global.stats.achieve["extinct_sludge"].m * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
            loc(`universe_micro`),
        ...
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.m >= 1
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "3/6/9/12/15"
            : global.stats.achieve["extinct_sludge"]
            ? global.stats.achieve["extinct_sludge"].mg * 3
            : 3;
          return loc("achieve_perks_extinct_sludge", [
            bonus,
            loc(`universe_magic`),
        ...
        active() {
          return global.stats.achieve["extinct_sludge"] &&
            global.stats.achieve.extinct_sludge.mg >= 1
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(
          `achieve_extinct_sludge_name`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_universe_scale`, [
        `<span class="has-text-caution">${loc(
          `achieve_extinct_sludge_name`
        )}</span>`,
      ]),
      ...
      },
      {
        desc(wiki) {
          let bonus = wiki
            ...
            ? global.stats.achieve["whitehole"].l * 5
            : 5;
  ...
  heavyweight: {
    ...
    desc(wiki) {
      let bonus = wiki
        ...
        ? global.stats.achieve["heavyweight"].l * 4
        : 4;
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["dissipated"] &&
        ...
        desc(wiki) {
          let bonus = wiki
            ...
            : global.stats.achieve["dissipated"] &&
              global.stats.achieve["dissipated"].l >= 5
            ? 2
            : 1;
  ...
  banana: {
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["banana"] &&
            global.stats.achieve.banana.l >= 1
            ? true
            : false;
        },
      },
      ...
      },
      {
        ...
        active() {
          return global.stats.achieve["banana"] &&
            global.stats.achieve.banana.l >= 3
            ? true
            : false;
        },
      },
      ...
      },
      {
        ...
        active() {
          return global.stats.achieve["banana"] &&
            global.stats.achieve.banana.l >= 5
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_banana_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task`, [
        `<span class="has-text-caution">${loc(`achieve_banana_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        1,
        `<span class="has-text-${
          global.stats.banana.b1.l ? `success` : `danger`
        }">${loc(`wiki_achieve_banana1`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        2,
        `<span class="has-text-${
          global.stats.banana.b2.l ? `success` : `danger`
        }">${loc(`wiki_achieve_banana2`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        3,
        `<span class="has-text-${
          global.stats.banana.b3.l ? `success` : `danger`
        }">${loc(`wiki_achieve_banana3`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        4,
        `<span class="has-text-${
          global.stats.banana.b4.l ? `success` : `danger`
        }">${loc(`wiki_achieve_banana4`, [500])}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        5,
        `<span class="has-text-${
          global.stats.banana.b5.l ? `success` : `danger`
        }">${loc(`wiki_achieve_banana5`, [50])}</span>`,
      ]),
    ],
  },
  anarchist: {
    ...
    desc(wiki) {
      let bonus = wiki
        ? "10/20/30/40/50"
        : global.stats.achieve["anarchist"]
        ? global.stats.achieve["anarchist"].l * 10
        : 10;
      return loc("achieve_perks_anarchist", [bonus]);
    },
    active() {
      return global.stats.achieve["anarchist"] &&
        global.stats.achieve["anarchist"].l >= 1
        ? true
        : false;
    },
    notes: [
  ...
  ascended: {
    ...
    group: [
      {
          ...
          if (wiki) {
            ...
            if (global.stats.achieve["ascended"]) {
              for (let i = 0; i < universe_affixes.length; i++) {
                if (
                  global.stats.achieve.ascended.hasOwnProperty(
                    universe_affixes[i]
                  )
                ) {
        ...
        active() {
          return global.stats.achieve["ascended"] &&
            global.stats.achieve["ascended"].l >= 1
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.stats.achieve["ascended"] &&
            global.stats.achieve["ascended"][universeAffix()] >= 1
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_ascended_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_ascended_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_universe`, [
        `<span class="has-text-caution">${loc(`achieve_ascended_name`)}</span>`,
      ]),
    ],
  },
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["technophobe"] &&
        ...
        desc(wiki) {
          ...
          if (wiki) {
            ...
            bonus =
              ...
              global.stats.achieve.technophobe.l >= 4
                ? 25
                : 10;
      ...
      },
      {
        desc(wiki) {
          ...
          return wiki || gems > 1
            ? loc("achieve_perks_technophobe3a", [gems])
            : loc("achieve_perks_technophobe3", [gems]);
        },
        active() {
          return global.stats.achieve["technophobe"] &&
            global.stats.achieve.technophobe.l >= 3
            ? true
            : false;
        },
      },
  ...
  iron_will: {
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["iron_will"] &&
      ...
      },
      {
        ...
        active() {
          return global.stats.achieve["iron_will"] &&
            global.stats.achieve.iron_will.l >= 2
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(
          `achieve_iron_will_name`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill`, [
        `<span class="has-text-caution">${loc(
          `evo_challenge_cataclysm`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill2`, [
        1,
        `<span class="has-text-caution">${loc(
          `space_red_ziggurat_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill3`, [
        2,
        `<span class="has-text-caution">${loc(`tech_elerium_mining`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill3`, [
        3,
        `<span class="has-text-caution">${loc(`tech_lasers`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill3`, [
        4,
        `<span class="has-text-caution">${loc(
          `tech_generational_ship`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_ironwill4`, [
        5,
        `<span class="has-text-caution">${loc(`wiki_resets_bioseed`)}</span>`,
      ]),
    ],
  },
  failed_history: {
    ...
    active() {
      return global.stats.achieve["failed_history"] &&
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(
          `achieve_failed_history_name`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_failed_history`, [
        `<span class="has-text-caution">${loc(
          `evo_challenge_cataclysm`
        )}</span>`,
      ]),
  ...
  lamentis: {
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_lamentis_name`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_scale`, [
        `<span class="has-text-caution">${loc(`achieve_lamentis_name`)}</span>`,
      ]),
  ...
  soul_sponge: {
    ...
    desc(wiki) {
      let soul = wiki
        ...
        ? global.stats.achieve.soul_sponge.mg * 100
        : 100;
      return loc("achieve_perks_soul_sponge", [soul]);
    },
    active() {
      return global.stats.achieve["soul_sponge"] &&
        global.stats.achieve.soul_sponge.mg >= 1
        ? true
        : false;
    },
    notes: [
  ...
  nightmare: {
    ...
    active() {
      return global.stats.achieve["nightmare"] &&
        global.stats.achieve.nightmare.mg >= 1
        ? true
        : false;
    },
    notes: [
  ...
  escape_velocity: {
    ...
    desc(wiki) {
      let ev = wiki
        ...
        ? global.stats.achieve.escape_velocity.h * 2
        : 2;
      return loc("achieve_perks_escape_velocity", [ev]);
    },
    active() {
      return global.stats.achieve["escape_velocity"] &&
        global.stats.achieve.escape_velocity.h >= 1
        ? true
        : false;
    },
    notes: [
  ...
  endless_hunger: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.stats.achieve["endless_hunger"] &&
            global.stats.achieve.endless_hunger.l >= 2
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.stats.achieve["endless_hunger"] &&
            global.stats.achieve.endless_hunger.l >= 3
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.stats.achieve["endless_hunger"] &&
            global.stats.achieve.endless_hunger.l >= 4
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.stats.achieve["endless_hunger"] &&
            global.stats.achieve.endless_hunger.l >= 5
            ? true
            : false;
        },
      },
    ...
    notes: [
      ...
      loc(`wiki_perks_achievement_note_task_num`, [
        1,
        `<span class="has-text-${
          global.stats.endless_hunger.b1.l ? `success` : `danger`
        }">${loc(`wiki_achieve_endless_hunger1`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        2,
        `<span class="has-text-${
          global.stats.endless_hunger.b2.l ? `success` : `danger`
        }">${loc(`wiki_achieve_endless_hunger2`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        3,
        `<span class="has-text-${
          global.stats.endless_hunger.b3.l ? `success` : `danger`
        }">${loc(`wiki_achieve_endless_hunger3`, [80])}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        4,
        `<span class="has-text-${
          global.stats.endless_hunger.b4.l ? `success` : `danger`
        }">${loc(`wiki_achieve_endless_hunger4`, [1200])}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_task_num`, [
        5,
        `<span class="has-text-${
          global.stats.endless_hunger.b5.l ? `success` : `danger`
        }">${loc(`wiki_achieve_endless_hunger5`)}</span>`,
      ]),
    ],
  },
  gladiator: {
    ...
    desc(wiki) {
      let mech = wiki
        ...
        ? global.stats.achieve.gladiator.l * 20
        : 20;
      return loc("achieve_perks_gladiator", [mech]);
    },
    active() {
      return global.stats.achieve["gladiator"] &&
        global.stats.achieve.gladiator.l >= 1
        ? true
        : false;
    },
    notes: [
  ...
  pathfinder: {
    ...
    group: [
      {
        ...
        active() {
          return global.stats.achieve["pathfinder"] &&
      ...
      },
      {
        ...
        active() {
          return global.stats.achieve["pathfinder"] &&
            global.stats.achieve.pathfinder.l >= 3
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(
          `achieve_pathfinder_name`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder`, [
        `<span class="has-text-caution">${loc(
          `evo_challenge_truepath`
        )}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder_reset`, [
        `<span class="has-text-${
          global.stats.achieve["ashanddust"] ? "success" : "danger"
        }">${loc(`wiki_resets_mad`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder_reset`, [
        `<span class="has-text-${
          global.stats.achieve["exodus"] ? "success" : "danger"
        }">${loc(`wiki_resets_bioseed`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder_reset`, [
        `<span class="has-text-${
          global.stats.achieve["obsolete"] ? "success" : "danger"
        }">${loc(`wiki_resets_ai`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder_reset`, [
        `<span class="has-text-${
          global.stats.achieve["bluepill"] ? "success" : "danger"
        }">${loc(`wiki_resets_matrix`)}</span>`,
      ]),
      loc(`wiki_perks_achievement_note_pathfinder_reset`, [
        `<span class="has-text-${
          global.stats.achieve["retired"] ? "success" : "danger"
        }">${loc(`wiki_resets_retired`)}</span>`,
      ]),
    ],
  },
  overlord: {
    ...
    active() {
      return global.stats.achieve["overlord"] &&
    ...
    notes: [
      loc(`wiki_perks_achievement_note`, [
        `<span class="has-text-caution">${loc(`achieve_overlord_name`)}</span>`,
      ]),
    ],
  },
  adam_eve: {
    ...
    active() {
      return global.stats.achieve["adam_eve"] &&
        global.stats.achieve.adam_eve.l >= 5
        ? true
        : false;
    },
    notes: [],
  ...
  creep: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_genetic_memory_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_animus_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_divine_remembrance_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_divine_proportion_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_genetic_repository_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  store: {
    ...
    desc(wiki) {
      let psb = wiki
        ? "0.04/0.06/0.08"
        : global.genes["store"] && global.genes.store > 1
        ? global.genes.store === 2
          ? 0.06
          : 0.08
        : 0.04;
      return loc(
        global.genes["store"] && global.genes.store >= 4
          ? "arpa_perks_store2"
          : "arpa_perks_store1",
        [psb]
      );
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_spatial_reasoning_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_spatial_superiority_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_spatial_supremacy_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_dimensional_warping_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  evolve: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 2
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 3
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 4
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 5
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 6
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 7
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["evolve"] && global.genes.evolve >= 8
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_morphogenesis_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          ...
          `<span class="has-text-caution">${loc(
            `arpa_genepool_homologous_recombination_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_genetic_reshuffling_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_recombinant_dna_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_chimeric_dna_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_molecular_cloning_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_transgenes_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  birth: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_replication_title`
        )}</span>`,
      ]),
    ],
  },
  enhance: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_enhanced_muscle_fiber_title`
        )}</span>`,
      ]),
    ],
  },
  crafty: {
    ...
    group: [
      ...
      },
      {
        desc(wiki) {
          let bonus = wiki
            ? "50/100"
            : global.genes["crafty"] && global.genes.crafty >= 3
            ? 100
            : 50;
          return loc("arpa_genepool_crafting_desc", [bonus]);
        },
        active() {
          return global.genes["crafty"] && global.genes.crafty >= 2
            ? true
            : false;
        },
      },
    ...
    notes: [
      ...
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_detail_oriented_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_rigorous_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  governor: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_governance_title`
        )}</span>`,
      ]),
    ],
  },
  synthesis: {
    ...
    desc(wiki) {
      let base = wiki
        ? "2/3/4"
        : global.genes["synthesis"] && global.genes["synthesis"] >= 2
        ? global.genes["synthesis"] >= 3
          ? 4
          : 3
        : 2;
      let auto = wiki
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_synthesis_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_karyokinesis_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_cytokinesis_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  challenge: {
    ...
    group: [
      ...
      },
      {
        desc(wiki) {
          return loc("arpa_perks_challenge2", [
            wiki
              ? "60/80"
              : global.genes["challenge"] && global.genes.challenge >= 4
              ? 80
              : 60,
            wiki
              ? "60/40"
        ...
        active() {
          return global.genes["challenge"] && global.genes.challenge >= 3
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["challenge"] && global.genes.challenge >= 5
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_hardened_genes_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          ...
          `<span class="has-text-caution">${loc(
            `arpa_genepool_universal_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_standard_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_mastered_title`
          )}</span>`,
        ].join(", "),
      ]),
      loc(`wiki_perks_crispr_note_challenge`, [
  ...
  ancients: {
    ...
    group: [
      ...
      {
        desc() {
          return global.genes["ancients"] && global.genes.ancients >= 4
            ? loc("arpa_perks_ancients3")
            : loc("arpa_perks_ancients2");
        },
        active() {
          return global.genes["ancients"] && global.genes.ancients >= 2
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          return loc("arpa_perks_ancients4", [
            wiki
              ? "25/50"
              : global.genes["ancients"] && global.genes.ancients >= 5
              ? 50
              : 25,
          ]);
        },
        active() {
          return global.genes["ancients"] && global.genes.ancients >= 3
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_ancients_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_faith_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_devotion_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_acolyte_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_conviction_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  trader: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_negotiator_title`
        )}</span>`,
      ]),
    ],
  },
  transcendence: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_transcendence_title`
        )}</span>`,
      ]),
    ],
  },
  queue: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.genes["queue"] && global.genes.queue >= 2
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_geographer_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_architect_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  plasma: {
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_mitosis_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_metaphase_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  mutation: {
    ...
    group: [
      {
        desc() {
          return global.genes["mutation"] && global.genes.mutation > 1
            ? loc("arpa_perks_mutation2")
            : loc("arpa_perks_mutation1");
        },
        active() {
      ...
      },
      {
        ...
        active() {
          return global.genes["mutation"] && global.genes.mutation >= 3
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_mutation_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_transformation_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_metamorphosis_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  bleed: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.genes["bleed"] && global.genes.bleed >= 2
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["bleed"] && global.genes.bleed >= 3
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_bleeding_effect_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_synchronicity_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_astral_awareness_title`
          )}</span>`,
        ].join(", "),
      ]),
      loc(`wiki_perks_crispr_note_bleed`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_bleeding_effect_title`
        )}</span>`,
      ]),
  ...
  blood: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.genes["blood"] && global.genes.blood >= 2
            ? true
            : false;
        },
      },
      {
        ...
        active() {
          return global.genes["blood"] && global.genes.blood >= 3
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_crispr_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_genepool_blood_remembrance_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_crispr_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_genepool_blood_sacrifice_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_genepool_essence_absorber_title`
          )}</span>`,
        ].join(", "),
      ]),
      loc(`wiki_perks_crispr_note_blood`, [
        loc(`arpa_genepool_blood_remembrance_title`),
      ]),
    ],
  },
  spire: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.blood["spire"] && global.blood.spire >= 2
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_blood_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_blood_purify_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_blood_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_blood_chum_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  lust: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_lust", [
            wiki
              ? 0.2
              : 0.2 * (global.blood["lust"] ? global.blood["lust"] : 1),
          ]);
        },
        active() {
    ...
    notes: [
      loc(`wiki_perks_blood_note`, [
        `<span class="has-text-caution">${loc(`arpa_blood_lust_title`)}</span>`,
      ]),
  ...
  illuminate: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_illuminate", [
            wiki
              ? 0.01
              : 0.01 *
                (global.blood["illuminate"] ? global.blood["illuminate"] : 1),
          ]);
        },
        active() {
  ...
  greed: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_greed", [
            wiki ? 1 : 1 * (global.blood["greed"] ? global.blood["greed"] : 1),
          ]);
        },
        active() {
  ...
  hoarder: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_hoarder", [
            wiki
              ? 1
              : 1 * (global.blood["hoarder"] ? global.blood["hoarder"] : 1),
          ]);
        },
        active() {
  ...
  artisan: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_artisan", [
            wiki
              ? 1
              : 1 * (global.blood["artisan"] ? global.blood["artisan"] : 1),
          ]);
        },
        active() {
  ...
  attract: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_attract", [
            wiki
              ? 5
              : 5 * (global.blood["attract"] ? global.blood["attract"] : 1),
          ]);
        },
        active() {
  ...
  wrath: {
    ...
    group: [
      {
        desc(wiki) {
          return loc("arpa_perks_wrath", [
            wiki ? 5 : 5 * (global.blood["wrath"] ? global.blood["wrath"] : 1),
          ]);
        },
        active() {
  ...
  prepared: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.blood["prepared"] && global.blood.prepared >= 2
            ? true
            : false;
        },
      },
  ...
  unbound: {
    ...
    group: [
      ...
      },
      {
        ...
        active() {
          return global.blood["unbound"] && global.blood.unbound >= 3
            ? true
            : false;
        },
      },
      {
        desc(wiki) {
          return loc("arpa_perks_unbound_resist", [
            wiki
              ? "10/5"
              : global.blood["unbound"] && global.blood.unbound >= 4
              ? 5
              : 10,
          ]);
        },
        active() {
          return global.blood["unbound"] && global.blood.unbound >= 2
            ? true
            : false;
        },
      },
    ...
    notes: [
      loc(`wiki_perks_blood_note`, [
        `<span class="has-text-caution">${loc(
          `arpa_blood_unbound_title`
        )}</span>`,
      ]),
      loc(`wiki_perks_blood_note_upgrade`, [
        [
          `<span class="has-text-caution">${loc(
            `arpa_blood_unbound_resistance_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_blood_shadow_war_title`
          )}</span>`,
          `<span class="has-text-caution">${loc(
            `arpa_blood_unbound_immunity_title`
          )}</span>`,
        ].join(", "),
      ]),
    ],
  ...
  harmonic: {
    ...
    group: [
      {
        desc(wiki) {
          ...
          return loc("perks_harmonic", [
            wiki
              ? `1-${Object.keys(races).length + 2}`
              : +((harmonic[0] - 1) * 100).toFixed(0),
            wiki
              ? `2-${(Object.keys(races).length + 2) * 2}`
              : +((harmonic[1] - 1) * 100).toFixed(0),
          ]);
        },
        active() {
          let harmonic = calcPillar();
      ...
      },
      {
        desc(wiki) {
          let harmonic = calcPillar();
          return loc("perks_harmonic2", [
            loc("portal_west_tower"),
            loc("portal_east_tower"),
            wiki
              ? `12-${(Object.keys(races).length - 1) * 12}`
              : +(Object.keys(global.pillars).length * 12),
          ]);
        },
        active() {
          let harmonic = calcPillar();
    ...
    notes: [loc(`wiki_perks_harmonic_note1`), loc(`wiki_perks_harmonic_note2`)],
  },
  novice: {
    ...
    desc(wiki) {
      ...
      return `<div>${loc("achieve_perks_novice", [rna, dna])}</div><div>${loc(
        "achieve_perks_novice2"
      )}</div>`;
    },
    active() {
  ...
  journeyman: {
    ...
    desc(wiki) {
      let rank =
        global.stats.feat["journeyman"] &&
        global.stats.achieve["seeder"] &&
        global.stats.achieve.seeder.l > 0
          ? Math.min(
              global.stats.achieve.seeder.l,
              global.stats.feat["journeyman"]
            )
          : 1;
      if (wiki || rank > 1) {
        ...
        return `<div>${loc("achieve_perks_journeyman2", [
          rqueue,
          queue,
        ])}</div><div>${loc("achieve_perks_journeyman3")}</div>`;
      } else {
        return `<div>${loc("achieve_perks_journeyman1", [1])}</div><div>${loc(
          "achieve_perks_journeyman3"
        )}</div>`;
      }
    },
    active() {
      return global.stats.feat["journeyman"] && global.stats.bioseed > 0
        ? true
        : false;
    },
    notes: [
  ...
  master: {
    ...
    desc(wiki) {
      ...
      return loc("achieve_perks_master", [
        boost1,
        boost2,
        loc("evo_mitochondria_title"),
        loc("evo_eukaryotic_title"),
        loc("evo_membrane_title"),
        loc("evo_organelles_title"),
        loc("evo_nucleus_title"),
      ]);
    },
    active() {
      return global.stats.feat["master"] &&
        global.stats.achieve["ascended"] &&
        global.stats.achieve.ascended.l > 0
        ? true
        : false;
  ...
  grandmaster: {
    ...
    active() {
      return global.stats.feat["grandmaster"] &&
        global.stats.achieve["corrupted"] &&
        global.stats.achieve.corrupted.l > 0
        ? true
        : false;
...
export function drawPerks() {
  ...
  Object.keys(perkList).forEach(function (perk) {
    if (perkList[perk].hasOwnProperty("group")) {
      let isactive = false;
      let gperk = $(
        `<div class="achievement"><span class="has-text-warning">${perkList[perk].name}</span></div>`
      );
      ...
      if (perkList[perk].active()) {
        ...
        perks.append(
          $(
            `<div class="achievement"><span class="has-text-warning">${
              perkList[perk].name
            }</span><span>${perkList[perk].desc()}</span></div>`
          )
        );
      }
    }
...
export function drawStats() {
  ...
  // Overall Stats
  stats.append(
    `<div><span class="has-text-success">${loc(
      "achieve_stats_overall"
    )}</span></div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_plasmid_earned"
    )}</span> {{ s.plasmid | format }}</div>`
  );
  if (global.stats.antiplasmid > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_antiplasmid_earned"
      )}</span> {{ s.antiplasmid | format }}</div>`
    );
  }
  if (global.stats.phage > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_phage_earned"
      )}</span> {{ s.phage | format }}</div>`
    );
  }
  if (global.stats.dark > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_dark_earned"
      )}</span> {{ s.dark | format }}</div>`
    );
  }
  if (global.stats.harmony > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_harmony_earned"
      )}</span> {{ s.harmony | format }}</div>`
    );
  }
  if (global.stats.blood > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_blood_earned"
      )}</span> {{ s.blood | format }}</div>`
    );
  }
  if (global.stats.artifact > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_artifact_earned"
      )}</span> {{ s.artifact | format }}</div>`
    );
  }
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_knowledge_spent"
    )}</span> {{ s.know | t_know | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_starved_to_death"
    )}</span> {{ s.starved | t_starved | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_died_in_combat"
    )}</span> {{ s.died | t_died | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_game_days_played"
    )}</span> {{ s.days | played | format }}</div>`
  );
  if (global.stats.portals > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_portals"
      )}</span> {{ s.portals | format }}</div>`
    );
  }
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_total_resets"
    )}</span> {{ s.reset | format }}</div>`
  );
  if (global.stats.mad > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_mad_resets"
      )}</span> {{ s.mad | format }}</div>`
    );
  }
  if (global.stats.bioseed > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_bioseed_resets"
      )}</span> {{ s.bioseed | format }}</div>`
    );
  }
  if (global.stats.cataclysm > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_cataclysm_resets"
      )}</span> {{ s.cataclysm | format }}</div>`
    );
  }
  if (global.stats.blackhole > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_blackhole_resets"
      )}</span> {{ s.blackhole | format }}</div>`
    );
  }
  if (global.stats.ascend > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_ascension_resets"
      )}</span> {{ s.ascend | format }}</div>`
    );
  }
  if (global.stats.descend > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_descension_resets"
      )}</span> {{ s.descend | format }}</div>`
    );
  }
  if (global.stats.apotheosis > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_apotheosis_resets"
      )}</span> {{ s.apotheosis | format }}</div>`
    );
  }
  if (global.stats.aiappoc > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_aiappoc_resets"
      )}</span> {{ s.aiappoc | format }}</div>`
    );
  }
  if (global.stats.matrix > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_matrix_resets"
      )}</span> {{ s.matrix | format }}</div>`
    );
  }
  if (global.stats.retire > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_retire_resets"
      )}</span> {{ s.retire | format }}</div>`
    );
  }
  if (global.stats.eden > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_eden_resets"
      )}</span> {{ s.eden | format }}</div>`
    );
  }
  if (global.stats.terraform > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_terraform_resets"
      )}</span> {{ s.terraform | format }}</div>`
    );
  }
  if (global.stats.geck > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_gecks"
      )}</span> {{ s.geck | format }}</div>`
    );
  }

  // Current Run Stats
  stats.append(
    `<div class="cstat"><span class="has-text-success">${loc(
      "achieve_stats_current_game"
    )}</span></div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_knowledge_spent"
    )}</span> {{ s.know | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_starved_to_death"
    )}</span> {{ s.starved | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_died_in_combat"
    )}</span> {{ s.died | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_attacks_made"
    )}</span> {{ s.attacks | format }}</div>`
  );
  stats.append(
    `<div><span class="has-text-warning">${loc(
      "achieve_stats_game_days_played"
    )}</span> {{ s.days | format }}</div>`
  );
  if (global.stats.dkills > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_demons_kills"
      )}</span> {{ s.dkills | format }}</div>`
    );
  }
  if (global.stats.sac > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_sacrificed"
      )}</span> {{ s.sac | format }}</div>`
    );
  }
  if (global.stats.murders > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_murders"
      )}</span> {{ s.murders | format }}</div>`
    );
  }
  if (global.stats.psykill > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_psymurders"
      )}</span> {{ s.psykill | format }}</div>`
    );
  }
  if (global.stats.uDead > 0) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_unstable"
      )}</span> {{ s.uDead | format }}</div>`
    );
  }

  if (
    global.resource.hasOwnProperty("Thermite") &&
    global.resource.Thermite.amount > 0
  ) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_thermite"
      )}</span> {{ r.Thermite.amount | res }}</div>`
    );
  }
  ...
  if (hallowed.active) {
    ...
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_trickortreat"
      )}</span> {{ s.cfood | format }} ${trick}</div>`
    );
  }
  ...
  if (global.race.hasOwnProperty("gods") && global.race.gods != "none") {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_gods"
      )}</span> {{ g.gods | species }}</div>`
    );
  }
  if (
    global.race.hasOwnProperty("old_gods") &&
    global.race.old_gods != "none"
  ) {
    stats.append(
      `<div><span class="has-text-warning">${loc(
        "achieve_stats_old_gods"
      )}</span> {{ g.old_gods | species }}</div>`
    );
  }
