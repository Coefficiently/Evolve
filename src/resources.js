  ...
  return function (res, type, recalc) {
    if (recalc) {
      ...
      if (global.race["casting"] && global.race.casting["crafting"]) {
        let boost_m =
          1 +
          global.race.casting["crafting"] /
            (global.race.casting["crafting"] + 75);
        let boost_a =
          1 +
          (2 * global.race.casting["crafting"]) /
            (2 * global.race.casting["crafting"] + 75);
        crafting.general.multi.push({
...
function loadResource(name, wiki, max, rate, tradable, stackable, color) {
  ...
  if (global.resource[name].max === -1 || global.resource[name].max === -2) {
    res_container = $(
      `<div id="res${name}" class="resource crafted" v-show="display"><h3 class="res has-text-${color}">{{ name | namespace }}</h3><span id="cnt${name}" class="count">{{ amount | diffSize }}</span></div>`
    );
  } else {
    res_container = $(
      `<div id="res${name}" class="resource" v-show="display"><h3 class="res has-text-${color}">{{ name | namespace }}</h3><span id="cnt${name}" class="count">{{ amount | size }} / {{ max | size }}</span></div>`
    );
  }
  ...
  if (stackable) {
    res_container.append(
      $(
        `<span><span id="con${name}" v-if="showTrigger()" class="interact has-text-success" @click="trigModal" role="button" aria-label="Open crate management for ${name}">+</span></span>`
      )
    );
  } else if (
    max !== -1 ||
  ...
  if (
    ...
    for (let i = 0; i < inc.length; i++) {
      craft.append(
        $(
          `<span id="inc${name}${inc[i]}"><a @click="craft('${name}',${inc[i]})" aria-label="craft ${inc[i]} ${name}">+<span class="craft" data-val="${inc[i]}">${inc[i]}</span></a></span>`
        )
      );
    }
    craft.append(
      $(
        `<span id="inc${name}A"><a @click="craft('${name}','A')" aria-label="craft max ${name}">+<span class="craft" data-val="${"A"}">A</span></a></span>`
      )
    );
    infopops = true;
  ...
  if (infopops) {
    ...
    for (let i = 0; i < inc.length; i++) {
      let extra = function () {
        ...
        popper.append(
          $(
            `<div class="has-text-info">${loc("manual_crafting_hover_bonus", [
        ...
        if (typeof vol !== "number") {
          ...
          crafts.append(
            $(
              `<span class="has-text-advanced">${sizeApproximation(
                (bonus / 100) * num_crafted,
                1
              )} ${global.resource[res].name}</span>`
            )
          );
        } else {
          ...
          crafts.append(
            $(
              `<span class="has-text-advanced"><span class="craft" data-val="${sizeApproximation(
                (bonus / 100) * vol
              )}">${total_crafted}</span> ${global.resource[res].name}</span>`
            )
          );
        }
        let costs = $(
          `<div><span class="has-text-danger">${loc(
            "manual_crafting_hover_use"
          )} </span></div>`
        );
        for (let i = 0; i < craft_costs[res].length; i++) {
          costs.append(
            $(
              `<span class="craft-elm has-text-caution">${sizeApproximation(
                num_crafted * craft_costs[res][i].a,
                1
              )} ${global.resource[craft_costs[res][i].r].name}</span>`
            )
          );
  ...
  if (stackable) {
    popover(`con${name}`, function () {
      var popper = $(
        `<div>${loc("resource_Crates_plural")} ${
          global.resource[name].crates
        }</div>`
      );
      if (global.tech["steel_container"]) {
        popper.append(
          $(
            `<div>${loc("resource_Containers_plural")} ${
              global.resource[name].containers
            }</div>`
          )
        );
      }
...
function loadSpecialResource(name, color) {
  ...
  var res_container = $(
    `<div id="res${name}" class="resource" v-show="count"><span class="res has-text-${color}">${loc(
      `resource_${name}_name`
    )}</span><span class="count">{{ count | round }}</span></div>`
  );
  $("#resources").append(res_container);
    ...
    switch (name) {
      case "Plasmid":
        {
          ...
          desc.append(
            $(
              `<span>${loc(`resource_${name}_desc`, [
                active,
                +(plasmidBonus("plasmid") * 100).toFixed(2),
              ])}</span>`
            )
          );
          if (
            ...
            if (plasmidSpatial > 1) {
              desc.append(
                $(
                  `<span> ${loc(`resource_Plasmid_desc2`, [
                    +((plasmidSpatial - 1) * 100).toFixed(2),
                  ])}</span>`
                )
              );
            }
          ...
          desc.append(
            $(
              `<span>${loc(`resource_${name}_desc`, [
                ...
                +(plasmidBonus("antiplasmid") * 100).toFixed(2),
              ])}</span>`
            )
          );
            ...
            $(
              `<span>${loc(
                global.prestige.AntiPlasmid.count > 0
                  ? `resource_Phage_desc2`
                  : `resource_Phage_desc`,
                [250 + global.prestige.Phage.count]
              )}</span>`
            )
          );
      ...
      case "Dark":
        {
          switch (global.race.universe) {
            case "standard":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_s`, [
                    +((darkEffect("standard") - 1) * 100).toFixed(2),
                  ])}</span>`
                )
              );
              break;
            ...
            case "evil":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_e`, [
                    +((darkEffect("evil") - 1) * 100).toFixed(2),
                  ])}</span>`
                )
              );
              break;
            ...
            case "micro":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_m`, [
                    darkEffect("micro", false),
                    darkEffect("micro", true),
                  ])}</span>`
                )
              );
              break;
            ...
            case "heavy":
              ...
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_h`, [
                    +(space * 100).toFixed(4),
                    +(int * 100).toFixed(4),
                  ])}</span>`
                )
            ...
            case "antimatter":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_a`, [
                    +((darkEffect("antimatter") - 1) * 100).toFixed(2),
                  ])}</span>`
                )
              );
              break;
            ...
            case "magic":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_mg`, [
                    loc("resource_Mana_name"),
                    +((darkEffect("magic") - 1) * 100).toFixed(2),
                  ])}</span>`
                )
              );
              break;
      ...
      case "Harmony":
        desc.append(
          $(
            `<span>${loc(`resource_${name}_desc`, [
              global.race.universe === "standard" ? 0.1 : 1,
              harmonyEffect(),
            ])}</span>`
          )
        );
      ...
      case "Supercoiled":
        {
          ...
          if (
            ...
            desc.append(
              $(
                `<span> ${loc(`resource_${name}_trade_desc`, [
                  +trade.toFixed(2),
                ])}</span>`
              )
            );
...
export function marketItem(mount, market_item, name, color, full) {
  ...
  if (!global.race["no_trade"]) {
    market_item.append(
      $(
        `<span class="buy"><span class="has-text-success">${loc(
          "resource_market_buy"
        )}</span></span>`
      )
    );
    market_item.append(
      $(
        `<span role="button" class="order" @click="purchase('${name}')">\${{ r.value | buy }}</span>`
      )
    );

    market_item.append(
      $(
        `<span class="sell"><span class="has-text-danger">${loc(
          "resource_market_sell"
        )}</span></span>`
      )
    );
    market_item.append(
      $(
        `<span role="button" class="order" @click="sell('${name}')">\${{ r.value | sell }}</span>`
      )
    );
  }
  ...
  if (
    ...
    let trade = $(
      `<span class="trade" v-show="m.active"><span class="has-text-warning">${loc(
        "resource_market_routes"
      )}</span></span>`
    );
    market_item.append(trade);
    trade.append(
      $(
        `<b-tooltip :label="aSell('${name}')" position="is-bottom" size="is-small" multilined animated><span role="button" aria-label="export ${name}" class="sub has-text-danger" @click="autoSell('${name}')"><span>-</span></span></b-tooltip>`
      )
    );
    trade.append(
      $(
    ...
    trade.append(
      $(
        `<b-tooltip :label="aBuy('${name}')" position="is-bottom" size="is-small" multilined animated><span role="button" aria-label="import ${name}" class="add has-text-success" @click="autoBuy('${name}')"><span>+</span></span></b-tooltip>`
      )
    );
    trade.append(
      $(
        `<span role="button" class="zero has-text-advanced" @click="zero('${name}')">${loc(
          "cancel_routes"
        )}</span>`
      )
    );
...
function initGalaxyTrade() {
  ...
  $("#market").append(
    $(
      `<div id="galaxyTrade" v-show="t.xeno && t.xeno >= 5" class="market-header galaxyTrade"><h2 class="is-sr-only">${loc(
        "galaxy_trade"
      )}</h2></div>`
    )
  );
...
export function galacticTrade(modal) {
  ...
  if (global.galaxy["trade"]) {
    galaxyTrade.append(
      $(
        `<div class="market-item trade-header"><span class="has-text-special">${loc(
          "galaxy_trade"
        )}</span></div>`
      )
    );
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      offer.append(
        $(
          `<span class="offer-item has-text-success">${
      ...
      offer.append(
        $(
          `<span class="offer-item has-text-danger">${
      ...
      let trade = $(
        `<span class="trade"><span class="has-text-warning">${loc(
          "resource_market_routes"
        )}</span></span>`
      );
      offer.append(trade);
      ...
      trade.append(
        $(
          `<b-tooltip :label="desc('${unassign}')" position="is-bottom" size="is-small" multilined animated><span role="button" aria-label="${unassign}" class="sub has-text-danger" @click="less('${i}')"><span>-</span></span></b-tooltip>`
        )
      );
      trade.append($(`<span class="current">{{ g.f${i} }}</span>`));
      trade.append(
        $(
          `<b-tooltip :label="desc('${assign}')" position="is-bottom" size="is-small" multilined animated><span role="button" aria-label="${assign}" class="add has-text-success" @click="more('${i}')"><span>+</span></span></b-tooltip>`
        )
      );
      trade.append(
        $(
          `<span role="button" class="zero has-text-advanced" @click="zero('${i}')">${loc(
            "cancel_routes"
          )}</span>`
        )
      );
    ...
    let totals = $(
      `<div class="market-item trade-offer"><div id="galacticTradeTotal"><span class="tradeTotal"><span class="has-text-caution">${loc(
        "resource_market_galactic_trade_routes"
      )}</span> {{ g.cur }} / {{ g.max }}</span></div></div>`
    );
    totals.append(
      $(
        `<span role="button" class="zero has-text-advanced" @click="zero()">${loc(
          "cancel_all_routes"
        )}</span>`
      )
    );
  ...
  popover(
    ...
    function () {
      ...
      bd.append(
        `<div class="modal_bd ${
          global.galaxy.trade.max > 0 ? "sum" : ""
        }"><span class="has-text-caution">${loc(
          "resource_market_galactic_trade_routes"
        )}</span> <span>${global.galaxy.trade.max}</span></div>`
      );
      return bd;
    },
    {
...
export function containerItem(mount, market_item, name, color) {
  ...
  if (global.resource.Crates.display) {
    ...
    crate.append(
      $(
        `<span role="button" aria-label="remove ${name} ${loc(
          "resource_Crates_name"
        )}" class="sub has-text-danger" @click="subCrate('${name}')"><span>&laquo;</span></span>`
      )
    );
    crate.append(
      $(
    ...
    crate.append(
      $(
        `<span role="button" aria-label="add ${name} ${loc(
          "resource_Crates_name"
        )}" class="add has-text-success" @click="addCrate('${name}')"><span>&raquo;</span></span>`
      )
    );
  }
  ...
  if (global.resource.Containers.display) {
    ...
    container.append(
      $(
        `<span role="button" aria-label="remove ${name} ${loc(
          "resource_Containers_name"
        )}" class="sub has-text-danger" @click="subCon('${name}')"><span>&laquo;</span></span>`
      )
    );
    container.append(
      $(
    ...
    container.append(
      $(
        `<span role="button" aria-label="add ${name} ${loc(
          "resource_Containers_name"
        )}" class="add has-text-success" @click="addCon('${name}')"><span>&raquo;</span></span>`
      )
    );
  }
  ...
  popover(
    ...
    function () {
      ...
      if (type === "auto" && breakdown.p[res]) {
        Object.keys(breakdown.p[res]).forEach(function (mod) {
          ...
          if (val != 0 && !isNaN(val)) {
            ...
            mod = mod.replace(/'/g, "\\'");
            col1.append(
              `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ ${[
                res,
              ]}['${mod}'] | translate }}</span></div>`
            );
          }
        });
      }
      ...
      if (
        ...
        bd.append(
          `<div class="modal_bd sum"><span>${loc(
            "to_empty"
          )}</span><span class="has-text-danger">{{ res.amount | counter }}</span></div>`
        );
      }
  ...
  popover(
    ...
    function () {
      ...
      if (type === "p" && name === global.race.species) {
        bd = $(
          `<div class="resBreakdown"><div class="has-text-info">${loc(
            "starvation_resist"
          )}</div></div>`
        );
      }
...
function loadRouteCounter() {
  ...
  var market_item = $(
    `<div id="tradeTotal" v-show="active" class="market-item"><div id="tradeTotalPopover"><span class="tradeTotal${no_market}"><span class="has-text-caution">${loc(
      "resource_market_trade_routes"
    )}</span> <span v-html="$options.filters.tdeCnt(trade)"></span> / {{ mtrade }}</span></div></div>`
  );
  market_item.append(
    $(
      `<span role="button" class="zero has-text-advanced" @click="zero()">${loc(
  ...
  popover(
    ...
    function () {
      ...
      bd.append(
        `<div class="modal_bd ${
          global.city.market.mtrade > 0 ? "sum" : ""
        }"><span class="has-text-caution">${loc(
          "resource_market_trade_routes"
        )}</span> <span>${global.city.market.mtrade}</span></div>`
      );
      return bd;
    },
    {
...
function loadContainerCounter() {
  ...
  var market_item = $(
    `<div id="crateTotal" class="market-item"><span v-show="cr.display" class="crtTotal"><span class="has-text-warning">${loc(
      "resource_Crates_name"
    )}</span><span>{{ cr.amount }} / {{ cr.max }}</span></span><span v-show="cn.display" class="cntTotal"><span class="has-text-warning">${loc(
      "resource_Containers_name"
    )}</span><span>{{ cn.amount }} / {{ cn.max }}</span></span></div>`
  );
  $("#resStorage").append(market_item);
...
function buildCrateLabel() {
  let material =
    global.race["kindling_kindred"] || global.race["smoldering"]
      ...
      : loc("resource_Plywood_name");
  let cost =
    global.race["kindling_kindred"] || global.race["smoldering"] ? 200 : 10;
...
function drawModal(name) {
  ...
  crates.append(
    $(
      `<div class="crateHead"><span>${loc(
        "resource_modal_crate_owned"
      )} {{ crates.amount }}/{{ crates.max }}</span><span>${loc(
        "resource_modal_crate_assigned"
      )} {{ res.crates }}</span></div>`
    )
  );

  let buildCr = $(
    `<button class="button construct" @click="buildCrate()">${loc(
      "resource_modal_crate_construct"
    )}</button>`
  );
  let removeCr = $(
    `<button class="button unassign" @click="subCrate('${name}')">${loc(
      "resource_modal_crate_unassign"
    )}</button>`
  );
  let addCr = $(
    `<button class="button assign" @click="addCrate('${name}')">${loc(
      "resource_modal_crate_assign"
    )}</button>`
  );
  ...
  if (global.resource.Containers.display) {
    ...
    containers.append(
      $(
        `<div class="crateHead"><span>${loc(
          "resource_modal_container_owned"
        )} {{ containers.amount }}/{{ containers.max }}</span><span>${loc(
          "resource_modal_container_assigned"
        )} {{ res.containers }}</span></div>`
      )
    );

    let buildCon = $(
      `<button class="button construct" @click="buildContainer()">${loc(
        "resource_modal_container_construct"
      )}</button>`
    );
    let removeCon = $(
      `<button class="button unassign" @click="removeContainer('${name}')">${loc(
        "resource_modal_container_unassign"
      )}</button>`
    );
    let addCon = $(
      `<button class="button assign" @click="addContainer('${name}')">${loc(
        "resource_modal_container_assign"
      )}</button>`
    );
...
function initMarket() {
  ...
  let market = $(
    `<div id="market-qty" class="market-header"><h2 class="is-sr-only">${loc(
      "resource_market"
    )}</h2></div>`
  );
  clearElement($("#market"));
...
function initStorage() {
  ...
  let store = $(
    `<div id="createHead" class="storage-header"><h2 class="is-sr-only">${loc(
      "tab_storage"
    )}</h2></div>`
  );
  ...
  if (global.resource["Crates"] && global.resource["Containers"]) {
    store.append(
      $(
        `<b-tooltip :label="buildCrateDesc()" position="is-bottom" class="crate" animated multilined><button :aria-label="buildCrateDesc()" v-show="cr.display" class="button" @click="crate">${loc(
          "resource_modal_crate_construct"
        )}</button></b-tooltip>`
      )
    );
    store.append(
      $(
        `<b-tooltip :label="buildContainerDesc()" position="is-bottom" class="container" animated multilined><button :aria-label="buildContainerDesc()" v-show="cn.display" class="button" @click="container">${loc(
          "resource_modal_container_construct"
        )}</button></b-tooltip>`
      )
    );
...
function loadMarket() {
  ...
  if (!global.race["no_trade"]) {
    ...
    market.append(
      $(
        `<b-field class="market"><span class="button has-text-danger" role="button" @click="less">-</span><b-numberinput :input="val()" min="1" :max="limit()" v-model="qty" :controls="false"></b-numberinput><span class="button has-text-success" role="button" @click="more">+</span></b-field>`
      )
    );
  }
...
function initEjector() {
  ...
  if (global.interstellar["mass_ejector"]) {
    let ejector = $(
      `<div id="eject" class="market-item"><h3 class="res has-text-warning">${loc(
        "interstellar_mass_ejector_vol"
      )}</h3></div>`
    );
    ...
    eject.append(
      $(
        `<span>{{ total }} / {{ on | max }}{{ on | real }}</span><span class="mass">${loc(
          "interstellar_mass_ejector_mass"
        )}: {{ mass | approx }} kt/s</span>`
      )
    );
      ...
      filters: {
        ...
        real(num) {
          if (p_on["mass_ejector"] < num) {
            return ` (${loc("interstellar_mass_ejector_active", [
...
export function loadEjector(name, color) {
  ...
  if (atomic_mass[name] && global.interstellar["mass_ejector"]) {
    ...
    res.append(
      $(
        `<span role="button" aria-label="eject less ${loc(
          "resource_" + name + "_name"
        )}" class="sub has-text-danger" @click="ejectLess('${name}')"><span>&laquo;</span></span>`
      )
    );
    res.append($(`<span class="current">{{ e.${name} }}</span>`));
    res.append(
      $(
        `<span role="button" aria-label="eject more ${loc(
          "resource_" + name + "_name"
        )}" class="add has-text-success" @click="ejectMore('${name}')"><span>&raquo;</span></span>`
      )
    );
...
function initSupply() {
  ...
  if (global.portal["transport"]) {
    let supply = $(
      `<div id="spireSupply"><h3 class="res has-text-warning pad">${loc(
        "portal_transport_supply"
      )}</h3></div>`
    );
  ...
  if (supplyValue[name] && global.portal["transport"]) {
    ...
    res.append(
      $(
        `<span role="button" aria-label="eject less ${loc(
          "resource_" + name + "_name"
        )}" class="sub has-text-danger" @click="supplyLess('${name}')"><span>&laquo;</span></span>`
      )
    );
    res.append($(`<span class="current">{{ e.${name} }}</span>`));
    res.append(
      $(
        `<span role="button" aria-label="eject more ${loc(
          "resource_" + name + "_name"
        )}" class="add has-text-success" @click="supplyMore('${name}')"><span>&raquo;</span></span>`
      )
    );
    ...
    res.append(
      $(
        `<span class="mass">${loc("portal_transport_item", [
          `<span class="has-text-caution">${volume}</span>`,
          `<span class="has-text-success">${supplyValue[name].in}</span>`,
        ])}</span>`
      )
    );
...
export function loadAlchemy(name, color, basic) {
  ...
  if (
    ...
    res.append(
      $(
        `<span role="button" aria-label="transmute less ${loc(
          "resource_" + name + "_name"
        )}" class="sub has-text-danger" @click="subSpell('${name}')"><span>&laquo;</span></span>`
      )
    );
    res.append($(`<span class="current">{{ a.${name} }}</span>`));
    res.append(
      $(
        `<span role="button" aria-label="transmute more ${loc(
          "resource_" + name + "_name"
        )}" class="add has-text-success" @click="addSpell('${name}')"><span>&raquo;</span></span>`
      )
    );
...
export const spatialReasoning = (function () {
  ...
  return function (value, type, recalc) {
    ...
    if (!spatial[tkey][key] || recalc) {
      ...
      if (
        global.race.universe === "antimatter" &&
        ((!noEarth && global.city["temple"] && global.city["temple"].count) ||
          (noEarth &&
            global.space["ziggurat"] &&
            global.space["ziggurat"].count))
      ) {
        let temple = 0.06;
        if (
        ...
        modifier *=
          1 +
          (noEarth ? global.space.ziggurat.count : global.city.temple.count) *
            temple;
      }
      if (!type) {
        if (global["pillars"]) {
